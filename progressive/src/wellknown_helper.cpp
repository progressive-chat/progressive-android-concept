...
namespace {
    WellknownHelperConfig g_config;
    std::mutex g_mutex;
    WellknownHelperStatus g_status;
    WellknownHelperStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* wellknown_helper_stateToString(WellknownHelperState s) {
    switch (s) {
        case WellknownHelperState::UNKNOWN: return "UNKNOWN";
        case WellknownHelperState::INIT: return "INIT";
        case WellknownHelperState::PROCESSING: return "PROCESSING";
        case WellknownHelperState::DONE: return "DONE";
        case WellknownHelperState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

WellknownHelperState wellknown_helper_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return WellknownHelperState::UNKNOWN;
    if (s == "INIT") return WellknownHelperState::INIT;
    if (s == "PROCESSING") return WellknownHelperState::PROCESSING;
    if (s == "DONE") return WellknownHelperState::DONE;
    if (s == "ERROR") return WellknownHelperState::ERROR;
    return WellknownHelperState::UNKNOWN;
}
...
bool wellknown_helper_init(const WellknownHelperConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = WellknownHelperState::INIT;
...
WellknownHelperResponse wellknown_helper_handle(const WellknownHelperRequest& req) {
    ...
    g_status.state = WellknownHelperState::PROCESSING;
    // Validate request
...
void wellknown_helper_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = WellknownHelperConfig{};
    g_status = WellknownHelperStatus{};
    g_status.state = WellknownHelperState::UNKNOWN;
}
...
std::string wellknown_helper_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "wellknown_helper";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
