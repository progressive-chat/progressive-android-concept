...
namespace {
    DisplayNameUtilsConfig g_config;
    std::mutex g_mutex;
    DisplayNameUtilsStatus g_status;
    DisplayNameUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* display_name_utils_stateToString(DisplayNameUtilsState s) {
    switch (s) {
        case DisplayNameUtilsState::UNKNOWN: return "UNKNOWN";
        case DisplayNameUtilsState::INIT: return "INIT";
        case DisplayNameUtilsState::PROCESSING: return "PROCESSING";
        case DisplayNameUtilsState::DONE: return "DONE";
        case DisplayNameUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

DisplayNameUtilsState display_name_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return DisplayNameUtilsState::UNKNOWN;
    if (s == "INIT") return DisplayNameUtilsState::INIT;
    if (s == "PROCESSING") return DisplayNameUtilsState::PROCESSING;
    if (s == "DONE") return DisplayNameUtilsState::DONE;
    if (s == "ERROR") return DisplayNameUtilsState::ERROR;
    return DisplayNameUtilsState::UNKNOWN;
}
...
bool display_name_utils_init(const DisplayNameUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = DisplayNameUtilsState::INIT;
...
DisplayNameUtilsResponse display_name_utils_handle(const DisplayNameUtilsRequest& req) {
    ...
    g_status.state = DisplayNameUtilsState::PROCESSING;
    // Validate request
...
void display_name_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = DisplayNameUtilsConfig{};
    g_status = DisplayNameUtilsStatus{};
    g_status.state = DisplayNameUtilsState::UNKNOWN;
}
...
std::string display_name_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "display_name_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
