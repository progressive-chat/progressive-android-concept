...
namespace {
    EmojiStickerConfig g_config;
    std::mutex g_mutex;
    EmojiStickerStatus g_status;
    EmojiStickerStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* emoji_sticker_stateToString(EmojiStickerState s) {
    switch (s) {
        case EmojiStickerState::UNKNOWN: return "UNKNOWN";
        case EmojiStickerState::INIT: return "INIT";
        case EmojiStickerState::PROCESSING: return "PROCESSING";
        case EmojiStickerState::DONE: return "DONE";
        case EmojiStickerState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

EmojiStickerState emoji_sticker_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return EmojiStickerState::UNKNOWN;
    if (s == "INIT") return EmojiStickerState::INIT;
    if (s == "PROCESSING") return EmojiStickerState::PROCESSING;
    if (s == "DONE") return EmojiStickerState::DONE;
    if (s == "ERROR") return EmojiStickerState::ERROR;
    return EmojiStickerState::UNKNOWN;
}
...
bool emoji_sticker_init(const EmojiStickerConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = EmojiStickerState::INIT;
...
EmojiStickerResponse emoji_sticker_handle(const EmojiStickerRequest& req) {
    ...
    g_status.state = EmojiStickerState::PROCESSING;
    // Validate request
...
void emoji_sticker_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = EmojiStickerConfig{};
    g_status = EmojiStickerStatus{};
    g_status.state = EmojiStickerState::UNKNOWN;
}
