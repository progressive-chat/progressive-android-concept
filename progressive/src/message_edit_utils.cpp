...
namespace {
    MessageEditUtilsConfig g_config;
    std::mutex g_mutex;
    MessageEditUtilsStatus g_status;
    MessageEditUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* message_edit_utils_stateToString(EditUtilsPhase s) {
    switch (s) {
        case EditUtilsPhase::UNKNOWN: return "UNKNOWN";
        case EditUtilsPhase::INIT: return "INIT";
        case EditUtilsPhase::PROCESSING: return "PROCESSING";
        case EditUtilsPhase::DONE: return "DONE";
        case EditUtilsPhase::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

EditUtilsPhase message_edit_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return EditUtilsPhase::UNKNOWN;
    if (s == "INIT") return EditUtilsPhase::INIT;
    if (s == "PROCESSING") return EditUtilsPhase::PROCESSING;
    if (s == "DONE") return EditUtilsPhase::DONE;
    if (s == "ERROR") return EditUtilsPhase::ERROR;
    return EditUtilsPhase::UNKNOWN;
}

bool message_edit_utils_init(const MessageEditUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = EditUtilsPhase::INIT;
...
MessageEditUtilsResponse message_edit_utils_handle(const MessageEditUtilsRequest& req) {
    MessageEditUtilsResponse resp;
    ...
    g_status.state = EditUtilsPhase::PROCESSING;
    // Validate request
...
MessageEditUtilsStatus message_edit_utils_getStatus() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_status;
}

void message_edit_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = MessageEditUtilsConfig{};
    g_status = MessageEditUtilsStatus{};
    g_status.state = EditUtilsPhase::UNKNOWN;
}
...
std::string message_edit_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "message_edit_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
MessageEditUtilsStats message_edit_utils_getStats() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_stats;
}

MessageEditUtilsConfig message_edit_utils_configFromJson(const json& j) {
    MessageEditUtilsConfig config;
...
json message_edit_utils_configToJson(const MessageEditUtilsConfig& config) {
    json j;
