...
namespace {
    ...
    std::mutex g_mutex;
    NotificationStatus g_status;
    NotificationStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* notification_stateToString(NotificationState s) {
    switch (s) {
        case NotificationState::UNKNOWN: return "UNKNOWN";
        case NotificationState::INIT: return "INIT";
        case NotificationState::PROCESSING: return "PROCESSING";
        case NotificationState::DONE: return "DONE";
...
NotificationState notification_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return NotificationState::UNKNOWN;
    if (s == "INIT") return NotificationState::INIT;
    if (s == "PROCESSING") return NotificationState::PROCESSING;
    if (s == "DONE") return NotificationState::DONE;
    if (s == "ERROR") return NotificationState::ERROR;
...
bool notification_init(const NotificationConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = NotificationState::INIT;
...
NotificationResponse notification_handle(const NotificationRequest& req) {
    ...
    g_status.state = NotificationState::PROCESSING;
    // Validate request
...
void notification_reset() {
    ...
    g_status = NotificationStatus{};
    g_status.state = NotificationState::UNKNOWN;
}
