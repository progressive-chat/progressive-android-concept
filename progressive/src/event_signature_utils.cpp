...
namespace {
    EventSignatureUtilsConfig g_config;
    std::mutex g_mutex;
    EventSignatureUtilsStatus g_status;
    EventSignatureUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* event_signature_utils_stateToString(SignatureUtilsState s) {
    switch (s) {
        case SignatureUtilsState::UNKNOWN: return "UNKNOWN";
        case SignatureUtilsState::INIT: return "INIT";
        case SignatureUtilsState::PROCESSING: return "PROCESSING";
        case SignatureUtilsState::DONE: return "DONE";
        case SignatureUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

SignatureUtilsState event_signature_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return SignatureUtilsState::UNKNOWN;
    if (s == "INIT") return SignatureUtilsState::INIT;
    if (s == "PROCESSING") return SignatureUtilsState::PROCESSING;
    if (s == "DONE") return SignatureUtilsState::DONE;
    if (s == "ERROR") return SignatureUtilsState::ERROR;
    return SignatureUtilsState::UNKNOWN;
}
...
bool event_signature_utils_init(const EventSignatureUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = SignatureUtilsState::INIT;
...
EventSignatureUtilsResponse event_signature_utils_handle(const EventSignatureUtilsRequest& req) {
    EventSignatureUtilsResponse resp;
    ...
    g_status.state = SignatureUtilsState::PROCESSING;
    // Validate request
...
void event_signature_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = EventSignatureUtilsConfig{};
    g_status = EventSignatureUtilsStatus{};
    g_status.state = SignatureUtilsState::UNKNOWN;
}
...
std::string event_signature_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "event_signature_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
EventSignatureUtilsConfig event_signature_utils_configFromJson(const json& j) {
    EventSignatureUtilsConfig config;
