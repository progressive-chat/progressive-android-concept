...
namespace {
    TypingSenderConfig g_config;
    std::mutex g_mutex;
    TypingSenderStatus g_status;
    TypingSenderStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* typing_sender_stateToString(TypingSenderState s) {
    switch (s) {
        case TypingSenderState::UNKNOWN: return "UNKNOWN";
        case TypingSenderState::INIT: return "INIT";
        case TypingSenderState::PROCESSING: return "PROCESSING";
        case TypingSenderState::DONE: return "DONE";
        case TypingSenderState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

TypingSenderState typing_sender_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return TypingSenderState::UNKNOWN;
    if (s == "INIT") return TypingSenderState::INIT;
    if (s == "PROCESSING") return TypingSenderState::PROCESSING;
    if (s == "DONE") return TypingSenderState::DONE;
    if (s == "ERROR") return TypingSenderState::ERROR;
    return TypingSenderState::UNKNOWN;
}
...
bool typing_sender_init(const TypingSenderConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = TypingSenderState::INIT;
...
TypingSenderResponse typing_sender_handle(const TypingSenderRequest& req) {
    ...
    g_status.state = TypingSenderState::PROCESSING;
    // Validate request
...
void typing_sender_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = TypingSenderConfig{};
    g_status = TypingSenderStatus{};
    g_status.state = TypingSenderState::UNKNOWN;
}
