...
namespace {
    AvatarColorGenConfig g_config;
    std::mutex g_mutex;
    AvatarColorGenStatus g_status;
    AvatarColorGenStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* avatar_color_gen_stateToString(AvatarColorGenState s) {
    switch (s) {
        case AvatarColorGenState::UNKNOWN: return "UNKNOWN";
        case AvatarColorGenState::INIT: return "INIT";
        case AvatarColorGenState::PROCESSING: return "PROCESSING";
        case AvatarColorGenState::DONE: return "DONE";
        case AvatarColorGenState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

AvatarColorGenState avatar_color_gen_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return AvatarColorGenState::UNKNOWN;
    if (s == "INIT") return AvatarColorGenState::INIT;
    if (s == "PROCESSING") return AvatarColorGenState::PROCESSING;
    if (s == "DONE") return AvatarColorGenState::DONE;
    if (s == "ERROR") return AvatarColorGenState::ERROR;
    return AvatarColorGenState::UNKNOWN;
}
...
bool avatar_color_gen_init(const AvatarColorGenConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = AvatarColorGenState::INIT;
...
AvatarColorGenResponse avatar_color_gen_handle(const AvatarColorGenRequest& req) {
    ...
    g_status.state = AvatarColorGenState::PROCESSING;
    // Validate request
...
void avatar_color_gen_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = AvatarColorGenConfig{};
    g_status = AvatarColorGenStatus{};
    g_status.state = AvatarColorGenState::UNKNOWN;
}
...
std::string avatar_color_gen_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "avatar_color_gen";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
