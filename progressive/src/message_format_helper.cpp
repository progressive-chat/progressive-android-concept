...
namespace {
    MessageFormatHelperConfig g_config;
    std::mutex g_mutex;
    MessageFormatHelperStatus g_status;
    MessageFormatHelperStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* message_format_helper_stateToString(FormatHelperPhase s) {
    switch (s) {
        case FormatHelperPhase::UNKNOWN: return "UNKNOWN";
        case FormatHelperPhase::INIT: return "INIT";
        case FormatHelperPhase::PROCESSING: return "PROCESSING";
        case FormatHelperPhase::DONE: return "DONE";
        case FormatHelperPhase::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

FormatHelperPhase message_format_helper_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return FormatHelperPhase::UNKNOWN;
    if (s == "INIT") return FormatHelperPhase::INIT;
    if (s == "PROCESSING") return FormatHelperPhase::PROCESSING;
    if (s == "DONE") return FormatHelperPhase::DONE;
    if (s == "ERROR") return FormatHelperPhase::ERROR;
    return FormatHelperPhase::UNKNOWN;
}
...
bool message_format_helper_init(const MessageFormatHelperConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = FormatHelperPhase::INIT;
...
MessageFormatHelperResponse message_format_helper_handle(const MessageFormatHelperRequest& req) {
    MessageFormatHelperResponse resp;
    ...
    g_status.state = FormatHelperPhase::PROCESSING;
    // Validate request
...
void message_format_helper_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = MessageFormatHelperConfig{};
    g_status = MessageFormatHelperStatus{};
    g_status.state = FormatHelperPhase::UNKNOWN;
}
...
std::string message_format_helper_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "message_format_helper";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
MessageFormatHelperConfig message_format_helper_configFromJson(const json& j) {
    MessageFormatHelperConfig config;
