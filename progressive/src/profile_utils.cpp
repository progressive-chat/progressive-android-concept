...
namespace {
    ProfileUtilsConfig g_config;
    std::mutex g_mutex;
    ProfileUtilsStatus g_status;
    ProfileUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* profile_utils_stateToString(ProfileUtilsState s) {
    switch (s) {
        case ProfileUtilsState::UNKNOWN: return "UNKNOWN";
        case ProfileUtilsState::INIT: return "INIT";
        case ProfileUtilsState::PROCESSING: return "PROCESSING";
        case ProfileUtilsState::DONE: return "DONE";
        case ProfileUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

ProfileUtilsState profile_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return ProfileUtilsState::UNKNOWN;
    if (s == "INIT") return ProfileUtilsState::INIT;
    if (s == "PROCESSING") return ProfileUtilsState::PROCESSING;
    if (s == "DONE") return ProfileUtilsState::DONE;
    if (s == "ERROR") return ProfileUtilsState::ERROR;
    return ProfileUtilsState::UNKNOWN;
}
...
bool profile_utils_init(const ProfileUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = ProfileUtilsState::INIT;
...
ProfileUtilsResponse profile_utils_handle(const ProfileUtilsRequest& req) {
    ...
    g_status.state = ProfileUtilsState::PROCESSING;
    // Validate request
...
void profile_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = ProfileUtilsConfig{};
    g_status = ProfileUtilsStatus{};
    g_status.state = ProfileUtilsState::UNKNOWN;
}
