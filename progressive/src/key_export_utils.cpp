...
namespace {
    KeyExportUtilsConfig g_config;
    std::mutex g_mutex;
    KeyExportUtilsStatus g_status;
    KeyExportUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* key_export_utils_stateToString(ExportUtilsStatus s) {
    switch (s) {
        case ExportUtilsStatus::UNKNOWN: return "UNKNOWN";
        case ExportUtilsStatus::INIT: return "INIT";
        case ExportUtilsStatus::PROCESSING: return "PROCESSING";
        case ExportUtilsStatus::DONE: return "DONE";
        case ExportUtilsStatus::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

ExportUtilsStatus key_export_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return ExportUtilsStatus::UNKNOWN;
    if (s == "INIT") return ExportUtilsStatus::INIT;
    if (s == "PROCESSING") return ExportUtilsStatus::PROCESSING;
    if (s == "DONE") return ExportUtilsStatus::DONE;
    if (s == "ERROR") return ExportUtilsStatus::ERROR;
    return ExportUtilsStatus::UNKNOWN;
}
...
bool key_export_utils_init(const KeyExportUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = ExportUtilsStatus::INIT;
...
KeyExportUtilsResponse key_export_utils_handle(const KeyExportUtilsRequest& req) {
    ...
    g_status.state = ExportUtilsStatus::PROCESSING;
    // Validate request
...
void key_export_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = KeyExportUtilsConfig{};
    g_status = KeyExportUtilsStatus{};
    g_status.state = ExportUtilsStatus::UNKNOWN;
}
...
std::string key_export_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "key_export_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
