...
namespace {
    NotificationGroupingConfig g_config;
    std::mutex g_mutex;
    NotificationGroupingStatus g_status;
    NotificationGroupingStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* notification_grouping_stateToString(GroupingLevel s) {
    switch (s) {
        case GroupingLevel::UNKNOWN: return "UNKNOWN";
        case GroupingLevel::INIT: return "INIT";
        case GroupingLevel::PROCESSING: return "PROCESSING";
        case GroupingLevel::DONE: return "DONE";
        case GroupingLevel::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

GroupingLevel notification_grouping_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return GroupingLevel::UNKNOWN;
    if (s == "INIT") return GroupingLevel::INIT;
    if (s == "PROCESSING") return GroupingLevel::PROCESSING;
    if (s == "DONE") return GroupingLevel::DONE;
    if (s == "ERROR") return GroupingLevel::ERROR;
    return GroupingLevel::UNKNOWN;
}
...
bool notification_grouping_init(const NotificationGroupingConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = GroupingLevel::INIT;
...
NotificationGroupingResponse notification_grouping_handle(const NotificationGroupingRequest& req) {
    NotificationGroupingResponse resp;
    ...
    g_status.state = GroupingLevel::PROCESSING;
    // Validate request
...
void notification_grouping_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = NotificationGroupingConfig{};
    g_status = NotificationGroupingStatus{};
    g_status.state = GroupingLevel::UNKNOWN;
}
...
std::string notification_grouping_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "notification_grouping";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
NotificationGroupingConfig notification_grouping_configFromJson(const json& j) {
    NotificationGroupingConfig config;
