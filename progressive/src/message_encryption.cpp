...
namespace {
    MessageEncryptionConfig g_config;
    std::mutex g_mutex;
    MessageEncryptionStatus g_status;
    MessageEncryptionStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* message_encryption_stateToString(EncryptionPhase s) {
    switch (s) {
        case EncryptionPhase::UNKNOWN: return "UNKNOWN";
        case EncryptionPhase::INIT: return "INIT";
        case EncryptionPhase::PROCESSING: return "PROCESSING";
        case EncryptionPhase::DONE: return "DONE";
        case EncryptionPhase::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

EncryptionPhase message_encryption_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return EncryptionPhase::UNKNOWN;
    if (s == "INIT") return EncryptionPhase::INIT;
    if (s == "PROCESSING") return EncryptionPhase::PROCESSING;
    if (s == "DONE") return EncryptionPhase::DONE;
    if (s == "ERROR") return EncryptionPhase::ERROR;
    return EncryptionPhase::UNKNOWN;
}
...
bool message_encryption_init(const MessageEncryptionConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = EncryptionPhase::INIT;
...
MessageEncryptionResponse message_encryption_handle(const MessageEncryptionRequest& req) {
    MessageEncryptionResponse resp;
    ...
    g_status.state = EncryptionPhase::PROCESSING;
    // Validate request
...
void message_encryption_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = MessageEncryptionConfig{};
    g_status = MessageEncryptionStatus{};
    g_status.state = EncryptionPhase::UNKNOWN;
}
...
std::string message_encryption_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "message_encryption";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
MessageEncryptionConfig message_encryption_configFromJson(const json& j) {
    MessageEncryptionConfig config;
