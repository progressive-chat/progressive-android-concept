...
namespace {
    RoomTopicUtilsConfig g_config;
    std::mutex g_mutex;
    RoomTopicUtilsStatus g_status;
    RoomTopicUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* room_topic_utils_stateToString(TopicUtilsAction s) {
    switch (s) {
        case TopicUtilsAction::UNKNOWN: return "UNKNOWN";
        case TopicUtilsAction::INIT: return "INIT";
        case TopicUtilsAction::PROCESSING: return "PROCESSING";
        case TopicUtilsAction::DONE: return "DONE";
        case TopicUtilsAction::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

TopicUtilsAction room_topic_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return TopicUtilsAction::UNKNOWN;
    if (s == "INIT") return TopicUtilsAction::INIT;
    if (s == "PROCESSING") return TopicUtilsAction::PROCESSING;
    if (s == "DONE") return TopicUtilsAction::DONE;
    if (s == "ERROR") return TopicUtilsAction::ERROR;
    return TopicUtilsAction::UNKNOWN;
}
...
bool room_topic_utils_init(const RoomTopicUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = TopicUtilsAction::INIT;
...
RoomTopicUtilsResponse room_topic_utils_handle(const RoomTopicUtilsRequest& req) {
    ...
    g_status.state = TopicUtilsAction::PROCESSING;
    // Validate request
...
void room_topic_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = RoomTopicUtilsConfig{};
    g_status = RoomTopicUtilsStatus{};
    g_status.state = TopicUtilsAction::UNKNOWN;
}
...
std::string room_topic_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "room_topic_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
