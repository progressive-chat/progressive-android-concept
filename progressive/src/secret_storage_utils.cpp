...
namespace {
    SecretStorageUtilsConfig g_config;
    std::mutex g_mutex;
    SecretStorageUtilsStatus g_status;
    SecretStorageUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* secret_storage_utils_stateToString(SecretStorageUtilsState s) {
    switch (s) {
        case SecretStorageUtilsState::UNKNOWN: return "UNKNOWN";
        case SecretStorageUtilsState::INIT: return "INIT";
        case SecretStorageUtilsState::PROCESSING: return "PROCESSING";
        case SecretStorageUtilsState::DONE: return "DONE";
        case SecretStorageUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

SecretStorageUtilsState secret_storage_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return SecretStorageUtilsState::UNKNOWN;
    if (s == "INIT") return SecretStorageUtilsState::INIT;
    if (s == "PROCESSING") return SecretStorageUtilsState::PROCESSING;
    if (s == "DONE") return SecretStorageUtilsState::DONE;
    if (s == "ERROR") return SecretStorageUtilsState::ERROR;
    return SecretStorageUtilsState::UNKNOWN;
}
...
bool secret_storage_utils_init(const SecretStorageUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = SecretStorageUtilsState::INIT;
...
SecretStorageUtilsResponse secret_storage_utils_handle(const SecretStorageUtilsRequest& req) {
    SecretStorageUtilsResponse resp;
    ...
    g_status.state = SecretStorageUtilsState::PROCESSING;
    // Validate request
...
void secret_storage_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = SecretStorageUtilsConfig{};
    g_status = SecretStorageUtilsStatus{};
    g_status.state = SecretStorageUtilsState::UNKNOWN;
}
...
std::string secret_storage_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "secret_storage_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
SecretStorageUtilsConfig secret_storage_utils_configFromJson(const json& j) {
    SecretStorageUtilsConfig config;
