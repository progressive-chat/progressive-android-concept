...
namespace {
    FontManagerConfig g_config;
    std::mutex g_mutex;
    FontManagerStatus g_status;
    FontManagerStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* font_manager_stateToString(FontManagerState s) {
    switch (s) {
        case FontManagerState::UNKNOWN: return "UNKNOWN";
        case FontManagerState::INIT: return "INIT";
        case FontManagerState::PROCESSING: return "PROCESSING";
        case FontManagerState::DONE: return "DONE";
        case FontManagerState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

FontManagerState font_manager_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return FontManagerState::UNKNOWN;
    if (s == "INIT") return FontManagerState::INIT;
    if (s == "PROCESSING") return FontManagerState::PROCESSING;
    if (s == "DONE") return FontManagerState::DONE;
    if (s == "ERROR") return FontManagerState::ERROR;
    return FontManagerState::UNKNOWN;
}
...
bool font_manager_init(const FontManagerConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = FontManagerState::INIT;
...
FontManagerResponse font_manager_handle(const FontManagerRequest& req) {
    ...
    g_status.state = FontManagerState::PROCESSING;
    // Validate request
...
void font_manager_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = FontManagerConfig{};
    g_status = FontManagerStatus{};
    g_status.state = FontManagerState::UNKNOWN;
}
