...
namespace {
    RoomForgotUtilsConfig g_config;
    std::mutex g_mutex;
    RoomForgotUtilsStatus g_status;
    RoomForgotUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* room_forgot_utils_stateToString(ForgotUtilsAction s) {
    switch (s) {
        case ForgotUtilsAction::UNKNOWN: return "UNKNOWN";
        case ForgotUtilsAction::INIT: return "INIT";
        case ForgotUtilsAction::PROCESSING: return "PROCESSING";
        case ForgotUtilsAction::DONE: return "DONE";
        case ForgotUtilsAction::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

ForgotUtilsAction room_forgot_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return ForgotUtilsAction::UNKNOWN;
    if (s == "INIT") return ForgotUtilsAction::INIT;
    if (s == "PROCESSING") return ForgotUtilsAction::PROCESSING;
    if (s == "DONE") return ForgotUtilsAction::DONE;
    if (s == "ERROR") return ForgotUtilsAction::ERROR;
    return ForgotUtilsAction::UNKNOWN;
}

bool room_forgot_utils_init(const RoomForgotUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = ForgotUtilsAction::INIT;
...
RoomForgotUtilsResponse room_forgot_utils_handle(const RoomForgotUtilsRequest& req) {
    RoomForgotUtilsResponse resp;
    ...
    g_status.state = ForgotUtilsAction::PROCESSING;
    // Validate request
...
RoomForgotUtilsStatus room_forgot_utils_getStatus() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_status;
}

void room_forgot_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = RoomForgotUtilsConfig{};
    g_status = RoomForgotUtilsStatus{};
    g_status.state = ForgotUtilsAction::UNKNOWN;
}
...
std::string room_forgot_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "room_forgot_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
RoomForgotUtilsStats room_forgot_utils_getStats() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_stats;
}

RoomForgotUtilsConfig room_forgot_utils_configFromJson(const json& j) {
    RoomForgotUtilsConfig config;
...
json room_forgot_utils_configToJson(const RoomForgotUtilsConfig& config) {
    json j;
