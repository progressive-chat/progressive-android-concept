...
namespace {
    WeatherUtilsConfig g_config;
    std::mutex g_mutex;
    WeatherUtilsStatus g_status;
    WeatherUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* weather_utils_stateToString(WeatherUtilsState s) {
    switch (s) {
        case WeatherUtilsState::UNKNOWN: return "UNKNOWN";
        case WeatherUtilsState::INIT: return "INIT";
        case WeatherUtilsState::PROCESSING: return "PROCESSING";
        case WeatherUtilsState::DONE: return "DONE";
        case WeatherUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

WeatherUtilsState weather_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return WeatherUtilsState::UNKNOWN;
    if (s == "INIT") return WeatherUtilsState::INIT;
    if (s == "PROCESSING") return WeatherUtilsState::PROCESSING;
    if (s == "DONE") return WeatherUtilsState::DONE;
    if (s == "ERROR") return WeatherUtilsState::ERROR;
    return WeatherUtilsState::UNKNOWN;
}
...
bool weather_utils_init(const WeatherUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = WeatherUtilsState::INIT;
...
WeatherUtilsResponse weather_utils_handle(const WeatherUtilsRequest& req) {
    ...
    g_status.state = WeatherUtilsState::PROCESSING;
    // Validate request
...
void weather_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = WeatherUtilsConfig{};
    g_status = WeatherUtilsStatus{};
    g_status.state = WeatherUtilsState::UNKNOWN;
}
