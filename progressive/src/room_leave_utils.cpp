...
namespace {
    RoomLeaveUtilsConfig g_config;
    std::mutex g_mutex;
    RoomLeaveUtilsStatus g_status;
    RoomLeaveUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* room_leave_utils_stateToString(LeaveUtilsAction s) {
    switch (s) {
        case LeaveUtilsAction::UNKNOWN: return "UNKNOWN";
        case LeaveUtilsAction::INIT: return "INIT";
        case LeaveUtilsAction::PROCESSING: return "PROCESSING";
        case LeaveUtilsAction::DONE: return "DONE";
        case LeaveUtilsAction::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

LeaveUtilsAction room_leave_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return LeaveUtilsAction::UNKNOWN;
    if (s == "INIT") return LeaveUtilsAction::INIT;
    if (s == "PROCESSING") return LeaveUtilsAction::PROCESSING;
    if (s == "DONE") return LeaveUtilsAction::DONE;
    if (s == "ERROR") return LeaveUtilsAction::ERROR;
    return LeaveUtilsAction::UNKNOWN;
}

bool room_leave_utils_init(const RoomLeaveUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = LeaveUtilsAction::INIT;
...
RoomLeaveUtilsResponse room_leave_utils_handle(const RoomLeaveUtilsRequest& req) {
    RoomLeaveUtilsResponse resp;
    ...
    g_status.state = LeaveUtilsAction::PROCESSING;
    // Validate request
...
RoomLeaveUtilsStatus room_leave_utils_getStatus() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_status;
}

void room_leave_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = RoomLeaveUtilsConfig{};
    g_status = RoomLeaveUtilsStatus{};
    g_status.state = LeaveUtilsAction::UNKNOWN;
}

bool room_leave_utils_validate(const std::string& input) {
...
std::string room_leave_utils_process(const std::string& input) {
    std::lock_guard<std::mutex> lock(g_mutex);
    ...
    try {
        ...
        result["module"] = "room_leave_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
json room_leave_utils_toJson(const std::string& input) {
    json j;
...
RoomLeaveUtilsStats room_leave_utils_getStats() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_stats;
}

RoomLeaveUtilsConfig room_leave_utils_configFromJson(const json& j) {
    RoomLeaveUtilsConfig config;
...
json room_leave_utils_configToJson(const RoomLeaveUtilsConfig& config) {
    json j;
