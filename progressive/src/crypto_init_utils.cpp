...
namespace {
    CryptoInitUtilsConfig g_config;
    std::mutex g_mutex;
    CryptoInitUtilsStatus g_status;
    CryptoInitUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* crypto_init_utils_stateToString(CryptoInitUtilsState s) {
    switch (s) {
        case CryptoInitUtilsState::UNKNOWN: return "UNKNOWN";
        case CryptoInitUtilsState::INIT: return "INIT";
        case CryptoInitUtilsState::PROCESSING: return "PROCESSING";
        case CryptoInitUtilsState::DONE: return "DONE";
        case CryptoInitUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

CryptoInitUtilsState crypto_init_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return CryptoInitUtilsState::UNKNOWN;
    if (s == "INIT") return CryptoInitUtilsState::INIT;
    if (s == "PROCESSING") return CryptoInitUtilsState::PROCESSING;
    if (s == "DONE") return CryptoInitUtilsState::DONE;
    if (s == "ERROR") return CryptoInitUtilsState::ERROR;
    return CryptoInitUtilsState::UNKNOWN;
}
...
bool crypto_init_utils_init(const CryptoInitUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = CryptoInitUtilsState::INIT;
...
CryptoInitUtilsResponse crypto_init_utils_handle(const CryptoInitUtilsRequest& req) {
    ...
    g_status.state = CryptoInitUtilsState::PROCESSING;
    // Validate request
...
void crypto_init_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = CryptoInitUtilsConfig{};
    g_status = CryptoInitUtilsStatus{};
    g_status.state = CryptoInitUtilsState::UNKNOWN;
}
...
std::string crypto_init_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "crypto_init_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
