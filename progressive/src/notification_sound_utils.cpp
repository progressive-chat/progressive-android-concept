...
namespace {
    NotificationSoundUtilsConfig g_config;
    std::mutex g_mutex;
    NotificationSoundUtilsStatus g_status;
    NotificationSoundUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* notification_sound_utils_stateToString(SoundUtilsLevel s) {
    switch (s) {
        case SoundUtilsLevel::UNKNOWN: return "UNKNOWN";
        case SoundUtilsLevel::INIT: return "INIT";
        case SoundUtilsLevel::PROCESSING: return "PROCESSING";
        case SoundUtilsLevel::DONE: return "DONE";
        case SoundUtilsLevel::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

SoundUtilsLevel notification_sound_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return SoundUtilsLevel::UNKNOWN;
    if (s == "INIT") return SoundUtilsLevel::INIT;
    if (s == "PROCESSING") return SoundUtilsLevel::PROCESSING;
    if (s == "DONE") return SoundUtilsLevel::DONE;
    if (s == "ERROR") return SoundUtilsLevel::ERROR;
    return SoundUtilsLevel::UNKNOWN;
}
...
bool notification_sound_utils_init(const NotificationSoundUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = SoundUtilsLevel::INIT;
...
NotificationSoundUtilsResponse notification_sound_utils_handle(const NotificationSoundUtilsRequest& req) {
    NotificationSoundUtilsResponse resp;
    ...
    g_status.state = SoundUtilsLevel::PROCESSING;
    // Validate request
...
void notification_sound_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = NotificationSoundUtilsConfig{};
    g_status = NotificationSoundUtilsStatus{};
    g_status.state = SoundUtilsLevel::UNKNOWN;
}
...
std::string notification_sound_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "notification_sound_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
NotificationSoundUtilsConfig notification_sound_utils_configFromJson(const json& j) {
    NotificationSoundUtilsConfig config;
