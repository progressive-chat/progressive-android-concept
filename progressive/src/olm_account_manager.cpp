...
namespace {
    OlmAccountManagerConfig g_config;
    std::mutex g_mutex;
    OlmAccountManagerStatus g_status;
    OlmAccountManagerStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* olm_account_manager_stateToString(AccountManagerStatus s) {
    switch (s) {
        case AccountManagerStatus::UNKNOWN: return "UNKNOWN";
        case AccountManagerStatus::INIT: return "INIT";
        case AccountManagerStatus::PROCESSING: return "PROCESSING";
        case AccountManagerStatus::DONE: return "DONE";
        case AccountManagerStatus::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

AccountManagerStatus olm_account_manager_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return AccountManagerStatus::UNKNOWN;
    if (s == "INIT") return AccountManagerStatus::INIT;
    if (s == "PROCESSING") return AccountManagerStatus::PROCESSING;
    if (s == "DONE") return AccountManagerStatus::DONE;
    if (s == "ERROR") return AccountManagerStatus::ERROR;
    return AccountManagerStatus::UNKNOWN;
}
...
bool olm_account_manager_init(const OlmAccountManagerConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = AccountManagerStatus::INIT;
...
OlmAccountManagerResponse olm_account_manager_handle(const OlmAccountManagerRequest& req) {
    OlmAccountManagerResponse resp;
    ...
    g_status.state = AccountManagerStatus::PROCESSING;
    // Validate request
...
void olm_account_manager_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = OlmAccountManagerConfig{};
    g_status = OlmAccountManagerStatus{};
    g_status.state = AccountManagerStatus::UNKNOWN;
}
...
std::string olm_account_manager_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "olm_account_manager";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
OlmAccountManagerConfig olm_account_manager_configFromJson(const json& j) {
    OlmAccountManagerConfig config;
