...
namespace {
    RoomMuteConfig g_config;
    std::mutex g_mutex;
    RoomMuteStatus g_status;
    RoomMuteStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* room_mute_stateToString(MuteAction s) {
    switch (s) {
        case MuteAction::UNKNOWN: return "UNKNOWN";
        case MuteAction::INIT: return "INIT";
        case MuteAction::PROCESSING: return "PROCESSING";
        case MuteAction::DONE: return "DONE";
        case MuteAction::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

MuteAction room_mute_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return MuteAction::UNKNOWN;
    if (s == "INIT") return MuteAction::INIT;
    if (s == "PROCESSING") return MuteAction::PROCESSING;
    if (s == "DONE") return MuteAction::DONE;
    if (s == "ERROR") return MuteAction::ERROR;
    return MuteAction::UNKNOWN;
}
...
bool room_mute_init(const RoomMuteConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = MuteAction::INIT;
...
RoomMuteResponse room_mute_handle(const RoomMuteRequest& req) {
    ...
    g_status.state = MuteAction::PROCESSING;
    // Validate request
...
void room_mute_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = RoomMuteConfig{};
    g_status = RoomMuteStatus{};
    g_status.state = MuteAction::UNKNOWN;
}
