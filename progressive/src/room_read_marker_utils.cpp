...
namespace {
    RoomReadMarkerUtilsConfig g_config;
    std::mutex g_mutex;
    RoomReadMarkerUtilsStatus g_status;
    RoomReadMarkerUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* room_read_marker_utils_stateToString(ReadMarkerUtilsAction s) {
    switch (s) {
        case ReadMarkerUtilsAction::UNKNOWN: return "UNKNOWN";
        case ReadMarkerUtilsAction::INIT: return "INIT";
        case ReadMarkerUtilsAction::PROCESSING: return "PROCESSING";
        case ReadMarkerUtilsAction::DONE: return "DONE";
        case ReadMarkerUtilsAction::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

ReadMarkerUtilsAction room_read_marker_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return ReadMarkerUtilsAction::UNKNOWN;
    if (s == "INIT") return ReadMarkerUtilsAction::INIT;
    if (s == "PROCESSING") return ReadMarkerUtilsAction::PROCESSING;
    if (s == "DONE") return ReadMarkerUtilsAction::DONE;
    if (s == "ERROR") return ReadMarkerUtilsAction::ERROR;
    return ReadMarkerUtilsAction::UNKNOWN;
}

bool room_read_marker_utils_init(const RoomReadMarkerUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = ReadMarkerUtilsAction::INIT;
...
RoomReadMarkerUtilsResponse room_read_marker_utils_handle(const RoomReadMarkerUtilsRequest& req) {
    RoomReadMarkerUtilsResponse resp;
    ...
    g_status.state = ReadMarkerUtilsAction::PROCESSING;
    // Validate request
...
RoomReadMarkerUtilsStatus room_read_marker_utils_getStatus() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_status;
}

void room_read_marker_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = RoomReadMarkerUtilsConfig{};
    g_status = RoomReadMarkerUtilsStatus{};
    g_status.state = ReadMarkerUtilsAction::UNKNOWN;
}
...
std::string room_read_marker_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "room_read_marker_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
RoomReadMarkerUtilsStats room_read_marker_utils_getStats() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_stats;
}

RoomReadMarkerUtilsConfig room_read_marker_utils_configFromJson(const json& j) {
    RoomReadMarkerUtilsConfig config;
...
json room_read_marker_utils_configToJson(const RoomReadMarkerUtilsConfig& config) {
    json j;
