...
namespace {
    MediaProgressUtilsConfig g_config;
    std::mutex g_mutex;
    MediaProgressUtilsStatus g_status;
    MediaProgressUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* media_progress_utils_stateToString(ProgressUtilsType s) {
    switch (s) {
        case ProgressUtilsType::UNKNOWN: return "UNKNOWN";
        case ProgressUtilsType::INIT: return "INIT";
        case ProgressUtilsType::PROCESSING: return "PROCESSING";
        case ProgressUtilsType::DONE: return "DONE";
        case ProgressUtilsType::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

ProgressUtilsType media_progress_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return ProgressUtilsType::UNKNOWN;
    if (s == "INIT") return ProgressUtilsType::INIT;
    if (s == "PROCESSING") return ProgressUtilsType::PROCESSING;
    if (s == "DONE") return ProgressUtilsType::DONE;
    if (s == "ERROR") return ProgressUtilsType::ERROR;
    return ProgressUtilsType::UNKNOWN;
}
...
bool media_progress_utils_init(const MediaProgressUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = ProgressUtilsType::INIT;
...
MediaProgressUtilsResponse media_progress_utils_handle(const MediaProgressUtilsRequest& req) {
    MediaProgressUtilsResponse resp;
    ...
    g_status.state = ProgressUtilsType::PROCESSING;
    // Validate request
...
void media_progress_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = MediaProgressUtilsConfig{};
    g_status = MediaProgressUtilsStatus{};
    g_status.state = ProgressUtilsType::UNKNOWN;
}
...
std::string media_progress_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "media_progress_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
MediaProgressUtilsConfig media_progress_utils_configFromJson(const json& j) {
    MediaProgressUtilsConfig config;
