...
namespace {
    VoiceMessageUtilsConfig g_config;
    std::mutex g_mutex;
    VoiceMessageUtilsStatus g_status;
    VoiceMessageUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* voice_message_utils_stateToString(VoiceMessageUtilsState s) {
    switch (s) {
        case VoiceMessageUtilsState::UNKNOWN: return "UNKNOWN";
        case VoiceMessageUtilsState::INIT: return "INIT";
        case VoiceMessageUtilsState::PROCESSING: return "PROCESSING";
        case VoiceMessageUtilsState::DONE: return "DONE";
        case VoiceMessageUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

VoiceMessageUtilsState voice_message_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return VoiceMessageUtilsState::UNKNOWN;
    if (s == "INIT") return VoiceMessageUtilsState::INIT;
    if (s == "PROCESSING") return VoiceMessageUtilsState::PROCESSING;
    if (s == "DONE") return VoiceMessageUtilsState::DONE;
    if (s == "ERROR") return VoiceMessageUtilsState::ERROR;
    return VoiceMessageUtilsState::UNKNOWN;
}
...
bool voice_message_utils_init(const VoiceMessageUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = VoiceMessageUtilsState::INIT;
...
VoiceMessageUtilsResponse voice_message_utils_handle(const VoiceMessageUtilsRequest& req) {
    VoiceMessageUtilsResponse resp;
    ...
    g_status.state = VoiceMessageUtilsState::PROCESSING;
    // Validate request
...
void voice_message_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = VoiceMessageUtilsConfig{};
    g_status = VoiceMessageUtilsStatus{};
    g_status.state = VoiceMessageUtilsState::UNKNOWN;
}
...
std::string voice_message_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "voice_message_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
VoiceMessageUtilsConfig voice_message_utils_configFromJson(const json& j) {
    VoiceMessageUtilsConfig config;
