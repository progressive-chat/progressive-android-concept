...
namespace {
    MediaScannerConfig g_config;
    std::mutex g_mutex;
    MediaScannerStatus g_status;
    MediaScannerStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* media_scanner_stateToString(ScannerType s) {
    switch (s) {
        case ScannerType::UNKNOWN: return "UNKNOWN";
        case ScannerType::INIT: return "INIT";
        case ScannerType::PROCESSING: return "PROCESSING";
        case ScannerType::DONE: return "DONE";
        case ScannerType::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

ScannerType media_scanner_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return ScannerType::UNKNOWN;
    if (s == "INIT") return ScannerType::INIT;
    if (s == "PROCESSING") return ScannerType::PROCESSING;
    if (s == "DONE") return ScannerType::DONE;
    if (s == "ERROR") return ScannerType::ERROR;
...
bool media_scanner_init(const MediaScannerConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = ScannerType::INIT;
...
MediaScannerResponse media_scanner_handle(const MediaScannerRequest& req) {
    ...
    g_status.state = ScannerType::PROCESSING;
    // Validate request
...
void media_scanner_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = MediaScannerConfig{};
    g_status = MediaScannerStatus{};
    g_status.state = ScannerType::UNKNOWN;
}
