...
namespace {
    MessageSearcherConfig g_config;
    std::mutex g_mutex;
    MessageSearcherStatus g_status;
    MessageSearcherStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* message_searcher_stateToString(SearcherPhase s) {
    switch (s) {
        case SearcherPhase::UNKNOWN: return "UNKNOWN";
        case SearcherPhase::INIT: return "INIT";
        case SearcherPhase::PROCESSING: return "PROCESSING";
        case SearcherPhase::DONE: return "DONE";
        case SearcherPhase::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

SearcherPhase message_searcher_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return SearcherPhase::UNKNOWN;
    if (s == "INIT") return SearcherPhase::INIT;
    if (s == "PROCESSING") return SearcherPhase::PROCESSING;
    if (s == "DONE") return SearcherPhase::DONE;
    if (s == "ERROR") return SearcherPhase::ERROR;
    return SearcherPhase::UNKNOWN;
}
...
bool message_searcher_init(const MessageSearcherConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = SearcherPhase::INIT;
...
MessageSearcherResponse message_searcher_handle(const MessageSearcherRequest& req) {
    ...
    g_status.state = SearcherPhase::PROCESSING;
    // Validate request
...
void message_searcher_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = MessageSearcherConfig{};
    g_status = MessageSearcherStatus{};
    g_status.state = SearcherPhase::UNKNOWN;
}
...
std::string message_searcher_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "message_searcher";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
