...
struct RoomInviteUtilsResponse {
    bool success = false;
...
struct RoomInviteUtilsStatus {
    InviteUtilsAction state = InviteUtilsAction::NONE;
...
// Initialize the room invite utils module with configuration
bool room_invite_utils_init(const RoomInviteUtilsConfig& config);

// Handle a room invite utils request, returning a response
