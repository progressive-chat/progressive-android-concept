...
namespace {
    RoomPermissionUtilsConfig g_config;
    std::mutex g_mutex;
    RoomPermissionUtilsStatus g_status;
    RoomPermissionUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* room_permission_utils_stateToString(PermissionUtilsAction s) {
    switch (s) {
        case PermissionUtilsAction::UNKNOWN: return "UNKNOWN";
        case PermissionUtilsAction::INIT: return "INIT";
        case PermissionUtilsAction::PROCESSING: return "PROCESSING";
        case PermissionUtilsAction::DONE: return "DONE";
        case PermissionUtilsAction::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

PermissionUtilsAction room_permission_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return PermissionUtilsAction::UNKNOWN;
    if (s == "INIT") return PermissionUtilsAction::INIT;
    if (s == "PROCESSING") return PermissionUtilsAction::PROCESSING;
    if (s == "DONE") return PermissionUtilsAction::DONE;
    if (s == "ERROR") return PermissionUtilsAction::ERROR;
    return PermissionUtilsAction::UNKNOWN;
}

bool room_permission_utils_init(const RoomPermissionUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = PermissionUtilsAction::INIT;
...
RoomPermissionUtilsResponse room_permission_utils_handle(const RoomPermissionUtilsRequest& req) {
    RoomPermissionUtilsResponse resp;
    ...
    g_status.state = PermissionUtilsAction::PROCESSING;
    // Validate request
...
RoomPermissionUtilsStatus room_permission_utils_getStatus() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_status;
}

void room_permission_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = RoomPermissionUtilsConfig{};
    g_status = RoomPermissionUtilsStatus{};
    g_status.state = PermissionUtilsAction::UNKNOWN;
}
...
std::string room_permission_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "room_permission_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
RoomPermissionUtilsStats room_permission_utils_getStats() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_stats;
}

RoomPermissionUtilsConfig room_permission_utils_configFromJson(const json& j) {
    RoomPermissionUtilsConfig config;
...
json room_permission_utils_configToJson(const RoomPermissionUtilsConfig& config) {
    json j;
