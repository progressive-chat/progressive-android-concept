...
namespace {
    RecoveryUtilsConfig g_config;
    std::mutex g_mutex;
    RecoveryUtilsStatus g_status;
    RecoveryUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* recovery_utils_stateToString(RecoveryUtilsState s) {
    switch (s) {
        case RecoveryUtilsState::UNKNOWN: return "UNKNOWN";
        case RecoveryUtilsState::INIT: return "INIT";
        case RecoveryUtilsState::PROCESSING: return "PROCESSING";
        case RecoveryUtilsState::DONE: return "DONE";
        case RecoveryUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

RecoveryUtilsState recovery_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return RecoveryUtilsState::UNKNOWN;
    if (s == "INIT") return RecoveryUtilsState::INIT;
    if (s == "PROCESSING") return RecoveryUtilsState::PROCESSING;
    if (s == "DONE") return RecoveryUtilsState::DONE;
    if (s == "ERROR") return RecoveryUtilsState::ERROR;
    return RecoveryUtilsState::UNKNOWN;
}
...
bool recovery_utils_init(const RecoveryUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = RecoveryUtilsState::INIT;
...
RecoveryUtilsResponse recovery_utils_handle(const RecoveryUtilsRequest& req) {
    ...
    g_status.state = RecoveryUtilsState::PROCESSING;
    // Validate request
...
void recovery_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = RecoveryUtilsConfig{};
    g_status = RecoveryUtilsStatus{};
    g_status.state = RecoveryUtilsState::UNKNOWN;
}
