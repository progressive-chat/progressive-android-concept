...
namespace {
    LoginUtilsConfig g_config;
    std::mutex g_mutex;
    LoginUtilsStatus g_status;
    LoginUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* login_utils_stateToString(LoginUtilsState s) {
    switch (s) {
        case LoginUtilsState::UNKNOWN: return "UNKNOWN";
        case LoginUtilsState::INIT: return "INIT";
        case LoginUtilsState::PROCESSING: return "PROCESSING";
        case LoginUtilsState::DONE: return "DONE";
        case LoginUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

LoginUtilsState login_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return LoginUtilsState::UNKNOWN;
    if (s == "INIT") return LoginUtilsState::INIT;
    if (s == "PROCESSING") return LoginUtilsState::PROCESSING;
    if (s == "DONE") return LoginUtilsState::DONE;
    if (s == "ERROR") return LoginUtilsState::ERROR;
    return LoginUtilsState::UNKNOWN;
}
...
bool login_utils_init(const LoginUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = LoginUtilsState::INIT;
...
LoginUtilsResponse login_utils_handle(const LoginUtilsRequest& req) {
    ...
    g_status.state = LoginUtilsState::PROCESSING;
    // Validate request
...
void login_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = LoginUtilsConfig{};
    g_status = LoginUtilsStatus{};
    g_status.state = LoginUtilsState::UNKNOWN;
}
