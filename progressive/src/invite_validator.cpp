...
namespace {
    InviteValidatorConfig g_config;
    std::mutex g_mutex;
    InviteValidatorStatus g_status;
    InviteValidatorStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* invite_validator_stateToString(InviteValidatorState s) {
    switch (s) {
        case InviteValidatorState::UNKNOWN: return "UNKNOWN";
        case InviteValidatorState::INIT: return "INIT";
        case InviteValidatorState::PROCESSING: return "PROCESSING";
        case InviteValidatorState::DONE: return "DONE";
        case InviteValidatorState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

InviteValidatorState invite_validator_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return InviteValidatorState::UNKNOWN;
    if (s == "INIT") return InviteValidatorState::INIT;
    if (s == "PROCESSING") return InviteValidatorState::PROCESSING;
    if (s == "DONE") return InviteValidatorState::DONE;
    if (s == "ERROR") return InviteValidatorState::ERROR;
    return InviteValidatorState::UNKNOWN;
}
...
bool invite_validator_init(const InviteValidatorConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = InviteValidatorState::INIT;
...
InviteValidatorResponse invite_validator_handle(const InviteValidatorRequest& req) {
    ...
    g_status.state = InviteValidatorState::PROCESSING;
    // Validate request
...
void invite_validator_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = InviteValidatorConfig{};
    g_status = InviteValidatorStatus{};
    g_status.state = InviteValidatorState::UNKNOWN;
}
...
std::string invite_validator_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "invite_validator";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
