...
namespace {
    LinkPreviewCacheConfig g_config;
    std::mutex g_mutex;
    LinkPreviewCacheStatus g_status;
    LinkPreviewCacheStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* link_preview_cache_stateToString(LinkPreviewCacheState s) {
    switch (s) {
        case LinkPreviewCacheState::UNKNOWN: return "UNKNOWN";
        case LinkPreviewCacheState::INIT: return "INIT";
        case LinkPreviewCacheState::PROCESSING: return "PROCESSING";
        case LinkPreviewCacheState::DONE: return "DONE";
        case LinkPreviewCacheState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

LinkPreviewCacheState link_preview_cache_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return LinkPreviewCacheState::UNKNOWN;
    if (s == "INIT") return LinkPreviewCacheState::INIT;
    if (s == "PROCESSING") return LinkPreviewCacheState::PROCESSING;
    if (s == "DONE") return LinkPreviewCacheState::DONE;
    if (s == "ERROR") return LinkPreviewCacheState::ERROR;
    return LinkPreviewCacheState::UNKNOWN;
}

bool link_preview_cache_init(const LinkPreviewCacheConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = LinkPreviewCacheState::INIT;
...
LinkPreviewCacheResponse link_preview_cache_handle(const LinkPreviewCacheRequest& req) {
    LinkPreviewCacheResponse resp;
    ...
    g_status.state = LinkPreviewCacheState::PROCESSING;
    // Validate request
...
LinkPreviewCacheStatus link_preview_cache_getStatus() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_status;
}

void link_preview_cache_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = LinkPreviewCacheConfig{};
    g_status = LinkPreviewCacheStatus{};
    g_status.state = LinkPreviewCacheState::UNKNOWN;
}
...
std::string link_preview_cache_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "link_preview_cache";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
LinkPreviewCacheStats link_preview_cache_getStats() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_stats;
}

LinkPreviewCacheConfig link_preview_cache_configFromJson(const json& j) {
    LinkPreviewCacheConfig config;
...
json link_preview_cache_configToJson(const LinkPreviewCacheConfig& config) {
    json j;
