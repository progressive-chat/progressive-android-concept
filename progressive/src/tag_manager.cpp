...
namespace {
    TagManagerConfig g_config;
    std::mutex g_mutex;
    TagManagerStatus g_status;
    TagManagerStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* tag_manager_stateToString(TagManagerState s) {
    switch (s) {
        case TagManagerState::UNKNOWN: return "UNKNOWN";
        case TagManagerState::INIT: return "INIT";
        case TagManagerState::PROCESSING: return "PROCESSING";
        case TagManagerState::DONE: return "DONE";
        case TagManagerState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

TagManagerState tag_manager_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return TagManagerState::UNKNOWN;
    if (s == "INIT") return TagManagerState::INIT;
    if (s == "PROCESSING") return TagManagerState::PROCESSING;
    if (s == "DONE") return TagManagerState::DONE;
    if (s == "ERROR") return TagManagerState::ERROR;
    return TagManagerState::UNKNOWN;
}
...
bool tag_manager_init(const TagManagerConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = TagManagerState::INIT;
...
TagManagerResponse tag_manager_handle(const TagManagerRequest& req) {
    ...
    g_status.state = TagManagerState::PROCESSING;
    // Validate request
...
void tag_manager_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = TagManagerConfig{};
    g_status = TagManagerStatus{};
    g_status.state = TagManagerState::UNKNOWN;
}
