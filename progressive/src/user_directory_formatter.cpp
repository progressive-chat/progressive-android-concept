...
namespace {
    UserDirectoryFormatterConfig g_config;
    std::mutex g_mutex;
    UserDirectoryFormatterStatus g_status;
    UserDirectoryFormatterStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* user_directory_formatter_stateToString(DirectoryFormatterState s) {
    switch (s) {
        case DirectoryFormatterState::UNKNOWN: return "UNKNOWN";
        case DirectoryFormatterState::INIT: return "INIT";
        case DirectoryFormatterState::PROCESSING: return "PROCESSING";
        case DirectoryFormatterState::DONE: return "DONE";
        case DirectoryFormatterState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

DirectoryFormatterState user_directory_formatter_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return DirectoryFormatterState::UNKNOWN;
    if (s == "INIT") return DirectoryFormatterState::INIT;
    if (s == "PROCESSING") return DirectoryFormatterState::PROCESSING;
    if (s == "DONE") return DirectoryFormatterState::DONE;
    if (s == "ERROR") return DirectoryFormatterState::ERROR;
    return DirectoryFormatterState::UNKNOWN;
}
...
bool user_directory_formatter_init(const UserDirectoryFormatterConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = DirectoryFormatterState::INIT;
...
UserDirectoryFormatterResponse user_directory_formatter_handle(const UserDirectoryFormatterRequest& req) {
    UserDirectoryFormatterResponse resp;
    ...
    g_status.state = DirectoryFormatterState::PROCESSING;
    // Validate request
...
void user_directory_formatter_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = UserDirectoryFormatterConfig{};
    g_status = UserDirectoryFormatterStatus{};
    g_status.state = DirectoryFormatterState::UNKNOWN;
}
...
std::string user_directory_formatter_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "user_directory_formatter";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
UserDirectoryFormatterConfig user_directory_formatter_configFromJson(const json& j) {
    UserDirectoryFormatterConfig config;
