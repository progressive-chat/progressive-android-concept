...
namespace {
    RoomPreviewerConfig g_config;
    std::mutex g_mutex;
    RoomPreviewerStatus g_status;
    RoomPreviewerStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* room_previewer_stateToString(PreviewerAction s) {
    switch (s) {
        case PreviewerAction::UNKNOWN: return "UNKNOWN";
        case PreviewerAction::INIT: return "INIT";
        case PreviewerAction::PROCESSING: return "PROCESSING";
        case PreviewerAction::DONE: return "DONE";
        case PreviewerAction::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

PreviewerAction room_previewer_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return PreviewerAction::UNKNOWN;
    if (s == "INIT") return PreviewerAction::INIT;
    if (s == "PROCESSING") return PreviewerAction::PROCESSING;
    if (s == "DONE") return PreviewerAction::DONE;
    if (s == "ERROR") return PreviewerAction::ERROR;
    return PreviewerAction::UNKNOWN;
}
...
bool room_previewer_init(const RoomPreviewerConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = PreviewerAction::INIT;
...
RoomPreviewerResponse room_previewer_handle(const RoomPreviewerRequest& req) {
    ...
    g_status.state = PreviewerAction::PROCESSING;
    // Validate request
...
void room_previewer_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = RoomPreviewerConfig{};
    g_status = RoomPreviewerStatus{};
    g_status.state = PreviewerAction::UNKNOWN;
}
