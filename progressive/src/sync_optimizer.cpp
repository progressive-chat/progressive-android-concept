...
namespace {
    SyncOptimizerConfig g_config;
    std::mutex g_mutex;
    SyncOptimizerStatus g_status;
    SyncOptimizerStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* sync_optimizer_stateToString(OptimizerPhase s) {
    switch (s) {
        case OptimizerPhase::UNKNOWN: return "UNKNOWN";
        case OptimizerPhase::INIT: return "INIT";
        case OptimizerPhase::PROCESSING: return "PROCESSING";
        case OptimizerPhase::DONE: return "DONE";
        case OptimizerPhase::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

OptimizerPhase sync_optimizer_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return OptimizerPhase::UNKNOWN;
    if (s == "INIT") return OptimizerPhase::INIT;
    if (s == "PROCESSING") return OptimizerPhase::PROCESSING;
    if (s == "DONE") return OptimizerPhase::DONE;
    if (s == "ERROR") return OptimizerPhase::ERROR;
    return OptimizerPhase::UNKNOWN;
}
...
bool sync_optimizer_init(const SyncOptimizerConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = OptimizerPhase::INIT;
...
SyncOptimizerResponse sync_optimizer_handle(const SyncOptimizerRequest& req) {
    ...
    g_status.state = OptimizerPhase::PROCESSING;
    // Validate request
...
void sync_optimizer_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = SyncOptimizerConfig{};
    g_status = SyncOptimizerStatus{};
    g_status.state = OptimizerPhase::UNKNOWN;
}
