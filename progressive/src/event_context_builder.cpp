...
namespace {
    EventContextBuilderConfig g_config;
    std::mutex g_mutex;
    EventContextBuilderStatus g_status;
    EventContextBuilderStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* event_context_builder_stateToString(ContextBuilderState s) {
    switch (s) {
        case ContextBuilderState::UNKNOWN: return "UNKNOWN";
        case ContextBuilderState::INIT: return "INIT";
        case ContextBuilderState::PROCESSING: return "PROCESSING";
        case ContextBuilderState::DONE: return "DONE";
        case ContextBuilderState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

ContextBuilderState event_context_builder_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return ContextBuilderState::UNKNOWN;
    if (s == "INIT") return ContextBuilderState::INIT;
    if (s == "PROCESSING") return ContextBuilderState::PROCESSING;
    if (s == "DONE") return ContextBuilderState::DONE;
    if (s == "ERROR") return ContextBuilderState::ERROR;
    return ContextBuilderState::UNKNOWN;
}
...
bool event_context_builder_init(const EventContextBuilderConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = ContextBuilderState::INIT;
...
EventContextBuilderResponse event_context_builder_handle(const EventContextBuilderRequest& req) {
    EventContextBuilderResponse resp;
    ...
    g_status.state = ContextBuilderState::PROCESSING;
    // Validate request
...
void event_context_builder_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = EventContextBuilderConfig{};
    g_status = EventContextBuilderStatus{};
    g_status.state = ContextBuilderState::UNKNOWN;
}
...
std::string event_context_builder_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "event_context_builder";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
EventContextBuilderConfig event_context_builder_configFromJson(const json& j) {
    EventContextBuilderConfig config;
