...
namespace {
    ReactionPickerUtilsConfig g_config;
    std::mutex g_mutex;
    ReactionPickerUtilsStatus g_status;
    ReactionPickerUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* reaction_picker_utils_stateToString(ReactionPickerUtilsState s) {
    switch (s) {
        case ReactionPickerUtilsState::UNKNOWN: return "UNKNOWN";
        case ReactionPickerUtilsState::INIT: return "INIT";
        case ReactionPickerUtilsState::PROCESSING: return "PROCESSING";
        case ReactionPickerUtilsState::DONE: return "DONE";
        case ReactionPickerUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

ReactionPickerUtilsState reaction_picker_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return ReactionPickerUtilsState::UNKNOWN;
    if (s == "INIT") return ReactionPickerUtilsState::INIT;
    if (s == "PROCESSING") return ReactionPickerUtilsState::PROCESSING;
    if (s == "DONE") return ReactionPickerUtilsState::DONE;
    if (s == "ERROR") return ReactionPickerUtilsState::ERROR;
    return ReactionPickerUtilsState::UNKNOWN;
}
...
bool reaction_picker_utils_init(const ReactionPickerUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = ReactionPickerUtilsState::INIT;
...
ReactionPickerUtilsResponse reaction_picker_utils_handle(const ReactionPickerUtilsRequest& req) {
    ReactionPickerUtilsResponse resp;
    ...
    g_status.state = ReactionPickerUtilsState::PROCESSING;
    // Validate request
...
void reaction_picker_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = ReactionPickerUtilsConfig{};
    g_status = ReactionPickerUtilsStatus{};
    g_status.state = ReactionPickerUtilsState::UNKNOWN;
}
...
std::string reaction_picker_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "reaction_picker_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
ReactionPickerUtilsConfig reaction_picker_utils_configFromJson(const json& j) {
    ReactionPickerUtilsConfig config;
