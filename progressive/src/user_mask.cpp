...
namespace {
    UserMaskConfig g_config;
    std::mutex g_mutex;
    UserMaskStatus g_status;
    UserMaskStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* user_mask_stateToString(MaskState s) {
    switch (s) {
        case MaskState::UNKNOWN: return "UNKNOWN";
        case MaskState::INIT: return "INIT";
        case MaskState::PROCESSING: return "PROCESSING";
        case MaskState::DONE: return "DONE";
        case MaskState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

MaskState user_mask_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return MaskState::UNKNOWN;
    if (s == "INIT") return MaskState::INIT;
    if (s == "PROCESSING") return MaskState::PROCESSING;
    if (s == "DONE") return MaskState::DONE;
    if (s == "ERROR") return MaskState::ERROR;
    return MaskState::UNKNOWN;
}
...
UserMaskResponse user_mask_handle(const UserMaskRequest& req) {
    ...
    g_status.state = MaskState::PROCESSING;
    // Validate request
...
void user_mask_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = UserMaskConfig{};
    g_status = UserMaskStatus{};
    g_status.state = MaskState::UNKNOWN;
}
