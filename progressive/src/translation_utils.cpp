...
namespace {
    TranslationUtilsConfig g_config;
    std::mutex g_mutex;
    TranslationUtilsStatus g_status;
    TranslationUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* translation_utils_stateToString(TranslationUtilsState s) {
    switch (s) {
        case TranslationUtilsState::UNKNOWN: return "UNKNOWN";
        case TranslationUtilsState::INIT: return "INIT";
        case TranslationUtilsState::PROCESSING: return "PROCESSING";
        case TranslationUtilsState::DONE: return "DONE";
        case TranslationUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

TranslationUtilsState translation_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return TranslationUtilsState::UNKNOWN;
    if (s == "INIT") return TranslationUtilsState::INIT;
    if (s == "PROCESSING") return TranslationUtilsState::PROCESSING;
    if (s == "DONE") return TranslationUtilsState::DONE;
    if (s == "ERROR") return TranslationUtilsState::ERROR;
    return TranslationUtilsState::UNKNOWN;
}
...
bool translation_utils_init(const TranslationUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = TranslationUtilsState::INIT;
...
TranslationUtilsResponse translation_utils_handle(const TranslationUtilsRequest& req) {
    ...
    g_status.state = TranslationUtilsState::PROCESSING;
    // Validate request
...
void translation_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = TranslationUtilsConfig{};
    g_status = TranslationUtilsStatus{};
    g_status.state = TranslationUtilsState::UNKNOWN;
}
...
std::string translation_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "translation_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
