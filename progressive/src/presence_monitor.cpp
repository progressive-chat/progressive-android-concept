...
namespace {
    PresenceMonitorConfig g_config;
    std::mutex g_mutex;
    PresenceMonitorStatus g_status;
    PresenceMonitorStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* presence_monitor_stateToString(PresenceMonitorState s) {
    switch (s) {
        case PresenceMonitorState::UNKNOWN: return "UNKNOWN";
        case PresenceMonitorState::INIT: return "INIT";
        case PresenceMonitorState::PROCESSING: return "PROCESSING";
        case PresenceMonitorState::DONE: return "DONE";
        case PresenceMonitorState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

PresenceMonitorState presence_monitor_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return PresenceMonitorState::UNKNOWN;
    if (s == "INIT") return PresenceMonitorState::INIT;
    if (s == "PROCESSING") return PresenceMonitorState::PROCESSING;
    if (s == "DONE") return PresenceMonitorState::DONE;
    if (s == "ERROR") return PresenceMonitorState::ERROR;
    return PresenceMonitorState::UNKNOWN;
}
...
bool presence_monitor_init(const PresenceMonitorConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = PresenceMonitorState::INIT;
...
PresenceMonitorResponse presence_monitor_handle(const PresenceMonitorRequest& req) {
    ...
    g_status.state = PresenceMonitorState::PROCESSING;
    // Validate request
...
void presence_monitor_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = PresenceMonitorConfig{};
    g_status = PresenceMonitorStatus{};
    g_status.state = PresenceMonitorState::UNKNOWN;
}
...
std::string presence_monitor_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "presence_monitor";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
