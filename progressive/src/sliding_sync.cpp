...
namespace {
    SlidingSyncConfig g_config;
    std::mutex g_mutex;
    SlidingSyncStatus g_status;
    SlidingSyncStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* sliding_sync_stateToString(SlidingSyncState s) {
    switch (s) {
        case SlidingSyncState::UNKNOWN: return "UNKNOWN";
        case SlidingSyncState::INIT: return "INIT";
        case SlidingSyncState::PROCESSING: return "PROCESSING";
        case SlidingSyncState::DONE: return "DONE";
        case SlidingSyncState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

SlidingSyncState sliding_sync_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return SlidingSyncState::UNKNOWN;
    if (s == "INIT") return SlidingSyncState::INIT;
    if (s == "PROCESSING") return SlidingSyncState::PROCESSING;
    if (s == "DONE") return SlidingSyncState::DONE;
    if (s == "ERROR") return SlidingSyncState::ERROR;
    return SlidingSyncState::UNKNOWN;
}
...
bool sliding_sync_init(const SlidingSyncConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = SlidingSyncState::INIT;
...
SlidingSyncResponse sliding_sync_handle(const SlidingSyncRequest& req) {
    ...
    g_status.state = SlidingSyncState::PROCESSING;
    // Validate request
...
void sliding_sync_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = SlidingSyncConfig{};
    g_status = SlidingSyncStatus{};
    g_status.state = SlidingSyncState::UNKNOWN;
}
