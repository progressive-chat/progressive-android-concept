...
namespace {
    RoomAccessControlConfig g_config;
    std::mutex g_mutex;
    RoomAccessControlStatus g_status;
    RoomAccessControlStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* room_access_control_stateToString(AccessControlAction s) {
    switch (s) {
        case AccessControlAction::UNKNOWN: return "UNKNOWN";
        case AccessControlAction::INIT: return "INIT";
        case AccessControlAction::PROCESSING: return "PROCESSING";
        case AccessControlAction::DONE: return "DONE";
        case AccessControlAction::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

AccessControlAction room_access_control_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return AccessControlAction::UNKNOWN;
    if (s == "INIT") return AccessControlAction::INIT;
    if (s == "PROCESSING") return AccessControlAction::PROCESSING;
    if (s == "DONE") return AccessControlAction::DONE;
    if (s == "ERROR") return AccessControlAction::ERROR;
...
bool room_access_control_init(const RoomAccessControlConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = AccessControlAction::INIT;
...
RoomAccessControlResponse room_access_control_handle(const RoomAccessControlRequest& req) {
    RoomAccessControlResponse resp;
    ...
    g_status.state = AccessControlAction::PROCESSING;
    // Validate request
...
void room_access_control_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = RoomAccessControlConfig{};
    g_status = RoomAccessControlStatus{};
    g_status.state = AccessControlAction::UNKNOWN;
}
...
std::string room_access_control_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "room_access_control";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
RoomAccessControlConfig room_access_control_configFromJson(const json& j) {
    RoomAccessControlConfig config;
