...
namespace {
    CallDurationConfig g_config;
    std::mutex g_mutex;
    CallDurationStatus g_status;
    CallDurationStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* call_duration_stateToString(DurationPhase s) {
    switch (s) {
        case DurationPhase::UNKNOWN: return "UNKNOWN";
        case DurationPhase::INIT: return "INIT";
        case DurationPhase::PROCESSING: return "PROCESSING";
        case DurationPhase::DONE: return "DONE";
        case DurationPhase::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

DurationPhase call_duration_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return DurationPhase::UNKNOWN;
    if (s == "INIT") return DurationPhase::INIT;
    if (s == "PROCESSING") return DurationPhase::PROCESSING;
    if (s == "DONE") return DurationPhase::DONE;
    if (s == "ERROR") return DurationPhase::ERROR;
    return DurationPhase::UNKNOWN;
}
...
bool call_duration_init(const CallDurationConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = DurationPhase::INIT;
...
CallDurationResponse call_duration_handle(const CallDurationRequest& req) {
    ...
    g_status.state = DurationPhase::PROCESSING;
    // Validate request
...
void call_duration_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = CallDurationConfig{};
    g_status = CallDurationStatus{};
    g_status.state = DurationPhase::UNKNOWN;
}
