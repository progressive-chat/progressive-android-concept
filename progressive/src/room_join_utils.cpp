...
namespace {
    RoomJoinUtilsConfig g_config;
    std::mutex g_mutex;
    RoomJoinUtilsStatus g_status;
    RoomJoinUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* room_join_utils_stateToString(JoinUtilsAction s) {
    switch (s) {
        case JoinUtilsAction::UNKNOWN: return "UNKNOWN";
        case JoinUtilsAction::INIT: return "INIT";
        case JoinUtilsAction::PROCESSING: return "PROCESSING";
        case JoinUtilsAction::DONE: return "DONE";
        case JoinUtilsAction::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

JoinUtilsAction room_join_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return JoinUtilsAction::UNKNOWN;
    if (s == "INIT") return JoinUtilsAction::INIT;
    if (s == "PROCESSING") return JoinUtilsAction::PROCESSING;
    if (s == "DONE") return JoinUtilsAction::DONE;
    if (s == "ERROR") return JoinUtilsAction::ERROR;
    return JoinUtilsAction::UNKNOWN;
}

bool room_join_utils_init(const RoomJoinUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = JoinUtilsAction::INIT;
...
RoomJoinUtilsResponse room_join_utils_handle(const RoomJoinUtilsRequest& req) {
    RoomJoinUtilsResponse resp;
    ...
    g_status.state = JoinUtilsAction::PROCESSING;
    // Validate request
...
RoomJoinUtilsStatus room_join_utils_getStatus() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_status;
}

void room_join_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = RoomJoinUtilsConfig{};
    g_status = RoomJoinUtilsStatus{};
    g_status.state = JoinUtilsAction::UNKNOWN;
}
...
RoomJoinUtilsStats room_join_utils_getStats() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_stats;
}

RoomJoinUtilsConfig room_join_utils_configFromJson(const json& j) {
    RoomJoinUtilsConfig config;
...
json room_join_utils_configToJson(const RoomJoinUtilsConfig& config) {
    json j;
