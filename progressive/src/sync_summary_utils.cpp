...
namespace {
    SyncSummaryUtilsConfig g_config;
    std::mutex g_mutex;
    SyncSummaryUtilsStatus g_status;
    SyncSummaryUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* sync_summary_utils_stateToString(SummaryUtilsPhase s) {
    switch (s) {
        case SummaryUtilsPhase::UNKNOWN: return "UNKNOWN";
        case SummaryUtilsPhase::INIT: return "INIT";
        case SummaryUtilsPhase::PROCESSING: return "PROCESSING";
        case SummaryUtilsPhase::DONE: return "DONE";
        case SummaryUtilsPhase::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

SummaryUtilsPhase sync_summary_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return SummaryUtilsPhase::UNKNOWN;
    if (s == "INIT") return SummaryUtilsPhase::INIT;
    if (s == "PROCESSING") return SummaryUtilsPhase::PROCESSING;
    if (s == "DONE") return SummaryUtilsPhase::DONE;
    if (s == "ERROR") return SummaryUtilsPhase::ERROR;
    return SummaryUtilsPhase::UNKNOWN;
}
...
bool sync_summary_utils_init(const SyncSummaryUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = SummaryUtilsPhase::INIT;
...
SyncSummaryUtilsResponse sync_summary_utils_handle(const SyncSummaryUtilsRequest& req) {
    ...
    g_status.state = SummaryUtilsPhase::PROCESSING;
    // Validate request
...
void sync_summary_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = SyncSummaryUtilsConfig{};
    g_status = SyncSummaryUtilsStatus{};
    g_status.state = SummaryUtilsPhase::UNKNOWN;
}
...
std::string sync_summary_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "sync_summary_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
