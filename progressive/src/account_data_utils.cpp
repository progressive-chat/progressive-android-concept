...
namespace {
    AccountDataUtilsConfig g_config;
    std::mutex g_mutex;
    AccountDataUtilsStatus g_status;
    AccountDataUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* account_data_utils_stateToString(AccountDataUtilsState s) {
    switch (s) {
        case AccountDataUtilsState::UNKNOWN: return "UNKNOWN";
        case AccountDataUtilsState::INIT: return "INIT";
        case AccountDataUtilsState::PROCESSING: return "PROCESSING";
        case AccountDataUtilsState::DONE: return "DONE";
        case AccountDataUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

AccountDataUtilsState account_data_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return AccountDataUtilsState::UNKNOWN;
    if (s == "INIT") return AccountDataUtilsState::INIT;
    if (s == "PROCESSING") return AccountDataUtilsState::PROCESSING;
    if (s == "DONE") return AccountDataUtilsState::DONE;
    if (s == "ERROR") return AccountDataUtilsState::ERROR;
    return AccountDataUtilsState::UNKNOWN;
}
...
bool account_data_utils_init(const AccountDataUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = AccountDataUtilsState::INIT;
...
AccountDataUtilsResponse account_data_utils_handle(const AccountDataUtilsRequest& req) {
    ...
    g_status.state = AccountDataUtilsState::PROCESSING;
    // Validate request
...
void account_data_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = AccountDataUtilsConfig{};
    g_status = AccountDataUtilsStatus{};
    g_status.state = AccountDataUtilsState::UNKNOWN;
}
...
std::string account_data_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "account_data_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
