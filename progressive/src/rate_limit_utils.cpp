...
namespace {
    RateLimitUtilsConfig g_config;
    std::mutex g_mutex;
    RateLimitUtilsStatus g_status;
    RateLimitUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* rate_limit_utils_stateToString(RateLimitUtilsState s) {
    switch (s) {
        case RateLimitUtilsState::UNKNOWN: return "UNKNOWN";
        case RateLimitUtilsState::INIT: return "INIT";
        case RateLimitUtilsState::PROCESSING: return "PROCESSING";
        case RateLimitUtilsState::DONE: return "DONE";
        case RateLimitUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

RateLimitUtilsState rate_limit_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return RateLimitUtilsState::UNKNOWN;
    if (s == "INIT") return RateLimitUtilsState::INIT;
    if (s == "PROCESSING") return RateLimitUtilsState::PROCESSING;
    if (s == "DONE") return RateLimitUtilsState::DONE;
    if (s == "ERROR") return RateLimitUtilsState::ERROR;
    return RateLimitUtilsState::UNKNOWN;
}
...
bool rate_limit_utils_init(const RateLimitUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = RateLimitUtilsState::INIT;
...
RateLimitUtilsResponse rate_limit_utils_handle(const RateLimitUtilsRequest& req) {
    ...
    g_status.state = RateLimitUtilsState::PROCESSING;
    // Validate request
...
void rate_limit_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = RateLimitUtilsConfig{};
    g_status = RateLimitUtilsStatus{};
    g_status.state = RateLimitUtilsState::UNKNOWN;
}
...
std::string rate_limit_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "rate_limit_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
