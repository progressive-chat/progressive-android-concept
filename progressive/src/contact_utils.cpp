...
namespace {
    ContactUtilsConfig g_config;
    std::mutex g_mutex;
    ContactUtilsStatus g_status;
    ContactUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* contact_utils_stateToString(ContactUtilsState s) {
    switch (s) {
        case ContactUtilsState::UNKNOWN: return "UNKNOWN";
        case ContactUtilsState::INIT: return "INIT";
        case ContactUtilsState::PROCESSING: return "PROCESSING";
        case ContactUtilsState::DONE: return "DONE";
        case ContactUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

ContactUtilsState contact_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return ContactUtilsState::UNKNOWN;
    if (s == "INIT") return ContactUtilsState::INIT;
    if (s == "PROCESSING") return ContactUtilsState::PROCESSING;
    if (s == "DONE") return ContactUtilsState::DONE;
    if (s == "ERROR") return ContactUtilsState::ERROR;
    return ContactUtilsState::UNKNOWN;
}
...
bool contact_utils_init(const ContactUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = ContactUtilsState::INIT;
...
ContactUtilsResponse contact_utils_handle(const ContactUtilsRequest& req) {
    ...
    g_status.state = ContactUtilsState::PROCESSING;
    // Validate request
...
void contact_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = ContactUtilsConfig{};
    g_status = ContactUtilsStatus{};
    g_status.state = ContactUtilsState::UNKNOWN;
}
