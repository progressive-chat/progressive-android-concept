...
namespace {
    CacheInvalidatorConfig g_config;
    std::mutex g_mutex;
    CacheInvalidatorStatus g_status;
    CacheInvalidatorStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* cache_invalidator_stateToString(CacheInvalidatorState s) {
    switch (s) {
        case CacheInvalidatorState::UNKNOWN: return "UNKNOWN";
        case CacheInvalidatorState::INIT: return "INIT";
        case CacheInvalidatorState::PROCESSING: return "PROCESSING";
        case CacheInvalidatorState::DONE: return "DONE";
        case CacheInvalidatorState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

CacheInvalidatorState cache_invalidator_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return CacheInvalidatorState::UNKNOWN;
    if (s == "INIT") return CacheInvalidatorState::INIT;
    if (s == "PROCESSING") return CacheInvalidatorState::PROCESSING;
    if (s == "DONE") return CacheInvalidatorState::DONE;
    if (s == "ERROR") return CacheInvalidatorState::ERROR;
    return CacheInvalidatorState::UNKNOWN;
}
...
bool cache_invalidator_init(const CacheInvalidatorConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = CacheInvalidatorState::INIT;
...
CacheInvalidatorResponse cache_invalidator_handle(const CacheInvalidatorRequest& req) {
    ...
    g_status.state = CacheInvalidatorState::PROCESSING;
    // Validate request
...
void cache_invalidator_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = CacheInvalidatorConfig{};
    g_status = CacheInvalidatorStatus{};
    g_status.state = CacheInvalidatorState::UNKNOWN;
}
...
std::string cache_invalidator_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "cache_invalidator";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
