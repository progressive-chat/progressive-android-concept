...
namespace {
    MessageSearchConfig g_config;
    std::mutex g_mutex;
    MessageSearchStatus g_status;
    MessageSearchStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* message_search_stateToString(SearchPhase s) {
    switch (s) {
        case SearchPhase::UNKNOWN: return "UNKNOWN";
        case SearchPhase::INIT: return "INIT";
        case SearchPhase::PROCESSING: return "PROCESSING";
        case SearchPhase::DONE: return "DONE";
        case SearchPhase::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

SearchPhase message_search_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return SearchPhase::UNKNOWN;
    if (s == "INIT") return SearchPhase::INIT;
    if (s == "PROCESSING") return SearchPhase::PROCESSING;
    if (s == "DONE") return SearchPhase::DONE;
    if (s == "ERROR") return SearchPhase::ERROR;
    return SearchPhase::UNKNOWN;
}
...
bool message_search_init(const MessageSearchConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = SearchPhase::INIT;
...
MessageSearchResponse message_search_handle(const MessageSearchRequest& req) {
    ...
    g_status.state = SearchPhase::PROCESSING;
    // Validate request
...
void message_search_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = MessageSearchConfig{};
    g_status = MessageSearchStatus{};
    g_status.state = SearchPhase::UNKNOWN;
}
