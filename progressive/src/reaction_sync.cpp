...
namespace {
    ReactionSyncConfig g_config;
    std::mutex g_mutex;
    ReactionSyncStatus g_status;
    ReactionSyncStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* reaction_sync_stateToString(ReactionSyncState s) {
    switch (s) {
        case ReactionSyncState::UNKNOWN: return "UNKNOWN";
        case ReactionSyncState::INIT: return "INIT";
        case ReactionSyncState::PROCESSING: return "PROCESSING";
        case ReactionSyncState::DONE: return "DONE";
        case ReactionSyncState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

ReactionSyncState reaction_sync_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return ReactionSyncState::UNKNOWN;
    if (s == "INIT") return ReactionSyncState::INIT;
    if (s == "PROCESSING") return ReactionSyncState::PROCESSING;
    if (s == "DONE") return ReactionSyncState::DONE;
    if (s == "ERROR") return ReactionSyncState::ERROR;
    return ReactionSyncState::UNKNOWN;
}
...
bool reaction_sync_init(const ReactionSyncConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = ReactionSyncState::INIT;
...
ReactionSyncResponse reaction_sync_handle(const ReactionSyncRequest& req) {
    ...
    g_status.state = ReactionSyncState::PROCESSING;
    // Validate request
...
void reaction_sync_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = ReactionSyncConfig{};
    g_status = ReactionSyncStatus{};
    g_status.state = ReactionSyncState::UNKNOWN;
}
