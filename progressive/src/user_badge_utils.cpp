...
namespace {
    UserBadgeUtilsConfig g_config;
    std::mutex g_mutex;
    UserBadgeUtilsStatus g_status;
    UserBadgeUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* user_badge_utils_stateToString(BadgeUtilsState s) {
    switch (s) {
        case BadgeUtilsState::UNKNOWN: return "UNKNOWN";
        case BadgeUtilsState::INIT: return "INIT";
        case BadgeUtilsState::PROCESSING: return "PROCESSING";
        case BadgeUtilsState::DONE: return "DONE";
        case BadgeUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

BadgeUtilsState user_badge_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return BadgeUtilsState::UNKNOWN;
    if (s == "INIT") return BadgeUtilsState::INIT;
    if (s == "PROCESSING") return BadgeUtilsState::PROCESSING;
    if (s == "DONE") return BadgeUtilsState::DONE;
    if (s == "ERROR") return BadgeUtilsState::ERROR;
    return BadgeUtilsState::UNKNOWN;
}
...
bool user_badge_utils_init(const UserBadgeUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = BadgeUtilsState::INIT;
...
UserBadgeUtilsResponse user_badge_utils_handle(const UserBadgeUtilsRequest& req) {
    ...
    g_status.state = BadgeUtilsState::PROCESSING;
    // Validate request
...
void user_badge_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = UserBadgeUtilsConfig{};
    g_status = UserBadgeUtilsStatus{};
    g_status.state = BadgeUtilsState::UNKNOWN;
}
...
std::string user_badge_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "user_badge_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
