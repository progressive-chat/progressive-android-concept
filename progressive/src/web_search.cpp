...
namespace {
    WebSearchConfig g_config;
    std::mutex g_mutex;
    WebSearchStatus g_status;
    WebSearchStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* web_search_stateToString(WebSearchState s) {
    switch (s) {
        case WebSearchState::UNKNOWN: return "UNKNOWN";
        case WebSearchState::INIT: return "INIT";
        case WebSearchState::PROCESSING: return "PROCESSING";
        case WebSearchState::DONE: return "DONE";
        case WebSearchState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

WebSearchState web_search_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return WebSearchState::UNKNOWN;
    if (s == "INIT") return WebSearchState::INIT;
    if (s == "PROCESSING") return WebSearchState::PROCESSING;
    if (s == "DONE") return WebSearchState::DONE;
    if (s == "ERROR") return WebSearchState::ERROR;
    return WebSearchState::UNKNOWN;
}
...
bool web_search_init(const WebSearchConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = WebSearchState::INIT;
...
WebSearchResponse web_search_handle(const WebSearchRequest& req) {
    ...
    g_status.state = WebSearchState::PROCESSING;
    // Validate request
...
void web_search_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = WebSearchConfig{};
    g_status = WebSearchStatus{};
    g_status.state = WebSearchState::UNKNOWN;
}
