...
namespace {
    NotificationActionUtilsConfig g_config;
    std::mutex g_mutex;
    NotificationActionUtilsStatus g_status;
    NotificationActionUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* notification_action_utils_stateToString(ActionUtilsLevel s) {
    switch (s) {
        case ActionUtilsLevel::UNKNOWN: return "UNKNOWN";
        case ActionUtilsLevel::INIT: return "INIT";
        case ActionUtilsLevel::PROCESSING: return "PROCESSING";
        case ActionUtilsLevel::DONE: return "DONE";
        case ActionUtilsLevel::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

ActionUtilsLevel notification_action_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return ActionUtilsLevel::UNKNOWN;
    if (s == "INIT") return ActionUtilsLevel::INIT;
    if (s == "PROCESSING") return ActionUtilsLevel::PROCESSING;
    if (s == "DONE") return ActionUtilsLevel::DONE;
    if (s == "ERROR") return ActionUtilsLevel::ERROR;
    return ActionUtilsLevel::UNKNOWN;
}
...
bool notification_action_utils_init(const NotificationActionUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = ActionUtilsLevel::INIT;
...
NotificationActionUtilsResponse notification_action_utils_handle(const NotificationActionUtilsRequest& req) {
    NotificationActionUtilsResponse resp;
    ...
    g_status.state = ActionUtilsLevel::PROCESSING;
    // Validate request
...
void notification_action_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = NotificationActionUtilsConfig{};
    g_status = NotificationActionUtilsStatus{};
    g_status.state = ActionUtilsLevel::UNKNOWN;
}
...
std::string notification_action_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "notification_action_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
NotificationActionUtilsConfig notification_action_utils_configFromJson(const json& j) {
    NotificationActionUtilsConfig config;
