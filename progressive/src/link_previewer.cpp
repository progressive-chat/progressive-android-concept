...
namespace {
    LinkPreviewerConfig g_config;
    std::mutex g_mutex;
    LinkPreviewerStatus g_status;
    LinkPreviewerStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* link_previewer_stateToString(LinkPreviewerState s) {
    switch (s) {
        case LinkPreviewerState::UNKNOWN: return "UNKNOWN";
        case LinkPreviewerState::INIT: return "INIT";
        case LinkPreviewerState::PROCESSING: return "PROCESSING";
        case LinkPreviewerState::DONE: return "DONE";
        case LinkPreviewerState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

LinkPreviewerState link_previewer_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return LinkPreviewerState::UNKNOWN;
    if (s == "INIT") return LinkPreviewerState::INIT;
    if (s == "PROCESSING") return LinkPreviewerState::PROCESSING;
    if (s == "DONE") return LinkPreviewerState::DONE;
    if (s == "ERROR") return LinkPreviewerState::ERROR;
    return LinkPreviewerState::UNKNOWN;
}
...
bool link_previewer_init(const LinkPreviewerConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = LinkPreviewerState::INIT;
...
LinkPreviewerResponse link_previewer_handle(const LinkPreviewerRequest& req) {
    ...
    g_status.state = LinkPreviewerState::PROCESSING;
    // Validate request
...
void link_previewer_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = LinkPreviewerConfig{};
    g_status = LinkPreviewerStatus{};
    g_status.state = LinkPreviewerState::UNKNOWN;
}
