...
namespace {
    SyncSchedulerConfig g_config;
    std::mutex g_mutex;
    SyncSchedulerStatus g_status;
    SyncSchedulerStats g_stats;
...
void sync_scheduler_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = SyncSchedulerConfig{};
    g_status = SyncSchedulerStatus{};
