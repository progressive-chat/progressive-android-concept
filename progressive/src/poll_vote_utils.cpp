...
namespace {
    PollVoteUtilsConfig g_config;
    std::mutex g_mutex;
    PollVoteUtilsStatus g_status;
    PollVoteUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* poll_vote_utils_stateToString(PollVoteUtilsState s) {
    switch (s) {
        case PollVoteUtilsState::UNKNOWN: return "UNKNOWN";
        case PollVoteUtilsState::INIT: return "INIT";
        case PollVoteUtilsState::PROCESSING: return "PROCESSING";
        case PollVoteUtilsState::DONE: return "DONE";
        case PollVoteUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

PollVoteUtilsState poll_vote_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return PollVoteUtilsState::UNKNOWN;
    if (s == "INIT") return PollVoteUtilsState::INIT;
    if (s == "PROCESSING") return PollVoteUtilsState::PROCESSING;
    if (s == "DONE") return PollVoteUtilsState::DONE;
    if (s == "ERROR") return PollVoteUtilsState::ERROR;
    return PollVoteUtilsState::UNKNOWN;
}
...
bool poll_vote_utils_init(const PollVoteUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = PollVoteUtilsState::INIT;
...
PollVoteUtilsResponse poll_vote_utils_handle(const PollVoteUtilsRequest& req) {
    ...
    g_status.state = PollVoteUtilsState::PROCESSING;
    // Validate request
...
void poll_vote_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = PollVoteUtilsConfig{};
    g_status = PollVoteUtilsStatus{};
    g_status.state = PollVoteUtilsState::UNKNOWN;
}
