...
namespace {
    KeyShareConfig g_config;
    std::mutex g_mutex;
    KeyShareStatus g_status;
    KeyShareStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* key_share_stateToString(ShareStatus s) {
    switch (s) {
        case ShareStatus::UNKNOWN: return "UNKNOWN";
        case ShareStatus::INIT: return "INIT";
        case ShareStatus::PROCESSING: return "PROCESSING";
        case ShareStatus::DONE: return "DONE";
        case ShareStatus::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

ShareStatus key_share_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return ShareStatus::UNKNOWN;
    if (s == "INIT") return ShareStatus::INIT;
    if (s == "PROCESSING") return ShareStatus::PROCESSING;
    if (s == "DONE") return ShareStatus::DONE;
    if (s == "ERROR") return ShareStatus::ERROR;
    return ShareStatus::UNKNOWN;
}
...
bool key_share_init(const KeyShareConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = ShareStatus::INIT;
...
KeyShareResponse key_share_handle(const KeyShareRequest& req) {
    ...
    g_status.state = ShareStatus::PROCESSING;
    // Validate request
...
void key_share_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = KeyShareConfig{};
    g_status = KeyShareStatus{};
    g_status.state = ShareStatus::UNKNOWN;
}
