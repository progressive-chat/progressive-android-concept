...
namespace {
    MatrixErrorUtilsConfig g_config;
    std::mutex g_mutex;
    MatrixErrorUtilsStatus g_status;
    MatrixErrorUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* matrix_error_utils_stateToString(MatrixErrorUtilsState s) {
    switch (s) {
        case MatrixErrorUtilsState::UNKNOWN: return "UNKNOWN";
        case MatrixErrorUtilsState::INIT: return "INIT";
        case MatrixErrorUtilsState::PROCESSING: return "PROCESSING";
        case MatrixErrorUtilsState::DONE: return "DONE";
        case MatrixErrorUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

MatrixErrorUtilsState matrix_error_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return MatrixErrorUtilsState::UNKNOWN;
    if (s == "INIT") return MatrixErrorUtilsState::INIT;
    if (s == "PROCESSING") return MatrixErrorUtilsState::PROCESSING;
    if (s == "DONE") return MatrixErrorUtilsState::DONE;
    if (s == "ERROR") return MatrixErrorUtilsState::ERROR;
    return MatrixErrorUtilsState::UNKNOWN;
}
...
bool matrix_error_utils_init(const MatrixErrorUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = MatrixErrorUtilsState::INIT;
...
MatrixErrorUtilsResponse matrix_error_utils_handle(const MatrixErrorUtilsRequest& req) {
    ...
    g_status.state = MatrixErrorUtilsState::PROCESSING;
    // Validate request
...
void matrix_error_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = MatrixErrorUtilsConfig{};
    g_status = MatrixErrorUtilsStatus{};
    g_status.state = MatrixErrorUtilsState::UNKNOWN;
}
...
std::string matrix_error_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "matrix_error_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
