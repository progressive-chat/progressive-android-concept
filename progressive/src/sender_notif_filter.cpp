...
namespace {
    SenderNotifFilterConfig g_config;
    std::mutex g_mutex;
    SenderNotifFilterStatus g_status;
    SenderNotifFilterStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* sender_notif_filter_stateToString(SenderNotifFilterState s) {
    switch (s) {
        case SenderNotifFilterState::UNKNOWN: return "UNKNOWN";
        case SenderNotifFilterState::INIT: return "INIT";
        case SenderNotifFilterState::PROCESSING: return "PROCESSING";
        case SenderNotifFilterState::DONE: return "DONE";
        case SenderNotifFilterState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

SenderNotifFilterState sender_notif_filter_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return SenderNotifFilterState::UNKNOWN;
    if (s == "INIT") return SenderNotifFilterState::INIT;
    if (s == "PROCESSING") return SenderNotifFilterState::PROCESSING;
    if (s == "DONE") return SenderNotifFilterState::DONE;
    if (s == "ERROR") return SenderNotifFilterState::ERROR;
    return SenderNotifFilterState::UNKNOWN;
}
...
bool sender_notif_filter_init(const SenderNotifFilterConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = SenderNotifFilterState::INIT;
...
SenderNotifFilterResponse sender_notif_filter_handle(const SenderNotifFilterRequest& req) {
    SenderNotifFilterResponse resp;
    ...
    g_status.state = SenderNotifFilterState::PROCESSING;
    // Validate request
...
void sender_notif_filter_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = SenderNotifFilterConfig{};
    g_status = SenderNotifFilterStatus{};
    g_status.state = SenderNotifFilterState::UNKNOWN;
}
...
std::string sender_notif_filter_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "sender_notif_filter";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
SenderNotifFilterConfig sender_notif_filter_configFromJson(const json& j) {
    SenderNotifFilterConfig config;
