...
namespace {
    RoomMuteSchedulerConfig g_config;
    std::mutex g_mutex;
    RoomMuteSchedulerStatus g_status;
    RoomMuteSchedulerStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* room_mute_scheduler_stateToString(MuteSchedulerAction s) {
    switch (s) {
        case MuteSchedulerAction::UNKNOWN: return "UNKNOWN";
        case MuteSchedulerAction::INIT: return "INIT";
        case MuteSchedulerAction::PROCESSING: return "PROCESSING";
        case MuteSchedulerAction::DONE: return "DONE";
        case MuteSchedulerAction::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

MuteSchedulerAction room_mute_scheduler_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return MuteSchedulerAction::UNKNOWN;
    if (s == "INIT") return MuteSchedulerAction::INIT;
    if (s == "PROCESSING") return MuteSchedulerAction::PROCESSING;
    if (s == "DONE") return MuteSchedulerAction::DONE;
    if (s == "ERROR") return MuteSchedulerAction::ERROR;
    return MuteSchedulerAction::UNKNOWN;
}
...
bool room_mute_scheduler_init(const RoomMuteSchedulerConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = MuteSchedulerAction::INIT;
...
RoomMuteSchedulerResponse room_mute_scheduler_handle(const RoomMuteSchedulerRequest& req) {
    RoomMuteSchedulerResponse resp;
    ...
    g_status.state = MuteSchedulerAction::PROCESSING;
    // Validate request
...
void room_mute_scheduler_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = RoomMuteSchedulerConfig{};
    g_status = RoomMuteSchedulerStatus{};
    g_status.state = MuteSchedulerAction::UNKNOWN;
}
...
std::string room_mute_scheduler_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "room_mute_scheduler";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
RoomMuteSchedulerConfig room_mute_scheduler_configFromJson(const json& j) {
    RoomMuteSchedulerConfig config;
