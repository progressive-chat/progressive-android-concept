...
namespace {
    OlmSessionUtilsConfig g_config;
    std::mutex g_mutex;
    OlmSessionUtilsStatus g_status;
    OlmSessionUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* olm_session_utils_stateToString(SessionUtilsStatus s) {
    switch (s) {
        case SessionUtilsStatus::UNKNOWN: return "UNKNOWN";
        case SessionUtilsStatus::INIT: return "INIT";
        case SessionUtilsStatus::PROCESSING: return "PROCESSING";
        case SessionUtilsStatus::DONE: return "DONE";
        case SessionUtilsStatus::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

SessionUtilsStatus olm_session_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return SessionUtilsStatus::UNKNOWN;
    if (s == "INIT") return SessionUtilsStatus::INIT;
    if (s == "PROCESSING") return SessionUtilsStatus::PROCESSING;
    if (s == "DONE") return SessionUtilsStatus::DONE;
    if (s == "ERROR") return SessionUtilsStatus::ERROR;
    return SessionUtilsStatus::UNKNOWN;
}
...
bool olm_session_utils_init(const OlmSessionUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = SessionUtilsStatus::INIT;
...
OlmSessionUtilsResponse olm_session_utils_handle(const OlmSessionUtilsRequest& req) {
    ...
    g_status.state = SessionUtilsStatus::PROCESSING;
    // Validate request
...
void olm_session_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = OlmSessionUtilsConfig{};
    g_status = OlmSessionUtilsStatus{};
    g_status.state = SessionUtilsStatus::UNKNOWN;
}
...
std::string olm_session_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "olm_session_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
