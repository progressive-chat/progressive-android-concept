...
namespace {
    LanguageSwitcherConfig g_config;
    std::mutex g_mutex;
    LanguageSwitcherStatus g_status;
    LanguageSwitcherStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* language_switcher_stateToString(LanguageSwitcherState s) {
    switch (s) {
        case LanguageSwitcherState::UNKNOWN: return "UNKNOWN";
        case LanguageSwitcherState::INIT: return "INIT";
        case LanguageSwitcherState::PROCESSING: return "PROCESSING";
        case LanguageSwitcherState::DONE: return "DONE";
        case LanguageSwitcherState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

LanguageSwitcherState language_switcher_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return LanguageSwitcherState::UNKNOWN;
    if (s == "INIT") return LanguageSwitcherState::INIT;
    if (s == "PROCESSING") return LanguageSwitcherState::PROCESSING;
    if (s == "DONE") return LanguageSwitcherState::DONE;
    if (s == "ERROR") return LanguageSwitcherState::ERROR;
    return LanguageSwitcherState::UNKNOWN;
}
...
bool language_switcher_init(const LanguageSwitcherConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = LanguageSwitcherState::INIT;
...
LanguageSwitcherResponse language_switcher_handle(const LanguageSwitcherRequest& req) {
    ...
    g_status.state = LanguageSwitcherState::PROCESSING;
    // Validate request
...
void language_switcher_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = LanguageSwitcherConfig{};
    g_status = LanguageSwitcherStatus{};
    g_status.state = LanguageSwitcherState::UNKNOWN;
}
...
std::string language_switcher_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "language_switcher";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
