...
namespace {
    PushConditionConfig g_config;
    std::mutex g_mutex;
    PushConditionStatus g_status;
    PushConditionStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* push_condition_stateToString(ConditionKind s) {
    switch (s) {
        case ConditionKind::UNKNOWN: return "UNKNOWN";
        case ConditionKind::INIT: return "INIT";
        case ConditionKind::PROCESSING: return "PROCESSING";
        case ConditionKind::DONE: return "DONE";
        case ConditionKind::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

ConditionKind push_condition_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return ConditionKind::UNKNOWN;
    if (s == "INIT") return ConditionKind::INIT;
    if (s == "PROCESSING") return ConditionKind::PROCESSING;
    if (s == "DONE") return ConditionKind::DONE;
    if (s == "ERROR") return ConditionKind::ERROR;
...
bool push_condition_init(const PushConditionConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = ConditionKind::INIT;
...
PushConditionResponse push_condition_handle(const PushConditionRequest& req) {
    ...
    g_status.state = ConditionKind::PROCESSING;
    // Validate request
...
void push_condition_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = PushConditionConfig{};
    g_status = PushConditionStatus{};
    g_status.state = ConditionKind::UNKNOWN;
}
