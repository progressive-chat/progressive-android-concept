...
namespace {
    MediaThumbnailerConfig g_config;
    std::mutex g_mutex;
    MediaThumbnailerStatus g_status;
    MediaThumbnailerStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* media_thumbnailer_stateToString(ThumbnailerType s) {
    switch (s) {
        case ThumbnailerType::UNKNOWN: return "UNKNOWN";
        case ThumbnailerType::INIT: return "INIT";
        case ThumbnailerType::PROCESSING: return "PROCESSING";
        case ThumbnailerType::DONE: return "DONE";
        case ThumbnailerType::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

ThumbnailerType media_thumbnailer_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return ThumbnailerType::UNKNOWN;
    if (s == "INIT") return ThumbnailerType::INIT;
    if (s == "PROCESSING") return ThumbnailerType::PROCESSING;
    if (s == "DONE") return ThumbnailerType::DONE;
    if (s == "ERROR") return ThumbnailerType::ERROR;
    return ThumbnailerType::UNKNOWN;
}
...
bool media_thumbnailer_init(const MediaThumbnailerConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = ThumbnailerType::INIT;
...
MediaThumbnailerResponse media_thumbnailer_handle(const MediaThumbnailerRequest& req) {
    ...
    g_status.state = ThumbnailerType::PROCESSING;
    // Validate request
...
void media_thumbnailer_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = MediaThumbnailerConfig{};
    g_status = MediaThumbnailerStatus{};
    g_status.state = ThumbnailerType::UNKNOWN;
}
...
std::string media_thumbnailer_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "media_thumbnailer";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
