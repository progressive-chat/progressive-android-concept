...
namespace {
    RoomParentUtilsConfig g_config;
    std::mutex g_mutex;
    RoomParentUtilsStatus g_status;
    RoomParentUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* room_parent_utils_stateToString(ParentUtilsAction s) {
    switch (s) {
        case ParentUtilsAction::UNKNOWN: return "UNKNOWN";
        case ParentUtilsAction::INIT: return "INIT";
        case ParentUtilsAction::PROCESSING: return "PROCESSING";
        case ParentUtilsAction::DONE: return "DONE";
        case ParentUtilsAction::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

ParentUtilsAction room_parent_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return ParentUtilsAction::UNKNOWN;
    if (s == "INIT") return ParentUtilsAction::INIT;
    if (s == "PROCESSING") return ParentUtilsAction::PROCESSING;
    if (s == "DONE") return ParentUtilsAction::DONE;
    if (s == "ERROR") return ParentUtilsAction::ERROR;
    return ParentUtilsAction::UNKNOWN;
}
...
bool room_parent_utils_init(const RoomParentUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = ParentUtilsAction::INIT;
...
RoomParentUtilsResponse room_parent_utils_handle(const RoomParentUtilsRequest& req) {
    ...
    g_status.state = ParentUtilsAction::PROCESSING;
    // Validate request
...
void room_parent_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = RoomParentUtilsConfig{};
    g_status = RoomParentUtilsStatus{};
    g_status.state = ParentUtilsAction::UNKNOWN;
}
...
std::string room_parent_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "room_parent_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
