...
namespace {
    EncryptionStatusConfig g_config;
    std::mutex g_mutex;
    EncryptionStatusStatus g_status;
    EncryptionStatusStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* encryption_status_stateToString(StatusMode s) {
    switch (s) {
        case StatusMode::UNKNOWN: return "UNKNOWN";
        case StatusMode::INIT: return "INIT";
        case StatusMode::PROCESSING: return "PROCESSING";
        case StatusMode::DONE: return "DONE";
        case StatusMode::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

StatusMode encryption_status_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return StatusMode::UNKNOWN;
    if (s == "INIT") return StatusMode::INIT;
    if (s == "PROCESSING") return StatusMode::PROCESSING;
    if (s == "DONE") return StatusMode::DONE;
    if (s == "ERROR") return StatusMode::ERROR;
...
bool encryption_status_init(const EncryptionStatusConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = StatusMode::INIT;
...
EncryptionStatusResponse encryption_status_handle(const EncryptionStatusRequest& req) {
    ...
    g_status.state = StatusMode::PROCESSING;
    // Validate request
...
void encryption_status_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = EncryptionStatusConfig{};
    g_status = EncryptionStatusStatus{};
    g_status.state = StatusMode::UNKNOWN;
}
...
std::string encryption_status_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "encryption_status";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
