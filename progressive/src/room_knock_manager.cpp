...
namespace {
    RoomKnockManagerConfig g_config;
    std::mutex g_mutex;
    RoomKnockManagerStatus g_status;
    RoomKnockManagerStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* room_knock_manager_stateToString(KnockManagerAction s) {
    switch (s) {
        case KnockManagerAction::UNKNOWN: return "UNKNOWN";
        case KnockManagerAction::INIT: return "INIT";
        case KnockManagerAction::PROCESSING: return "PROCESSING";
        case KnockManagerAction::DONE: return "DONE";
        case KnockManagerAction::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

KnockManagerAction room_knock_manager_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return KnockManagerAction::UNKNOWN;
    if (s == "INIT") return KnockManagerAction::INIT;
    if (s == "PROCESSING") return KnockManagerAction::PROCESSING;
    if (s == "DONE") return KnockManagerAction::DONE;
    if (s == "ERROR") return KnockManagerAction::ERROR;
    return KnockManagerAction::UNKNOWN;
}
...
bool room_knock_manager_init(const RoomKnockManagerConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = KnockManagerAction::INIT;
...
RoomKnockManagerResponse room_knock_manager_handle(const RoomKnockManagerRequest& req) {
    ...
    g_status.state = KnockManagerAction::PROCESSING;
    // Validate request
...
void room_knock_manager_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = RoomKnockManagerConfig{};
    g_status = RoomKnockManagerStatus{};
    g_status.state = KnockManagerAction::UNKNOWN;
}
...
std::string room_knock_manager_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "room_knock_manager";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
