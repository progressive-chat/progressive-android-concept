...
namespace {
    AdminUtilsConfig g_config;
    std::mutex g_mutex;
    AdminUtilsStatus g_status;
    AdminUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* admin_utils_stateToString(AdminUtilsState s) {
    switch (s) {
        case AdminUtilsState::UNKNOWN: return "UNKNOWN";
        case AdminUtilsState::INIT: return "INIT";
        case AdminUtilsState::PROCESSING: return "PROCESSING";
        case AdminUtilsState::DONE: return "DONE";
        case AdminUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

AdminUtilsState admin_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return AdminUtilsState::UNKNOWN;
    if (s == "INIT") return AdminUtilsState::INIT;
    if (s == "PROCESSING") return AdminUtilsState::PROCESSING;
    if (s == "DONE") return AdminUtilsState::DONE;
    if (s == "ERROR") return AdminUtilsState::ERROR;
    return AdminUtilsState::UNKNOWN;
}

bool admin_utils_init(const AdminUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = AdminUtilsState::INIT;
...
AdminUtilsResponse admin_utils_handle(const AdminUtilsRequest& req) {
    AdminUtilsResponse resp;
    ...
    g_status.state = AdminUtilsState::PROCESSING;
    // Validate request
...
AdminUtilsStatus admin_utils_getStatus() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_status;
}

void admin_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = AdminUtilsConfig{};
    g_status = AdminUtilsStatus{};
    g_status.state = AdminUtilsState::UNKNOWN;
}
...
AdminUtilsStats admin_utils_getStats() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_stats;
}

AdminUtilsConfig admin_utils_configFromJson(const json& j) {
    AdminUtilsConfig config;
...
json admin_utils_configToJson(const AdminUtilsConfig& config) {
    json j;
