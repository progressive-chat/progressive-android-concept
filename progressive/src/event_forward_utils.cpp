...
namespace {
    EventForwardUtilsConfig g_config;
    std::mutex g_mutex;
    EventForwardUtilsStatus g_status;
    EventForwardUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* event_forward_utils_stateToString(ForwardUtilsState s) {
    switch (s) {
        case ForwardUtilsState::UNKNOWN: return "UNKNOWN";
        case ForwardUtilsState::INIT: return "INIT";
        case ForwardUtilsState::PROCESSING: return "PROCESSING";
        case ForwardUtilsState::DONE: return "DONE";
        case ForwardUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

ForwardUtilsState event_forward_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return ForwardUtilsState::UNKNOWN;
    if (s == "INIT") return ForwardUtilsState::INIT;
    if (s == "PROCESSING") return ForwardUtilsState::PROCESSING;
    if (s == "DONE") return ForwardUtilsState::DONE;
    if (s == "ERROR") return ForwardUtilsState::ERROR;
    return ForwardUtilsState::UNKNOWN;
}
...
bool event_forward_utils_init(const EventForwardUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = ForwardUtilsState::INIT;
...
EventForwardUtilsResponse event_forward_utils_handle(const EventForwardUtilsRequest& req) {
    EventForwardUtilsResponse resp;
    ...
    g_status.state = ForwardUtilsState::PROCESSING;
    // Validate request
...
void event_forward_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = EventForwardUtilsConfig{};
    g_status = EventForwardUtilsStatus{};
    g_status.state = ForwardUtilsState::UNKNOWN;
}
...
std::string event_forward_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "event_forward_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
EventForwardUtilsConfig event_forward_utils_configFromJson(const json& j) {
    EventForwardUtilsConfig config;
