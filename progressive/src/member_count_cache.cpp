...
namespace {
    MemberCountCacheConfig g_config;
    std::mutex g_mutex;
    MemberCountCacheStatus g_status;
    MemberCountCacheStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* member_count_cache_stateToString(MemberCountCacheState s) {
    switch (s) {
        case MemberCountCacheState::UNKNOWN: return "UNKNOWN";
        case MemberCountCacheState::INIT: return "INIT";
        case MemberCountCacheState::PROCESSING: return "PROCESSING";
        case MemberCountCacheState::DONE: return "DONE";
        case MemberCountCacheState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

MemberCountCacheState member_count_cache_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return MemberCountCacheState::UNKNOWN;
    if (s == "INIT") return MemberCountCacheState::INIT;
    if (s == "PROCESSING") return MemberCountCacheState::PROCESSING;
    if (s == "DONE") return MemberCountCacheState::DONE;
    if (s == "ERROR") return MemberCountCacheState::ERROR;
    return MemberCountCacheState::UNKNOWN;
}

bool member_count_cache_init(const MemberCountCacheConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = MemberCountCacheState::INIT;
...
MemberCountCacheResponse member_count_cache_handle(const MemberCountCacheRequest& req) {
    MemberCountCacheResponse resp;
    ...
    g_status.state = MemberCountCacheState::PROCESSING;
    // Validate request
...
MemberCountCacheStatus member_count_cache_getStatus() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_status;
}

void member_count_cache_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = MemberCountCacheConfig{};
    g_status = MemberCountCacheStatus{};
    g_status.state = MemberCountCacheState::UNKNOWN;
}
...
std::string member_count_cache_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "member_count_cache";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
MemberCountCacheStats member_count_cache_getStats() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_stats;
}

MemberCountCacheConfig member_count_cache_configFromJson(const json& j) {
    MemberCountCacheConfig config;
...
json member_count_cache_configToJson(const MemberCountCacheConfig& config) {
    json j;
