...
namespace {
    CanonicalJsonConfig g_config;
    std::mutex g_mutex;
    CanonicalJsonStatus g_status;
    CanonicalJsonStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* canonical_json_stateToString(CanonicalJsonState s) {
    switch (s) {
        case CanonicalJsonState::UNKNOWN: return "UNKNOWN";
        case CanonicalJsonState::INIT: return "INIT";
        case CanonicalJsonState::PROCESSING: return "PROCESSING";
        case CanonicalJsonState::DONE: return "DONE";
        case CanonicalJsonState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

CanonicalJsonState canonical_json_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return CanonicalJsonState::UNKNOWN;
    if (s == "INIT") return CanonicalJsonState::INIT;
    if (s == "PROCESSING") return CanonicalJsonState::PROCESSING;
    if (s == "DONE") return CanonicalJsonState::DONE;
    if (s == "ERROR") return CanonicalJsonState::ERROR;
    return CanonicalJsonState::UNKNOWN;
}
...
bool canonical_json_init(const CanonicalJsonConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = CanonicalJsonState::INIT;
...
CanonicalJsonResponse canonical_json_handle(const CanonicalJsonRequest& req) {
    ...
    g_status.state = CanonicalJsonState::PROCESSING;
    // Validate request
...
void canonical_json_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = CanonicalJsonConfig{};
    g_status = CanonicalJsonStatus{};
    g_status.state = CanonicalJsonState::UNKNOWN;
}
