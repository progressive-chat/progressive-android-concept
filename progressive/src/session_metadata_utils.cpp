...
namespace {
    SessionMetadataUtilsConfig g_config;
    std::mutex g_mutex;
    SessionMetadataUtilsStatus g_status;
    SessionMetadataUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* session_metadata_utils_stateToString(MetadataUtilsPhase s) {
    switch (s) {
        case MetadataUtilsPhase::UNKNOWN: return "UNKNOWN";
        case MetadataUtilsPhase::INIT: return "INIT";
        case MetadataUtilsPhase::PROCESSING: return "PROCESSING";
        case MetadataUtilsPhase::DONE: return "DONE";
        case MetadataUtilsPhase::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

MetadataUtilsPhase session_metadata_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return MetadataUtilsPhase::UNKNOWN;
    if (s == "INIT") return MetadataUtilsPhase::INIT;
    if (s == "PROCESSING") return MetadataUtilsPhase::PROCESSING;
    if (s == "DONE") return MetadataUtilsPhase::DONE;
    if (s == "ERROR") return MetadataUtilsPhase::ERROR;
    return MetadataUtilsPhase::UNKNOWN;
}
...
bool session_metadata_utils_init(const SessionMetadataUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = MetadataUtilsPhase::INIT;
...
SessionMetadataUtilsResponse session_metadata_utils_handle(const SessionMetadataUtilsRequest& req) {
    SessionMetadataUtilsResponse resp;
    ...
    g_status.state = MetadataUtilsPhase::PROCESSING;
    // Validate request
...
void session_metadata_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = SessionMetadataUtilsConfig{};
    g_status = SessionMetadataUtilsStatus{};
    g_status.state = MetadataUtilsPhase::UNKNOWN;
}
...
std::string session_metadata_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "session_metadata_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
SessionMetadataUtilsConfig session_metadata_utils_configFromJson(const json& j) {
    SessionMetadataUtilsConfig config;
