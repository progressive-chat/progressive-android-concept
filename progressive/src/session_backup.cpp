...
namespace {
    SessionBackupConfig g_config;
    std::mutex g_mutex;
    SessionBackupStatus g_status;
    SessionBackupStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* session_backup_stateToString(BackupPhase s) {
    switch (s) {
        case BackupPhase::UNKNOWN: return "UNKNOWN";
        case BackupPhase::INIT: return "INIT";
        case BackupPhase::PROCESSING: return "PROCESSING";
        case BackupPhase::DONE: return "DONE";
        case BackupPhase::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

BackupPhase session_backup_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return BackupPhase::UNKNOWN;
    if (s == "INIT") return BackupPhase::INIT;
    if (s == "PROCESSING") return BackupPhase::PROCESSING;
    if (s == "DONE") return BackupPhase::DONE;
    if (s == "ERROR") return BackupPhase::ERROR;
    return BackupPhase::UNKNOWN;
}
...
bool session_backup_init(const SessionBackupConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = BackupPhase::INIT;
...
SessionBackupResponse session_backup_handle(const SessionBackupRequest& req) {
    ...
    g_status.state = BackupPhase::PROCESSING;
    // Validate request
...
void session_backup_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = SessionBackupConfig{};
    g_status = SessionBackupStatus{};
    g_status.state = BackupPhase::UNKNOWN;
}
