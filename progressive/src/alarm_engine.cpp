...
namespace {
    AlarmEngineConfig g_config;
    std::mutex g_mutex;
    AlarmEngineStatus g_status;
    AlarmEngineStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* alarm_engine_stateToString(AlarmEngineState s) {
    switch (s) {
        case AlarmEngineState::UNKNOWN: return "UNKNOWN";
        case AlarmEngineState::INIT: return "INIT";
        case AlarmEngineState::PROCESSING: return "PROCESSING";
        case AlarmEngineState::DONE: return "DONE";
        case AlarmEngineState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

AlarmEngineState alarm_engine_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return AlarmEngineState::UNKNOWN;
    if (s == "INIT") return AlarmEngineState::INIT;
    if (s == "PROCESSING") return AlarmEngineState::PROCESSING;
    if (s == "DONE") return AlarmEngineState::DONE;
    if (s == "ERROR") return AlarmEngineState::ERROR;
    return AlarmEngineState::UNKNOWN;
}
...
bool alarm_engine_init(const AlarmEngineConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = AlarmEngineState::INIT;
...
AlarmEngineResponse alarm_engine_handle(const AlarmEngineRequest& req) {
    ...
    g_status.state = AlarmEngineState::PROCESSING;
    // Validate request
...
void alarm_engine_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = AlarmEngineConfig{};
    g_status = AlarmEngineStatus{};
    g_status.state = AlarmEngineState::UNKNOWN;
}
