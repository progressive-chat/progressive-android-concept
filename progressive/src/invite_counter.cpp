...
namespace {
    InviteCounterConfig g_config;
    std::mutex g_mutex;
    InviteCounterStatus g_status;
    InviteCounterStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* invite_counter_stateToString(InviteCounterState s) {
    switch (s) {
        case InviteCounterState::UNKNOWN: return "UNKNOWN";
        case InviteCounterState::INIT: return "INIT";
        case InviteCounterState::PROCESSING: return "PROCESSING";
        case InviteCounterState::DONE: return "DONE";
        case InviteCounterState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

InviteCounterState invite_counter_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return InviteCounterState::UNKNOWN;
    if (s == "INIT") return InviteCounterState::INIT;
    if (s == "PROCESSING") return InviteCounterState::PROCESSING;
    if (s == "DONE") return InviteCounterState::DONE;
    if (s == "ERROR") return InviteCounterState::ERROR;
    return InviteCounterState::UNKNOWN;
}
...
bool invite_counter_init(const InviteCounterConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = InviteCounterState::INIT;
...
InviteCounterResponse invite_counter_handle(const InviteCounterRequest& req) {
    ...
    g_status.state = InviteCounterState::PROCESSING;
    // Validate request
...
void invite_counter_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = InviteCounterConfig{};
    g_status = InviteCounterStatus{};
    g_status.state = InviteCounterState::UNKNOWN;
}
