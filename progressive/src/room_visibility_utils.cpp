...
namespace {
    RoomVisibilityUtilsConfig g_config;
    std::mutex g_mutex;
    RoomVisibilityUtilsStatus g_status;
    RoomVisibilityUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* room_visibility_utils_stateToString(VisibilityUtilsAction s) {
    switch (s) {
        case VisibilityUtilsAction::UNKNOWN: return "UNKNOWN";
        case VisibilityUtilsAction::INIT: return "INIT";
        case VisibilityUtilsAction::PROCESSING: return "PROCESSING";
        case VisibilityUtilsAction::DONE: return "DONE";
        case VisibilityUtilsAction::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

VisibilityUtilsAction room_visibility_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return VisibilityUtilsAction::UNKNOWN;
    if (s == "INIT") return VisibilityUtilsAction::INIT;
    if (s == "PROCESSING") return VisibilityUtilsAction::PROCESSING;
    if (s == "DONE") return VisibilityUtilsAction::DONE;
    if (s == "ERROR") return VisibilityUtilsAction::ERROR;
    return VisibilityUtilsAction::UNKNOWN;
}

bool room_visibility_utils_init(const RoomVisibilityUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = VisibilityUtilsAction::INIT;
...
RoomVisibilityUtilsResponse room_visibility_utils_handle(const RoomVisibilityUtilsRequest& req) {
    RoomVisibilityUtilsResponse resp;
    ...
    g_status.state = VisibilityUtilsAction::PROCESSING;
    // Validate request
...
RoomVisibilityUtilsStatus room_visibility_utils_getStatus() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_status;
}

void room_visibility_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = RoomVisibilityUtilsConfig{};
    g_status = RoomVisibilityUtilsStatus{};
    g_status.state = VisibilityUtilsAction::UNKNOWN;
}
...
std::string room_visibility_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "room_visibility_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
RoomVisibilityUtilsStats room_visibility_utils_getStats() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_stats;
}

RoomVisibilityUtilsConfig room_visibility_utils_configFromJson(const json& j) {
    RoomVisibilityUtilsConfig config;
...
json room_visibility_utils_configToJson(const RoomVisibilityUtilsConfig& config) {
    json j;
