...
namespace {
    ...
    std::mutex g_mutex;
    CrashReporterStatus g_status;
    CrashReporterStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* crash_reporter_stateToString(CrashReporterState s) {
    switch (s) {
        case CrashReporterState::UNKNOWN: return "UNKNOWN";
        case CrashReporterState::INIT: return "INIT";
        case CrashReporterState::PROCESSING: return "PROCESSING";
        case CrashReporterState::DONE: return "DONE";
        case CrashReporterState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

CrashReporterState crash_reporter_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return CrashReporterState::UNKNOWN;
    if (s == "INIT") return CrashReporterState::INIT;
    if (s == "PROCESSING") return CrashReporterState::PROCESSING;
    if (s == "DONE") return CrashReporterState::DONE;
    if (s == "ERROR") return CrashReporterState::ERROR;
    return CrashReporterState::UNKNOWN;
}
...
bool crash_reporter_init(const CrashReporterConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = CrashReporterState::INIT;
...
CrashReporterResponse crash_reporter_handle(const CrashReporterRequest& req) {
    ...
    g_status.state = CrashReporterState::PROCESSING;
    // Validate request
...
void crash_reporter_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = CrashReporterConfig{};
    g_status = CrashReporterStatus{};
    g_status.state = CrashReporterState::UNKNOWN;
}
