...
namespace {
    TimelineStateUtilsConfig g_config;
    std::mutex g_mutex;
    TimelineStateUtilsStatus g_status;
    TimelineStateUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* timeline_state_utils_stateToString(TimelineStateUtilsState s) {
    switch (s) {
        case TimelineStateUtilsState::UNKNOWN: return "UNKNOWN";
        case TimelineStateUtilsState::INIT: return "INIT";
        case TimelineStateUtilsState::PROCESSING: return "PROCESSING";
        case TimelineStateUtilsState::DONE: return "DONE";
        case TimelineStateUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

TimelineStateUtilsState timeline_state_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return TimelineStateUtilsState::UNKNOWN;
    if (s == "INIT") return TimelineStateUtilsState::INIT;
    if (s == "PROCESSING") return TimelineStateUtilsState::PROCESSING;
    if (s == "DONE") return TimelineStateUtilsState::DONE;
    if (s == "ERROR") return TimelineStateUtilsState::ERROR;
    return TimelineStateUtilsState::UNKNOWN;
}

bool timeline_state_utils_init(const TimelineStateUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = TimelineStateUtilsState::INIT;
...
TimelineStateUtilsResponse timeline_state_utils_handle(const TimelineStateUtilsRequest& req) {
    TimelineStateUtilsResponse resp;
    ...
    g_status.state = TimelineStateUtilsState::PROCESSING;
    // Validate request
...
TimelineStateUtilsStatus timeline_state_utils_getStatus() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_status;
}

void timeline_state_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = TimelineStateUtilsConfig{};
    g_status = TimelineStateUtilsStatus{};
    g_status.state = TimelineStateUtilsState::UNKNOWN;
}
...
std::string timeline_state_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "timeline_state_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
TimelineStateUtilsStats timeline_state_utils_getStats() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_stats;
}

TimelineStateUtilsConfig timeline_state_utils_configFromJson(const json& j) {
    TimelineStateUtilsConfig config;
...
json timeline_state_utils_configToJson(const TimelineStateUtilsConfig& config) {
    json j;
