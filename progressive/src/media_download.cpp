...
namespace {
    MediaDownloadConfig g_config;
    std::mutex g_mutex;
    MediaDownloadStatus g_status;
    MediaDownloadStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* media_download_stateToString(DownloadType s) {
    switch (s) {
        case DownloadType::UNKNOWN: return "UNKNOWN";
        case DownloadType::INIT: return "INIT";
        case DownloadType::PROCESSING: return "PROCESSING";
        case DownloadType::DONE: return "DONE";
        case DownloadType::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

DownloadType media_download_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return DownloadType::UNKNOWN;
    if (s == "INIT") return DownloadType::INIT;
    if (s == "PROCESSING") return DownloadType::PROCESSING;
    if (s == "DONE") return DownloadType::DONE;
    if (s == "ERROR") return DownloadType::ERROR;
...
bool media_download_init(const MediaDownloadConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = DownloadType::INIT;
...
MediaDownloadResponse media_download_handle(const MediaDownloadRequest& req) {
    ...
    g_status.state = DownloadType::PROCESSING;
    // Validate request
...
void media_download_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = MediaDownloadConfig{};
    g_status = MediaDownloadStatus{};
    g_status.state = DownloadType::UNKNOWN;
}
