...
namespace {
    RoomNoticeFormatterConfig g_config;
    std::mutex g_mutex;
    RoomNoticeFormatterStatus g_status;
    RoomNoticeFormatterStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* room_notice_formatter_stateToString(NoticeFormatterAction s) {
    switch (s) {
        case NoticeFormatterAction::UNKNOWN: return "UNKNOWN";
        case NoticeFormatterAction::INIT: return "INIT";
        case NoticeFormatterAction::PROCESSING: return "PROCESSING";
        case NoticeFormatterAction::DONE: return "DONE";
        case NoticeFormatterAction::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

NoticeFormatterAction room_notice_formatter_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return NoticeFormatterAction::UNKNOWN;
    if (s == "INIT") return NoticeFormatterAction::INIT;
    if (s == "PROCESSING") return NoticeFormatterAction::PROCESSING;
    if (s == "DONE") return NoticeFormatterAction::DONE;
    if (s == "ERROR") return NoticeFormatterAction::ERROR;
    return NoticeFormatterAction::UNKNOWN;
}
...
bool room_notice_formatter_init(const RoomNoticeFormatterConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = NoticeFormatterAction::INIT;
...
RoomNoticeFormatterResponse room_notice_formatter_handle(const RoomNoticeFormatterRequest& req) {
    RoomNoticeFormatterResponse resp;
    ...
    g_status.state = NoticeFormatterAction::PROCESSING;
    // Validate request
...
void room_notice_formatter_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = RoomNoticeFormatterConfig{};
    g_status = RoomNoticeFormatterStatus{};
    g_status.state = NoticeFormatterAction::UNKNOWN;
}
...
std::string room_notice_formatter_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "room_notice_formatter";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
RoomNoticeFormatterConfig room_notice_formatter_configFromJson(const json& j) {
    RoomNoticeFormatterConfig config;
