...
namespace {
    RoomSidebarConfig g_config;
    std::mutex g_mutex;
    RoomSidebarStatus g_status;
    RoomSidebarStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* room_sidebar_stateToString(SidebarAction s) {
    switch (s) {
        case SidebarAction::UNKNOWN: return "UNKNOWN";
        case SidebarAction::INIT: return "INIT";
        case SidebarAction::PROCESSING: return "PROCESSING";
        case SidebarAction::DONE: return "DONE";
        case SidebarAction::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

SidebarAction room_sidebar_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return SidebarAction::UNKNOWN;
    if (s == "INIT") return SidebarAction::INIT;
    if (s == "PROCESSING") return SidebarAction::PROCESSING;
    if (s == "DONE") return SidebarAction::DONE;
    if (s == "ERROR") return SidebarAction::ERROR;
    return SidebarAction::UNKNOWN;
}
...
bool room_sidebar_init(const RoomSidebarConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = SidebarAction::INIT;
...
RoomSidebarResponse room_sidebar_handle(const RoomSidebarRequest& req) {
    ...
    g_status.state = SidebarAction::PROCESSING;
    // Validate request
...
void room_sidebar_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = RoomSidebarConfig{};
    g_status = RoomSidebarStatus{};
    g_status.state = SidebarAction::UNKNOWN;
}
