...
namespace {
    CrossSignBootstrapConfig g_config;
    std::mutex g_mutex;
    CrossSignBootstrapStatus g_status;
    CrossSignBootstrapStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* cross_sign_bootstrap_stateToString(BootstrapPhase s) {
    switch (s) {
        case BootstrapPhase::UNKNOWN: return "UNKNOWN";
        case BootstrapPhase::INIT: return "INIT";
        case BootstrapPhase::PROCESSING: return "PROCESSING";
        case BootstrapPhase::DONE: return "DONE";
        case BootstrapPhase::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

BootstrapPhase cross_sign_bootstrap_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return BootstrapPhase::UNKNOWN;
    if (s == "INIT") return BootstrapPhase::INIT;
    if (s == "PROCESSING") return BootstrapPhase::PROCESSING;
    if (s == "DONE") return BootstrapPhase::DONE;
    if (s == "ERROR") return BootstrapPhase::ERROR;
    return BootstrapPhase::UNKNOWN;
}
...
bool cross_sign_bootstrap_init(const CrossSignBootstrapConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = BootstrapPhase::INIT;
...
CrossSignBootstrapResponse cross_sign_bootstrap_handle(const CrossSignBootstrapRequest& req) {
    CrossSignBootstrapResponse resp;
    ...
    g_status.state = BootstrapPhase::PROCESSING;
    // Validate request
...
void cross_sign_bootstrap_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = CrossSignBootstrapConfig{};
    g_status = CrossSignBootstrapStatus{};
    g_status.state = BootstrapPhase::UNKNOWN;
}
...
std::string cross_sign_bootstrap_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "cross_sign_bootstrap";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
CrossSignBootstrapConfig cross_sign_bootstrap_configFromJson(const json& j) {
    CrossSignBootstrapConfig config;
