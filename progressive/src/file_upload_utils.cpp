...
namespace {
    FileUploadUtilsConfig g_config;
    std::mutex g_mutex;
    FileUploadUtilsStatus g_status;
    FileUploadUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* file_upload_utils_stateToString(FileUploadUtilsState s) {
    switch (s) {
        case FileUploadUtilsState::UNKNOWN: return "UNKNOWN";
        case FileUploadUtilsState::INIT: return "INIT";
        case FileUploadUtilsState::PROCESSING: return "PROCESSING";
        case FileUploadUtilsState::DONE: return "DONE";
        case FileUploadUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

FileUploadUtilsState file_upload_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return FileUploadUtilsState::UNKNOWN;
    if (s == "INIT") return FileUploadUtilsState::INIT;
    if (s == "PROCESSING") return FileUploadUtilsState::PROCESSING;
    if (s == "DONE") return FileUploadUtilsState::DONE;
    if (s == "ERROR") return FileUploadUtilsState::ERROR;
    return FileUploadUtilsState::UNKNOWN;
}
...
bool file_upload_utils_init(const FileUploadUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = FileUploadUtilsState::INIT;
...
FileUploadUtilsResponse file_upload_utils_handle(const FileUploadUtilsRequest& req) {
    ...
    g_status.state = FileUploadUtilsState::PROCESSING;
    // Validate request
...
void file_upload_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = FileUploadUtilsConfig{};
    g_status = FileUploadUtilsStatus{};
    g_status.state = FileUploadUtilsState::UNKNOWN;
}
...
std::string file_upload_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "file_upload_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
