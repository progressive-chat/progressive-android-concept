...
namespace {
    ContentBuilderUtilsConfig g_config;
    std::mutex g_mutex;
    ContentBuilderUtilsStatus g_status;
    ContentBuilderUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* content_builder_utils_stateToString(ContentBuilderUtilsState s) {
    switch (s) {
        case ContentBuilderUtilsState::UNKNOWN: return "UNKNOWN";
        case ContentBuilderUtilsState::INIT: return "INIT";
        case ContentBuilderUtilsState::PROCESSING: return "PROCESSING";
        case ContentBuilderUtilsState::DONE: return "DONE";
        case ContentBuilderUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

ContentBuilderUtilsState content_builder_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return ContentBuilderUtilsState::UNKNOWN;
    if (s == "INIT") return ContentBuilderUtilsState::INIT;
    if (s == "PROCESSING") return ContentBuilderUtilsState::PROCESSING;
    if (s == "DONE") return ContentBuilderUtilsState::DONE;
    if (s == "ERROR") return ContentBuilderUtilsState::ERROR;
    return ContentBuilderUtilsState::UNKNOWN;
}
...
bool content_builder_utils_init(const ContentBuilderUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = ContentBuilderUtilsState::INIT;
...
ContentBuilderUtilsResponse content_builder_utils_handle(const ContentBuilderUtilsRequest& req) {
    ContentBuilderUtilsResponse resp;
    ...
    g_status.state = ContentBuilderUtilsState::PROCESSING;
    // Validate request
...
void content_builder_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = ContentBuilderUtilsConfig{};
    g_status = ContentBuilderUtilsStatus{};
    g_status.state = ContentBuilderUtilsState::UNKNOWN;
}
...
std::string content_builder_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "content_builder_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
ContentBuilderUtilsConfig content_builder_utils_configFromJson(const json& j) {
    ContentBuilderUtilsConfig config;
