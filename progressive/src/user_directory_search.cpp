...
namespace {
    UserDirectorySearchConfig g_config;
    std::mutex g_mutex;
    UserDirectorySearchStatus g_status;
    UserDirectorySearchStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* user_directory_search_stateToString(DirectorySearchState s) {
    switch (s) {
        case DirectorySearchState::UNKNOWN: return "UNKNOWN";
        case DirectorySearchState::INIT: return "INIT";
        case DirectorySearchState::PROCESSING: return "PROCESSING";
        case DirectorySearchState::DONE: return "DONE";
        case DirectorySearchState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

DirectorySearchState user_directory_search_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return DirectorySearchState::UNKNOWN;
    if (s == "INIT") return DirectorySearchState::INIT;
    if (s == "PROCESSING") return DirectorySearchState::PROCESSING;
    if (s == "DONE") return DirectorySearchState::DONE;
    if (s == "ERROR") return DirectorySearchState::ERROR;
    return DirectorySearchState::UNKNOWN;
}
...
bool user_directory_search_init(const UserDirectorySearchConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = DirectorySearchState::INIT;
...
UserDirectorySearchResponse user_directory_search_handle(const UserDirectorySearchRequest& req) {
    UserDirectorySearchResponse resp;
    ...
    g_status.state = DirectorySearchState::PROCESSING;
    // Validate request
...
void user_directory_search_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = UserDirectorySearchConfig{};
    g_status = UserDirectorySearchStatus{};
    g_status.state = DirectorySearchState::UNKNOWN;
}
...
std::string user_directory_search_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "user_directory_search";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
UserDirectorySearchConfig user_directory_search_configFromJson(const json& j) {
    UserDirectorySearchConfig config;
