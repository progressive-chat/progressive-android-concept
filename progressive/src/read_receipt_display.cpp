...
namespace {
    ReadReceiptDisplayConfig g_config;
    std::mutex g_mutex;
    ReadReceiptDisplayStatus g_status;
    ReadReceiptDisplayStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* read_receipt_display_stateToString(ReadReceiptDisplayState s) {
    switch (s) {
        case ReadReceiptDisplayState::UNKNOWN: return "UNKNOWN";
        case ReadReceiptDisplayState::INIT: return "INIT";
        case ReadReceiptDisplayState::PROCESSING: return "PROCESSING";
        case ReadReceiptDisplayState::DONE: return "DONE";
        case ReadReceiptDisplayState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

ReadReceiptDisplayState read_receipt_display_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return ReadReceiptDisplayState::UNKNOWN;
    if (s == "INIT") return ReadReceiptDisplayState::INIT;
    if (s == "PROCESSING") return ReadReceiptDisplayState::PROCESSING;
    if (s == "DONE") return ReadReceiptDisplayState::DONE;
    if (s == "ERROR") return ReadReceiptDisplayState::ERROR;
    return ReadReceiptDisplayState::UNKNOWN;
}
...
bool read_receipt_display_init(const ReadReceiptDisplayConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = ReadReceiptDisplayState::INIT;
...
ReadReceiptDisplayResponse read_receipt_display_handle(const ReadReceiptDisplayRequest& req) {
    ReadReceiptDisplayResponse resp;
    ...
    g_status.state = ReadReceiptDisplayState::PROCESSING;
    // Validate request
...
void read_receipt_display_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = ReadReceiptDisplayConfig{};
    g_status = ReadReceiptDisplayStatus{};
    g_status.state = ReadReceiptDisplayState::UNKNOWN;
}
...
std::string read_receipt_display_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "read_receipt_display";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
ReadReceiptDisplayConfig read_receipt_display_configFromJson(const json& j) {
    ReadReceiptDisplayConfig config;
