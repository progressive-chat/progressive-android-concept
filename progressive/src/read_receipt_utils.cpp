...
namespace {
    ReadReceiptUtilsConfig g_config;
    std::mutex g_mutex;
    ReadReceiptUtilsStatus g_status;
    ReadReceiptUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* read_receipt_utils_stateToString(ReadReceiptUtilsState s) {
    switch (s) {
        case ReadReceiptUtilsState::UNKNOWN: return "UNKNOWN";
        case ReadReceiptUtilsState::INIT: return "INIT";
        case ReadReceiptUtilsState::PROCESSING: return "PROCESSING";
        case ReadReceiptUtilsState::DONE: return "DONE";
        case ReadReceiptUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

ReadReceiptUtilsState read_receipt_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return ReadReceiptUtilsState::UNKNOWN;
    if (s == "INIT") return ReadReceiptUtilsState::INIT;
    if (s == "PROCESSING") return ReadReceiptUtilsState::PROCESSING;
    if (s == "DONE") return ReadReceiptUtilsState::DONE;
    if (s == "ERROR") return ReadReceiptUtilsState::ERROR;
    return ReadReceiptUtilsState::UNKNOWN;
}
...
bool read_receipt_utils_init(const ReadReceiptUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = ReadReceiptUtilsState::INIT;
...
ReadReceiptUtilsResponse read_receipt_utils_handle(const ReadReceiptUtilsRequest& req) {
    ...
    g_status.state = ReadReceiptUtilsState::PROCESSING;
    // Validate request
...
void read_receipt_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = ReadReceiptUtilsConfig{};
    g_status = ReadReceiptUtilsStatus{};
    g_status.state = ReadReceiptUtilsState::UNKNOWN;
}
...
std::string read_receipt_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "read_receipt_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
