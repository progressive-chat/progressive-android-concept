...
namespace {
    CapabilitiesUtilsConfig g_config;
    std::mutex g_mutex;
    CapabilitiesUtilsStatus g_status;
    CapabilitiesUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* capabilities_utils_stateToString(CapabilitiesUtilsState s) {
    switch (s) {
        case CapabilitiesUtilsState::UNKNOWN: return "UNKNOWN";
        case CapabilitiesUtilsState::INIT: return "INIT";
        case CapabilitiesUtilsState::PROCESSING: return "PROCESSING";
        case CapabilitiesUtilsState::DONE: return "DONE";
        case CapabilitiesUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

CapabilitiesUtilsState capabilities_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return CapabilitiesUtilsState::UNKNOWN;
    if (s == "INIT") return CapabilitiesUtilsState::INIT;
    if (s == "PROCESSING") return CapabilitiesUtilsState::PROCESSING;
    if (s == "DONE") return CapabilitiesUtilsState::DONE;
    if (s == "ERROR") return CapabilitiesUtilsState::ERROR;
    return CapabilitiesUtilsState::UNKNOWN;
}
...
bool capabilities_utils_init(const CapabilitiesUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = CapabilitiesUtilsState::INIT;
...
CapabilitiesUtilsResponse capabilities_utils_handle(const CapabilitiesUtilsRequest& req) {
    CapabilitiesUtilsResponse resp;
    ...
    g_status.state = CapabilitiesUtilsState::PROCESSING;
    // Validate request
...
void capabilities_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = CapabilitiesUtilsConfig{};
    g_status = CapabilitiesUtilsStatus{};
    g_status.state = CapabilitiesUtilsState::UNKNOWN;
}
...
std::string capabilities_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "capabilities_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
CapabilitiesUtilsConfig capabilities_utils_configFromJson(const json& j) {
    CapabilitiesUtilsConfig config;
