...
namespace {
    FederationTesterConfig g_config;
    std::mutex g_mutex;
    FederationTesterStatus g_status;
    FederationTesterStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* federation_tester_stateToString(FederationTesterState s) {
    switch (s) {
        case FederationTesterState::UNKNOWN: return "UNKNOWN";
        case FederationTesterState::INIT: return "INIT";
        case FederationTesterState::PROCESSING: return "PROCESSING";
        case FederationTesterState::DONE: return "DONE";
        case FederationTesterState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

FederationTesterState federation_tester_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return FederationTesterState::UNKNOWN;
    if (s == "INIT") return FederationTesterState::INIT;
    if (s == "PROCESSING") return FederationTesterState::PROCESSING;
    if (s == "DONE") return FederationTesterState::DONE;
    if (s == "ERROR") return FederationTesterState::ERROR;
    return FederationTesterState::UNKNOWN;
}
...
bool federation_tester_init(const FederationTesterConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = FederationTesterState::INIT;
...
FederationTesterResponse federation_tester_handle(const FederationTesterRequest& req) {
    ...
    g_status.state = FederationTesterState::PROCESSING;
    // Validate request
...
void federation_tester_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = FederationTesterConfig{};
    g_status = FederationTesterStatus{};
    g_status.state = FederationTesterState::UNKNOWN;
}
...
std::string federation_tester_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "federation_tester";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
