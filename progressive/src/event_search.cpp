...
namespace {
    EventSearchConfig g_config;
    std::mutex g_mutex;
    EventSearchStatus g_status;
    EventSearchStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* event_search_stateToString(SearchState s) {
    switch (s) {
        case SearchState::UNKNOWN: return "UNKNOWN";
        case SearchState::INIT: return "INIT";
        case SearchState::PROCESSING: return "PROCESSING";
        case SearchState::DONE: return "DONE";
        case SearchState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

SearchState event_search_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return SearchState::UNKNOWN;
    if (s == "INIT") return SearchState::INIT;
    if (s == "PROCESSING") return SearchState::PROCESSING;
    if (s == "DONE") return SearchState::DONE;
    if (s == "ERROR") return SearchState::ERROR;
...
bool event_search_init(const EventSearchConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = SearchState::INIT;
...
EventSearchResponse event_search_handle(const EventSearchRequest& req) {
    ...
    g_status.state = SearchState::PROCESSING;
    // Validate request
...
void event_search_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = EventSearchConfig{};
    g_status = EventSearchStatus{};
    g_status.state = SearchState::UNKNOWN;
}
