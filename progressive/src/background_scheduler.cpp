...
namespace {
    BackgroundSchedulerConfig g_config;
    std::mutex g_mutex;
    BackgroundSchedulerStatus g_status;
    BackgroundSchedulerStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* background_scheduler_stateToString(BackgroundSchedulerState s) {
    switch (s) {
        case BackgroundSchedulerState::UNKNOWN: return "UNKNOWN";
        case BackgroundSchedulerState::INIT: return "INIT";
        case BackgroundSchedulerState::PROCESSING: return "PROCESSING";
        case BackgroundSchedulerState::DONE: return "DONE";
        case BackgroundSchedulerState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

BackgroundSchedulerState background_scheduler_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return BackgroundSchedulerState::UNKNOWN;
    if (s == "INIT") return BackgroundSchedulerState::INIT;
    if (s == "PROCESSING") return BackgroundSchedulerState::PROCESSING;
    if (s == "DONE") return BackgroundSchedulerState::DONE;
    if (s == "ERROR") return BackgroundSchedulerState::ERROR;
    return BackgroundSchedulerState::UNKNOWN;
}

bool background_scheduler_init(const BackgroundSchedulerConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = BackgroundSchedulerState::INIT;
...
BackgroundSchedulerResponse background_scheduler_handle(const BackgroundSchedulerRequest& req) {
    BackgroundSchedulerResponse resp;
    ...
    g_status.state = BackgroundSchedulerState::PROCESSING;
    // Validate request
...
BackgroundSchedulerStatus background_scheduler_getStatus() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_status;
}

void background_scheduler_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = BackgroundSchedulerConfig{};
    g_status = BackgroundSchedulerStatus{};
    g_status.state = BackgroundSchedulerState::UNKNOWN;
}
...
std::string background_scheduler_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "background_scheduler";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
BackgroundSchedulerStats background_scheduler_getStats() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_stats;
}

BackgroundSchedulerConfig background_scheduler_configFromJson(const json& j) {
    BackgroundSchedulerConfig config;
...
json background_scheduler_configToJson(const BackgroundSchedulerConfig& config) {
    json j;
