...
namespace {
    ContentReportUtilsConfig g_config;
    std::mutex g_mutex;
    ContentReportUtilsStatus g_status;
    ContentReportUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* content_report_utils_stateToString(ContentReportUtilsState s) {
    switch (s) {
        case ContentReportUtilsState::UNKNOWN: return "UNKNOWN";
        case ContentReportUtilsState::INIT: return "INIT";
        case ContentReportUtilsState::PROCESSING: return "PROCESSING";
        case ContentReportUtilsState::DONE: return "DONE";
        case ContentReportUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

ContentReportUtilsState content_report_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return ContentReportUtilsState::UNKNOWN;
    if (s == "INIT") return ContentReportUtilsState::INIT;
    if (s == "PROCESSING") return ContentReportUtilsState::PROCESSING;
    if (s == "DONE") return ContentReportUtilsState::DONE;
    if (s == "ERROR") return ContentReportUtilsState::ERROR;
    return ContentReportUtilsState::UNKNOWN;
}
...
bool content_report_utils_init(const ContentReportUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = ContentReportUtilsState::INIT;
...
ContentReportUtilsResponse content_report_utils_handle(const ContentReportUtilsRequest& req) {
    ContentReportUtilsResponse resp;
    ...
    g_status.state = ContentReportUtilsState::PROCESSING;
    // Validate request
...
void content_report_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = ContentReportUtilsConfig{};
    g_status = ContentReportUtilsStatus{};
    g_status.state = ContentReportUtilsState::UNKNOWN;
}
...
std::string content_report_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "content_report_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
ContentReportUtilsConfig content_report_utils_configFromJson(const json& j) {
    ContentReportUtilsConfig config;
