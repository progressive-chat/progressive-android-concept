...
namespace {
    ServerDiscoveryConfig g_config;
    std::mutex g_mutex;
    ServerDiscoveryStatus g_status;
    ServerDiscoveryStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* server_discovery_stateToString(DiscoveryStatus s) {
    switch (s) {
        case DiscoveryStatus::UNKNOWN: return "UNKNOWN";
        case DiscoveryStatus::INIT: return "INIT";
        case DiscoveryStatus::PROCESSING: return "PROCESSING";
        case DiscoveryStatus::DONE: return "DONE";
        case DiscoveryStatus::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

DiscoveryStatus server_discovery_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return DiscoveryStatus::UNKNOWN;
    if (s == "INIT") return DiscoveryStatus::INIT;
    if (s == "PROCESSING") return DiscoveryStatus::PROCESSING;
    if (s == "DONE") return DiscoveryStatus::DONE;
    if (s == "ERROR") return DiscoveryStatus::ERROR;
...
bool server_discovery_init(const ServerDiscoveryConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = DiscoveryStatus::INIT;
...
ServerDiscoveryResponse server_discovery_handle(const ServerDiscoveryRequest& req) {
    ...
    g_status.state = DiscoveryStatus::PROCESSING;
    // Validate request
...
void server_discovery_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = ServerDiscoveryConfig{};
    g_status = ServerDiscoveryStatus{};
    g_status.state = DiscoveryStatus::UNKNOWN;
}
...
std::string server_discovery_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "server_discovery";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
