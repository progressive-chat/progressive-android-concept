...
namespace {
    DeviceNamingHelperConfig g_config;
    std::mutex g_mutex;
    DeviceNamingHelperStatus g_status;
    DeviceNamingHelperStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* device_naming_helper_stateToString(DeviceNamingHelperState s) {
    switch (s) {
        case DeviceNamingHelperState::UNKNOWN: return "UNKNOWN";
        case DeviceNamingHelperState::INIT: return "INIT";
        case DeviceNamingHelperState::PROCESSING: return "PROCESSING";
        case DeviceNamingHelperState::DONE: return "DONE";
        case DeviceNamingHelperState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

DeviceNamingHelperState device_naming_helper_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return DeviceNamingHelperState::UNKNOWN;
    if (s == "INIT") return DeviceNamingHelperState::INIT;
    if (s == "PROCESSING") return DeviceNamingHelperState::PROCESSING;
    if (s == "DONE") return DeviceNamingHelperState::DONE;
    if (s == "ERROR") return DeviceNamingHelperState::ERROR;
    return DeviceNamingHelperState::UNKNOWN;
}
...
bool device_naming_helper_init(const DeviceNamingHelperConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = DeviceNamingHelperState::INIT;
...
DeviceNamingHelperResponse device_naming_helper_handle(const DeviceNamingHelperRequest& req) {
    DeviceNamingHelperResponse resp;
    ...
    g_status.state = DeviceNamingHelperState::PROCESSING;
    // Validate request
...
void device_naming_helper_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = DeviceNamingHelperConfig{};
    g_status = DeviceNamingHelperStatus{};
    g_status.state = DeviceNamingHelperState::UNKNOWN;
}
...
std::string device_naming_helper_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "device_naming_helper";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
DeviceNamingHelperConfig device_naming_helper_configFromJson(const json& j) {
    DeviceNamingHelperConfig config;
