...
namespace {
    SecretSharingConfig g_config;
    std::mutex g_mutex;
    SecretSharingStatus g_status;
    SecretSharingStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* secret_sharing_stateToString(SecretSharingState s) {
    switch (s) {
        case SecretSharingState::UNKNOWN: return "UNKNOWN";
        case SecretSharingState::INIT: return "INIT";
        case SecretSharingState::PROCESSING: return "PROCESSING";
        case SecretSharingState::DONE: return "DONE";
        case SecretSharingState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

SecretSharingState secret_sharing_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return SecretSharingState::UNKNOWN;
    if (s == "INIT") return SecretSharingState::INIT;
    if (s == "PROCESSING") return SecretSharingState::PROCESSING;
    if (s == "DONE") return SecretSharingState::DONE;
    if (s == "ERROR") return SecretSharingState::ERROR;
    return SecretSharingState::UNKNOWN;
}
...
bool secret_sharing_init(const SecretSharingConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = SecretSharingState::INIT;
...
SecretSharingResponse secret_sharing_handle(const SecretSharingRequest& req) {
    ...
    g_status.state = SecretSharingState::PROCESSING;
    // Validate request
...
void secret_sharing_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = SecretSharingConfig{};
    g_status = SecretSharingStatus{};
    g_status.state = SecretSharingState::UNKNOWN;
}
