...
namespace {
    ServerVersionCheckConfig g_config;
    std::mutex g_mutex;
    ServerVersionCheckStatus g_status;
    ServerVersionCheckStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* server_version_check_stateToString(VersionCheckStatus s) {
    switch (s) {
        case VersionCheckStatus::UNKNOWN: return "UNKNOWN";
        case VersionCheckStatus::INIT: return "INIT";
        case VersionCheckStatus::PROCESSING: return "PROCESSING";
        case VersionCheckStatus::DONE: return "DONE";
        case VersionCheckStatus::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

VersionCheckStatus server_version_check_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return VersionCheckStatus::UNKNOWN;
    if (s == "INIT") return VersionCheckStatus::INIT;
    if (s == "PROCESSING") return VersionCheckStatus::PROCESSING;
    if (s == "DONE") return VersionCheckStatus::DONE;
    if (s == "ERROR") return VersionCheckStatus::ERROR;
    return VersionCheckStatus::UNKNOWN;
}
...
bool server_version_check_init(const ServerVersionCheckConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = VersionCheckStatus::INIT;
...
ServerVersionCheckResponse server_version_check_handle(const ServerVersionCheckRequest& req) {
    ServerVersionCheckResponse resp;
    ...
    g_status.state = VersionCheckStatus::PROCESSING;
    // Validate request
...
void server_version_check_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = ServerVersionCheckConfig{};
    g_status = ServerVersionCheckStatus{};
    g_status.state = VersionCheckStatus::UNKNOWN;
}
...
std::string server_version_check_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "server_version_check";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
ServerVersionCheckConfig server_version_check_configFromJson(const json& j) {
    ServerVersionCheckConfig config;
