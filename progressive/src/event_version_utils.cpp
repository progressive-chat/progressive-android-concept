...
namespace {
    EventVersionUtilsConfig g_config;
    std::mutex g_mutex;
    EventVersionUtilsStatus g_status;
    EventVersionUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* event_version_utils_stateToString(VersionUtilsState s) {
    switch (s) {
        case VersionUtilsState::UNKNOWN: return "UNKNOWN";
        case VersionUtilsState::INIT: return "INIT";
        case VersionUtilsState::PROCESSING: return "PROCESSING";
        case VersionUtilsState::DONE: return "DONE";
        case VersionUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

VersionUtilsState event_version_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return VersionUtilsState::UNKNOWN;
    if (s == "INIT") return VersionUtilsState::INIT;
    if (s == "PROCESSING") return VersionUtilsState::PROCESSING;
    if (s == "DONE") return VersionUtilsState::DONE;
    if (s == "ERROR") return VersionUtilsState::ERROR;
    return VersionUtilsState::UNKNOWN;
}

bool event_version_utils_init(const EventVersionUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = VersionUtilsState::INIT;
...
EventVersionUtilsResponse event_version_utils_handle(const EventVersionUtilsRequest& req) {
    EventVersionUtilsResponse resp;
    ...
    g_status.state = VersionUtilsState::PROCESSING;
    // Validate request
...
EventVersionUtilsStatus event_version_utils_getStatus() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_status;
}

void event_version_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = EventVersionUtilsConfig{};
    g_status = EventVersionUtilsStatus{};
    g_status.state = VersionUtilsState::UNKNOWN;
}
...
std::string event_version_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "event_version_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
EventVersionUtilsStats event_version_utils_getStats() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_stats;
}

EventVersionUtilsConfig event_version_utils_configFromJson(const json& j) {
    EventVersionUtilsConfig config;
...
json event_version_utils_configToJson(const EventVersionUtilsConfig& config) {
    json j;
