...
namespace {
    AvatarUtilsConfig g_config;
    std::mutex g_mutex;
    AvatarUtilsStatus g_status;
    AvatarUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* avatar_utils_stateToString(AvatarUtilsState s) {
    switch (s) {
        case AvatarUtilsState::UNKNOWN: return "UNKNOWN";
        case AvatarUtilsState::INIT: return "INIT";
        case AvatarUtilsState::PROCESSING: return "PROCESSING";
        case AvatarUtilsState::DONE: return "DONE";
        case AvatarUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

AvatarUtilsState avatar_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return AvatarUtilsState::UNKNOWN;
    if (s == "INIT") return AvatarUtilsState::INIT;
    if (s == "PROCESSING") return AvatarUtilsState::PROCESSING;
    if (s == "DONE") return AvatarUtilsState::DONE;
    if (s == "ERROR") return AvatarUtilsState::ERROR;
    return AvatarUtilsState::UNKNOWN;
}

bool avatar_utils_init(const AvatarUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = AvatarUtilsState::INIT;
...
AvatarUtilsResponse avatar_utils_handle(const AvatarUtilsRequest& req) {
    AvatarUtilsResponse resp;
    ...
    g_status.state = AvatarUtilsState::PROCESSING;
    // Validate request
...
AvatarUtilsStatus avatar_utils_getStatus() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_status;
}

void avatar_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = AvatarUtilsConfig{};
    g_status = AvatarUtilsStatus{};
    g_status.state = AvatarUtilsState::UNKNOWN;
}
...
AvatarUtilsStats avatar_utils_getStats() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_stats;
}

AvatarUtilsConfig avatar_utils_configFromJson(const json& j) {
    AvatarUtilsConfig config;
...
json avatar_utils_configToJson(const AvatarUtilsConfig& config) {
    json j;
