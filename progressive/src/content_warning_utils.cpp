...
namespace {
    ContentWarningUtilsConfig g_config;
    std::mutex g_mutex;
    ContentWarningUtilsStatus g_status;
    ContentWarningUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* content_warning_utils_stateToString(ContentWarningUtilsState s) {
    switch (s) {
        case ContentWarningUtilsState::UNKNOWN: return "UNKNOWN";
        case ContentWarningUtilsState::INIT: return "INIT";
        case ContentWarningUtilsState::PROCESSING: return "PROCESSING";
        case ContentWarningUtilsState::DONE: return "DONE";
        case ContentWarningUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

ContentWarningUtilsState content_warning_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return ContentWarningUtilsState::UNKNOWN;
    if (s == "INIT") return ContentWarningUtilsState::INIT;
    if (s == "PROCESSING") return ContentWarningUtilsState::PROCESSING;
    if (s == "DONE") return ContentWarningUtilsState::DONE;
    if (s == "ERROR") return ContentWarningUtilsState::ERROR;
    return ContentWarningUtilsState::UNKNOWN;
}
...
bool content_warning_utils_init(const ContentWarningUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = ContentWarningUtilsState::INIT;
...
ContentWarningUtilsResponse content_warning_utils_handle(const ContentWarningUtilsRequest& req) {
    ContentWarningUtilsResponse resp;
    ...
    g_status.state = ContentWarningUtilsState::PROCESSING;
    // Validate request
...
void content_warning_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = ContentWarningUtilsConfig{};
    g_status = ContentWarningUtilsStatus{};
    g_status.state = ContentWarningUtilsState::UNKNOWN;
}
...
std::string content_warning_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "content_warning_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
ContentWarningUtilsConfig content_warning_utils_configFromJson(const json& j) {
    ContentWarningUtilsConfig config;
