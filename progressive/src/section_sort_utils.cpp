...
namespace {
    SectionSortUtilsConfig g_config;
    std::mutex g_mutex;
    SectionSortUtilsStatus g_status;
    SectionSortUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* section_sort_utils_stateToString(SectionSortUtilsState s) {
    switch (s) {
        case SectionSortUtilsState::UNKNOWN: return "UNKNOWN";
        case SectionSortUtilsState::INIT: return "INIT";
        case SectionSortUtilsState::PROCESSING: return "PROCESSING";
        case SectionSortUtilsState::DONE: return "DONE";
        case SectionSortUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

SectionSortUtilsState section_sort_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return SectionSortUtilsState::UNKNOWN;
    if (s == "INIT") return SectionSortUtilsState::INIT;
    if (s == "PROCESSING") return SectionSortUtilsState::PROCESSING;
    if (s == "DONE") return SectionSortUtilsState::DONE;
    if (s == "ERROR") return SectionSortUtilsState::ERROR;
    return SectionSortUtilsState::UNKNOWN;
}

bool section_sort_utils_init(const SectionSortUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = SectionSortUtilsState::INIT;
...
SectionSortUtilsResponse section_sort_utils_handle(const SectionSortUtilsRequest& req) {
    SectionSortUtilsResponse resp;
    ...
    g_status.state = SectionSortUtilsState::PROCESSING;
    // Validate request
...
SectionSortUtilsStatus section_sort_utils_getStatus() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_status;
}

void section_sort_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = SectionSortUtilsConfig{};
    g_status = SectionSortUtilsStatus{};
    g_status.state = SectionSortUtilsState::UNKNOWN;
}
...
std::string section_sort_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "section_sort_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
SectionSortUtilsStats section_sort_utils_getStats() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_stats;
}

SectionSortUtilsConfig section_sort_utils_configFromJson(const json& j) {
    SectionSortUtilsConfig config;
...
json section_sort_utils_configToJson(const SectionSortUtilsConfig& config) {
    json j;
