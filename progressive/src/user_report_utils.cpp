...
namespace {
    UserReportUtilsConfig g_config;
    std::mutex g_mutex;
    UserReportUtilsStatus g_status;
    UserReportUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* user_report_utils_stateToString(ReportUtilsState s) {
    switch (s) {
        case ReportUtilsState::UNKNOWN: return "UNKNOWN";
        case ReportUtilsState::INIT: return "INIT";
        case ReportUtilsState::PROCESSING: return "PROCESSING";
        case ReportUtilsState::DONE: return "DONE";
        case ReportUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

ReportUtilsState user_report_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return ReportUtilsState::UNKNOWN;
    if (s == "INIT") return ReportUtilsState::INIT;
    if (s == "PROCESSING") return ReportUtilsState::PROCESSING;
    if (s == "DONE") return ReportUtilsState::DONE;
    if (s == "ERROR") return ReportUtilsState::ERROR;
    return ReportUtilsState::UNKNOWN;
}
...
bool user_report_utils_init(const UserReportUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = ReportUtilsState::INIT;
...
UserReportUtilsResponse user_report_utils_handle(const UserReportUtilsRequest& req) {
    ...
    g_status.state = ReportUtilsState::PROCESSING;
    // Validate request
...
void user_report_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = UserReportUtilsConfig{};
    g_status = UserReportUtilsStatus{};
    g_status.state = ReportUtilsState::UNKNOWN;
}
...
std::string user_report_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "user_report_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
