...
namespace {
    SpotlightSearchUtilsConfig g_config;
    std::mutex g_mutex;
    SpotlightSearchUtilsStatus g_status;
    SpotlightSearchUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* spotlight_search_utils_stateToString(SpotlightSearchUtilsState s) {
    switch (s) {
        case SpotlightSearchUtilsState::UNKNOWN: return "UNKNOWN";
        case SpotlightSearchUtilsState::INIT: return "INIT";
        case SpotlightSearchUtilsState::PROCESSING: return "PROCESSING";
        case SpotlightSearchUtilsState::DONE: return "DONE";
        case SpotlightSearchUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

SpotlightSearchUtilsState spotlight_search_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return SpotlightSearchUtilsState::UNKNOWN;
    if (s == "INIT") return SpotlightSearchUtilsState::INIT;
    if (s == "PROCESSING") return SpotlightSearchUtilsState::PROCESSING;
    if (s == "DONE") return SpotlightSearchUtilsState::DONE;
    if (s == "ERROR") return SpotlightSearchUtilsState::ERROR;
    return SpotlightSearchUtilsState::UNKNOWN;
}
...
bool spotlight_search_utils_init(const SpotlightSearchUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = SpotlightSearchUtilsState::INIT;
...
SpotlightSearchUtilsResponse spotlight_search_utils_handle(const SpotlightSearchUtilsRequest& req) {
    SpotlightSearchUtilsResponse resp;
    ...
    g_status.state = SpotlightSearchUtilsState::PROCESSING;
    // Validate request
...
void spotlight_search_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = SpotlightSearchUtilsConfig{};
    g_status = SpotlightSearchUtilsStatus{};
    g_status.state = SpotlightSearchUtilsState::UNKNOWN;
}
...
std::string spotlight_search_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "spotlight_search_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
SpotlightSearchUtilsConfig spotlight_search_utils_configFromJson(const json& j) {
    SpotlightSearchUtilsConfig config;
