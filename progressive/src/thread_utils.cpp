...
namespace {
    ThreadUtilsConfig g_config;
    std::mutex g_mutex;
    ThreadUtilsStatus g_status;
    ThreadUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* thread_utils_stateToString(ThreadUtilsState s) {
    switch (s) {
        case ThreadUtilsState::UNKNOWN: return "UNKNOWN";
        case ThreadUtilsState::INIT: return "INIT";
        case ThreadUtilsState::PROCESSING: return "PROCESSING";
        case ThreadUtilsState::DONE: return "DONE";
        case ThreadUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

ThreadUtilsState thread_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return ThreadUtilsState::UNKNOWN;
    if (s == "INIT") return ThreadUtilsState::INIT;
    if (s == "PROCESSING") return ThreadUtilsState::PROCESSING;
    if (s == "DONE") return ThreadUtilsState::DONE;
    if (s == "ERROR") return ThreadUtilsState::ERROR;
    return ThreadUtilsState::UNKNOWN;
}
...
bool thread_utils_init(const ThreadUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = ThreadUtilsState::INIT;
...
ThreadUtilsResponse thread_utils_handle(const ThreadUtilsRequest& req) {
    ...
    g_status.state = ThreadUtilsState::PROCESSING;
    // Validate request
...
void thread_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = ThreadUtilsConfig{};
    g_status = ThreadUtilsStatus{};
    g_status.state = ThreadUtilsState::UNKNOWN;
}
