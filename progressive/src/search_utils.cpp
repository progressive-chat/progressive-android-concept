...
namespace {
    SearchUtilsConfig g_config;
    std::mutex g_mutex;
    SearchUtilsStatus g_status;
    SearchUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* search_utils_stateToString(SearchUtilsState s) {
    switch (s) {
        case SearchUtilsState::UNKNOWN: return "UNKNOWN";
        case SearchUtilsState::INIT: return "INIT";
        case SearchUtilsState::PROCESSING: return "PROCESSING";
        case SearchUtilsState::DONE: return "DONE";
        case SearchUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

SearchUtilsState search_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return SearchUtilsState::UNKNOWN;
    if (s == "INIT") return SearchUtilsState::INIT;
    if (s == "PROCESSING") return SearchUtilsState::PROCESSING;
    if (s == "DONE") return SearchUtilsState::DONE;
    if (s == "ERROR") return SearchUtilsState::ERROR;
    return SearchUtilsState::UNKNOWN;
}
...
bool search_utils_init(const SearchUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = SearchUtilsState::INIT;
...
SearchUtilsResponse search_utils_handle(const SearchUtilsRequest& req) {
    ...
    g_status.state = SearchUtilsState::PROCESSING;
    // Validate request
...
void search_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = SearchUtilsConfig{};
    g_status = SearchUtilsStatus{};
    g_status.state = SearchUtilsState::UNKNOWN;
}
