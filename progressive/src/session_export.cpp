...
namespace {
    SessionExportConfig g_config;
    std::mutex g_mutex;
    SessionExportStatus g_status;
    SessionExportStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* session_export_stateToString(ExportPhase s) {
    switch (s) {
        case ExportPhase::UNKNOWN: return "UNKNOWN";
        case ExportPhase::INIT: return "INIT";
        case ExportPhase::PROCESSING: return "PROCESSING";
        case ExportPhase::DONE: return "DONE";
        case ExportPhase::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

ExportPhase session_export_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return ExportPhase::UNKNOWN;
    if (s == "INIT") return ExportPhase::INIT;
    if (s == "PROCESSING") return ExportPhase::PROCESSING;
    if (s == "DONE") return ExportPhase::DONE;
    if (s == "ERROR") return ExportPhase::ERROR;
    return ExportPhase::UNKNOWN;
}
...
bool session_export_init(const SessionExportConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = ExportPhase::INIT;
...
SessionExportResponse session_export_handle(const SessionExportRequest& req) {
    ...
    g_status.state = ExportPhase::PROCESSING;
    // Validate request
...
void session_export_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = SessionExportConfig{};
    g_status = SessionExportStatus{};
    g_status.state = ExportPhase::UNKNOWN;
}
