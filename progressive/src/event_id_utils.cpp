...
namespace {
    EventIdUtilsConfig g_config;
    std::mutex g_mutex;
    EventIdUtilsStatus g_status;
    EventIdUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* event_id_utils_stateToString(IdUtilsState s) {
    switch (s) {
        case IdUtilsState::UNKNOWN: return "UNKNOWN";
        case IdUtilsState::INIT: return "INIT";
        case IdUtilsState::PROCESSING: return "PROCESSING";
        case IdUtilsState::DONE: return "DONE";
        case IdUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

IdUtilsState event_id_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return IdUtilsState::UNKNOWN;
    if (s == "INIT") return IdUtilsState::INIT;
    if (s == "PROCESSING") return IdUtilsState::PROCESSING;
    if (s == "DONE") return IdUtilsState::DONE;
    if (s == "ERROR") return IdUtilsState::ERROR;
    return IdUtilsState::UNKNOWN;
}
...
bool event_id_utils_init(const EventIdUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = IdUtilsState::INIT;
...
EventIdUtilsResponse event_id_utils_handle(const EventIdUtilsRequest& req) {
    ...
    g_status.state = IdUtilsState::PROCESSING;
    // Validate request
...
void event_id_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = EventIdUtilsConfig{};
    g_status = EventIdUtilsStatus{};
    g_status.state = IdUtilsState::UNKNOWN;
}
