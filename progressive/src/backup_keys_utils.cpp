...
namespace {
    BackupKeysUtilsConfig g_config;
    std::mutex g_mutex;
    BackupKeysUtilsStatus g_status;
    BackupKeysUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* backup_keys_utils_stateToString(BackupKeysUtilsState s) {
    switch (s) {
        case BackupKeysUtilsState::UNKNOWN: return "UNKNOWN";
        case BackupKeysUtilsState::INIT: return "INIT";
        case BackupKeysUtilsState::PROCESSING: return "PROCESSING";
        case BackupKeysUtilsState::DONE: return "DONE";
        case BackupKeysUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

BackupKeysUtilsState backup_keys_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return BackupKeysUtilsState::UNKNOWN;
    if (s == "INIT") return BackupKeysUtilsState::INIT;
    if (s == "PROCESSING") return BackupKeysUtilsState::PROCESSING;
    if (s == "DONE") return BackupKeysUtilsState::DONE;
    if (s == "ERROR") return BackupKeysUtilsState::ERROR;
    return BackupKeysUtilsState::UNKNOWN;
}
...
bool backup_keys_utils_init(const BackupKeysUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = BackupKeysUtilsState::INIT;
...
BackupKeysUtilsResponse backup_keys_utils_handle(const BackupKeysUtilsRequest& req) {
    ...
    g_status.state = BackupKeysUtilsState::PROCESSING;
    // Validate request
...
void backup_keys_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = BackupKeysUtilsConfig{};
    g_status = BackupKeysUtilsStatus{};
    g_status.state = BackupKeysUtilsState::UNKNOWN;
}
...
std::string backup_keys_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "backup_keys_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
