...
namespace {
    ThumbnailUtilsConfig g_config;
    std::mutex g_mutex;
    ThumbnailUtilsStatus g_status;
    ThumbnailUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* thumbnail_utils_stateToString(ThumbnailUtilsState s) {
    switch (s) {
        case ThumbnailUtilsState::UNKNOWN: return "UNKNOWN";
        case ThumbnailUtilsState::INIT: return "INIT";
        case ThumbnailUtilsState::PROCESSING: return "PROCESSING";
        case ThumbnailUtilsState::DONE: return "DONE";
        case ThumbnailUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

ThumbnailUtilsState thumbnail_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return ThumbnailUtilsState::UNKNOWN;
    if (s == "INIT") return ThumbnailUtilsState::INIT;
    if (s == "PROCESSING") return ThumbnailUtilsState::PROCESSING;
    if (s == "DONE") return ThumbnailUtilsState::DONE;
    if (s == "ERROR") return ThumbnailUtilsState::ERROR;
    return ThumbnailUtilsState::UNKNOWN;
}
...
bool thumbnail_utils_init(const ThumbnailUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = ThumbnailUtilsState::INIT;
...
ThumbnailUtilsResponse thumbnail_utils_handle(const ThumbnailUtilsRequest& req) {
    ...
    g_status.state = ThumbnailUtilsState::PROCESSING;
    // Validate request
...
void thumbnail_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = ThumbnailUtilsConfig{};
    g_status = ThumbnailUtilsStatus{};
    g_status.state = ThumbnailUtilsState::UNKNOWN;
}
