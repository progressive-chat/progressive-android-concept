...
namespace {
    LiveDraftConfig g_config;
    std::mutex g_mutex;
    LiveDraftStatus g_status;
    LiveDraftStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* live_draft_stateToString(LiveDraftState s) {
    switch (s) {
        case LiveDraftState::UNKNOWN: return "UNKNOWN";
        case LiveDraftState::INIT: return "INIT";
        case LiveDraftState::PROCESSING: return "PROCESSING";
        case LiveDraftState::DONE: return "DONE";
        case LiveDraftState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

LiveDraftState live_draft_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return LiveDraftState::UNKNOWN;
    if (s == "INIT") return LiveDraftState::INIT;
    if (s == "PROCESSING") return LiveDraftState::PROCESSING;
    if (s == "DONE") return LiveDraftState::DONE;
    if (s == "ERROR") return LiveDraftState::ERROR;
    return LiveDraftState::UNKNOWN;
}
...
bool live_draft_init(const LiveDraftConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = LiveDraftState::INIT;
...
LiveDraftResponse live_draft_handle(const LiveDraftRequest& req) {
    ...
    g_status.state = LiveDraftState::PROCESSING;
    // Validate request
...
void live_draft_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = LiveDraftConfig{};
    g_status = LiveDraftStatus{};
    g_status.state = LiveDraftState::UNKNOWN;
}
