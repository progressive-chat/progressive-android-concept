...
namespace {
    SyncStateMonitorConfig g_config;
    std::mutex g_mutex;
    SyncStateMonitorStatus g_status;
    SyncStateMonitorStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* sync_state_monitor_stateToString(StateMonitorPhase s) {
    switch (s) {
        case StateMonitorPhase::UNKNOWN: return "UNKNOWN";
        case StateMonitorPhase::INIT: return "INIT";
        case StateMonitorPhase::PROCESSING: return "PROCESSING";
        case StateMonitorPhase::DONE: return "DONE";
        case StateMonitorPhase::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

StateMonitorPhase sync_state_monitor_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return StateMonitorPhase::UNKNOWN;
    if (s == "INIT") return StateMonitorPhase::INIT;
    if (s == "PROCESSING") return StateMonitorPhase::PROCESSING;
    if (s == "DONE") return StateMonitorPhase::DONE;
    if (s == "ERROR") return StateMonitorPhase::ERROR;
    return StateMonitorPhase::UNKNOWN;
}
...
bool sync_state_monitor_init(const SyncStateMonitorConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = StateMonitorPhase::INIT;
...
SyncStateMonitorResponse sync_state_monitor_handle(const SyncStateMonitorRequest& req) {
    ...
    g_status.state = StateMonitorPhase::PROCESSING;
    // Validate request
...
void sync_state_monitor_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = SyncStateMonitorConfig{};
    g_status = SyncStateMonitorStatus{};
    g_status.state = StateMonitorPhase::UNKNOWN;
}
...
std::string sync_state_monitor_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "sync_state_monitor";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
