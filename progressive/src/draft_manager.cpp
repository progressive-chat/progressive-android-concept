...
namespace {
    DraftManagerConfig g_config;
    std::mutex g_mutex;
    DraftManagerStatus g_status;
    DraftManagerStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* draft_manager_stateToString(DraftManagerState s) {
    switch (s) {
        case DraftManagerState::UNKNOWN: return "UNKNOWN";
        case DraftManagerState::INIT: return "INIT";
        case DraftManagerState::PROCESSING: return "PROCESSING";
        case DraftManagerState::DONE: return "DONE";
        case DraftManagerState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

DraftManagerState draft_manager_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return DraftManagerState::UNKNOWN;
    if (s == "INIT") return DraftManagerState::INIT;
    if (s == "PROCESSING") return DraftManagerState::PROCESSING;
    if (s == "DONE") return DraftManagerState::DONE;
    if (s == "ERROR") return DraftManagerState::ERROR;
    return DraftManagerState::UNKNOWN;
}
...
bool draft_manager_init(const DraftManagerConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = DraftManagerState::INIT;
...
DraftManagerResponse draft_manager_handle(const DraftManagerRequest& req) {
    ...
    g_status.state = DraftManagerState::PROCESSING;
    // Validate request
...
void draft_manager_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = DraftManagerConfig{};
    g_status = DraftManagerStatus{};
    g_status.state = DraftManagerState::UNKNOWN;
}
