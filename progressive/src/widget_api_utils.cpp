...
namespace {
    WidgetApiUtilsConfig g_config;
    std::mutex g_mutex;
    WidgetApiUtilsStatus g_status;
    WidgetApiUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* widget_api_utils_stateToString(WidgetApiUtilsState s) {
    switch (s) {
        case WidgetApiUtilsState::UNKNOWN: return "UNKNOWN";
        case WidgetApiUtilsState::INIT: return "INIT";
        case WidgetApiUtilsState::PROCESSING: return "PROCESSING";
        case WidgetApiUtilsState::DONE: return "DONE";
        case WidgetApiUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

WidgetApiUtilsState widget_api_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return WidgetApiUtilsState::UNKNOWN;
    if (s == "INIT") return WidgetApiUtilsState::INIT;
    if (s == "PROCESSING") return WidgetApiUtilsState::PROCESSING;
    if (s == "DONE") return WidgetApiUtilsState::DONE;
    if (s == "ERROR") return WidgetApiUtilsState::ERROR;
    return WidgetApiUtilsState::UNKNOWN;
}

bool widget_api_utils_init(const WidgetApiUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = WidgetApiUtilsState::INIT;
...
WidgetApiUtilsResponse widget_api_utils_handle(const WidgetApiUtilsRequest& req) {
    WidgetApiUtilsResponse resp;
    ...
    g_status.state = WidgetApiUtilsState::PROCESSING;
    // Validate request
...
WidgetApiUtilsStatus widget_api_utils_getStatus() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_status;
}

void widget_api_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = WidgetApiUtilsConfig{};
    g_status = WidgetApiUtilsStatus{};
    g_status.state = WidgetApiUtilsState::UNKNOWN;
}
...
std::string widget_api_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "widget_api_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
WidgetApiUtilsStats widget_api_utils_getStats() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_stats;
}

WidgetApiUtilsConfig widget_api_utils_configFromJson(const json& j) {
    WidgetApiUtilsConfig config;
...
json widget_api_utils_configToJson(const WidgetApiUtilsConfig& config) {
    json j;
