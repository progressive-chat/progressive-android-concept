...
namespace {
    MatrixIdUtilsConfig g_config;
    std::mutex g_mutex;
    MatrixIdUtilsStatus g_status;
    MatrixIdUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* matrix_id_utils_stateToString(MatrixIdUtilsState s) {
    switch (s) {
        case MatrixIdUtilsState::UNKNOWN: return "UNKNOWN";
        case MatrixIdUtilsState::INIT: return "INIT";
        case MatrixIdUtilsState::PROCESSING: return "PROCESSING";
        case MatrixIdUtilsState::DONE: return "DONE";
        case MatrixIdUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

MatrixIdUtilsState matrix_id_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return MatrixIdUtilsState::UNKNOWN;
    if (s == "INIT") return MatrixIdUtilsState::INIT;
    if (s == "PROCESSING") return MatrixIdUtilsState::PROCESSING;
    if (s == "DONE") return MatrixIdUtilsState::DONE;
    if (s == "ERROR") return MatrixIdUtilsState::ERROR;
    return MatrixIdUtilsState::UNKNOWN;
}
...
bool matrix_id_utils_init(const MatrixIdUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = MatrixIdUtilsState::INIT;
...
MatrixIdUtilsResponse matrix_id_utils_handle(const MatrixIdUtilsRequest& req) {
    ...
    g_status.state = MatrixIdUtilsState::PROCESSING;
    // Validate request
...
void matrix_id_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = MatrixIdUtilsConfig{};
    g_status = MatrixIdUtilsStatus{};
    g_status.state = MatrixIdUtilsState::UNKNOWN;
}
