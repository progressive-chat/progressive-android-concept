...
namespace {
    CustomStatusConfig g_config;
    std::mutex g_mutex;
    CustomStatusStatus g_status;
    CustomStatusStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* custom_status_stateToString(CustomStatusState s) {
    switch (s) {
        case CustomStatusState::UNKNOWN: return "UNKNOWN";
        case CustomStatusState::INIT: return "INIT";
        case CustomStatusState::PROCESSING: return "PROCESSING";
        case CustomStatusState::DONE: return "DONE";
        case CustomStatusState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

CustomStatusState custom_status_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return CustomStatusState::UNKNOWN;
    if (s == "INIT") return CustomStatusState::INIT;
    if (s == "PROCESSING") return CustomStatusState::PROCESSING;
    if (s == "DONE") return CustomStatusState::DONE;
    if (s == "ERROR") return CustomStatusState::ERROR;
    return CustomStatusState::UNKNOWN;
}
...
bool custom_status_init(const CustomStatusConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = CustomStatusState::INIT;
...
CustomStatusResponse custom_status_handle(const CustomStatusRequest& req) {
    ...
    g_status.state = CustomStatusState::PROCESSING;
    // Validate request
...
void custom_status_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = CustomStatusConfig{};
    g_status = CustomStatusStatus{};
    g_status.state = CustomStatusState::UNKNOWN;
}
