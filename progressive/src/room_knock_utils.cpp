...
namespace {
    RoomKnockUtilsConfig g_config;
    std::mutex g_mutex;
    RoomKnockUtilsStatus g_status;
    RoomKnockUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* room_knock_utils_stateToString(KnockUtilsAction s) {
    switch (s) {
        case KnockUtilsAction::UNKNOWN: return "UNKNOWN";
        case KnockUtilsAction::INIT: return "INIT";
        case KnockUtilsAction::PROCESSING: return "PROCESSING";
        case KnockUtilsAction::DONE: return "DONE";
        case KnockUtilsAction::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

KnockUtilsAction room_knock_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return KnockUtilsAction::UNKNOWN;
    if (s == "INIT") return KnockUtilsAction::INIT;
    if (s == "PROCESSING") return KnockUtilsAction::PROCESSING;
    if (s == "DONE") return KnockUtilsAction::DONE;
    if (s == "ERROR") return KnockUtilsAction::ERROR;
    return KnockUtilsAction::UNKNOWN;
}
...
bool room_knock_utils_init(const RoomKnockUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = KnockUtilsAction::INIT;
...
RoomKnockUtilsResponse room_knock_utils_handle(const RoomKnockUtilsRequest& req) {
    ...
    g_status.state = KnockUtilsAction::PROCESSING;
    // Validate request
...
void room_knock_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = RoomKnockUtilsConfig{};
    g_status = RoomKnockUtilsStatus{};
    g_status.state = KnockUtilsAction::UNKNOWN;
}
...
std::string room_knock_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "room_knock_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
