...
namespace {
    EventReminderConfig g_config;
    std::mutex g_mutex;
    EventReminderStatus g_status;
    EventReminderStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* event_reminder_stateToString(ReminderState s) {
    switch (s) {
        case ReminderState::UNKNOWN: return "UNKNOWN";
        case ReminderState::INIT: return "INIT";
        case ReminderState::PROCESSING: return "PROCESSING";
        case ReminderState::DONE: return "DONE";
...
ReminderState event_reminder_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return ReminderState::UNKNOWN;
    if (s == "INIT") return ReminderState::INIT;
    if (s == "PROCESSING") return ReminderState::PROCESSING;
    if (s == "DONE") return ReminderState::DONE;
    if (s == "ERROR") return ReminderState::ERROR;
...
bool event_reminder_init(const EventReminderConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = ReminderState::INIT;
...
EventReminderResponse event_reminder_handle(const EventReminderRequest& req) {
    ...
    g_status.state = ReminderState::PROCESSING;
    // Validate request
...
void event_reminder_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = EventReminderConfig{};
    g_status = EventReminderStatus{};
    g_status.state = ReminderState::UNKNOWN;
}
