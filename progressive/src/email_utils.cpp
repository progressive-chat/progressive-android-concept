...
namespace {
    EmailUtilsConfig g_config;
    std::mutex g_mutex;
    EmailUtilsStatus g_status;
    EmailUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* email_utils_stateToString(EmailUtilsState s) {
    switch (s) {
        case EmailUtilsState::UNKNOWN: return "UNKNOWN";
        case EmailUtilsState::INIT: return "INIT";
        case EmailUtilsState::PROCESSING: return "PROCESSING";
        case EmailUtilsState::DONE: return "DONE";
        case EmailUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

EmailUtilsState email_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return EmailUtilsState::UNKNOWN;
    if (s == "INIT") return EmailUtilsState::INIT;
    if (s == "PROCESSING") return EmailUtilsState::PROCESSING;
    if (s == "DONE") return EmailUtilsState::DONE;
    if (s == "ERROR") return EmailUtilsState::ERROR;
    return EmailUtilsState::UNKNOWN;
}

bool email_utils_init(const EmailUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = EmailUtilsState::INIT;
...
EmailUtilsResponse email_utils_handle(const EmailUtilsRequest& req) {
    EmailUtilsResponse resp;
    ...
    g_status.state = EmailUtilsState::PROCESSING;
    // Validate request
...
EmailUtilsStatus email_utils_getStatus() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_status;
}

void email_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = EmailUtilsConfig{};
    g_status = EmailUtilsStatus{};
    g_status.state = EmailUtilsState::UNKNOWN;
}
...
EmailUtilsStats email_utils_getStats() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_stats;
}

EmailUtilsConfig email_utils_configFromJson(const json& j) {
    EmailUtilsConfig config;
...
json email_utils_configToJson(const EmailUtilsConfig& config) {
    json j;
