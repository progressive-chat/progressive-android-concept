...
namespace {
    UserDirectoryConfig g_config;
    std::mutex g_mutex;
    UserDirectoryStatus g_status;
    UserDirectoryStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* user_directory_stateToString(DirectoryState s) {
    switch (s) {
        case DirectoryState::UNKNOWN: return "UNKNOWN";
        case DirectoryState::INIT: return "INIT";
        case DirectoryState::PROCESSING: return "PROCESSING";
        case DirectoryState::DONE: return "DONE";
        case DirectoryState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

DirectoryState user_directory_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return DirectoryState::UNKNOWN;
    if (s == "INIT") return DirectoryState::INIT;
    if (s == "PROCESSING") return DirectoryState::PROCESSING;
    if (s == "DONE") return DirectoryState::DONE;
    if (s == "ERROR") return DirectoryState::ERROR;
...
bool user_directory_init(const UserDirectoryConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = DirectoryState::INIT;
...
UserDirectoryResponse user_directory_handle(const UserDirectoryRequest& req) {
    ...
    g_status.state = DirectoryState::PROCESSING;
    // Validate request
...
void user_directory_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = UserDirectoryConfig{};
    g_status = UserDirectoryStatus{};
    g_status.state = DirectoryState::UNKNOWN;
}
