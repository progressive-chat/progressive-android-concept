...
namespace progressive {
std::string computeRecoveryKey(const std::string& keyBytes) { return keyBytes; }
bool isValidBase58Char(char c) { return (c >= '1' && c <= '9') || (c >= 'A' && c <= 'Z') || (c >= 'a' && c <= 'z'); }
std::string extractCurveKeyFromRecoveryKey(const std::string& key) { return key; }
} // namespace progressive
