...
namespace {
    CryptoDeviceUtilsConfig g_config;
    std::mutex g_mutex;
    CryptoDeviceUtilsStatus g_status;
    CryptoDeviceUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* crypto_device_utils_stateToString(CryptoDeviceUtilsState s) {
    switch (s) {
        case CryptoDeviceUtilsState::UNKNOWN: return "UNKNOWN";
        case CryptoDeviceUtilsState::INIT: return "INIT";
        case CryptoDeviceUtilsState::PROCESSING: return "PROCESSING";
        case CryptoDeviceUtilsState::DONE: return "DONE";
        case CryptoDeviceUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

CryptoDeviceUtilsState crypto_device_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return CryptoDeviceUtilsState::UNKNOWN;
    if (s == "INIT") return CryptoDeviceUtilsState::INIT;
    if (s == "PROCESSING") return CryptoDeviceUtilsState::PROCESSING;
    if (s == "DONE") return CryptoDeviceUtilsState::DONE;
    if (s == "ERROR") return CryptoDeviceUtilsState::ERROR;
    return CryptoDeviceUtilsState::UNKNOWN;
}
...
bool crypto_device_utils_init(const CryptoDeviceUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = CryptoDeviceUtilsState::INIT;
...
CryptoDeviceUtilsResponse crypto_device_utils_handle(const CryptoDeviceUtilsRequest& req) {
    CryptoDeviceUtilsResponse resp;
    ...
    g_status.state = CryptoDeviceUtilsState::PROCESSING;
    // Validate request
...
void crypto_device_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = CryptoDeviceUtilsConfig{};
    g_status = CryptoDeviceUtilsStatus{};
    g_status.state = CryptoDeviceUtilsState::UNKNOWN;
}
...
std::string crypto_device_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "crypto_device_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
CryptoDeviceUtilsConfig crypto_device_utils_configFromJson(const json& j) {
    CryptoDeviceUtilsConfig config;
