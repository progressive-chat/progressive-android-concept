...
namespace {
    EncryptionKeyUtilsConfig g_config;
    std::mutex g_mutex;
    EncryptionKeyUtilsStatus g_status;
    EncryptionKeyUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* encryption_key_utils_stateToString(KeyUtilsMode s) {
    switch (s) {
        case KeyUtilsMode::UNKNOWN: return "UNKNOWN";
        case KeyUtilsMode::INIT: return "INIT";
        case KeyUtilsMode::PROCESSING: return "PROCESSING";
        case KeyUtilsMode::DONE: return "DONE";
        case KeyUtilsMode::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

KeyUtilsMode encryption_key_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return KeyUtilsMode::UNKNOWN;
    if (s == "INIT") return KeyUtilsMode::INIT;
    if (s == "PROCESSING") return KeyUtilsMode::PROCESSING;
    if (s == "DONE") return KeyUtilsMode::DONE;
    if (s == "ERROR") return KeyUtilsMode::ERROR;
    return KeyUtilsMode::UNKNOWN;
}
...
bool encryption_key_utils_init(const EncryptionKeyUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = KeyUtilsMode::INIT;
...
EncryptionKeyUtilsResponse encryption_key_utils_handle(const EncryptionKeyUtilsRequest& req) {
    EncryptionKeyUtilsResponse resp;
    ...
    g_status.state = KeyUtilsMode::PROCESSING;
    // Validate request
...
void encryption_key_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = EncryptionKeyUtilsConfig{};
    g_status = EncryptionKeyUtilsStatus{};
    g_status.state = KeyUtilsMode::UNKNOWN;
}
...
std::string encryption_key_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "encryption_key_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
EncryptionKeyUtilsConfig encryption_key_utils_configFromJson(const json& j) {
    EncryptionKeyUtilsConfig config;
