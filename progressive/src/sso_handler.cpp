...
namespace {
    SsoHandlerConfig g_config;
    std::mutex g_mutex;
    SsoHandlerStatus g_status;
    SsoHandlerStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* sso_handler_stateToString(SsoHandlerState s) {
    switch (s) {
        case SsoHandlerState::UNKNOWN: return "UNKNOWN";
        case SsoHandlerState::INIT: return "INIT";
        case SsoHandlerState::PROCESSING: return "PROCESSING";
        case SsoHandlerState::DONE: return "DONE";
        case SsoHandlerState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

SsoHandlerState sso_handler_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return SsoHandlerState::UNKNOWN;
    if (s == "INIT") return SsoHandlerState::INIT;
    if (s == "PROCESSING") return SsoHandlerState::PROCESSING;
    if (s == "DONE") return SsoHandlerState::DONE;
    if (s == "ERROR") return SsoHandlerState::ERROR;
    return SsoHandlerState::UNKNOWN;
}
...
bool sso_handler_init(const SsoHandlerConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = SsoHandlerState::INIT;
...
SsoHandlerResponse sso_handler_handle(const SsoHandlerRequest& req) {
    ...
    g_status.state = SsoHandlerState::PROCESSING;
    // Validate request
...
void sso_handler_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = SsoHandlerConfig{};
    g_status = SsoHandlerStatus{};
    g_status.state = SsoHandlerState::UNKNOWN;
}
