...
namespace {
    RoomAutocompleteConfig g_config;
    std::mutex g_mutex;
    RoomAutocompleteStatus g_status;
    RoomAutocompleteStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* room_autocomplete_stateToString(AutocompleteAction s) {
    switch (s) {
        case AutocompleteAction::UNKNOWN: return "UNKNOWN";
        case AutocompleteAction::INIT: return "INIT";
        case AutocompleteAction::PROCESSING: return "PROCESSING";
        case AutocompleteAction::DONE: return "DONE";
        case AutocompleteAction::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

AutocompleteAction room_autocomplete_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return AutocompleteAction::UNKNOWN;
    if (s == "INIT") return AutocompleteAction::INIT;
    if (s == "PROCESSING") return AutocompleteAction::PROCESSING;
    if (s == "DONE") return AutocompleteAction::DONE;
    if (s == "ERROR") return AutocompleteAction::ERROR;
    return AutocompleteAction::UNKNOWN;
}

bool room_autocomplete_init(const RoomAutocompleteConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = AutocompleteAction::INIT;
...
RoomAutocompleteResponse room_autocomplete_handle(const RoomAutocompleteRequest& req) {
    RoomAutocompleteResponse resp;
    ...
    g_status.state = AutocompleteAction::PROCESSING;
    // Validate request
...
RoomAutocompleteStatus room_autocomplete_getStatus() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_status;
}

void room_autocomplete_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = RoomAutocompleteConfig{};
    g_status = RoomAutocompleteStatus{};
    g_status.state = AutocompleteAction::UNKNOWN;
}
...
std::string room_autocomplete_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "room_autocomplete";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
RoomAutocompleteStats room_autocomplete_getStats() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_stats;
}

RoomAutocompleteConfig room_autocomplete_configFromJson(const json& j) {
    RoomAutocompleteConfig config;
...
json room_autocomplete_configToJson(const RoomAutocompleteConfig& config) {
    json j;
