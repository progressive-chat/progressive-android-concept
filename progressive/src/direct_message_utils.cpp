...
namespace {
    DirectMessageUtilsConfig g_config;
    std::mutex g_mutex;
    DirectMessageUtilsStatus g_status;
    DirectMessageUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* direct_message_utils_stateToString(DirectMessageUtilsState s) {
    switch (s) {
        case DirectMessageUtilsState::UNKNOWN: return "UNKNOWN";
        case DirectMessageUtilsState::INIT: return "INIT";
        case DirectMessageUtilsState::PROCESSING: return "PROCESSING";
        case DirectMessageUtilsState::DONE: return "DONE";
        case DirectMessageUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

DirectMessageUtilsState direct_message_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return DirectMessageUtilsState::UNKNOWN;
    if (s == "INIT") return DirectMessageUtilsState::INIT;
    if (s == "PROCESSING") return DirectMessageUtilsState::PROCESSING;
    if (s == "DONE") return DirectMessageUtilsState::DONE;
    if (s == "ERROR") return DirectMessageUtilsState::ERROR;
    return DirectMessageUtilsState::UNKNOWN;
}
...
bool direct_message_utils_init(const DirectMessageUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = DirectMessageUtilsState::INIT;
...
DirectMessageUtilsResponse direct_message_utils_handle(const DirectMessageUtilsRequest& req) {
    DirectMessageUtilsResponse resp;
    ...
    g_status.state = DirectMessageUtilsState::PROCESSING;
    // Validate request
...
void direct_message_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = DirectMessageUtilsConfig{};
    g_status = DirectMessageUtilsStatus{};
    g_status.state = DirectMessageUtilsState::UNKNOWN;
}
...
std::string direct_message_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "direct_message_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
DirectMessageUtilsConfig direct_message_utils_configFromJson(const json& j) {
    DirectMessageUtilsConfig config;
