...
namespace {
    RoomEncryptionEventUtilsConfig g_config;
    std::mutex g_mutex;
    RoomEncryptionEventUtilsStatus g_status;
    RoomEncryptionEventUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* room_encryption_event_utils_stateToString(EncryptionEventUtilsAction s) {
    switch (s) {
        case EncryptionEventUtilsAction::UNKNOWN: return "UNKNOWN";
        case EncryptionEventUtilsAction::INIT: return "INIT";
        case EncryptionEventUtilsAction::PROCESSING: return "PROCESSING";
        case EncryptionEventUtilsAction::DONE: return "DONE";
        case EncryptionEventUtilsAction::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

EncryptionEventUtilsAction room_encryption_event_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return EncryptionEventUtilsAction::UNKNOWN;
    if (s == "INIT") return EncryptionEventUtilsAction::INIT;
    if (s == "PROCESSING") return EncryptionEventUtilsAction::PROCESSING;
    if (s == "DONE") return EncryptionEventUtilsAction::DONE;
    if (s == "ERROR") return EncryptionEventUtilsAction::ERROR;
    return EncryptionEventUtilsAction::UNKNOWN;
}
...
bool room_encryption_event_utils_init(const RoomEncryptionEventUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = EncryptionEventUtilsAction::INIT;
...
RoomEncryptionEventUtilsResponse room_encryption_event_utils_handle(const RoomEncryptionEventUtilsRequest& req) {
    RoomEncryptionEventUtilsResponse resp;
    ...
    g_status.state = EncryptionEventUtilsAction::PROCESSING;
    // Validate request
...
void room_encryption_event_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = RoomEncryptionEventUtilsConfig{};
    g_status = RoomEncryptionEventUtilsStatus{};
    g_status.state = EncryptionEventUtilsAction::UNKNOWN;
}
...
std::string room_encryption_event_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "room_encryption_event_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
RoomEncryptionEventUtilsConfig room_encryption_event_utils_configFromJson(const json& j) {
    RoomEncryptionEventUtilsConfig config;
