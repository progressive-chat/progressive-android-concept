...
namespace {
    UrlPreviewParserConfig g_config;
    std::mutex g_mutex;
    UrlPreviewParserStatus g_status;
    UrlPreviewParserStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* url_preview_parser_stateToString(UrlPreviewParserState s) {
    switch (s) {
        case UrlPreviewParserState::UNKNOWN: return "UNKNOWN";
        case UrlPreviewParserState::INIT: return "INIT";
        case UrlPreviewParserState::PROCESSING: return "PROCESSING";
        case UrlPreviewParserState::DONE: return "DONE";
        case UrlPreviewParserState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

UrlPreviewParserState url_preview_parser_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return UrlPreviewParserState::UNKNOWN;
    if (s == "INIT") return UrlPreviewParserState::INIT;
    if (s == "PROCESSING") return UrlPreviewParserState::PROCESSING;
    if (s == "DONE") return UrlPreviewParserState::DONE;
    if (s == "ERROR") return UrlPreviewParserState::ERROR;
    return UrlPreviewParserState::UNKNOWN;
}
...
bool url_preview_parser_init(const UrlPreviewParserConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = UrlPreviewParserState::INIT;
...
UrlPreviewParserResponse url_preview_parser_handle(const UrlPreviewParserRequest& req) {
    ...
    g_status.state = UrlPreviewParserState::PROCESSING;
    // Validate request
...
void url_preview_parser_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = UrlPreviewParserConfig{};
    g_status = UrlPreviewParserStatus{};
    g_status.state = UrlPreviewParserState::UNKNOWN;
}
...
std::string url_preview_parser_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "url_preview_parser";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
