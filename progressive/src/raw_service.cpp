...
namespace {
    RawServiceConfig g_config;
    std::mutex g_mutex;
    RawServiceStatus g_status;
    RawServiceStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* raw_service_stateToString(RawServiceState s) {
    switch (s) {
        case RawServiceState::UNKNOWN: return "UNKNOWN";
        case RawServiceState::INIT: return "INIT";
        case RawServiceState::PROCESSING: return "PROCESSING";
        case RawServiceState::DONE: return "DONE";
        case RawServiceState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

RawServiceState raw_service_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return RawServiceState::UNKNOWN;
    if (s == "INIT") return RawServiceState::INIT;
    if (s == "PROCESSING") return RawServiceState::PROCESSING;
    if (s == "DONE") return RawServiceState::DONE;
    if (s == "ERROR") return RawServiceState::ERROR;
    return RawServiceState::UNKNOWN;
}
...
bool raw_service_init(const RawServiceConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = RawServiceState::INIT;
...
RawServiceResponse raw_service_handle(const RawServiceRequest& req) {
    ...
    g_status.state = RawServiceState::PROCESSING;
    // Validate request
...
void raw_service_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = RawServiceConfig{};
    g_status = RawServiceStatus{};
    g_status.state = RawServiceState::UNKNOWN;
}
