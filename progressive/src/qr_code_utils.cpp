...
namespace {
    QrCodeUtilsConfig g_config;
    std::mutex g_mutex;
    QrCodeUtilsStatus g_status;
    QrCodeUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* qr_code_utils_stateToString(QrCodeUtilsState s) {
    switch (s) {
        case QrCodeUtilsState::UNKNOWN: return "UNKNOWN";
        case QrCodeUtilsState::INIT: return "INIT";
        case QrCodeUtilsState::PROCESSING: return "PROCESSING";
        case QrCodeUtilsState::DONE: return "DONE";
        case QrCodeUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

QrCodeUtilsState qr_code_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return QrCodeUtilsState::UNKNOWN;
    if (s == "INIT") return QrCodeUtilsState::INIT;
    if (s == "PROCESSING") return QrCodeUtilsState::PROCESSING;
    if (s == "DONE") return QrCodeUtilsState::DONE;
    if (s == "ERROR") return QrCodeUtilsState::ERROR;
    return QrCodeUtilsState::UNKNOWN;
}
...
bool qr_code_utils_init(const QrCodeUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = QrCodeUtilsState::INIT;
...
QrCodeUtilsResponse qr_code_utils_handle(const QrCodeUtilsRequest& req) {
    ...
    g_status.state = QrCodeUtilsState::PROCESSING;
    // Validate request
...
void qr_code_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = QrCodeUtilsConfig{};
    g_status = QrCodeUtilsStatus{};
    g_status.state = QrCodeUtilsState::UNKNOWN;
}
