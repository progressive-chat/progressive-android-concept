...
namespace {
    ServerCapabilitiesConfig g_config;
    std::mutex g_mutex;
    ServerCapabilitiesStatus g_status;
    ServerCapabilitiesStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* server_capabilities_stateToString(CapabilitiesStatus s) {
    switch (s) {
        case CapabilitiesStatus::UNKNOWN: return "UNKNOWN";
        case CapabilitiesStatus::INIT: return "INIT";
        case CapabilitiesStatus::PROCESSING: return "PROCESSING";
        case CapabilitiesStatus::DONE: return "DONE";
        case CapabilitiesStatus::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

CapabilitiesStatus server_capabilities_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return CapabilitiesStatus::UNKNOWN;
    if (s == "INIT") return CapabilitiesStatus::INIT;
    if (s == "PROCESSING") return CapabilitiesStatus::PROCESSING;
    if (s == "DONE") return CapabilitiesStatus::DONE;
    if (s == "ERROR") return CapabilitiesStatus::ERROR;
...
bool server_capabilities_init(const ServerCapabilitiesConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = CapabilitiesStatus::INIT;
...
ServerCapabilitiesResponse server_capabilities_handle(const ServerCapabilitiesRequest& req) {
    ServerCapabilitiesResponse resp;
    ...
    g_status.state = CapabilitiesStatus::PROCESSING;
    // Validate request
...
void server_capabilities_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = ServerCapabilitiesConfig{};
    g_status = ServerCapabilitiesStatus{};
    g_status.state = CapabilitiesStatus::UNKNOWN;
}
...
std::string server_capabilities_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "server_capabilities";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
ServerCapabilitiesConfig server_capabilities_configFromJson(const json& j) {
    ServerCapabilitiesConfig config;
