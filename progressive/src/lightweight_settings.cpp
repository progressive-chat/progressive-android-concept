...
namespace {
    LightweightSettingsConfig g_config;
    std::mutex g_mutex;
    LightweightSettingsStatus g_status;
    LightweightSettingsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* lightweight_settings_stateToString(LightweightSettingsState s) {
    switch (s) {
        case LightweightSettingsState::UNKNOWN: return "UNKNOWN";
        case LightweightSettingsState::INIT: return "INIT";
        case LightweightSettingsState::PROCESSING: return "PROCESSING";
        case LightweightSettingsState::DONE: return "DONE";
        case LightweightSettingsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

LightweightSettingsState lightweight_settings_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return LightweightSettingsState::UNKNOWN;
    if (s == "INIT") return LightweightSettingsState::INIT;
    if (s == "PROCESSING") return LightweightSettingsState::PROCESSING;
    if (s == "DONE") return LightweightSettingsState::DONE;
    if (s == "ERROR") return LightweightSettingsState::ERROR;
    return LightweightSettingsState::UNKNOWN;
}

bool lightweight_settings_init(const LightweightSettingsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = LightweightSettingsState::INIT;
...
LightweightSettingsResponse lightweight_settings_handle(const LightweightSettingsRequest& req) {
    LightweightSettingsResponse resp;
    ...
    g_status.state = LightweightSettingsState::PROCESSING;
    // Validate request
...
LightweightSettingsStatus lightweight_settings_getStatus() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_status;
}

void lightweight_settings_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = LightweightSettingsConfig{};
    g_status = LightweightSettingsStatus{};
    g_status.state = LightweightSettingsState::UNKNOWN;
}
...
std::string lightweight_settings_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "lightweight_settings";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
LightweightSettingsStats lightweight_settings_getStats() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_stats;
}

LightweightSettingsConfig lightweight_settings_configFromJson(const json& j) {
    LightweightSettingsConfig config;
...
json lightweight_settings_configToJson(const LightweightSettingsConfig& config) {
    json j;
