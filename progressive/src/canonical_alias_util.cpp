...
namespace {
    CanonicalAliasUtilConfig g_config;
    std::mutex g_mutex;
    CanonicalAliasUtilStatus g_status;
    CanonicalAliasUtilStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* canonical_alias_util_stateToString(CanonicalAliasUtilState s) {
    switch (s) {
        case CanonicalAliasUtilState::UNKNOWN: return "UNKNOWN";
        case CanonicalAliasUtilState::INIT: return "INIT";
        case CanonicalAliasUtilState::PROCESSING: return "PROCESSING";
        case CanonicalAliasUtilState::DONE: return "DONE";
        case CanonicalAliasUtilState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

CanonicalAliasUtilState canonical_alias_util_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return CanonicalAliasUtilState::UNKNOWN;
    if (s == "INIT") return CanonicalAliasUtilState::INIT;
    if (s == "PROCESSING") return CanonicalAliasUtilState::PROCESSING;
    if (s == "DONE") return CanonicalAliasUtilState::DONE;
    if (s == "ERROR") return CanonicalAliasUtilState::ERROR;
    return CanonicalAliasUtilState::UNKNOWN;
}
...
bool canonical_alias_util_init(const CanonicalAliasUtilConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = CanonicalAliasUtilState::INIT;
...
CanonicalAliasUtilResponse canonical_alias_util_handle(const CanonicalAliasUtilRequest& req) {
    CanonicalAliasUtilResponse resp;
    ...
    g_status.state = CanonicalAliasUtilState::PROCESSING;
    // Validate request
...
void canonical_alias_util_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = CanonicalAliasUtilConfig{};
    g_status = CanonicalAliasUtilStatus{};
    g_status.state = CanonicalAliasUtilState::UNKNOWN;
}
...
std::string canonical_alias_util_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "canonical_alias_util";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
CanonicalAliasUtilConfig canonical_alias_util_configFromJson(const json& j) {
    CanonicalAliasUtilConfig config;
