...
namespace {
    ScheduledEditConfig g_config;
    std::mutex g_mutex;
    ScheduledEditStatus g_status;
    ScheduledEditStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* scheduled_edit_stateToString(ScheduledEditState s) {
    switch (s) {
        case ScheduledEditState::UNKNOWN: return "UNKNOWN";
        case ScheduledEditState::INIT: return "INIT";
        case ScheduledEditState::PROCESSING: return "PROCESSING";
        case ScheduledEditState::DONE: return "DONE";
        case ScheduledEditState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

ScheduledEditState scheduled_edit_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return ScheduledEditState::UNKNOWN;
    if (s == "INIT") return ScheduledEditState::INIT;
    if (s == "PROCESSING") return ScheduledEditState::PROCESSING;
    if (s == "DONE") return ScheduledEditState::DONE;
    if (s == "ERROR") return ScheduledEditState::ERROR;
    return ScheduledEditState::UNKNOWN;
}
...
bool scheduled_edit_init(const ScheduledEditConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = ScheduledEditState::INIT;
...
ScheduledEditResponse scheduled_edit_handle(const ScheduledEditRequest& req) {
    ...
    g_status.state = ScheduledEditState::PROCESSING;
    // Validate request
...
void scheduled_edit_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = ScheduledEditConfig{};
    g_status = ScheduledEditStatus{};
    g_status.state = ScheduledEditState::UNKNOWN;
}
