...
namespace {
    RoomHistoryUtilsConfig g_config;
    std::mutex g_mutex;
    RoomHistoryUtilsStatus g_status;
    RoomHistoryUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* room_history_utils_stateToString(HistoryUtilsAction s) {
    switch (s) {
        case HistoryUtilsAction::UNKNOWN: return "UNKNOWN";
        case HistoryUtilsAction::INIT: return "INIT";
        case HistoryUtilsAction::PROCESSING: return "PROCESSING";
        case HistoryUtilsAction::DONE: return "DONE";
        case HistoryUtilsAction::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

HistoryUtilsAction room_history_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return HistoryUtilsAction::UNKNOWN;
    if (s == "INIT") return HistoryUtilsAction::INIT;
    if (s == "PROCESSING") return HistoryUtilsAction::PROCESSING;
    if (s == "DONE") return HistoryUtilsAction::DONE;
    if (s == "ERROR") return HistoryUtilsAction::ERROR;
    return HistoryUtilsAction::UNKNOWN;
}

bool room_history_utils_init(const RoomHistoryUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = HistoryUtilsAction::INIT;
...
RoomHistoryUtilsResponse room_history_utils_handle(const RoomHistoryUtilsRequest& req) {
    RoomHistoryUtilsResponse resp;
    ...
    g_status.state = HistoryUtilsAction::PROCESSING;
    // Validate request
...
RoomHistoryUtilsStatus room_history_utils_getStatus() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_status;
}

void room_history_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = RoomHistoryUtilsConfig{};
    g_status = RoomHistoryUtilsStatus{};
    g_status.state = HistoryUtilsAction::UNKNOWN;
}

bool room_history_utils_validate(const std::string& input) {
...
std::string room_history_utils_process(const std::string& input) {
    std::lock_guard<std::mutex> lock(g_mutex);
    ...
    try {
        ...
        result["module"] = "room_history_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
json room_history_utils_toJson(const std::string& input) {
    json j;
...
RoomHistoryUtilsStats room_history_utils_getStats() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_stats;
}

RoomHistoryUtilsConfig room_history_utils_configFromJson(const json& j) {
    RoomHistoryUtilsConfig config;
...
json room_history_utils_configToJson(const RoomHistoryUtilsConfig& config) {
    json j;
