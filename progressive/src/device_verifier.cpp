...
namespace {
    DeviceVerifierConfig g_config;
    std::mutex g_mutex;
    DeviceVerifierStatus g_status;
    DeviceVerifierStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* device_verifier_stateToString(DeviceVerifierState s) {
    switch (s) {
        case DeviceVerifierState::UNKNOWN: return "UNKNOWN";
        case DeviceVerifierState::INIT: return "INIT";
        case DeviceVerifierState::PROCESSING: return "PROCESSING";
        case DeviceVerifierState::DONE: return "DONE";
        case DeviceVerifierState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

DeviceVerifierState device_verifier_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return DeviceVerifierState::UNKNOWN;
    if (s == "INIT") return DeviceVerifierState::INIT;
    if (s == "PROCESSING") return DeviceVerifierState::PROCESSING;
    if (s == "DONE") return DeviceVerifierState::DONE;
    if (s == "ERROR") return DeviceVerifierState::ERROR;
    return DeviceVerifierState::UNKNOWN;
}

bool device_verifier_init(const DeviceVerifierConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = DeviceVerifierState::INIT;
...
DeviceVerifierResponse device_verifier_handle(const DeviceVerifierRequest& req) {
    DeviceVerifierResponse resp;
    ...
    g_status.state = DeviceVerifierState::PROCESSING;
    // Validate request
...
DeviceVerifierStatus device_verifier_getStatus() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_status;
}

void device_verifier_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = DeviceVerifierConfig{};
    g_status = DeviceVerifierStatus{};
    g_status.state = DeviceVerifierState::UNKNOWN;
}
...
DeviceVerifierStats device_verifier_getStats() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_stats;
}

DeviceVerifierConfig device_verifier_configFromJson(const json& j) {
    DeviceVerifierConfig config;
...
json device_verifier_configToJson(const DeviceVerifierConfig& config) {
    json j;
