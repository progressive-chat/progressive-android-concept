...
namespace {
    RoomUnreadUtilsConfig g_config;
    std::mutex g_mutex;
    RoomUnreadUtilsStatus g_status;
    RoomUnreadUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* room_unread_utils_stateToString(UnreadUtilsAction s) {
    switch (s) {
        case UnreadUtilsAction::UNKNOWN: return "UNKNOWN";
        case UnreadUtilsAction::INIT: return "INIT";
        case UnreadUtilsAction::PROCESSING: return "PROCESSING";
        case UnreadUtilsAction::DONE: return "DONE";
        case UnreadUtilsAction::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

UnreadUtilsAction room_unread_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return UnreadUtilsAction::UNKNOWN;
    if (s == "INIT") return UnreadUtilsAction::INIT;
    if (s == "PROCESSING") return UnreadUtilsAction::PROCESSING;
    if (s == "DONE") return UnreadUtilsAction::DONE;
    if (s == "ERROR") return UnreadUtilsAction::ERROR;
    return UnreadUtilsAction::UNKNOWN;
}

bool room_unread_utils_init(const RoomUnreadUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = UnreadUtilsAction::INIT;
...
RoomUnreadUtilsResponse room_unread_utils_handle(const RoomUnreadUtilsRequest& req) {
    RoomUnreadUtilsResponse resp;
    ...
    g_status.state = UnreadUtilsAction::PROCESSING;
    // Validate request
...
RoomUnreadUtilsStatus room_unread_utils_getStatus() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_status;
}

void room_unread_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = RoomUnreadUtilsConfig{};
    g_status = RoomUnreadUtilsStatus{};
    g_status.state = UnreadUtilsAction::UNKNOWN;
}

bool room_unread_utils_validate(const std::string& input) {
...
std::string room_unread_utils_process(const std::string& input) {
    std::lock_guard<std::mutex> lock(g_mutex);
    ...
    try {
        ...
        result["module"] = "room_unread_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
json room_unread_utils_toJson(const std::string& input) {
    json j;
...
RoomUnreadUtilsStats room_unread_utils_getStats() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_stats;
}

RoomUnreadUtilsConfig room_unread_utils_configFromJson(const json& j) {
    RoomUnreadUtilsConfig config;
...
json room_unread_utils_configToJson(const RoomUnreadUtilsConfig& config) {
    json j;
