...
namespace {
    PaginationUtilsConfig g_config;
    std::mutex g_mutex;
    PaginationUtilsStatus g_status;
    PaginationUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* pagination_utils_stateToString(PaginationUtilsState s) {
    switch (s) {
        case PaginationUtilsState::UNKNOWN: return "UNKNOWN";
        case PaginationUtilsState::INIT: return "INIT";
        case PaginationUtilsState::PROCESSING: return "PROCESSING";
        case PaginationUtilsState::DONE: return "DONE";
        case PaginationUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

PaginationUtilsState pagination_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return PaginationUtilsState::UNKNOWN;
    if (s == "INIT") return PaginationUtilsState::INIT;
    if (s == "PROCESSING") return PaginationUtilsState::PROCESSING;
    if (s == "DONE") return PaginationUtilsState::DONE;
    if (s == "ERROR") return PaginationUtilsState::ERROR;
    return PaginationUtilsState::UNKNOWN;
}

bool pagination_utils_init(const PaginationUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = PaginationUtilsState::INIT;
...
PaginationUtilsResponse pagination_utils_handle(const PaginationUtilsRequest& req) {
    PaginationUtilsResponse resp;
    ...
    g_status.state = PaginationUtilsState::PROCESSING;
    // Validate request
...
PaginationUtilsStatus pagination_utils_getStatus() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_status;
}

void pagination_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = PaginationUtilsConfig{};
    g_status = PaginationUtilsStatus{};
    g_status.state = PaginationUtilsState::UNKNOWN;
}
...
std::string pagination_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "pagination_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
PaginationUtilsStats pagination_utils_getStats() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_stats;
}

PaginationUtilsConfig pagination_utils_configFromJson(const json& j) {
    PaginationUtilsConfig config;
...
json pagination_utils_configToJson(const PaginationUtilsConfig& config) {
    json j;
