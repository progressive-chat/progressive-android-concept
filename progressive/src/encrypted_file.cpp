...
namespace {
    EncryptedFileConfig g_config;
    std::mutex g_mutex;
    EncryptedFileStatus g_status;
    EncryptedFileStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* encrypted_file_stateToString(EncryptedFileState s) {
    switch (s) {
        case EncryptedFileState::UNKNOWN: return "UNKNOWN";
        case EncryptedFileState::INIT: return "INIT";
        case EncryptedFileState::PROCESSING: return "PROCESSING";
        case EncryptedFileState::DONE: return "DONE";
        case EncryptedFileState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

EncryptedFileState encrypted_file_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return EncryptedFileState::UNKNOWN;
    if (s == "INIT") return EncryptedFileState::INIT;
    if (s == "PROCESSING") return EncryptedFileState::PROCESSING;
    if (s == "DONE") return EncryptedFileState::DONE;
    if (s == "ERROR") return EncryptedFileState::ERROR;
    return EncryptedFileState::UNKNOWN;
}
...
bool encrypted_file_init(const EncryptedFileConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = EncryptedFileState::INIT;
...
EncryptedFileResponse encrypted_file_handle(const EncryptedFileRequest& req) {
    ...
    g_status.state = EncryptedFileState::PROCESSING;
    // Validate request
...
void encrypted_file_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = EncryptedFileConfig{};
    g_status = EncryptedFileStatus{};
    g_status.state = EncryptedFileState::UNKNOWN;
}
