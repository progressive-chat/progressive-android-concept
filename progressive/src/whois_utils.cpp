...
namespace {
    WhoisUtilsConfig g_config;
    std::mutex g_mutex;
    WhoisUtilsStatus g_status;
    WhoisUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* whois_utils_stateToString(WhoisUtilsState s) {
    switch (s) {
        case WhoisUtilsState::UNKNOWN: return "UNKNOWN";
        case WhoisUtilsState::INIT: return "INIT";
        case WhoisUtilsState::PROCESSING: return "PROCESSING";
        case WhoisUtilsState::DONE: return "DONE";
        case WhoisUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

WhoisUtilsState whois_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return WhoisUtilsState::UNKNOWN;
    if (s == "INIT") return WhoisUtilsState::INIT;
    if (s == "PROCESSING") return WhoisUtilsState::PROCESSING;
    if (s == "DONE") return WhoisUtilsState::DONE;
    if (s == "ERROR") return WhoisUtilsState::ERROR;
    return WhoisUtilsState::UNKNOWN;
}
...
bool whois_utils_init(const WhoisUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = WhoisUtilsState::INIT;
...
WhoisUtilsResponse whois_utils_handle(const WhoisUtilsRequest& req) {
    ...
    g_status.state = WhoisUtilsState::PROCESSING;
    // Validate request
...
void whois_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = WhoisUtilsConfig{};
    g_status = WhoisUtilsStatus{};
    g_status.state = WhoisUtilsState::UNKNOWN;
}
