...
namespace {
    SessionSignoutConfig g_config;
    std::mutex g_mutex;
    SessionSignoutStatus g_status;
    SessionSignoutStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* session_signout_stateToString(SignoutPhase s) {
    switch (s) {
        case SignoutPhase::UNKNOWN: return "UNKNOWN";
        case SignoutPhase::INIT: return "INIT";
        case SignoutPhase::PROCESSING: return "PROCESSING";
        case SignoutPhase::DONE: return "DONE";
        case SignoutPhase::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

SignoutPhase session_signout_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return SignoutPhase::UNKNOWN;
    if (s == "INIT") return SignoutPhase::INIT;
    if (s == "PROCESSING") return SignoutPhase::PROCESSING;
    if (s == "DONE") return SignoutPhase::DONE;
    if (s == "ERROR") return SignoutPhase::ERROR;
    return SignoutPhase::UNKNOWN;
}
...
bool session_signout_init(const SessionSignoutConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = SignoutPhase::INIT;
...
SessionSignoutResponse session_signout_handle(const SessionSignoutRequest& req) {
    ...
    g_status.state = SignoutPhase::PROCESSING;
    // Validate request
...
void session_signout_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = SessionSignoutConfig{};
    g_status = SessionSignoutStatus{};
    g_status.state = SignoutPhase::UNKNOWN;
}
