...
namespace {
    OlmUtilsConfig g_config;
    std::mutex g_mutex;
    OlmUtilsStatus g_status;
    OlmUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* olm_utils_stateToString(UtilsStatus s) {
    switch (s) {
        case UtilsStatus::UNKNOWN: return "UNKNOWN";
        case UtilsStatus::INIT: return "INIT";
        case UtilsStatus::PROCESSING: return "PROCESSING";
        case UtilsStatus::DONE: return "DONE";
        case UtilsStatus::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

UtilsStatus olm_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return UtilsStatus::UNKNOWN;
    if (s == "INIT") return UtilsStatus::INIT;
    if (s == "PROCESSING") return UtilsStatus::PROCESSING;
    if (s == "DONE") return UtilsStatus::DONE;
    if (s == "ERROR") return UtilsStatus::ERROR;
    return UtilsStatus::UNKNOWN;
}
...
bool olm_utils_init(const OlmUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = UtilsStatus::INIT;
...
OlmUtilsResponse olm_utils_handle(const OlmUtilsRequest& req) {
    ...
    g_status.state = UtilsStatus::PROCESSING;
    // Validate request
...
void olm_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = OlmUtilsConfig{};
    g_status = OlmUtilsStatus{};
    g_status.state = UtilsStatus::UNKNOWN;
}
