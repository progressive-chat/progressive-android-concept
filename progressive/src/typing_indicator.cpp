...
namespace {
    TypingIndicatorConfig g_config;
    std::mutex g_mutex;
    TypingIndicatorStatus g_status;
    TypingIndicatorStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* typing_indicator_stateToString(TypingIndicatorState s) {
    switch (s) {
        case TypingIndicatorState::UNKNOWN: return "UNKNOWN";
        case TypingIndicatorState::INIT: return "INIT";
        case TypingIndicatorState::PROCESSING: return "PROCESSING";
        case TypingIndicatorState::DONE: return "DONE";
        case TypingIndicatorState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

TypingIndicatorState typing_indicator_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return TypingIndicatorState::UNKNOWN;
    if (s == "INIT") return TypingIndicatorState::INIT;
    if (s == "PROCESSING") return TypingIndicatorState::PROCESSING;
    if (s == "DONE") return TypingIndicatorState::DONE;
    if (s == "ERROR") return TypingIndicatorState::ERROR;
    return TypingIndicatorState::UNKNOWN;
}
...
bool typing_indicator_init(const TypingIndicatorConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = TypingIndicatorState::INIT;
...
TypingIndicatorResponse typing_indicator_handle(const TypingIndicatorRequest& req) {
    ...
    g_status.state = TypingIndicatorState::PROCESSING;
    // Validate request
...
void typing_indicator_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = TypingIndicatorConfig{};
    g_status = TypingIndicatorStatus{};
    g_status.state = TypingIndicatorState::UNKNOWN;
}
...
std::string typing_indicator_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "typing_indicator";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
