...
namespace {
    EventDeliveryUtilsConfig g_config;
    std::mutex g_mutex;
    EventDeliveryUtilsStatus g_status;
    EventDeliveryUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* event_delivery_utils_stateToString(DeliveryUtilsState s) {
    switch (s) {
        case DeliveryUtilsState::UNKNOWN: return "UNKNOWN";
        case DeliveryUtilsState::INIT: return "INIT";
        case DeliveryUtilsState::PROCESSING: return "PROCESSING";
        case DeliveryUtilsState::DONE: return "DONE";
        case DeliveryUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

DeliveryUtilsState event_delivery_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return DeliveryUtilsState::UNKNOWN;
    if (s == "INIT") return DeliveryUtilsState::INIT;
    if (s == "PROCESSING") return DeliveryUtilsState::PROCESSING;
    if (s == "DONE") return DeliveryUtilsState::DONE;
    if (s == "ERROR") return DeliveryUtilsState::ERROR;
    return DeliveryUtilsState::UNKNOWN;
}
...
bool event_delivery_utils_init(const EventDeliveryUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = DeliveryUtilsState::INIT;
...
EventDeliveryUtilsResponse event_delivery_utils_handle(const EventDeliveryUtilsRequest& req) {
    EventDeliveryUtilsResponse resp;
    ...
    g_status.state = DeliveryUtilsState::PROCESSING;
    // Validate request
...
void event_delivery_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = EventDeliveryUtilsConfig{};
    g_status = EventDeliveryUtilsStatus{};
    g_status.state = DeliveryUtilsState::UNKNOWN;
}
...
std::string event_delivery_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "event_delivery_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
EventDeliveryUtilsConfig event_delivery_utils_configFromJson(const json& j) {
    EventDeliveryUtilsConfig config;
