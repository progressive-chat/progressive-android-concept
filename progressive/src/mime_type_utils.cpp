...
namespace {
    MimeTypeUtilsConfig g_config;
    std::mutex g_mutex;
    MimeTypeUtilsStatus g_status;
    MimeTypeUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* mime_type_utils_stateToString(MimeTypeUtilsState s) {
    switch (s) {
        case MimeTypeUtilsState::UNKNOWN: return "UNKNOWN";
        case MimeTypeUtilsState::INIT: return "INIT";
        case MimeTypeUtilsState::PROCESSING: return "PROCESSING";
        case MimeTypeUtilsState::DONE: return "DONE";
        case MimeTypeUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

MimeTypeUtilsState mime_type_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return MimeTypeUtilsState::UNKNOWN;
    if (s == "INIT") return MimeTypeUtilsState::INIT;
    if (s == "PROCESSING") return MimeTypeUtilsState::PROCESSING;
    if (s == "DONE") return MimeTypeUtilsState::DONE;
    if (s == "ERROR") return MimeTypeUtilsState::ERROR;
    return MimeTypeUtilsState::UNKNOWN;
}
...
bool mime_type_utils_init(const MimeTypeUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = MimeTypeUtilsState::INIT;
...
MimeTypeUtilsResponse mime_type_utils_handle(const MimeTypeUtilsRequest& req) {
    ...
    g_status.state = MimeTypeUtilsState::PROCESSING;
    // Validate request
...
void mime_type_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = MimeTypeUtilsConfig{};
    g_status = MimeTypeUtilsStatus{};
    g_status.state = MimeTypeUtilsState::UNKNOWN;
}
