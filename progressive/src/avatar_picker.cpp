...
namespace {
    AvatarPickerConfig g_config;
    std::mutex g_mutex;
    AvatarPickerStatus g_status;
    AvatarPickerStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* avatar_picker_stateToString(AvatarPickerState s) {
    switch (s) {
        case AvatarPickerState::UNKNOWN: return "UNKNOWN";
        case AvatarPickerState::INIT: return "INIT";
        case AvatarPickerState::PROCESSING: return "PROCESSING";
        case AvatarPickerState::DONE: return "DONE";
        case AvatarPickerState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

AvatarPickerState avatar_picker_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return AvatarPickerState::UNKNOWN;
    if (s == "INIT") return AvatarPickerState::INIT;
    if (s == "PROCESSING") return AvatarPickerState::PROCESSING;
    if (s == "DONE") return AvatarPickerState::DONE;
    if (s == "ERROR") return AvatarPickerState::ERROR;
    return AvatarPickerState::UNKNOWN;
}
...
bool avatar_picker_init(const AvatarPickerConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = AvatarPickerState::INIT;
...
AvatarPickerResponse avatar_picker_handle(const AvatarPickerRequest& req) {
    ...
    g_status.state = AvatarPickerState::PROCESSING;
    // Validate request
...
void avatar_picker_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = AvatarPickerConfig{};
    g_status = AvatarPickerStatus{};
    g_status.state = AvatarPickerState::UNKNOWN;
}
