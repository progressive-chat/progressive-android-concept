...
namespace {
    ThemeManagerConfig g_config;
    std::mutex g_mutex;
    ThemeManagerStatus g_status;
    ThemeManagerStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* theme_manager_stateToString(ThemeManagerState s) {
    switch (s) {
        case ThemeManagerState::UNKNOWN: return "UNKNOWN";
        case ThemeManagerState::INIT: return "INIT";
        case ThemeManagerState::PROCESSING: return "PROCESSING";
        case ThemeManagerState::DONE: return "DONE";
        case ThemeManagerState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

ThemeManagerState theme_manager_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return ThemeManagerState::UNKNOWN;
    if (s == "INIT") return ThemeManagerState::INIT;
    if (s == "PROCESSING") return ThemeManagerState::PROCESSING;
    if (s == "DONE") return ThemeManagerState::DONE;
    if (s == "ERROR") return ThemeManagerState::ERROR;
    return ThemeManagerState::UNKNOWN;
}

bool theme_manager_init(const ThemeManagerConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = ThemeManagerState::INIT;
...
ThemeManagerResponse theme_manager_handle(const ThemeManagerRequest& req) {
    ThemeManagerResponse resp;
    ...
    g_status.state = ThemeManagerState::PROCESSING;
    // Validate request
...
ThemeManagerStatus theme_manager_getStatus() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_status;
}

void theme_manager_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = ThemeManagerConfig{};
    g_status = ThemeManagerStatus{};
    g_status.state = ThemeManagerState::UNKNOWN;
}
...
ThemeManagerStats theme_manager_getStats() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_stats;
}

ThemeManagerConfig theme_manager_configFromJson(const json& j) {
    ThemeManagerConfig config;
...
json theme_manager_configToJson(const ThemeManagerConfig& config) {
    json j;
