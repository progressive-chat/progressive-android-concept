...
namespace {
    StateResolverConfig g_config;
    std::mutex g_mutex;
    StateResolverStatus g_status;
    StateResolverStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* state_resolver_stateToString(StateResolverState s) {
    switch (s) {
        case StateResolverState::UNKNOWN: return "UNKNOWN";
        case StateResolverState::INIT: return "INIT";
        case StateResolverState::PROCESSING: return "PROCESSING";
        case StateResolverState::DONE: return "DONE";
        case StateResolverState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

StateResolverState state_resolver_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return StateResolverState::UNKNOWN;
    if (s == "INIT") return StateResolverState::INIT;
    if (s == "PROCESSING") return StateResolverState::PROCESSING;
    if (s == "DONE") return StateResolverState::DONE;
    if (s == "ERROR") return StateResolverState::ERROR;
    return StateResolverState::UNKNOWN;
}
...
bool state_resolver_init(const StateResolverConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = StateResolverState::INIT;
...
StateResolverResponse state_resolver_handle(const StateResolverRequest& req) {
    ...
    g_status.state = StateResolverState::PROCESSING;
    // Validate request
...
void state_resolver_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = StateResolverConfig{};
    g_status = StateResolverStatus{};
    g_status.state = StateResolverState::UNKNOWN;
}
