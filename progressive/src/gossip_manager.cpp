...
namespace {
    GossipManagerConfig g_config;
    std::mutex g_mutex;
    GossipManagerStatus g_status;
    GossipManagerStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* gossip_manager_stateToString(GossipManagerState s) {
    switch (s) {
        case GossipManagerState::UNKNOWN: return "UNKNOWN";
        case GossipManagerState::INIT: return "INIT";
        case GossipManagerState::PROCESSING: return "PROCESSING";
        case GossipManagerState::DONE: return "DONE";
        case GossipManagerState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

GossipManagerState gossip_manager_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return GossipManagerState::UNKNOWN;
    if (s == "INIT") return GossipManagerState::INIT;
    if (s == "PROCESSING") return GossipManagerState::PROCESSING;
    if (s == "DONE") return GossipManagerState::DONE;
    if (s == "ERROR") return GossipManagerState::ERROR;
    return GossipManagerState::UNKNOWN;
}
...
bool gossip_manager_init(const GossipManagerConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = GossipManagerState::INIT;
...
GossipManagerResponse gossip_manager_handle(const GossipManagerRequest& req) {
    ...
    g_status.state = GossipManagerState::PROCESSING;
    // Validate request
...
void gossip_manager_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = GossipManagerConfig{};
    g_status = GossipManagerStatus{};
    g_status.state = GossipManagerState::UNKNOWN;
}
