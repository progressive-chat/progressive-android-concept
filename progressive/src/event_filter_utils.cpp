...
namespace {
    EventFilterUtilsConfig g_config;
    std::mutex g_mutex;
    EventFilterUtilsStatus g_status;
    EventFilterUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* event_filter_utils_stateToString(FilterUtilsState s) {
    switch (s) {
        case FilterUtilsState::UNKNOWN: return "UNKNOWN";
        case FilterUtilsState::INIT: return "INIT";
        case FilterUtilsState::PROCESSING: return "PROCESSING";
        case FilterUtilsState::DONE: return "DONE";
        case FilterUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

FilterUtilsState event_filter_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return FilterUtilsState::UNKNOWN;
    if (s == "INIT") return FilterUtilsState::INIT;
    if (s == "PROCESSING") return FilterUtilsState::PROCESSING;
    if (s == "DONE") return FilterUtilsState::DONE;
    if (s == "ERROR") return FilterUtilsState::ERROR;
    return FilterUtilsState::UNKNOWN;
}
...
bool event_filter_utils_init(const EventFilterUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = FilterUtilsState::INIT;
...
EventFilterUtilsResponse event_filter_utils_handle(const EventFilterUtilsRequest& req) {
    ...
    g_status.state = FilterUtilsState::PROCESSING;
    // Validate request
...
void event_filter_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = EventFilterUtilsConfig{};
    g_status = EventFilterUtilsStatus{};
    g_status.state = FilterUtilsState::UNKNOWN;
}
...
std::string event_filter_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "event_filter_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
