...
namespace {
    EventRedactionConfig g_config;
    std::mutex g_mutex;
    EventRedactionStatus g_status;
    EventRedactionStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* event_redaction_stateToString(RedactionState s) {
    switch (s) {
        case RedactionState::UNKNOWN: return "UNKNOWN";
        case RedactionState::INIT: return "INIT";
        case RedactionState::PROCESSING: return "PROCESSING";
        case RedactionState::DONE: return "DONE";
        case RedactionState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

RedactionState event_redaction_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return RedactionState::UNKNOWN;
    if (s == "INIT") return RedactionState::INIT;
    if (s == "PROCESSING") return RedactionState::PROCESSING;
    if (s == "DONE") return RedactionState::DONE;
    if (s == "ERROR") return RedactionState::ERROR;
...
bool event_redaction_init(const EventRedactionConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = RedactionState::INIT;
...
EventRedactionResponse event_redaction_handle(const EventRedactionRequest& req) {
    ...
    g_status.state = RedactionState::PROCESSING;
    // Validate request
...
void event_redaction_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = EventRedactionConfig{};
    g_status = EventRedactionStatus{};
    g_status.state = RedactionState::UNKNOWN;
}
