...
namespace {
    PresenceSyncConfig g_config;
    std::mutex g_mutex;
    PresenceSyncStatus g_status;
    PresenceSyncStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* presence_sync_stateToString(PresenceSyncState s) {
    switch (s) {
        case PresenceSyncState::UNKNOWN: return "UNKNOWN";
        case PresenceSyncState::INIT: return "INIT";
        case PresenceSyncState::PROCESSING: return "PROCESSING";
        case PresenceSyncState::DONE: return "DONE";
        case PresenceSyncState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

PresenceSyncState presence_sync_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return PresenceSyncState::UNKNOWN;
    if (s == "INIT") return PresenceSyncState::INIT;
    if (s == "PROCESSING") return PresenceSyncState::PROCESSING;
    if (s == "DONE") return PresenceSyncState::DONE;
    if (s == "ERROR") return PresenceSyncState::ERROR;
    return PresenceSyncState::UNKNOWN;
}
...
bool presence_sync_init(const PresenceSyncConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = PresenceSyncState::INIT;
...
PresenceSyncResponse presence_sync_handle(const PresenceSyncRequest& req) {
    ...
    g_status.state = PresenceSyncState::PROCESSING;
    // Validate request
...
void presence_sync_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = PresenceSyncConfig{};
    g_status = PresenceSyncStatus{};
    g_status.state = PresenceSyncState::UNKNOWN;
}
