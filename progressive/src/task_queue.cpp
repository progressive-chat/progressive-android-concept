...
namespace {
    TaskQueueConfig g_config;
    std::mutex g_mutex;
    TaskQueueStatus g_status;
    TaskQueueStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* task_queue_stateToString(TaskQueueState s) {
    switch (s) {
        case TaskQueueState::UNKNOWN: return "UNKNOWN";
        case TaskQueueState::INIT: return "INIT";
        case TaskQueueState::PROCESSING: return "PROCESSING";
        case TaskQueueState::DONE: return "DONE";
        case TaskQueueState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

TaskQueueState task_queue_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return TaskQueueState::UNKNOWN;
    if (s == "INIT") return TaskQueueState::INIT;
    if (s == "PROCESSING") return TaskQueueState::PROCESSING;
    if (s == "DONE") return TaskQueueState::DONE;
    if (s == "ERROR") return TaskQueueState::ERROR;
    return TaskQueueState::UNKNOWN;
}
...
bool task_queue_init(const TaskQueueConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = TaskQueueState::INIT;
...
TaskQueueResponse task_queue_handle(const TaskQueueRequest& req) {
    ...
    g_status.state = TaskQueueState::PROCESSING;
    // Validate request
...
void task_queue_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = TaskQueueConfig{};
    g_status = TaskQueueStatus{};
    g_status.state = TaskQueueState::UNKNOWN;
}
