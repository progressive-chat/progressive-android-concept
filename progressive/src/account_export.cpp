...
namespace {
    AccountExportConfig g_config;
    std::mutex g_mutex;
    AccountExportStatus g_status;
    AccountExportStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* account_export_stateToString(AccountExportState s) {
    switch (s) {
        case AccountExportState::UNKNOWN: return "UNKNOWN";
        case AccountExportState::INIT: return "INIT";
        case AccountExportState::PROCESSING: return "PROCESSING";
        case AccountExportState::DONE: return "DONE";
        case AccountExportState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

AccountExportState account_export_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return AccountExportState::UNKNOWN;
    if (s == "INIT") return AccountExportState::INIT;
    if (s == "PROCESSING") return AccountExportState::PROCESSING;
    if (s == "DONE") return AccountExportState::DONE;
    if (s == "ERROR") return AccountExportState::ERROR;
    return AccountExportState::UNKNOWN;
}
...
bool account_export_init(const AccountExportConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = AccountExportState::INIT;
...
AccountExportResponse account_export_handle(const AccountExportRequest& req) {
    ...
    g_status.state = AccountExportState::PROCESSING;
    // Validate request
...
void account_export_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = AccountExportConfig{};
    g_status = AccountExportStatus{};
    g_status.state = AccountExportState::UNKNOWN;
}
