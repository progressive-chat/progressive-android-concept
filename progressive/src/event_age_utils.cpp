...
namespace {
    EventAgeUtilsConfig g_config;
    std::mutex g_mutex;
    EventAgeUtilsStatus g_status;
    EventAgeUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* event_age_utils_stateToString(AgeUtilsState s) {
    switch (s) {
        case AgeUtilsState::UNKNOWN: return "UNKNOWN";
        case AgeUtilsState::INIT: return "INIT";
        case AgeUtilsState::PROCESSING: return "PROCESSING";
        case AgeUtilsState::DONE: return "DONE";
        case AgeUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

AgeUtilsState event_age_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return AgeUtilsState::UNKNOWN;
    if (s == "INIT") return AgeUtilsState::INIT;
    if (s == "PROCESSING") return AgeUtilsState::PROCESSING;
    if (s == "DONE") return AgeUtilsState::DONE;
    if (s == "ERROR") return AgeUtilsState::ERROR;
    return AgeUtilsState::UNKNOWN;
}
...
bool event_age_utils_init(const EventAgeUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = AgeUtilsState::INIT;
...
EventAgeUtilsResponse event_age_utils_handle(const EventAgeUtilsRequest& req) {
    ...
    g_status.state = AgeUtilsState::PROCESSING;
    // Validate request
...
void event_age_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = EventAgeUtilsConfig{};
    g_status = EventAgeUtilsStatus{};
    g_status.state = AgeUtilsState::UNKNOWN;
}
