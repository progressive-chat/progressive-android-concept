...
namespace {
    MessageSchedulerConfig g_config;
    std::mutex g_mutex;
    MessageSchedulerStatus g_status;
    MessageSchedulerStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* message_scheduler_stateToString(SchedulerPhase s) {
    switch (s) {
        case SchedulerPhase::UNKNOWN: return "UNKNOWN";
        case SchedulerPhase::INIT: return "INIT";
        case SchedulerPhase::PROCESSING: return "PROCESSING";
        case SchedulerPhase::DONE: return "DONE";
        case SchedulerPhase::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

SchedulerPhase message_scheduler_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return SchedulerPhase::UNKNOWN;
    if (s == "INIT") return SchedulerPhase::INIT;
    if (s == "PROCESSING") return SchedulerPhase::PROCESSING;
    if (s == "DONE") return SchedulerPhase::DONE;
    if (s == "ERROR") return SchedulerPhase::ERROR;
...
bool message_scheduler_init(const MessageSchedulerConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = SchedulerPhase::INIT;
...
MessageSchedulerResponse message_scheduler_handle(const MessageSchedulerRequest& req) {
    ...
    g_status.state = SchedulerPhase::PROCESSING;
    // Validate request
...
void message_scheduler_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = MessageSchedulerConfig{};
    g_status = MessageSchedulerStatus{};
    g_status.state = SchedulerPhase::UNKNOWN;
}
...
std::string message_scheduler_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "message_scheduler";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
