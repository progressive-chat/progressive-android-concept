...
namespace {
    MediaCompressorConfig g_config;
    std::mutex g_mutex;
    MediaCompressorStatus g_status;
    MediaCompressorStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* media_compressor_stateToString(CompressorType s) {
    switch (s) {
        case CompressorType::UNKNOWN: return "UNKNOWN";
        case CompressorType::INIT: return "INIT";
        case CompressorType::PROCESSING: return "PROCESSING";
        case CompressorType::DONE: return "DONE";
        case CompressorType::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

CompressorType media_compressor_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return CompressorType::UNKNOWN;
    if (s == "INIT") return CompressorType::INIT;
    if (s == "PROCESSING") return CompressorType::PROCESSING;
    if (s == "DONE") return CompressorType::DONE;
    if (s == "ERROR") return CompressorType::ERROR;
    return CompressorType::UNKNOWN;
}
...
bool media_compressor_init(const MediaCompressorConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = CompressorType::INIT;
...
MediaCompressorResponse media_compressor_handle(const MediaCompressorRequest& req) {
    ...
    g_status.state = CompressorType::PROCESSING;
    // Validate request
...
void media_compressor_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = MediaCompressorConfig{};
    g_status = MediaCompressorStatus{};
    g_status.state = CompressorType::UNKNOWN;
}
...
std::string media_compressor_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "media_compressor";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
