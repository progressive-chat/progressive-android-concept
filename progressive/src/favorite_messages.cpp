...
namespace {
    FavoriteMessagesConfig g_config;
    std::mutex g_mutex;
    FavoriteMessagesStatus g_status;
    FavoriteMessagesStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* favorite_messages_stateToString(FavoriteMessagesState s) {
    switch (s) {
        case FavoriteMessagesState::UNKNOWN: return "UNKNOWN";
        case FavoriteMessagesState::INIT: return "INIT";
        case FavoriteMessagesState::PROCESSING: return "PROCESSING";
        case FavoriteMessagesState::DONE: return "DONE";
        case FavoriteMessagesState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

FavoriteMessagesState favorite_messages_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return FavoriteMessagesState::UNKNOWN;
    if (s == "INIT") return FavoriteMessagesState::INIT;
    if (s == "PROCESSING") return FavoriteMessagesState::PROCESSING;
    if (s == "DONE") return FavoriteMessagesState::DONE;
    if (s == "ERROR") return FavoriteMessagesState::ERROR;
    return FavoriteMessagesState::UNKNOWN;
}

bool favorite_messages_init(const FavoriteMessagesConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = FavoriteMessagesState::INIT;
...
FavoriteMessagesResponse favorite_messages_handle(const FavoriteMessagesRequest& req) {
    FavoriteMessagesResponse resp;
    ...
    g_status.state = FavoriteMessagesState::PROCESSING;
    // Validate request
...
FavoriteMessagesStatus favorite_messages_getStatus() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_status;
}

void favorite_messages_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = FavoriteMessagesConfig{};
    g_status = FavoriteMessagesStatus{};
    g_status.state = FavoriteMessagesState::UNKNOWN;
}
...
std::string favorite_messages_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "favorite_messages";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
FavoriteMessagesStats favorite_messages_getStats() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_stats;
}

FavoriteMessagesConfig favorite_messages_configFromJson(const json& j) {
    FavoriteMessagesConfig config;
...
json favorite_messages_configToJson(const FavoriteMessagesConfig& config) {
    json j;
