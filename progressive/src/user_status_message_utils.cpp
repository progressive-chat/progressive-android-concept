...
namespace {
    UserStatusMessageUtilsConfig g_config;
    std::mutex g_mutex;
    UserStatusMessageUtilsStatus g_status;
    UserStatusMessageUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* user_status_message_utils_stateToString(StatusMessageUtilsState s) {
    switch (s) {
        case StatusMessageUtilsState::UNKNOWN: return "UNKNOWN";
        case StatusMessageUtilsState::INIT: return "INIT";
        case StatusMessageUtilsState::PROCESSING: return "PROCESSING";
        case StatusMessageUtilsState::DONE: return "DONE";
        case StatusMessageUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

StatusMessageUtilsState user_status_message_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return StatusMessageUtilsState::UNKNOWN;
    if (s == "INIT") return StatusMessageUtilsState::INIT;
    if (s == "PROCESSING") return StatusMessageUtilsState::PROCESSING;
    if (s == "DONE") return StatusMessageUtilsState::DONE;
    if (s == "ERROR") return StatusMessageUtilsState::ERROR;
    return StatusMessageUtilsState::UNKNOWN;
}
...
bool user_status_message_utils_init(const UserStatusMessageUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = StatusMessageUtilsState::INIT;
...
UserStatusMessageUtilsResponse user_status_message_utils_handle(const UserStatusMessageUtilsRequest& req) {
    UserStatusMessageUtilsResponse resp;
    ...
    g_status.state = StatusMessageUtilsState::PROCESSING;
    // Validate request
...
void user_status_message_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = UserStatusMessageUtilsConfig{};
    g_status = UserStatusMessageUtilsStatus{};
    g_status.state = StatusMessageUtilsState::UNKNOWN;
}
...
std::string user_status_message_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "user_status_message_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
UserStatusMessageUtilsConfig user_status_message_utils_configFromJson(const json& j) {
    UserStatusMessageUtilsConfig config;
