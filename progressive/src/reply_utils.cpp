...
namespace {
    ReplyUtilsConfig g_config;
    std::mutex g_mutex;
    ReplyUtilsStatus g_status;
    ReplyUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* reply_utils_stateToString(ReplyUtilsState s) {
    switch (s) {
        case ReplyUtilsState::UNKNOWN: return "UNKNOWN";
        case ReplyUtilsState::INIT: return "INIT";
        case ReplyUtilsState::PROCESSING: return "PROCESSING";
        case ReplyUtilsState::DONE: return "DONE";
        case ReplyUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

ReplyUtilsState reply_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return ReplyUtilsState::UNKNOWN;
    if (s == "INIT") return ReplyUtilsState::INIT;
    if (s == "PROCESSING") return ReplyUtilsState::PROCESSING;
    if (s == "DONE") return ReplyUtilsState::DONE;
    if (s == "ERROR") return ReplyUtilsState::ERROR;
    return ReplyUtilsState::UNKNOWN;
}
...
bool reply_utils_init(const ReplyUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = ReplyUtilsState::INIT;
...
ReplyUtilsResponse reply_utils_handle(const ReplyUtilsRequest& req) {
    ...
    g_status.state = ReplyUtilsState::PROCESSING;
    // Validate request
...
void reply_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = ReplyUtilsConfig{};
    g_status = ReplyUtilsStatus{};
    g_status.state = ReplyUtilsState::UNKNOWN;
}
