...
namespace {
    MediaGalleryConfig g_config;
    std::mutex g_mutex;
    MediaGalleryStatus g_status;
    MediaGalleryStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* media_gallery_stateToString(GalleryType s) {
    switch (s) {
        case GalleryType::UNKNOWN: return "UNKNOWN";
        case GalleryType::INIT: return "INIT";
        case GalleryType::PROCESSING: return "PROCESSING";
        case GalleryType::DONE: return "DONE";
        case GalleryType::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

GalleryType media_gallery_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return GalleryType::UNKNOWN;
    if (s == "INIT") return GalleryType::INIT;
    if (s == "PROCESSING") return GalleryType::PROCESSING;
    if (s == "DONE") return GalleryType::DONE;
    if (s == "ERROR") return GalleryType::ERROR;
...
bool media_gallery_init(const MediaGalleryConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = GalleryType::INIT;
...
MediaGalleryResponse media_gallery_handle(const MediaGalleryRequest& req) {
    ...
    g_status.state = GalleryType::PROCESSING;
    // Validate request
...
void media_gallery_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = MediaGalleryConfig{};
    g_status = MediaGalleryStatus{};
    g_status.state = GalleryType::UNKNOWN;
}
