...
namespace {
    SsoProviderUtilsConfig g_config;
    std::mutex g_mutex;
    SsoProviderUtilsStatus g_status;
    SsoProviderUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* sso_provider_utils_stateToString(SsoProviderUtilsState s) {
    switch (s) {
        case SsoProviderUtilsState::UNKNOWN: return "UNKNOWN";
        case SsoProviderUtilsState::INIT: return "INIT";
        case SsoProviderUtilsState::PROCESSING: return "PROCESSING";
        case SsoProviderUtilsState::DONE: return "DONE";
        case SsoProviderUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

SsoProviderUtilsState sso_provider_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return SsoProviderUtilsState::UNKNOWN;
    if (s == "INIT") return SsoProviderUtilsState::INIT;
    if (s == "PROCESSING") return SsoProviderUtilsState::PROCESSING;
    if (s == "DONE") return SsoProviderUtilsState::DONE;
    if (s == "ERROR") return SsoProviderUtilsState::ERROR;
    return SsoProviderUtilsState::UNKNOWN;
}

bool sso_provider_utils_init(const SsoProviderUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = SsoProviderUtilsState::INIT;
...
SsoProviderUtilsResponse sso_provider_utils_handle(const SsoProviderUtilsRequest& req) {
    SsoProviderUtilsResponse resp;
    ...
    g_status.state = SsoProviderUtilsState::PROCESSING;
    // Validate request
...
SsoProviderUtilsStatus sso_provider_utils_getStatus() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_status;
}

void sso_provider_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = SsoProviderUtilsConfig{};
    g_status = SsoProviderUtilsStatus{};
    g_status.state = SsoProviderUtilsState::UNKNOWN;
}
...
std::string sso_provider_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "sso_provider_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
SsoProviderUtilsStats sso_provider_utils_getStats() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_stats;
}

SsoProviderUtilsConfig sso_provider_utils_configFromJson(const json& j) {
    SsoProviderUtilsConfig config;
...
json sso_provider_utils_configToJson(const SsoProviderUtilsConfig& config) {
    json j;
