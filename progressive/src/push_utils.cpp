...
namespace {
    PushUtilsConfig g_config;
    std::mutex g_mutex;
    PushUtilsStatus g_status;
    PushUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* push_utils_stateToString(UtilsKind s) {
    switch (s) {
        case UtilsKind::UNKNOWN: return "UNKNOWN";
        case UtilsKind::INIT: return "INIT";
        case UtilsKind::PROCESSING: return "PROCESSING";
        case UtilsKind::DONE: return "DONE";
        case UtilsKind::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

UtilsKind push_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return UtilsKind::UNKNOWN;
    if (s == "INIT") return UtilsKind::INIT;
    if (s == "PROCESSING") return UtilsKind::PROCESSING;
    if (s == "DONE") return UtilsKind::DONE;
    if (s == "ERROR") return UtilsKind::ERROR;
    return UtilsKind::UNKNOWN;
}
...
PushUtilsResponse push_utils_handle(const PushUtilsRequest& req) {
    ...
    g_status.state = UtilsKind::PROCESSING;
    // Validate request
...
void push_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = PushUtilsConfig{};
    g_status = PushUtilsStatus{};
    g_status.state = UtilsKind::UNKNOWN;
}
