...
namespace {
    EventAggregationUtilsConfig g_config;
    std::mutex g_mutex;
    EventAggregationUtilsStatus g_status;
    EventAggregationUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* event_aggregation_utils_stateToString(AggregationUtilsState s) {
    switch (s) {
        case AggregationUtilsState::UNKNOWN: return "UNKNOWN";
        case AggregationUtilsState::INIT: return "INIT";
        case AggregationUtilsState::PROCESSING: return "PROCESSING";
        case AggregationUtilsState::DONE: return "DONE";
        case AggregationUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

AggregationUtilsState event_aggregation_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return AggregationUtilsState::UNKNOWN;
    if (s == "INIT") return AggregationUtilsState::INIT;
    if (s == "PROCESSING") return AggregationUtilsState::PROCESSING;
    if (s == "DONE") return AggregationUtilsState::DONE;
    if (s == "ERROR") return AggregationUtilsState::ERROR;
    return AggregationUtilsState::UNKNOWN;
}

bool event_aggregation_utils_init(const EventAggregationUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = AggregationUtilsState::INIT;
...
EventAggregationUtilsResponse event_aggregation_utils_handle(const EventAggregationUtilsRequest& req) {
    EventAggregationUtilsResponse resp;
    ...
    g_status.state = AggregationUtilsState::PROCESSING;
    // Validate request
...
EventAggregationUtilsStatus event_aggregation_utils_getStatus() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_status;
}

void event_aggregation_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = EventAggregationUtilsConfig{};
    g_status = EventAggregationUtilsStatus{};
    g_status.state = AggregationUtilsState::UNKNOWN;
}
...
std::string event_aggregation_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "event_aggregation_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
EventAggregationUtilsStats event_aggregation_utils_getStats() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_stats;
}

EventAggregationUtilsConfig event_aggregation_utils_configFromJson(const json& j) {
    EventAggregationUtilsConfig config;
...
json event_aggregation_utils_configToJson(const EventAggregationUtilsConfig& config) {
    json j;
