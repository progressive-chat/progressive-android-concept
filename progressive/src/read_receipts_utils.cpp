...
namespace {
    ReadReceiptsUtilsConfig g_config;
    std::mutex g_mutex;
    ReadReceiptsUtilsStatus g_status;
    ReadReceiptsUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* read_receipts_utils_stateToString(ReadReceiptsUtilsState s) {
    switch (s) {
        case ReadReceiptsUtilsState::UNKNOWN: return "UNKNOWN";
        case ReadReceiptsUtilsState::INIT: return "INIT";
        case ReadReceiptsUtilsState::PROCESSING: return "PROCESSING";
        case ReadReceiptsUtilsState::DONE: return "DONE";
        case ReadReceiptsUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

ReadReceiptsUtilsState read_receipts_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return ReadReceiptsUtilsState::UNKNOWN;
    if (s == "INIT") return ReadReceiptsUtilsState::INIT;
    if (s == "PROCESSING") return ReadReceiptsUtilsState::PROCESSING;
    if (s == "DONE") return ReadReceiptsUtilsState::DONE;
    if (s == "ERROR") return ReadReceiptsUtilsState::ERROR;
    return ReadReceiptsUtilsState::UNKNOWN;
}
...
bool read_receipts_utils_init(const ReadReceiptsUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = ReadReceiptsUtilsState::INIT;
...
ReadReceiptsUtilsResponse read_receipts_utils_handle(const ReadReceiptsUtilsRequest& req) {
    ReadReceiptsUtilsResponse resp;
    ...
    g_status.state = ReadReceiptsUtilsState::PROCESSING;
    // Validate request
...
void read_receipts_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = ReadReceiptsUtilsConfig{};
    g_status = ReadReceiptsUtilsStatus{};
    g_status.state = ReadReceiptsUtilsState::UNKNOWN;
}
...
std::string read_receipts_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "read_receipts_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
ReadReceiptsUtilsConfig read_receipts_utils_configFromJson(const json& j) {
    ReadReceiptsUtilsConfig config;
