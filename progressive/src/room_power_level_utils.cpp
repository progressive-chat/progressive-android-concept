...
namespace {
    RoomPowerLevelUtilsConfig g_config;
    std::mutex g_mutex;
    RoomPowerLevelUtilsStatus g_status;
    RoomPowerLevelUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* room_power_level_utils_stateToString(PowerLevelUtilsAction s) {
    switch (s) {
        case PowerLevelUtilsAction::UNKNOWN: return "UNKNOWN";
        case PowerLevelUtilsAction::INIT: return "INIT";
        case PowerLevelUtilsAction::PROCESSING: return "PROCESSING";
        case PowerLevelUtilsAction::DONE: return "DONE";
        case PowerLevelUtilsAction::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

PowerLevelUtilsAction room_power_level_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return PowerLevelUtilsAction::UNKNOWN;
    if (s == "INIT") return PowerLevelUtilsAction::INIT;
    if (s == "PROCESSING") return PowerLevelUtilsAction::PROCESSING;
    if (s == "DONE") return PowerLevelUtilsAction::DONE;
    if (s == "ERROR") return PowerLevelUtilsAction::ERROR;
    return PowerLevelUtilsAction::UNKNOWN;
}
...
bool room_power_level_utils_init(const RoomPowerLevelUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = PowerLevelUtilsAction::INIT;
...
RoomPowerLevelUtilsResponse room_power_level_utils_handle(const RoomPowerLevelUtilsRequest& req) {
    RoomPowerLevelUtilsResponse resp;
    ...
    g_status.state = PowerLevelUtilsAction::PROCESSING;
    // Validate request
...
void room_power_level_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = RoomPowerLevelUtilsConfig{};
    g_status = RoomPowerLevelUtilsStatus{};
    g_status.state = PowerLevelUtilsAction::UNKNOWN;
}
...
std::string room_power_level_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "room_power_level_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
RoomPowerLevelUtilsConfig room_power_level_utils_configFromJson(const json& j) {
    RoomPowerLevelUtilsConfig config;
