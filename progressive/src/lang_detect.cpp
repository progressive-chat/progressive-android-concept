...
namespace {
    LangDetectConfig g_config;
    std::mutex g_mutex;
    LangDetectStatus g_status;
    LangDetectStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* lang_detect_stateToString(LangDetectState s) {
    switch (s) {
        case LangDetectState::UNKNOWN: return "UNKNOWN";
        case LangDetectState::INIT: return "INIT";
        case LangDetectState::PROCESSING: return "PROCESSING";
        case LangDetectState::DONE: return "DONE";
        case LangDetectState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

LangDetectState lang_detect_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return LangDetectState::UNKNOWN;
    if (s == "INIT") return LangDetectState::INIT;
    if (s == "PROCESSING") return LangDetectState::PROCESSING;
    if (s == "DONE") return LangDetectState::DONE;
    if (s == "ERROR") return LangDetectState::ERROR;
    return LangDetectState::UNKNOWN;
}
...
bool lang_detect_init(const LangDetectConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = LangDetectState::INIT;
...
LangDetectResponse lang_detect_handle(const LangDetectRequest& req) {
    ...
    g_status.state = LangDetectState::PROCESSING;
    // Validate request
...
void lang_detect_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = LangDetectConfig{};
    g_status = LangDetectStatus{};
    g_status.state = LangDetectState::UNKNOWN;
}
