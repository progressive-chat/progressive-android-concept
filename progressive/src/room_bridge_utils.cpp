...
namespace {
    RoomBridgeUtilsConfig g_config;
    std::mutex g_mutex;
    RoomBridgeUtilsStatus g_status;
    RoomBridgeUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* room_bridge_utils_stateToString(BridgeUtilsAction s) {
    switch (s) {
        case BridgeUtilsAction::UNKNOWN: return "UNKNOWN";
        case BridgeUtilsAction::INIT: return "INIT";
        case BridgeUtilsAction::PROCESSING: return "PROCESSING";
        case BridgeUtilsAction::DONE: return "DONE";
        case BridgeUtilsAction::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

BridgeUtilsAction room_bridge_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return BridgeUtilsAction::UNKNOWN;
    if (s == "INIT") return BridgeUtilsAction::INIT;
    if (s == "PROCESSING") return BridgeUtilsAction::PROCESSING;
    if (s == "DONE") return BridgeUtilsAction::DONE;
    if (s == "ERROR") return BridgeUtilsAction::ERROR;
    return BridgeUtilsAction::UNKNOWN;
}
...
bool room_bridge_utils_init(const RoomBridgeUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = BridgeUtilsAction::INIT;
...
RoomBridgeUtilsResponse room_bridge_utils_handle(const RoomBridgeUtilsRequest& req) {
    ...
    g_status.state = BridgeUtilsAction::PROCESSING;
    // Validate request
...
void room_bridge_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = RoomBridgeUtilsConfig{};
    g_status = RoomBridgeUtilsStatus{};
    g_status.state = BridgeUtilsAction::UNKNOWN;
}
...
std::string room_bridge_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "room_bridge_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
