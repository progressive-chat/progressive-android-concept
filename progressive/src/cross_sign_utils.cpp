...
namespace {
    CrossSignUtilsConfig g_config;
    std::mutex g_mutex;
    CrossSignUtilsStatus g_status;
    CrossSignUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* cross_sign_utils_stateToString(UtilsPhase s) {
    switch (s) {
        case UtilsPhase::UNKNOWN: return "UNKNOWN";
        case UtilsPhase::INIT: return "INIT";
        case UtilsPhase::PROCESSING: return "PROCESSING";
        case UtilsPhase::DONE: return "DONE";
        case UtilsPhase::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

UtilsPhase cross_sign_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return UtilsPhase::UNKNOWN;
    if (s == "INIT") return UtilsPhase::INIT;
    if (s == "PROCESSING") return UtilsPhase::PROCESSING;
    if (s == "DONE") return UtilsPhase::DONE;
    if (s == "ERROR") return UtilsPhase::ERROR;
    return UtilsPhase::UNKNOWN;
}

bool cross_sign_utils_init(const CrossSignUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = UtilsPhase::INIT;
...
CrossSignUtilsResponse cross_sign_utils_handle(const CrossSignUtilsRequest& req) {
    CrossSignUtilsResponse resp;
    ...
    g_status.state = UtilsPhase::PROCESSING;
    // Validate request
...
CrossSignUtilsStatus cross_sign_utils_getStatus() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_status;
}

void cross_sign_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = CrossSignUtilsConfig{};
    g_status = CrossSignUtilsStatus{};
    g_status.state = UtilsPhase::UNKNOWN;
}
...
std::string cross_sign_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "cross_sign_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
CrossSignUtilsStats cross_sign_utils_getStats() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_stats;
}

CrossSignUtilsConfig cross_sign_utils_configFromJson(const json& j) {
    CrossSignUtilsConfig config;
...
json cross_sign_utils_configToJson(const CrossSignUtilsConfig& config) {
    json j;
