...
namespace {
    ConfigLoaderConfig g_config;
    std::mutex g_mutex;
    ConfigLoaderStatus g_status;
    ConfigLoaderStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* config_loader_stateToString(ConfigLoaderState s) {
    switch (s) {
        case ConfigLoaderState::UNKNOWN: return "UNKNOWN";
        case ConfigLoaderState::INIT: return "INIT";
        case ConfigLoaderState::PROCESSING: return "PROCESSING";
        case ConfigLoaderState::DONE: return "DONE";
        case ConfigLoaderState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

ConfigLoaderState config_loader_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return ConfigLoaderState::UNKNOWN;
    if (s == "INIT") return ConfigLoaderState::INIT;
    if (s == "PROCESSING") return ConfigLoaderState::PROCESSING;
    if (s == "DONE") return ConfigLoaderState::DONE;
    if (s == "ERROR") return ConfigLoaderState::ERROR;
    return ConfigLoaderState::UNKNOWN;
}
...
bool config_loader_init(const ConfigLoaderConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = ConfigLoaderState::INIT;
...
ConfigLoaderResponse config_loader_handle(const ConfigLoaderRequest& req) {
    ...
    g_status.state = ConfigLoaderState::PROCESSING;
    // Validate request
...
void config_loader_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = ConfigLoaderConfig{};
    g_status = ConfigLoaderStatus{};
    g_status.state = ConfigLoaderState::UNKNOWN;
}
