...
namespace {
    MediaConfigUtilsConfig g_config;
    std::mutex g_mutex;
    MediaConfigUtilsStatus g_status;
    MediaConfigUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* media_config_utils_stateToString(ConfigUtilsType s) {
    switch (s) {
        case ConfigUtilsType::UNKNOWN: return "UNKNOWN";
        case ConfigUtilsType::INIT: return "INIT";
        case ConfigUtilsType::PROCESSING: return "PROCESSING";
        case ConfigUtilsType::DONE: return "DONE";
        case ConfigUtilsType::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

ConfigUtilsType media_config_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return ConfigUtilsType::UNKNOWN;
    if (s == "INIT") return ConfigUtilsType::INIT;
    if (s == "PROCESSING") return ConfigUtilsType::PROCESSING;
    if (s == "DONE") return ConfigUtilsType::DONE;
    if (s == "ERROR") return ConfigUtilsType::ERROR;
    return ConfigUtilsType::UNKNOWN;
}
...
bool media_config_utils_init(const MediaConfigUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = ConfigUtilsType::INIT;
...
MediaConfigUtilsResponse media_config_utils_handle(const MediaConfigUtilsRequest& req) {
    ...
    g_status.state = ConfigUtilsType::PROCESSING;
    // Validate request
...
void media_config_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = MediaConfigUtilsConfig{};
    g_status = MediaConfigUtilsStatus{};
    g_status.state = ConfigUtilsType::UNKNOWN;
}
...
std::string media_config_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "media_config_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
