...
namespace {
    EmojiUtilsConfig g_config;
    std::mutex g_mutex;
    EmojiUtilsStatus g_status;
    EmojiUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* emoji_utils_stateToString(EmojiUtilsState s) {
    switch (s) {
        case EmojiUtilsState::UNKNOWN: return "UNKNOWN";
        case EmojiUtilsState::INIT: return "INIT";
        case EmojiUtilsState::PROCESSING: return "PROCESSING";
        case EmojiUtilsState::DONE: return "DONE";
        case EmojiUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

EmojiUtilsState emoji_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return EmojiUtilsState::UNKNOWN;
    if (s == "INIT") return EmojiUtilsState::INIT;
    if (s == "PROCESSING") return EmojiUtilsState::PROCESSING;
    if (s == "DONE") return EmojiUtilsState::DONE;
    if (s == "ERROR") return EmojiUtilsState::ERROR;
    return EmojiUtilsState::UNKNOWN;
}
...
bool emoji_utils_init(const EmojiUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = EmojiUtilsState::INIT;
...
EmojiUtilsResponse emoji_utils_handle(const EmojiUtilsRequest& req) {
    ...
    g_status.state = EmojiUtilsState::PROCESSING;
    // Validate request
...
void emoji_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = EmojiUtilsConfig{};
    g_status = EmojiUtilsStatus{};
    g_status.state = EmojiUtilsState::UNKNOWN;
}
