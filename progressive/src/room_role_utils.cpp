...
namespace {
    RoomRoleUtilsConfig g_config;
    std::mutex g_mutex;
    RoomRoleUtilsStatus g_status;
    RoomRoleUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* room_role_utils_stateToString(RoleUtilsAction s) {
    switch (s) {
        case RoleUtilsAction::UNKNOWN: return "UNKNOWN";
        case RoleUtilsAction::INIT: return "INIT";
        case RoleUtilsAction::PROCESSING: return "PROCESSING";
        case RoleUtilsAction::DONE: return "DONE";
        case RoleUtilsAction::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

RoleUtilsAction room_role_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return RoleUtilsAction::UNKNOWN;
    if (s == "INIT") return RoleUtilsAction::INIT;
    if (s == "PROCESSING") return RoleUtilsAction::PROCESSING;
    if (s == "DONE") return RoleUtilsAction::DONE;
    if (s == "ERROR") return RoleUtilsAction::ERROR;
    return RoleUtilsAction::UNKNOWN;
}
...
bool room_role_utils_init(const RoomRoleUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = RoleUtilsAction::INIT;
...
RoomRoleUtilsResponse room_role_utils_handle(const RoomRoleUtilsRequest& req) {
    ...
    g_status.state = RoleUtilsAction::PROCESSING;
    // Validate request
...
void room_role_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = RoomRoleUtilsConfig{};
    g_status = RoomRoleUtilsStatus{};
    g_status.state = RoleUtilsAction::UNKNOWN;
}
