...
namespace {
    IdentityHashConfig g_config;
    std::mutex g_mutex;
    IdentityHashStatus g_status;
    IdentityHashStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* identity_hash_stateToString(HashState s) {
    switch (s) {
        case HashState::UNKNOWN: return "UNKNOWN";
        case HashState::INIT: return "INIT";
        case HashState::PROCESSING: return "PROCESSING";
        case HashState::DONE: return "DONE";
        case HashState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

HashState identity_hash_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return HashState::UNKNOWN;
    if (s == "INIT") return HashState::INIT;
    if (s == "PROCESSING") return HashState::PROCESSING;
    if (s == "DONE") return HashState::DONE;
    if (s == "ERROR") return HashState::ERROR;
...
IdentityHashResponse identity_hash_handle(const IdentityHashRequest& req) {
    ...
    g_status.state = HashState::PROCESSING;
    // Validate request
...
void identity_hash_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = IdentityHashConfig{};
    g_status = IdentityHashStatus{};
    g_status.state = HashState::UNKNOWN;
}
