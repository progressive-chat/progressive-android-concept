...
namespace {
    IgnoreUtilsConfig g_config;
    std::mutex g_mutex;
    IgnoreUtilsStatus g_status;
    IgnoreUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* ignore_utils_stateToString(IgnoreUtilsState s) {
    switch (s) {
        case IgnoreUtilsState::UNKNOWN: return "UNKNOWN";
        case IgnoreUtilsState::INIT: return "INIT";
        case IgnoreUtilsState::PROCESSING: return "PROCESSING";
        case IgnoreUtilsState::DONE: return "DONE";
        case IgnoreUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

IgnoreUtilsState ignore_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return IgnoreUtilsState::UNKNOWN;
    if (s == "INIT") return IgnoreUtilsState::INIT;
    if (s == "PROCESSING") return IgnoreUtilsState::PROCESSING;
    if (s == "DONE") return IgnoreUtilsState::DONE;
    if (s == "ERROR") return IgnoreUtilsState::ERROR;
    return IgnoreUtilsState::UNKNOWN;
}
...
bool ignore_utils_init(const IgnoreUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = IgnoreUtilsState::INIT;
...
IgnoreUtilsResponse ignore_utils_handle(const IgnoreUtilsRequest& req) {
    ...
    g_status.state = IgnoreUtilsState::PROCESSING;
    // Validate request
...
void ignore_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = IgnoreUtilsConfig{};
    g_status = IgnoreUtilsStatus{};
    g_status.state = IgnoreUtilsState::UNKNOWN;
}
