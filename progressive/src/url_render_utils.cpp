...
namespace {
    UrlRenderUtilsConfig g_config;
    std::mutex g_mutex;
    UrlRenderUtilsStatus g_status;
    UrlRenderUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* url_render_utils_stateToString(UrlRenderUtilsState s) {
    switch (s) {
        case UrlRenderUtilsState::UNKNOWN: return "UNKNOWN";
        case UrlRenderUtilsState::INIT: return "INIT";
        case UrlRenderUtilsState::PROCESSING: return "PROCESSING";
        case UrlRenderUtilsState::DONE: return "DONE";
        case UrlRenderUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

UrlRenderUtilsState url_render_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return UrlRenderUtilsState::UNKNOWN;
    if (s == "INIT") return UrlRenderUtilsState::INIT;
    if (s == "PROCESSING") return UrlRenderUtilsState::PROCESSING;
    if (s == "DONE") return UrlRenderUtilsState::DONE;
    if (s == "ERROR") return UrlRenderUtilsState::ERROR;
    return UrlRenderUtilsState::UNKNOWN;
}
...
bool url_render_utils_init(const UrlRenderUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = UrlRenderUtilsState::INIT;
...
UrlRenderUtilsResponse url_render_utils_handle(const UrlRenderUtilsRequest& req) {
    ...
    g_status.state = UrlRenderUtilsState::PROCESSING;
    // Validate request
...
void url_render_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = UrlRenderUtilsConfig{};
    g_status = UrlRenderUtilsStatus{};
    g_status.state = UrlRenderUtilsState::UNKNOWN;
}
...
std::string url_render_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "url_render_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
