...
namespace {
    CommonUtilsConfig g_config;
    std::mutex g_mutex;
    CommonUtilsStatus g_status;
    CommonUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* common_utils_stateToString(CommonUtilsState s) {
    switch (s) {
        case CommonUtilsState::UNKNOWN: return "UNKNOWN";
        case CommonUtilsState::INIT: return "INIT";
        case CommonUtilsState::PROCESSING: return "PROCESSING";
        case CommonUtilsState::DONE: return "DONE";
        case CommonUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

CommonUtilsState common_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return CommonUtilsState::UNKNOWN;
    if (s == "INIT") return CommonUtilsState::INIT;
    if (s == "PROCESSING") return CommonUtilsState::PROCESSING;
    if (s == "DONE") return CommonUtilsState::DONE;
    if (s == "ERROR") return CommonUtilsState::ERROR;
    return CommonUtilsState::UNKNOWN;
}

bool common_utils_init(const CommonUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = CommonUtilsState::INIT;
...
CommonUtilsResponse common_utils_handle(const CommonUtilsRequest& req) {
    CommonUtilsResponse resp;
    ...
    g_status.state = CommonUtilsState::PROCESSING;
    // Validate request
...
CommonUtilsStatus common_utils_getStatus() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_status;
}

void common_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = CommonUtilsConfig{};
    g_status = CommonUtilsStatus{};
    g_status.state = CommonUtilsState::UNKNOWN;
}
...
CommonUtilsStats common_utils_getStats() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_stats;
}

CommonUtilsConfig common_utils_configFromJson(const json& j) {
    CommonUtilsConfig config;
...
json common_utils_configToJson(const CommonUtilsConfig& config) {
    json j;
