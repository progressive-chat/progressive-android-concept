...
namespace {
    EventContextUtilsConfig g_config;
    std::mutex g_mutex;
    EventContextUtilsStatus g_status;
    EventContextUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* event_context_utils_stateToString(ContextUtilsState s) {
    switch (s) {
        case ContextUtilsState::UNKNOWN: return "UNKNOWN";
        case ContextUtilsState::INIT: return "INIT";
        case ContextUtilsState::PROCESSING: return "PROCESSING";
        case ContextUtilsState::DONE: return "DONE";
        case ContextUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

ContextUtilsState event_context_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return ContextUtilsState::UNKNOWN;
    if (s == "INIT") return ContextUtilsState::INIT;
    if (s == "PROCESSING") return ContextUtilsState::PROCESSING;
    if (s == "DONE") return ContextUtilsState::DONE;
    if (s == "ERROR") return ContextUtilsState::ERROR;
    return ContextUtilsState::UNKNOWN;
}
...
bool event_context_utils_init(const EventContextUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = ContextUtilsState::INIT;
...
EventContextUtilsResponse event_context_utils_handle(const EventContextUtilsRequest& req) {
    EventContextUtilsResponse resp;
    ...
    g_status.state = ContextUtilsState::PROCESSING;
    // Validate request
...
void event_context_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = EventContextUtilsConfig{};
    g_status = EventContextUtilsStatus{};
    g_status.state = ContextUtilsState::UNKNOWN;
}
...
std::string event_context_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "event_context_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
EventContextUtilsConfig event_context_utils_configFromJson(const json& j) {
    EventContextUtilsConfig config;
