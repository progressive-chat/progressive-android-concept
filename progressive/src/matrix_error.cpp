...
namespace {
    MatrixErrorConfig g_config;
    std::mutex g_mutex;
    MatrixErrorStatus g_status;
    MatrixErrorStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* matrix_error_stateToString(MatrixErrorState s) {
    switch (s) {
        case MatrixErrorState::UNKNOWN: return "UNKNOWN";
        case MatrixErrorState::INIT: return "INIT";
        case MatrixErrorState::PROCESSING: return "PROCESSING";
        case MatrixErrorState::DONE: return "DONE";
        case MatrixErrorState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

MatrixErrorState matrix_error_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return MatrixErrorState::UNKNOWN;
    if (s == "INIT") return MatrixErrorState::INIT;
    if (s == "PROCESSING") return MatrixErrorState::PROCESSING;
    if (s == "DONE") return MatrixErrorState::DONE;
    if (s == "ERROR") return MatrixErrorState::ERROR;
    return MatrixErrorState::UNKNOWN;
}
...
bool matrix_error_init(const MatrixErrorConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = MatrixErrorState::INIT;
...
MatrixErrorResponse matrix_error_handle(const MatrixErrorRequest& req) {
    ...
    g_status.state = MatrixErrorState::PROCESSING;
    // Validate request
...
void matrix_error_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = MatrixErrorConfig{};
    g_status = MatrixErrorStatus{};
    g_status.state = MatrixErrorState::UNKNOWN;
}
