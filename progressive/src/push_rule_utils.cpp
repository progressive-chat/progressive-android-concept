...
namespace {
    PushRuleUtilsConfig g_config;
    std::mutex g_mutex;
    PushRuleUtilsStatus g_status;
    PushRuleUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* push_rule_utils_stateToString(RuleUtilsKind s) {
    switch (s) {
        case RuleUtilsKind::UNKNOWN: return "UNKNOWN";
        case RuleUtilsKind::INIT: return "INIT";
        case RuleUtilsKind::PROCESSING: return "PROCESSING";
        case RuleUtilsKind::DONE: return "DONE";
        case RuleUtilsKind::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

RuleUtilsKind push_rule_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return RuleUtilsKind::UNKNOWN;
    if (s == "INIT") return RuleUtilsKind::INIT;
    if (s == "PROCESSING") return RuleUtilsKind::PROCESSING;
    if (s == "DONE") return RuleUtilsKind::DONE;
    if (s == "ERROR") return RuleUtilsKind::ERROR;
    return RuleUtilsKind::UNKNOWN;
}
...
bool push_rule_utils_init(const PushRuleUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = RuleUtilsKind::INIT;
...
PushRuleUtilsResponse push_rule_utils_handle(const PushRuleUtilsRequest& req) {
    ...
    g_status.state = RuleUtilsKind::PROCESSING;
    // Validate request
...
void push_rule_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = PushRuleUtilsConfig{};
    g_status = PushRuleUtilsStatus{};
    g_status.state = RuleUtilsKind::UNKNOWN;
}
