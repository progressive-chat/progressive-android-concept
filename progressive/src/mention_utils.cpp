...
namespace {
    MentionUtilsConfig g_config;
    std::mutex g_mutex;
    MentionUtilsStatus g_status;
    MentionUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* mention_utils_stateToString(MentionUtilsState s) {
    switch (s) {
        case MentionUtilsState::UNKNOWN: return "UNKNOWN";
        case MentionUtilsState::INIT: return "INIT";
        case MentionUtilsState::PROCESSING: return "PROCESSING";
        case MentionUtilsState::DONE: return "DONE";
        case MentionUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

MentionUtilsState mention_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return MentionUtilsState::UNKNOWN;
    if (s == "INIT") return MentionUtilsState::INIT;
    if (s == "PROCESSING") return MentionUtilsState::PROCESSING;
    if (s == "DONE") return MentionUtilsState::DONE;
    if (s == "ERROR") return MentionUtilsState::ERROR;
    return MentionUtilsState::UNKNOWN;
}
...
bool mention_utils_init(const MentionUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = MentionUtilsState::INIT;
...
MentionUtilsResponse mention_utils_handle(const MentionUtilsRequest& req) {
    ...
    g_status.state = MentionUtilsState::PROCESSING;
    // Validate request
...
void mention_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = MentionUtilsConfig{};
    g_status = MentionUtilsStatus{};
    g_status.state = MentionUtilsState::UNKNOWN;
}
