...
namespace {
    PasswordStrengthConfig g_config;
    std::mutex g_mutex;
    PasswordStrengthStatus g_status;
    PasswordStrengthStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* password_strength_stateToString(PasswordStrengthState s) {
    switch (s) {
        case PasswordStrengthState::UNKNOWN: return "UNKNOWN";
        case PasswordStrengthState::INIT: return "INIT";
        case PasswordStrengthState::PROCESSING: return "PROCESSING";
        case PasswordStrengthState::DONE: return "DONE";
        case PasswordStrengthState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

PasswordStrengthState password_strength_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return PasswordStrengthState::UNKNOWN;
    if (s == "INIT") return PasswordStrengthState::INIT;
    if (s == "PROCESSING") return PasswordStrengthState::PROCESSING;
    if (s == "DONE") return PasswordStrengthState::DONE;
    if (s == "ERROR") return PasswordStrengthState::ERROR;
    return PasswordStrengthState::UNKNOWN;
}
...
bool password_strength_init(const PasswordStrengthConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = PasswordStrengthState::INIT;
...
PasswordStrengthResponse password_strength_handle(const PasswordStrengthRequest& req) {
    ...
    g_status.state = PasswordStrengthState::PROCESSING;
    // Validate request
...
void password_strength_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = PasswordStrengthConfig{};
    g_status = PasswordStrengthStatus{};
    g_status.state = PasswordStrengthState::UNKNOWN;
}
...
std::string password_strength_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "password_strength";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
