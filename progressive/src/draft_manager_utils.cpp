...
namespace {
    DraftManagerUtilsConfig g_config;
    std::mutex g_mutex;
    DraftManagerUtilsStatus g_status;
    DraftManagerUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* draft_manager_utils_stateToString(DraftManagerUtilsState s) {
    switch (s) {
        case DraftManagerUtilsState::UNKNOWN: return "UNKNOWN";
        case DraftManagerUtilsState::INIT: return "INIT";
        case DraftManagerUtilsState::PROCESSING: return "PROCESSING";
        case DraftManagerUtilsState::DONE: return "DONE";
        case DraftManagerUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

DraftManagerUtilsState draft_manager_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return DraftManagerUtilsState::UNKNOWN;
    if (s == "INIT") return DraftManagerUtilsState::INIT;
    if (s == "PROCESSING") return DraftManagerUtilsState::PROCESSING;
    if (s == "DONE") return DraftManagerUtilsState::DONE;
    if (s == "ERROR") return DraftManagerUtilsState::ERROR;
    return DraftManagerUtilsState::UNKNOWN;
}

bool draft_manager_utils_init(const DraftManagerUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = DraftManagerUtilsState::INIT;
...
DraftManagerUtilsResponse draft_manager_utils_handle(const DraftManagerUtilsRequest& req) {
    DraftManagerUtilsResponse resp;
    ...
    g_status.state = DraftManagerUtilsState::PROCESSING;
    // Validate request
...
DraftManagerUtilsStatus draft_manager_utils_getStatus() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_status;
}

void draft_manager_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = DraftManagerUtilsConfig{};
    g_status = DraftManagerUtilsStatus{};
    g_status.state = DraftManagerUtilsState::UNKNOWN;
}
...
std::string draft_manager_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "draft_manager_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
DraftManagerUtilsStats draft_manager_utils_getStats() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_stats;
}

DraftManagerUtilsConfig draft_manager_utils_configFromJson(const json& j) {
    DraftManagerUtilsConfig config;
...
json draft_manager_utils_configToJson(const DraftManagerUtilsConfig& config) {
    json j;
