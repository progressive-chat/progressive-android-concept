...
namespace {
    RoomFavoriteUtilsConfig g_config;
    std::mutex g_mutex;
    RoomFavoriteUtilsStatus g_status;
    RoomFavoriteUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* room_favorite_utils_stateToString(FavoriteUtilsAction s) {
    switch (s) {
        case FavoriteUtilsAction::UNKNOWN: return "UNKNOWN";
        case FavoriteUtilsAction::INIT: return "INIT";
        case FavoriteUtilsAction::PROCESSING: return "PROCESSING";
        case FavoriteUtilsAction::DONE: return "DONE";
        case FavoriteUtilsAction::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

FavoriteUtilsAction room_favorite_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return FavoriteUtilsAction::UNKNOWN;
    if (s == "INIT") return FavoriteUtilsAction::INIT;
    if (s == "PROCESSING") return FavoriteUtilsAction::PROCESSING;
    if (s == "DONE") return FavoriteUtilsAction::DONE;
    if (s == "ERROR") return FavoriteUtilsAction::ERROR;
    return FavoriteUtilsAction::UNKNOWN;
}
...
bool room_favorite_utils_init(const RoomFavoriteUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = FavoriteUtilsAction::INIT;
...
RoomFavoriteUtilsResponse room_favorite_utils_handle(const RoomFavoriteUtilsRequest& req) {
    RoomFavoriteUtilsResponse resp;
    ...
    g_status.state = FavoriteUtilsAction::PROCESSING;
    // Validate request
...
void room_favorite_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = RoomFavoriteUtilsConfig{};
    g_status = RoomFavoriteUtilsStatus{};
    g_status.state = FavoriteUtilsAction::UNKNOWN;
}
...
std::string room_favorite_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "room_favorite_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
RoomFavoriteUtilsConfig room_favorite_utils_configFromJson(const json& j) {
    RoomFavoriteUtilsConfig config;
