...
namespace {
    EmojiSearchConfig g_config;
    std::mutex g_mutex;
    EmojiSearchStatus g_status;
    EmojiSearchStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* emoji_search_stateToString(EmojiSearchState s) {
    switch (s) {
        case EmojiSearchState::UNKNOWN: return "UNKNOWN";
        case EmojiSearchState::INIT: return "INIT";
        case EmojiSearchState::PROCESSING: return "PROCESSING";
        case EmojiSearchState::DONE: return "DONE";
        case EmojiSearchState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

EmojiSearchState emoji_search_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return EmojiSearchState::UNKNOWN;
    if (s == "INIT") return EmojiSearchState::INIT;
    if (s == "PROCESSING") return EmojiSearchState::PROCESSING;
    if (s == "DONE") return EmojiSearchState::DONE;
    if (s == "ERROR") return EmojiSearchState::ERROR;
    return EmojiSearchState::UNKNOWN;
}
...
bool emoji_search_init(const EmojiSearchConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = EmojiSearchState::INIT;
...
EmojiSearchResponse emoji_search_handle(const EmojiSearchRequest& req) {
    ...
    g_status.state = EmojiSearchState::PROCESSING;
    // Validate request
...
void emoji_search_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = EmojiSearchConfig{};
    g_status = EmojiSearchStatus{};
    g_status.state = EmojiSearchState::UNKNOWN;
}
