...
namespace {
    MessageRetryUtilsConfig g_config;
    std::mutex g_mutex;
    MessageRetryUtilsStatus g_status;
    MessageRetryUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* message_retry_utils_stateToString(RetryUtilsPhase s) {
    switch (s) {
        case RetryUtilsPhase::UNKNOWN: return "UNKNOWN";
        case RetryUtilsPhase::INIT: return "INIT";
        case RetryUtilsPhase::PROCESSING: return "PROCESSING";
        case RetryUtilsPhase::DONE: return "DONE";
        case RetryUtilsPhase::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

RetryUtilsPhase message_retry_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return RetryUtilsPhase::UNKNOWN;
    if (s == "INIT") return RetryUtilsPhase::INIT;
    if (s == "PROCESSING") return RetryUtilsPhase::PROCESSING;
    if (s == "DONE") return RetryUtilsPhase::DONE;
    if (s == "ERROR") return RetryUtilsPhase::ERROR;
    return RetryUtilsPhase::UNKNOWN;
}

bool message_retry_utils_init(const MessageRetryUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = RetryUtilsPhase::INIT;
...
MessageRetryUtilsResponse message_retry_utils_handle(const MessageRetryUtilsRequest& req) {
    MessageRetryUtilsResponse resp;
    ...
    g_status.state = RetryUtilsPhase::PROCESSING;
    // Validate request
...
MessageRetryUtilsStatus message_retry_utils_getStatus() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_status;
}

void message_retry_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = MessageRetryUtilsConfig{};
    g_status = MessageRetryUtilsStatus{};
    g_status.state = RetryUtilsPhase::UNKNOWN;
}

bool message_retry_utils_validate(const std::string& input) {
...
std::string message_retry_utils_process(const std::string& input) {
    std::lock_guard<std::mutex> lock(g_mutex);
    ...
    try {
        ...
        result["module"] = "message_retry_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
json message_retry_utils_toJson(const std::string& input) {
    json j;
...
MessageRetryUtilsStats message_retry_utils_getStats() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_stats;
}

MessageRetryUtilsConfig message_retry_utils_configFromJson(const json& j) {
    MessageRetryUtilsConfig config;
...
json message_retry_utils_configToJson(const MessageRetryUtilsConfig& config) {
    json j;
