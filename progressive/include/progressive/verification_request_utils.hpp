...
enum class VerificationRequestUtilsState {
    UNKNOWN = 0,
    INIT = 1,
    PROCESSING = 2,
    DONE = 3,
...
struct VerificationRequestUtilsConfig {
...
struct VerificationRequestUtilsRequest {
...
struct VerificationRequestUtilsResponse {
    bool success = false;
...
struct VerificationRequestUtilsStatus {
    VerificationRequestUtilsState state = VerificationRequestUtilsState::UNKNOWN;
...
struct VerificationRequestUtilsStats {
...
// Initialize the verification request utils module with configuration
bool verification_request_utils_init(const VerificationRequestUtilsConfig& config);

// Handle a verification request utils request, returning a response
