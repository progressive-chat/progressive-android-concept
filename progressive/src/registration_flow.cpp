...
namespace {
    RegistrationFlowConfig g_config;
    std::mutex g_mutex;
    RegistrationFlowStatus g_status;
    RegistrationFlowStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* registration_flow_stateToString(RegistrationFlowState s) {
    switch (s) {
        case RegistrationFlowState::UNKNOWN: return "UNKNOWN";
        case RegistrationFlowState::INIT: return "INIT";
        case RegistrationFlowState::PROCESSING: return "PROCESSING";
        case RegistrationFlowState::DONE: return "DONE";
        case RegistrationFlowState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

RegistrationFlowState registration_flow_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return RegistrationFlowState::UNKNOWN;
    if (s == "INIT") return RegistrationFlowState::INIT;
    if (s == "PROCESSING") return RegistrationFlowState::PROCESSING;
    if (s == "DONE") return RegistrationFlowState::DONE;
    if (s == "ERROR") return RegistrationFlowState::ERROR;
    return RegistrationFlowState::UNKNOWN;
}
...
bool registration_flow_init(const RegistrationFlowConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = RegistrationFlowState::INIT;
...
RegistrationFlowResponse registration_flow_handle(const RegistrationFlowRequest& req) {
    ...
    g_status.state = RegistrationFlowState::PROCESSING;
    // Validate request
...
void registration_flow_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = RegistrationFlowConfig{};
    g_status = RegistrationFlowStatus{};
    g_status.state = RegistrationFlowState::UNKNOWN;
}
...
std::string registration_flow_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "registration_flow";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
