...
namespace {
    RoomMemberUtilsConfig g_config;
    std::mutex g_mutex;
    RoomMemberUtilsStatus g_status;
    RoomMemberUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* room_member_utils_stateToString(MemberUtilsAction s) {
    switch (s) {
        case MemberUtilsAction::UNKNOWN: return "UNKNOWN";
        case MemberUtilsAction::INIT: return "INIT";
        case MemberUtilsAction::PROCESSING: return "PROCESSING";
        case MemberUtilsAction::DONE: return "DONE";
        case MemberUtilsAction::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

MemberUtilsAction room_member_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return MemberUtilsAction::UNKNOWN;
    if (s == "INIT") return MemberUtilsAction::INIT;
    if (s == "PROCESSING") return MemberUtilsAction::PROCESSING;
    if (s == "DONE") return MemberUtilsAction::DONE;
    if (s == "ERROR") return MemberUtilsAction::ERROR;
    return MemberUtilsAction::UNKNOWN;
}
...
bool room_member_utils_init(const RoomMemberUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = MemberUtilsAction::INIT;
...
RoomMemberUtilsResponse room_member_utils_handle(const RoomMemberUtilsRequest& req) {
    ...
    g_status.state = MemberUtilsAction::PROCESSING;
    // Validate request
...
void room_member_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = RoomMemberUtilsConfig{};
    g_status = RoomMemberUtilsStatus{};
    g_status.state = MemberUtilsAction::UNKNOWN;
}
...
std::string room_member_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "room_member_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
