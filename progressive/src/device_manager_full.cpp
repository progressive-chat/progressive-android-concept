...
namespace {
    DeviceManagerFullConfig g_config;
    std::mutex g_mutex;
    DeviceManagerFullStatus g_status;
    DeviceManagerFullStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* device_manager_full_stateToString(DeviceManagerFullState s) {
    switch (s) {
        case DeviceManagerFullState::UNKNOWN: return "UNKNOWN";
        case DeviceManagerFullState::INIT: return "INIT";
        case DeviceManagerFullState::PROCESSING: return "PROCESSING";
        case DeviceManagerFullState::DONE: return "DONE";
        case DeviceManagerFullState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

DeviceManagerFullState device_manager_full_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return DeviceManagerFullState::UNKNOWN;
    if (s == "INIT") return DeviceManagerFullState::INIT;
    if (s == "PROCESSING") return DeviceManagerFullState::PROCESSING;
    if (s == "DONE") return DeviceManagerFullState::DONE;
    if (s == "ERROR") return DeviceManagerFullState::ERROR;
    return DeviceManagerFullState::UNKNOWN;
}
...
bool device_manager_full_init(const DeviceManagerFullConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = DeviceManagerFullState::INIT;
...
DeviceManagerFullResponse device_manager_full_handle(const DeviceManagerFullRequest& req) {
    DeviceManagerFullResponse resp;
    ...
    g_status.state = DeviceManagerFullState::PROCESSING;
    // Validate request
...
void device_manager_full_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = DeviceManagerFullConfig{};
    g_status = DeviceManagerFullStatus{};
    g_status.state = DeviceManagerFullState::UNKNOWN;
}
...
std::string device_manager_full_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "device_manager_full";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
DeviceManagerFullConfig device_manager_full_configFromJson(const json& j) {
    DeviceManagerFullConfig config;
