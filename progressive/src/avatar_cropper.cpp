...
namespace {
    AvatarCropperConfig g_config;
    std::mutex g_mutex;
    AvatarCropperStatus g_status;
    AvatarCropperStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* avatar_cropper_stateToString(AvatarCropperState s) {
    switch (s) {
        case AvatarCropperState::UNKNOWN: return "UNKNOWN";
        case AvatarCropperState::INIT: return "INIT";
        case AvatarCropperState::PROCESSING: return "PROCESSING";
        case AvatarCropperState::DONE: return "DONE";
        case AvatarCropperState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

AvatarCropperState avatar_cropper_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return AvatarCropperState::UNKNOWN;
    if (s == "INIT") return AvatarCropperState::INIT;
    if (s == "PROCESSING") return AvatarCropperState::PROCESSING;
    if (s == "DONE") return AvatarCropperState::DONE;
    if (s == "ERROR") return AvatarCropperState::ERROR;
    return AvatarCropperState::UNKNOWN;
}
...
bool avatar_cropper_init(const AvatarCropperConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = AvatarCropperState::INIT;
...
AvatarCropperResponse avatar_cropper_handle(const AvatarCropperRequest& req) {
    ...
    g_status.state = AvatarCropperState::PROCESSING;
    // Validate request
...
void avatar_cropper_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = AvatarCropperConfig{};
    g_status = AvatarCropperStatus{};
    g_status.state = AvatarCropperState::UNKNOWN;
}
