...
namespace {
    MessageReplyUtilsConfig g_config;
    std::mutex g_mutex;
    MessageReplyUtilsStatus g_status;
    MessageReplyUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* message_reply_utils_stateToString(ReplyUtilsPhase s) {
    switch (s) {
        case ReplyUtilsPhase::UNKNOWN: return "UNKNOWN";
        case ReplyUtilsPhase::INIT: return "INIT";
        case ReplyUtilsPhase::PROCESSING: return "PROCESSING";
        case ReplyUtilsPhase::DONE: return "DONE";
        case ReplyUtilsPhase::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

ReplyUtilsPhase message_reply_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return ReplyUtilsPhase::UNKNOWN;
    if (s == "INIT") return ReplyUtilsPhase::INIT;
    if (s == "PROCESSING") return ReplyUtilsPhase::PROCESSING;
    if (s == "DONE") return ReplyUtilsPhase::DONE;
    if (s == "ERROR") return ReplyUtilsPhase::ERROR;
    return ReplyUtilsPhase::UNKNOWN;
}
...
bool message_reply_utils_init(const MessageReplyUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = ReplyUtilsPhase::INIT;
...
MessageReplyUtilsResponse message_reply_utils_handle(const MessageReplyUtilsRequest& req) {
    MessageReplyUtilsResponse resp;
    ...
    g_status.state = ReplyUtilsPhase::PROCESSING;
    // Validate request
...
void message_reply_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = MessageReplyUtilsConfig{};
    g_status = MessageReplyUtilsStatus{};
    g_status.state = ReplyUtilsPhase::UNKNOWN;
}
...
std::string message_reply_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "message_reply_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
MessageReplyUtilsConfig message_reply_utils_configFromJson(const json& j) {
    MessageReplyUtilsConfig config;
