...
namespace {
    DeviceTrustConfig g_config;
    std::mutex g_mutex;
    DeviceTrustStatus g_status;
    DeviceTrustStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* device_trust_stateToString(DeviceTrustState s) {
    switch (s) {
        case DeviceTrustState::UNKNOWN: return "UNKNOWN";
        case DeviceTrustState::INIT: return "INIT";
        case DeviceTrustState::PROCESSING: return "PROCESSING";
        case DeviceTrustState::DONE: return "DONE";
        case DeviceTrustState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

DeviceTrustState device_trust_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return DeviceTrustState::UNKNOWN;
    if (s == "INIT") return DeviceTrustState::INIT;
    if (s == "PROCESSING") return DeviceTrustState::PROCESSING;
    if (s == "DONE") return DeviceTrustState::DONE;
    if (s == "ERROR") return DeviceTrustState::ERROR;
...
bool device_trust_init(const DeviceTrustConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = DeviceTrustState::INIT;
...
DeviceTrustResponse device_trust_handle(const DeviceTrustRequest& req) {
    ...
    g_status.state = DeviceTrustState::PROCESSING;
    // Validate request
...
void device_trust_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = DeviceTrustConfig{};
    g_status = DeviceTrustStatus{};
    g_status.state = DeviceTrustState::UNKNOWN;
}
