...
namespace {
    RoomGuestAccessUtilsConfig g_config;
    std::mutex g_mutex;
    RoomGuestAccessUtilsStatus g_status;
    RoomGuestAccessUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* room_guest_access_utils_stateToString(GuestAccessUtilsAction s) {
    switch (s) {
        case GuestAccessUtilsAction::UNKNOWN: return "UNKNOWN";
        case GuestAccessUtilsAction::INIT: return "INIT";
        case GuestAccessUtilsAction::PROCESSING: return "PROCESSING";
        case GuestAccessUtilsAction::DONE: return "DONE";
        case GuestAccessUtilsAction::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

GuestAccessUtilsAction room_guest_access_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return GuestAccessUtilsAction::UNKNOWN;
    if (s == "INIT") return GuestAccessUtilsAction::INIT;
    if (s == "PROCESSING") return GuestAccessUtilsAction::PROCESSING;
    if (s == "DONE") return GuestAccessUtilsAction::DONE;
    if (s == "ERROR") return GuestAccessUtilsAction::ERROR;
    return GuestAccessUtilsAction::UNKNOWN;
}
...
bool room_guest_access_utils_init(const RoomGuestAccessUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = GuestAccessUtilsAction::INIT;
...
RoomGuestAccessUtilsResponse room_guest_access_utils_handle(const RoomGuestAccessUtilsRequest& req) {
    RoomGuestAccessUtilsResponse resp;
    ...
    g_status.state = GuestAccessUtilsAction::PROCESSING;
    // Validate request
...
void room_guest_access_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = RoomGuestAccessUtilsConfig{};
    g_status = RoomGuestAccessUtilsStatus{};
    g_status.state = GuestAccessUtilsAction::UNKNOWN;
}
...
std::string room_guest_access_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "room_guest_access_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
RoomGuestAccessUtilsConfig room_guest_access_utils_configFromJson(const json& j) {
    RoomGuestAccessUtilsConfig config;
