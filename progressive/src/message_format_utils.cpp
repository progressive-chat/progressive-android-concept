...
namespace {
    MessageFormatUtilsConfig g_config;
    std::mutex g_mutex;
    MessageFormatUtilsStatus g_status;
    MessageFormatUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* message_format_utils_stateToString(FormatUtilsPhase s) {
    switch (s) {
        case FormatUtilsPhase::UNKNOWN: return "UNKNOWN";
        case FormatUtilsPhase::INIT: return "INIT";
        case FormatUtilsPhase::PROCESSING: return "PROCESSING";
        case FormatUtilsPhase::DONE: return "DONE";
        case FormatUtilsPhase::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

FormatUtilsPhase message_format_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return FormatUtilsPhase::UNKNOWN;
    if (s == "INIT") return FormatUtilsPhase::INIT;
    if (s == "PROCESSING") return FormatUtilsPhase::PROCESSING;
    if (s == "DONE") return FormatUtilsPhase::DONE;
    if (s == "ERROR") return FormatUtilsPhase::ERROR;
    return FormatUtilsPhase::UNKNOWN;
}
...
bool message_format_utils_init(const MessageFormatUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = FormatUtilsPhase::INIT;
...
MessageFormatUtilsResponse message_format_utils_handle(const MessageFormatUtilsRequest& req) {
    MessageFormatUtilsResponse resp;
    ...
    g_status.state = FormatUtilsPhase::PROCESSING;
    // Validate request
...
void message_format_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = MessageFormatUtilsConfig{};
    g_status = MessageFormatUtilsStatus{};
    g_status.state = FormatUtilsPhase::UNKNOWN;
}
...
std::string message_format_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "message_format_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
MessageFormatUtilsConfig message_format_utils_configFromJson(const json& j) {
    MessageFormatUtilsConfig config;
