...
namespace {
    NotificationGroupConfig g_config;
    std::mutex g_mutex;
    NotificationGroupStatus g_status;
    NotificationGroupStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* notification_group_stateToString(GroupLevel s) {
    switch (s) {
        case GroupLevel::UNKNOWN: return "UNKNOWN";
        case GroupLevel::INIT: return "INIT";
        case GroupLevel::PROCESSING: return "PROCESSING";
        case GroupLevel::DONE: return "DONE";
        case GroupLevel::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

GroupLevel notification_group_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return GroupLevel::UNKNOWN;
    if (s == "INIT") return GroupLevel::INIT;
    if (s == "PROCESSING") return GroupLevel::PROCESSING;
    if (s == "DONE") return GroupLevel::DONE;
    if (s == "ERROR") return GroupLevel::ERROR;
    return GroupLevel::UNKNOWN;
}
...
bool notification_group_init(const NotificationGroupConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = GroupLevel::INIT;
...
NotificationGroupResponse notification_group_handle(const NotificationGroupRequest& req) {
    NotificationGroupResponse resp;
    ...
    g_status.state = GroupLevel::PROCESSING;
    // Validate request
...
void notification_group_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = NotificationGroupConfig{};
    g_status = NotificationGroupStatus{};
    g_status.state = GroupLevel::UNKNOWN;
}
...
std::string notification_group_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "notification_group";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
