...
namespace {
    WellKnownConfig g_config;
    std::mutex g_mutex;
    WellKnownStatus g_status;
    WellKnownStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* well_known_stateToString(WellKnownState s) {
    switch (s) {
        case WellKnownState::UNKNOWN: return "UNKNOWN";
        case WellKnownState::INIT: return "INIT";
        case WellKnownState::PROCESSING: return "PROCESSING";
        case WellKnownState::DONE: return "DONE";
        case WellKnownState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

WellKnownState well_known_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return WellKnownState::UNKNOWN;
    if (s == "INIT") return WellKnownState::INIT;
    if (s == "PROCESSING") return WellKnownState::PROCESSING;
    if (s == "DONE") return WellKnownState::DONE;
    if (s == "ERROR") return WellKnownState::ERROR;
    return WellKnownState::UNKNOWN;
}
...
bool well_known_init(const WellKnownConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = WellKnownState::INIT;
...
WellKnownResponse well_known_handle(const WellKnownRequest& req) {
    ...
    g_status.state = WellKnownState::PROCESSING;
    // Validate request
...
void well_known_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = WellKnownConfig{};
    g_status = WellKnownStatus{};
    g_status.state = WellKnownState::UNKNOWN;
}
