...
namespace {
    RoomTagUtilsConfig g_config;
    std::mutex g_mutex;
    RoomTagUtilsStatus g_status;
    RoomTagUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* room_tag_utils_stateToString(TagUtilsAction s) {
    switch (s) {
        case TagUtilsAction::UNKNOWN: return "UNKNOWN";
        case TagUtilsAction::INIT: return "INIT";
        case TagUtilsAction::PROCESSING: return "PROCESSING";
        case TagUtilsAction::DONE: return "DONE";
        case TagUtilsAction::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

TagUtilsAction room_tag_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return TagUtilsAction::UNKNOWN;
    if (s == "INIT") return TagUtilsAction::INIT;
    if (s == "PROCESSING") return TagUtilsAction::PROCESSING;
    if (s == "DONE") return TagUtilsAction::DONE;
    if (s == "ERROR") return TagUtilsAction::ERROR;
    return TagUtilsAction::UNKNOWN;
}
...
bool room_tag_utils_init(const RoomTagUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = TagUtilsAction::INIT;
...
RoomTagUtilsResponse room_tag_utils_handle(const RoomTagUtilsRequest& req) {
    ...
    g_status.state = TagUtilsAction::PROCESSING;
    // Validate request
...
void room_tag_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = RoomTagUtilsConfig{};
    g_status = RoomTagUtilsStatus{};
    g_status.state = TagUtilsAction::UNKNOWN;
}
