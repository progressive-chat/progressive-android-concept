...
namespace {
    EventStateUtilsConfig g_config;
    std::mutex g_mutex;
    EventStateUtilsStatus g_status;
    EventStateUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* event_state_utils_stateToString(StateUtilsState s) {
    switch (s) {
        case StateUtilsState::UNKNOWN: return "UNKNOWN";
        case StateUtilsState::INIT: return "INIT";
        case StateUtilsState::PROCESSING: return "PROCESSING";
        case StateUtilsState::DONE: return "DONE";
        case StateUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

StateUtilsState event_state_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return StateUtilsState::UNKNOWN;
    if (s == "INIT") return StateUtilsState::INIT;
    if (s == "PROCESSING") return StateUtilsState::PROCESSING;
    if (s == "DONE") return StateUtilsState::DONE;
    if (s == "ERROR") return StateUtilsState::ERROR;
    return StateUtilsState::UNKNOWN;
}
...
bool event_state_utils_init(const EventStateUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = StateUtilsState::INIT;
...
EventStateUtilsResponse event_state_utils_handle(const EventStateUtilsRequest& req) {
    ...
    g_status.state = StateUtilsState::PROCESSING;
    // Validate request
...
void event_state_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = EventStateUtilsConfig{};
    g_status = EventStateUtilsStatus{};
    g_status.state = StateUtilsState::UNKNOWN;
}
...
std::string event_state_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "event_state_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
