...
namespace {
    EventContentUtilsConfig g_config;
    std::mutex g_mutex;
    EventContentUtilsStatus g_status;
    EventContentUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* event_content_utils_stateToString(ContentUtilsState s) {
    switch (s) {
        case ContentUtilsState::UNKNOWN: return "UNKNOWN";
        case ContentUtilsState::INIT: return "INIT";
        case ContentUtilsState::PROCESSING: return "PROCESSING";
        case ContentUtilsState::DONE: return "DONE";
        case ContentUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

ContentUtilsState event_content_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return ContentUtilsState::UNKNOWN;
    if (s == "INIT") return ContentUtilsState::INIT;
    if (s == "PROCESSING") return ContentUtilsState::PROCESSING;
    if (s == "DONE") return ContentUtilsState::DONE;
    if (s == "ERROR") return ContentUtilsState::ERROR;
    return ContentUtilsState::UNKNOWN;
}
...
bool event_content_utils_init(const EventContentUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = ContentUtilsState::INIT;
...
EventContentUtilsResponse event_content_utils_handle(const EventContentUtilsRequest& req) {
    EventContentUtilsResponse resp;
    ...
    g_status.state = ContentUtilsState::PROCESSING;
    // Validate request
...
void event_content_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = EventContentUtilsConfig{};
    g_status = EventContentUtilsStatus{};
    g_status.state = ContentUtilsState::UNKNOWN;
}
...
std::string event_content_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "event_content_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
EventContentUtilsConfig event_content_utils_configFromJson(const json& j) {
    EventContentUtilsConfig config;
