...
namespace {
    WidgetManagerExtConfig g_config;
    std::mutex g_mutex;
    WidgetManagerExtStatus g_status;
    WidgetManagerExtStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* widget_manager_ext_stateToString(WidgetManagerExtState s) {
    switch (s) {
        case WidgetManagerExtState::UNKNOWN: return "UNKNOWN";
        case WidgetManagerExtState::INIT: return "INIT";
        case WidgetManagerExtState::PROCESSING: return "PROCESSING";
        case WidgetManagerExtState::DONE: return "DONE";
        case WidgetManagerExtState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

WidgetManagerExtState widget_manager_ext_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return WidgetManagerExtState::UNKNOWN;
    if (s == "INIT") return WidgetManagerExtState::INIT;
    if (s == "PROCESSING") return WidgetManagerExtState::PROCESSING;
    if (s == "DONE") return WidgetManagerExtState::DONE;
    if (s == "ERROR") return WidgetManagerExtState::ERROR;
    return WidgetManagerExtState::UNKNOWN;
}
...
bool widget_manager_ext_init(const WidgetManagerExtConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = WidgetManagerExtState::INIT;
...
WidgetManagerExtResponse widget_manager_ext_handle(const WidgetManagerExtRequest& req) {
    ...
    g_status.state = WidgetManagerExtState::PROCESSING;
    // Validate request
...
void widget_manager_ext_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = WidgetManagerExtConfig{};
    g_status = WidgetManagerExtStatus{};
    g_status.state = WidgetManagerExtState::UNKNOWN;
}
...
std::string widget_manager_ext_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "widget_manager_ext";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
