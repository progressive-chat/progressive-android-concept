...
namespace {
    LocationShareConfig g_config;
    std::mutex g_mutex;
    LocationShareStatus g_status;
    LocationShareStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* location_share_stateToString(LocationShareState s) {
    switch (s) {
        case LocationShareState::UNKNOWN: return "UNKNOWN";
        case LocationShareState::INIT: return "INIT";
        case LocationShareState::PROCESSING: return "PROCESSING";
        case LocationShareState::DONE: return "DONE";
        case LocationShareState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

LocationShareState location_share_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return LocationShareState::UNKNOWN;
    if (s == "INIT") return LocationShareState::INIT;
    if (s == "PROCESSING") return LocationShareState::PROCESSING;
    if (s == "DONE") return LocationShareState::DONE;
    if (s == "ERROR") return LocationShareState::ERROR;
    return LocationShareState::UNKNOWN;
}
...
bool location_share_init(const LocationShareConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = LocationShareState::INIT;
...
LocationShareResponse location_share_handle(const LocationShareRequest& req) {
    ...
    g_status.state = LocationShareState::PROCESSING;
    // Validate request
...
void location_share_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = LocationShareConfig{};
    g_status = LocationShareStatus{};
    g_status.state = LocationShareState::UNKNOWN;
}
