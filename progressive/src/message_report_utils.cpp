...
namespace {
    MessageReportUtilsConfig g_config;
    std::mutex g_mutex;
    MessageReportUtilsStatus g_status;
    MessageReportUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* message_report_utils_stateToString(ReportUtilsPhase s) {
    switch (s) {
        case ReportUtilsPhase::UNKNOWN: return "UNKNOWN";
        case ReportUtilsPhase::INIT: return "INIT";
        case ReportUtilsPhase::PROCESSING: return "PROCESSING";
        case ReportUtilsPhase::DONE: return "DONE";
        case ReportUtilsPhase::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

ReportUtilsPhase message_report_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return ReportUtilsPhase::UNKNOWN;
    if (s == "INIT") return ReportUtilsPhase::INIT;
    if (s == "PROCESSING") return ReportUtilsPhase::PROCESSING;
    if (s == "DONE") return ReportUtilsPhase::DONE;
    if (s == "ERROR") return ReportUtilsPhase::ERROR;
    return ReportUtilsPhase::UNKNOWN;
}
...
bool message_report_utils_init(const MessageReportUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = ReportUtilsPhase::INIT;
...
MessageReportUtilsResponse message_report_utils_handle(const MessageReportUtilsRequest& req) {
    MessageReportUtilsResponse resp;
    ...
    g_status.state = ReportUtilsPhase::PROCESSING;
    // Validate request
...
void message_report_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = MessageReportUtilsConfig{};
    g_status = MessageReportUtilsStatus{};
    g_status.state = ReportUtilsPhase::UNKNOWN;
}
...
std::string message_report_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "message_report_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
MessageReportUtilsConfig message_report_utils_configFromJson(const json& j) {
    MessageReportUtilsConfig config;
