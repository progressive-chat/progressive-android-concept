...
namespace {
    PushAnalyticsConfig g_config;
    std::mutex g_mutex;
    PushAnalyticsStatus g_status;
    PushAnalyticsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* push_analytics_stateToString(AnalyticsKind s) {
    switch (s) {
        case AnalyticsKind::UNKNOWN: return "UNKNOWN";
        case AnalyticsKind::INIT: return "INIT";
        case AnalyticsKind::PROCESSING: return "PROCESSING";
        case AnalyticsKind::DONE: return "DONE";
        case AnalyticsKind::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

AnalyticsKind push_analytics_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return AnalyticsKind::UNKNOWN;
    if (s == "INIT") return AnalyticsKind::INIT;
    if (s == "PROCESSING") return AnalyticsKind::PROCESSING;
    if (s == "DONE") return AnalyticsKind::DONE;
    if (s == "ERROR") return AnalyticsKind::ERROR;
    return AnalyticsKind::UNKNOWN;
}
...
bool push_analytics_init(const PushAnalyticsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = AnalyticsKind::INIT;
...
PushAnalyticsResponse push_analytics_handle(const PushAnalyticsRequest& req) {
    ...
    g_status.state = AnalyticsKind::PROCESSING;
    // Validate request
...
void push_analytics_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = PushAnalyticsConfig{};
    g_status = PushAnalyticsStatus{};
    g_status.state = AnalyticsKind::UNKNOWN;
}
