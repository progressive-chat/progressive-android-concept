...
namespace {
    DehydrateUtilsConfig g_config;
    std::mutex g_mutex;
    DehydrateUtilsStatus g_status;
    DehydrateUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* dehydrate_utils_stateToString(DehydrateUtilsState s) {
    switch (s) {
        case DehydrateUtilsState::UNKNOWN: return "UNKNOWN";
        case DehydrateUtilsState::INIT: return "INIT";
        case DehydrateUtilsState::PROCESSING: return "PROCESSING";
        case DehydrateUtilsState::DONE: return "DONE";
        case DehydrateUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

DehydrateUtilsState dehydrate_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return DehydrateUtilsState::UNKNOWN;
    if (s == "INIT") return DehydrateUtilsState::INIT;
    if (s == "PROCESSING") return DehydrateUtilsState::PROCESSING;
    if (s == "DONE") return DehydrateUtilsState::DONE;
    if (s == "ERROR") return DehydrateUtilsState::ERROR;
    return DehydrateUtilsState::UNKNOWN;
}
...
bool dehydrate_utils_init(const DehydrateUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = DehydrateUtilsState::INIT;
...
DehydrateUtilsResponse dehydrate_utils_handle(const DehydrateUtilsRequest& req) {
    ...
    g_status.state = DehydrateUtilsState::PROCESSING;
    // Validate request
...
void dehydrate_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = DehydrateUtilsConfig{};
    g_status = DehydrateUtilsStatus{};
    g_status.state = DehydrateUtilsState::UNKNOWN;
}
