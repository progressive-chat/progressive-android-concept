...
namespace {
    TermsManagerConfig g_config;
    std::mutex g_mutex;
    TermsManagerStatus g_status;
    TermsManagerStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* terms_manager_stateToString(TermsManagerState s) {
    switch (s) {
        case TermsManagerState::UNKNOWN: return "UNKNOWN";
        case TermsManagerState::INIT: return "INIT";
        case TermsManagerState::PROCESSING: return "PROCESSING";
        case TermsManagerState::DONE: return "DONE";
        case TermsManagerState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

TermsManagerState terms_manager_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return TermsManagerState::UNKNOWN;
    if (s == "INIT") return TermsManagerState::INIT;
    if (s == "PROCESSING") return TermsManagerState::PROCESSING;
    if (s == "DONE") return TermsManagerState::DONE;
    if (s == "ERROR") return TermsManagerState::ERROR;
    return TermsManagerState::UNKNOWN;
}
...
bool terms_manager_init(const TermsManagerConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = TermsManagerState::INIT;
...
TermsManagerResponse terms_manager_handle(const TermsManagerRequest& req) {
    ...
    g_status.state = TermsManagerState::PROCESSING;
    // Validate request
...
void terms_manager_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = TermsManagerConfig{};
    g_status = TermsManagerStatus{};
    g_status.state = TermsManagerState::UNKNOWN;
}
