...
namespace {
    SessionRestoreConfig g_config;
    std::mutex g_mutex;
    SessionRestoreStatus g_status;
    SessionRestoreStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* session_restore_stateToString(RestorePhase s) {
    switch (s) {
        case RestorePhase::UNKNOWN: return "UNKNOWN";
        case RestorePhase::INIT: return "INIT";
        case RestorePhase::PROCESSING: return "PROCESSING";
        case RestorePhase::DONE: return "DONE";
        case RestorePhase::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

RestorePhase session_restore_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return RestorePhase::UNKNOWN;
    if (s == "INIT") return RestorePhase::INIT;
    if (s == "PROCESSING") return RestorePhase::PROCESSING;
    if (s == "DONE") return RestorePhase::DONE;
    if (s == "ERROR") return RestorePhase::ERROR;
    return RestorePhase::UNKNOWN;
}
...
bool session_restore_init(const SessionRestoreConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = RestorePhase::INIT;
...
SessionRestoreResponse session_restore_handle(const SessionRestoreRequest& req) {
    ...
    g_status.state = RestorePhase::PROCESSING;
    // Validate request
...
void session_restore_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = SessionRestoreConfig{};
    g_status = SessionRestoreStatus{};
    g_status.state = RestorePhase::UNKNOWN;
}
