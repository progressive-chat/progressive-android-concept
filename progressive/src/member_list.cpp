...
namespace {
    MemberListConfig g_config;
    std::mutex g_mutex;
    MemberListStatus g_status;
    MemberListStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* member_list_stateToString(MemberListState s) {
    switch (s) {
        case MemberListState::UNKNOWN: return "UNKNOWN";
        case MemberListState::INIT: return "INIT";
        case MemberListState::PROCESSING: return "PROCESSING";
        case MemberListState::DONE: return "DONE";
        case MemberListState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

MemberListState member_list_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return MemberListState::UNKNOWN;
    if (s == "INIT") return MemberListState::INIT;
    if (s == "PROCESSING") return MemberListState::PROCESSING;
    if (s == "DONE") return MemberListState::DONE;
    if (s == "ERROR") return MemberListState::ERROR;
    return MemberListState::UNKNOWN;
}
...
bool member_list_init(const MemberListConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = MemberListState::INIT;
...
MemberListResponse member_list_handle(const MemberListRequest& req) {
    ...
    g_status.state = MemberListState::PROCESSING;
    // Validate request
...
void member_list_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = MemberListConfig{};
    g_status = MemberListStatus{};
    g_status.state = MemberListState::UNKNOWN;
}
