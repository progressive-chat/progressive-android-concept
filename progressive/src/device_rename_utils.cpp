...
namespace {
    DeviceRenameUtilsConfig g_config;
    std::mutex g_mutex;
    DeviceRenameUtilsStatus g_status;
    DeviceRenameUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* device_rename_utils_stateToString(DeviceRenameUtilsState s) {
    switch (s) {
        case DeviceRenameUtilsState::UNKNOWN: return "UNKNOWN";
        case DeviceRenameUtilsState::INIT: return "INIT";
        case DeviceRenameUtilsState::PROCESSING: return "PROCESSING";
        case DeviceRenameUtilsState::DONE: return "DONE";
        case DeviceRenameUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

DeviceRenameUtilsState device_rename_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return DeviceRenameUtilsState::UNKNOWN;
    if (s == "INIT") return DeviceRenameUtilsState::INIT;
    if (s == "PROCESSING") return DeviceRenameUtilsState::PROCESSING;
    if (s == "DONE") return DeviceRenameUtilsState::DONE;
    if (s == "ERROR") return DeviceRenameUtilsState::ERROR;
    return DeviceRenameUtilsState::UNKNOWN;
}
...
bool device_rename_utils_init(const DeviceRenameUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = DeviceRenameUtilsState::INIT;
...
DeviceRenameUtilsResponse device_rename_utils_handle(const DeviceRenameUtilsRequest& req) {
    DeviceRenameUtilsResponse resp;
    ...
    g_status.state = DeviceRenameUtilsState::PROCESSING;
    // Validate request
...
void device_rename_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = DeviceRenameUtilsConfig{};
    g_status = DeviceRenameUtilsStatus{};
    g_status.state = DeviceRenameUtilsState::UNKNOWN;
}
...
std::string device_rename_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "device_rename_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
DeviceRenameUtilsConfig device_rename_utils_configFromJson(const json& j) {
    DeviceRenameUtilsConfig config;
