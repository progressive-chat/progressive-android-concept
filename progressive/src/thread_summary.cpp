...
namespace {
    ThreadSummaryConfig g_config;
    std::mutex g_mutex;
    ThreadSummaryStatus g_status;
    ThreadSummaryStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* thread_summary_stateToString(ThreadSummaryState s) {
    switch (s) {
        case ThreadSummaryState::UNKNOWN: return "UNKNOWN";
        case ThreadSummaryState::INIT: return "INIT";
        case ThreadSummaryState::PROCESSING: return "PROCESSING";
        case ThreadSummaryState::DONE: return "DONE";
        case ThreadSummaryState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

ThreadSummaryState thread_summary_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return ThreadSummaryState::UNKNOWN;
    if (s == "INIT") return ThreadSummaryState::INIT;
    if (s == "PROCESSING") return ThreadSummaryState::PROCESSING;
    if (s == "DONE") return ThreadSummaryState::DONE;
    if (s == "ERROR") return ThreadSummaryState::ERROR;
    return ThreadSummaryState::UNKNOWN;
}
...
bool thread_summary_init(const ThreadSummaryConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = ThreadSummaryState::INIT;
...
ThreadSummaryResponse thread_summary_handle(const ThreadSummaryRequest& req) {
    ...
    g_status.state = ThreadSummaryState::PROCESSING;
    // Validate request
...
void thread_summary_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = ThreadSummaryConfig{};
    g_status = ThreadSummaryStatus{};
    g_status.state = ThreadSummaryState::UNKNOWN;
}
