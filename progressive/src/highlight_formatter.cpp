...
namespace {
    HighlightFormatterConfig g_config;
    std::mutex g_mutex;
    HighlightFormatterStatus g_status;
    HighlightFormatterStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* highlight_formatter_stateToString(HighlightFormatterState s) {
    switch (s) {
        case HighlightFormatterState::UNKNOWN: return "UNKNOWN";
        case HighlightFormatterState::INIT: return "INIT";
        case HighlightFormatterState::PROCESSING: return "PROCESSING";
        case HighlightFormatterState::DONE: return "DONE";
        case HighlightFormatterState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

HighlightFormatterState highlight_formatter_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return HighlightFormatterState::UNKNOWN;
    if (s == "INIT") return HighlightFormatterState::INIT;
    if (s == "PROCESSING") return HighlightFormatterState::PROCESSING;
    if (s == "DONE") return HighlightFormatterState::DONE;
    if (s == "ERROR") return HighlightFormatterState::ERROR;
    return HighlightFormatterState::UNKNOWN;
}
...
bool highlight_formatter_init(const HighlightFormatterConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = HighlightFormatterState::INIT;
...
HighlightFormatterResponse highlight_formatter_handle(const HighlightFormatterRequest& req) {
    HighlightFormatterResponse resp;
    ...
    g_status.state = HighlightFormatterState::PROCESSING;
    // Validate request
...
void highlight_formatter_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = HighlightFormatterConfig{};
    g_status = HighlightFormatterStatus{};
    g_status.state = HighlightFormatterState::UNKNOWN;
}
...
std::string highlight_formatter_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "highlight_formatter";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
HighlightFormatterConfig highlight_formatter_configFromJson(const json& j) {
    HighlightFormatterConfig config;
