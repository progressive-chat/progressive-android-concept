...
namespace {
    RoomUploadProgressConfig g_config;
    std::mutex g_mutex;
    RoomUploadProgressStatus g_status;
    RoomUploadProgressStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* room_upload_progress_stateToString(UploadProgressAction s) {
    switch (s) {
        case UploadProgressAction::UNKNOWN: return "UNKNOWN";
        case UploadProgressAction::INIT: return "INIT";
        case UploadProgressAction::PROCESSING: return "PROCESSING";
        case UploadProgressAction::DONE: return "DONE";
        case UploadProgressAction::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

UploadProgressAction room_upload_progress_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return UploadProgressAction::UNKNOWN;
    if (s == "INIT") return UploadProgressAction::INIT;
    if (s == "PROCESSING") return UploadProgressAction::PROCESSING;
    if (s == "DONE") return UploadProgressAction::DONE;
    if (s == "ERROR") return UploadProgressAction::ERROR;
    return UploadProgressAction::UNKNOWN;
}
...
bool room_upload_progress_init(const RoomUploadProgressConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = UploadProgressAction::INIT;
...
RoomUploadProgressResponse room_upload_progress_handle(const RoomUploadProgressRequest& req) {
    RoomUploadProgressResponse resp;
    ...
    g_status.state = UploadProgressAction::PROCESSING;
    // Validate request
...
void room_upload_progress_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = RoomUploadProgressConfig{};
    g_status = RoomUploadProgressStatus{};
    g_status.state = UploadProgressAction::UNKNOWN;
}
...
std::string room_upload_progress_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "room_upload_progress";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
RoomUploadProgressConfig room_upload_progress_configFromJson(const json& j) {
    RoomUploadProgressConfig config;
