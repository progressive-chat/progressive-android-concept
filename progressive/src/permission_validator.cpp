...
namespace {
    PermissionValidatorConfig g_config;
    std::mutex g_mutex;
    PermissionValidatorStatus g_status;
    PermissionValidatorStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* permission_validator_stateToString(PermissionValidatorState s) {
    switch (s) {
        case PermissionValidatorState::UNKNOWN: return "UNKNOWN";
        case PermissionValidatorState::INIT: return "INIT";
        case PermissionValidatorState::PROCESSING: return "PROCESSING";
        case PermissionValidatorState::DONE: return "DONE";
        case PermissionValidatorState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

PermissionValidatorState permission_validator_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return PermissionValidatorState::UNKNOWN;
    if (s == "INIT") return PermissionValidatorState::INIT;
    if (s == "PROCESSING") return PermissionValidatorState::PROCESSING;
    if (s == "DONE") return PermissionValidatorState::DONE;
    if (s == "ERROR") return PermissionValidatorState::ERROR;
    return PermissionValidatorState::UNKNOWN;
}
...
bool permission_validator_init(const PermissionValidatorConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = PermissionValidatorState::INIT;
...
PermissionValidatorResponse permission_validator_handle(const PermissionValidatorRequest& req) {
    PermissionValidatorResponse resp;
    ...
    g_status.state = PermissionValidatorState::PROCESSING;
    // Validate request
...
void permission_validator_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = PermissionValidatorConfig{};
    g_status = PermissionValidatorStatus{};
    g_status.state = PermissionValidatorState::UNKNOWN;
}
...
std::string permission_validator_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "permission_validator";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
PermissionValidatorConfig permission_validator_configFromJson(const json& j) {
    PermissionValidatorConfig config;
