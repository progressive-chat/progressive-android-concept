...
namespace {
    SessionLifecycleUtilsConfig g_config;
    std::mutex g_mutex;
    SessionLifecycleUtilsStatus g_status;
    SessionLifecycleUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* session_lifecycle_utils_stateToString(LifecycleUtilsPhase s) {
    switch (s) {
        case LifecycleUtilsPhase::UNKNOWN: return "UNKNOWN";
        case LifecycleUtilsPhase::INIT: return "INIT";
        case LifecycleUtilsPhase::PROCESSING: return "PROCESSING";
        case LifecycleUtilsPhase::DONE: return "DONE";
        case LifecycleUtilsPhase::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

LifecycleUtilsPhase session_lifecycle_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return LifecycleUtilsPhase::UNKNOWN;
    if (s == "INIT") return LifecycleUtilsPhase::INIT;
    if (s == "PROCESSING") return LifecycleUtilsPhase::PROCESSING;
    if (s == "DONE") return LifecycleUtilsPhase::DONE;
    if (s == "ERROR") return LifecycleUtilsPhase::ERROR;
    return LifecycleUtilsPhase::UNKNOWN;
}
...
bool session_lifecycle_utils_init(const SessionLifecycleUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = LifecycleUtilsPhase::INIT;
...
SessionLifecycleUtilsResponse session_lifecycle_utils_handle(const SessionLifecycleUtilsRequest& req) {
    SessionLifecycleUtilsResponse resp;
    ...
    g_status.state = LifecycleUtilsPhase::PROCESSING;
    // Validate request
...
void session_lifecycle_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = SessionLifecycleUtilsConfig{};
    g_status = SessionLifecycleUtilsStatus{};
    g_status.state = LifecycleUtilsPhase::UNKNOWN;
}
...
std::string session_lifecycle_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "session_lifecycle_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
SessionLifecycleUtilsConfig session_lifecycle_utils_configFromJson(const json& j) {
    SessionLifecycleUtilsConfig config;
