...
namespace {
    TermsServiceConfig g_config;
    std::mutex g_mutex;
    TermsServiceStatus g_status;
    TermsServiceStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* terms_service_stateToString(TermsServiceState s) {
    switch (s) {
        case TermsServiceState::UNKNOWN: return "UNKNOWN";
        case TermsServiceState::INIT: return "INIT";
        case TermsServiceState::PROCESSING: return "PROCESSING";
        case TermsServiceState::DONE: return "DONE";
        case TermsServiceState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

TermsServiceState terms_service_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return TermsServiceState::UNKNOWN;
    if (s == "INIT") return TermsServiceState::INIT;
    if (s == "PROCESSING") return TermsServiceState::PROCESSING;
    if (s == "DONE") return TermsServiceState::DONE;
    if (s == "ERROR") return TermsServiceState::ERROR;
    return TermsServiceState::UNKNOWN;
}

bool terms_service_init(const TermsServiceConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = TermsServiceState::INIT;
...
TermsServiceResponse terms_service_handle(const TermsServiceRequest& req) {
    TermsServiceResponse resp;
    ...
    g_status.state = TermsServiceState::PROCESSING;
    // Validate request
...
TermsServiceStatus terms_service_getStatus() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_status;
}

void terms_service_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = TermsServiceConfig{};
    g_status = TermsServiceStatus{};
    g_status.state = TermsServiceState::UNKNOWN;
}
...
TermsServiceStats terms_service_getStats() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_stats;
}

TermsServiceConfig terms_service_configFromJson(const json& j) {
    TermsServiceConfig config;
...
json terms_service_configToJson(const TermsServiceConfig& config) {
    json j;
