...
namespace {
    BackupRestoreUtilsConfig g_config;
    std::mutex g_mutex;
    BackupRestoreUtilsStatus g_status;
    BackupRestoreUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* backup_restore_utils_stateToString(BackupRestoreUtilsState s) {
    switch (s) {
        case BackupRestoreUtilsState::UNKNOWN: return "UNKNOWN";
        case BackupRestoreUtilsState::INIT: return "INIT";
        case BackupRestoreUtilsState::PROCESSING: return "PROCESSING";
        case BackupRestoreUtilsState::DONE: return "DONE";
        case BackupRestoreUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

BackupRestoreUtilsState backup_restore_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return BackupRestoreUtilsState::UNKNOWN;
    if (s == "INIT") return BackupRestoreUtilsState::INIT;
    if (s == "PROCESSING") return BackupRestoreUtilsState::PROCESSING;
    if (s == "DONE") return BackupRestoreUtilsState::DONE;
    if (s == "ERROR") return BackupRestoreUtilsState::ERROR;
    return BackupRestoreUtilsState::UNKNOWN;
}
...
bool backup_restore_utils_init(const BackupRestoreUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = BackupRestoreUtilsState::INIT;
...
BackupRestoreUtilsResponse backup_restore_utils_handle(const BackupRestoreUtilsRequest& req) {
    BackupRestoreUtilsResponse resp;
    ...
    g_status.state = BackupRestoreUtilsState::PROCESSING;
    // Validate request
...
void backup_restore_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = BackupRestoreUtilsConfig{};
    g_status = BackupRestoreUtilsStatus{};
    g_status.state = BackupRestoreUtilsState::UNKNOWN;
}
...
std::string backup_restore_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "backup_restore_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
BackupRestoreUtilsConfig backup_restore_utils_configFromJson(const json& j) {
    BackupRestoreUtilsConfig config;
