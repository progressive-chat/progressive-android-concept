...
namespace {
    RoomChildUtilsConfig g_config;
    std::mutex g_mutex;
    RoomChildUtilsStatus g_status;
    RoomChildUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* room_child_utils_stateToString(ChildUtilsAction s) {
    switch (s) {
        case ChildUtilsAction::UNKNOWN: return "UNKNOWN";
        case ChildUtilsAction::INIT: return "INIT";
        case ChildUtilsAction::PROCESSING: return "PROCESSING";
        case ChildUtilsAction::DONE: return "DONE";
        case ChildUtilsAction::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

ChildUtilsAction room_child_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return ChildUtilsAction::UNKNOWN;
    if (s == "INIT") return ChildUtilsAction::INIT;
    if (s == "PROCESSING") return ChildUtilsAction::PROCESSING;
    if (s == "DONE") return ChildUtilsAction::DONE;
    if (s == "ERROR") return ChildUtilsAction::ERROR;
    return ChildUtilsAction::UNKNOWN;
}

bool room_child_utils_init(const RoomChildUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = ChildUtilsAction::INIT;
...
RoomChildUtilsResponse room_child_utils_handle(const RoomChildUtilsRequest& req) {
    RoomChildUtilsResponse resp;
    ...
    g_status.state = ChildUtilsAction::PROCESSING;
    // Validate request
...
RoomChildUtilsStatus room_child_utils_getStatus() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_status;
}

void room_child_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = RoomChildUtilsConfig{};
    g_status = RoomChildUtilsStatus{};
    g_status.state = ChildUtilsAction::UNKNOWN;
}
...
std::string room_child_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "room_child_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
RoomChildUtilsStats room_child_utils_getStats() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_stats;
}

RoomChildUtilsConfig room_child_utils_configFromJson(const json& j) {
    RoomChildUtilsConfig config;
...
json room_child_utils_configToJson(const RoomChildUtilsConfig& config) {
    json j;
