...
namespace {
    AvatarUrlUtilsConfig g_config;
    std::mutex g_mutex;
    AvatarUrlUtilsStatus g_status;
    AvatarUrlUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* avatar_url_utils_stateToString(AvatarUrlUtilsState s) {
    switch (s) {
        case AvatarUrlUtilsState::UNKNOWN: return "UNKNOWN";
        case AvatarUrlUtilsState::INIT: return "INIT";
        case AvatarUrlUtilsState::PROCESSING: return "PROCESSING";
        case AvatarUrlUtilsState::DONE: return "DONE";
        case AvatarUrlUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

AvatarUrlUtilsState avatar_url_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return AvatarUrlUtilsState::UNKNOWN;
    if (s == "INIT") return AvatarUrlUtilsState::INIT;
    if (s == "PROCESSING") return AvatarUrlUtilsState::PROCESSING;
    if (s == "DONE") return AvatarUrlUtilsState::DONE;
    if (s == "ERROR") return AvatarUrlUtilsState::ERROR;
    return AvatarUrlUtilsState::UNKNOWN;
}
...
bool avatar_url_utils_init(const AvatarUrlUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = AvatarUrlUtilsState::INIT;
...
AvatarUrlUtilsResponse avatar_url_utils_handle(const AvatarUrlUtilsRequest& req) {
    ...
    g_status.state = AvatarUrlUtilsState::PROCESSING;
    // Validate request
...
void avatar_url_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = AvatarUrlUtilsConfig{};
    g_status = AvatarUrlUtilsStatus{};
    g_status.state = AvatarUrlUtilsState::UNKNOWN;
}
...
std::string avatar_url_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "avatar_url_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
