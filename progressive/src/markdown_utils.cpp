...
namespace {
    MarkdownUtilsConfig g_config;
    std::mutex g_mutex;
    MarkdownUtilsStatus g_status;
    MarkdownUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* markdown_utils_stateToString(MarkdownUtilsState s) {
    switch (s) {
        case MarkdownUtilsState::UNKNOWN: return "UNKNOWN";
        case MarkdownUtilsState::INIT: return "INIT";
        case MarkdownUtilsState::PROCESSING: return "PROCESSING";
        case MarkdownUtilsState::DONE: return "DONE";
        case MarkdownUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

MarkdownUtilsState markdown_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return MarkdownUtilsState::UNKNOWN;
    if (s == "INIT") return MarkdownUtilsState::INIT;
    if (s == "PROCESSING") return MarkdownUtilsState::PROCESSING;
    if (s == "DONE") return MarkdownUtilsState::DONE;
    if (s == "ERROR") return MarkdownUtilsState::ERROR;
    return MarkdownUtilsState::UNKNOWN;
}
...
bool markdown_utils_init(const MarkdownUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = MarkdownUtilsState::INIT;
...
MarkdownUtilsResponse markdown_utils_handle(const MarkdownUtilsRequest& req) {
    ...
    g_status.state = MarkdownUtilsState::PROCESSING;
    // Validate request
...
void markdown_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = MarkdownUtilsConfig{};
    g_status = MarkdownUtilsStatus{};
    g_status.state = MarkdownUtilsState::UNKNOWN;
}
