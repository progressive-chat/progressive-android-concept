...
namespace {
    RoomInfoUtilsConfig g_config;
    std::mutex g_mutex;
    RoomInfoUtilsStatus g_status;
    RoomInfoUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* room_info_utils_stateToString(InfoUtilsAction s) {
    switch (s) {
        case InfoUtilsAction::UNKNOWN: return "UNKNOWN";
        case InfoUtilsAction::INIT: return "INIT";
        case InfoUtilsAction::PROCESSING: return "PROCESSING";
        case InfoUtilsAction::DONE: return "DONE";
        case InfoUtilsAction::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

InfoUtilsAction room_info_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return InfoUtilsAction::UNKNOWN;
    if (s == "INIT") return InfoUtilsAction::INIT;
    if (s == "PROCESSING") return InfoUtilsAction::PROCESSING;
    if (s == "DONE") return InfoUtilsAction::DONE;
    if (s == "ERROR") return InfoUtilsAction::ERROR;
    return InfoUtilsAction::UNKNOWN;
}
...
bool room_info_utils_init(const RoomInfoUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = InfoUtilsAction::INIT;
...
RoomInfoUtilsResponse room_info_utils_handle(const RoomInfoUtilsRequest& req) {
    ...
    g_status.state = InfoUtilsAction::PROCESSING;
    // Validate request
...
void room_info_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = RoomInfoUtilsConfig{};
    g_status = RoomInfoUtilsStatus{};
    g_status.state = InfoUtilsAction::UNKNOWN;
}
