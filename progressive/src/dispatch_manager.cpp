...
namespace {
    DispatchManagerConfig g_config;
    std::mutex g_mutex;
    DispatchManagerStatus g_status;
    DispatchManagerStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* dispatch_manager_stateToString(DispatchManagerState s) {
    switch (s) {
        case DispatchManagerState::UNKNOWN: return "UNKNOWN";
        case DispatchManagerState::INIT: return "INIT";
        case DispatchManagerState::PROCESSING: return "PROCESSING";
        case DispatchManagerState::DONE: return "DONE";
        case DispatchManagerState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

DispatchManagerState dispatch_manager_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return DispatchManagerState::UNKNOWN;
    if (s == "INIT") return DispatchManagerState::INIT;
    if (s == "PROCESSING") return DispatchManagerState::PROCESSING;
    if (s == "DONE") return DispatchManagerState::DONE;
    if (s == "ERROR") return DispatchManagerState::ERROR;
    return DispatchManagerState::UNKNOWN;
}
...
bool dispatch_manager_init(const DispatchManagerConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = DispatchManagerState::INIT;
...
DispatchManagerResponse dispatch_manager_handle(const DispatchManagerRequest& req) {
    ...
    g_status.state = DispatchManagerState::PROCESSING;
    // Validate request
...
void dispatch_manager_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = DispatchManagerConfig{};
    g_status = DispatchManagerStatus{};
    g_status.state = DispatchManagerState::UNKNOWN;
}
...
std::string dispatch_manager_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "dispatch_manager";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
