...
namespace {
    InvitationHideConfig g_config;
    std::mutex g_mutex;
    InvitationHideStatus g_status;
    InvitationHideStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* invitation_hide_stateToString(InvitationHideState s) {
    switch (s) {
        case InvitationHideState::UNKNOWN: return "UNKNOWN";
        case InvitationHideState::INIT: return "INIT";
        case InvitationHideState::PROCESSING: return "PROCESSING";
        case InvitationHideState::DONE: return "DONE";
        case InvitationHideState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

InvitationHideState invitation_hide_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return InvitationHideState::UNKNOWN;
    if (s == "INIT") return InvitationHideState::INIT;
    if (s == "PROCESSING") return InvitationHideState::PROCESSING;
    if (s == "DONE") return InvitationHideState::DONE;
    if (s == "ERROR") return InvitationHideState::ERROR;
    return InvitationHideState::UNKNOWN;
}
...
bool invitation_hide_init(const InvitationHideConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = InvitationHideState::INIT;
...
InvitationHideResponse invitation_hide_handle(const InvitationHideRequest& req) {
    ...
    g_status.state = InvitationHideState::PROCESSING;
    // Validate request
...
void invitation_hide_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = InvitationHideConfig{};
    g_status = InvitationHideStatus{};
    g_status.state = InvitationHideState::UNKNOWN;
}
