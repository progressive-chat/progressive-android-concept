...
namespace {
    NotificationCleanerConfig g_config;
    std::mutex g_mutex;
    NotificationCleanerStatus g_status;
    NotificationCleanerStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* notification_cleaner_stateToString(CleanerLevel s) {
    switch (s) {
        case CleanerLevel::UNKNOWN: return "UNKNOWN";
        case CleanerLevel::INIT: return "INIT";
        case CleanerLevel::PROCESSING: return "PROCESSING";
        case CleanerLevel::DONE: return "DONE";
        case CleanerLevel::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

CleanerLevel notification_cleaner_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return CleanerLevel::UNKNOWN;
    if (s == "INIT") return CleanerLevel::INIT;
    if (s == "PROCESSING") return CleanerLevel::PROCESSING;
    if (s == "DONE") return CleanerLevel::DONE;
    if (s == "ERROR") return CleanerLevel::ERROR;
    return CleanerLevel::UNKNOWN;
}
...
bool notification_cleaner_init(const NotificationCleanerConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = CleanerLevel::INIT;
...
NotificationCleanerResponse notification_cleaner_handle(const NotificationCleanerRequest& req) {
    NotificationCleanerResponse resp;
    ...
    g_status.state = CleanerLevel::PROCESSING;
    // Validate request
...
void notification_cleaner_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = NotificationCleanerConfig{};
    g_status = NotificationCleanerStatus{};
    g_status.state = CleanerLevel::UNKNOWN;
}
...
std::string notification_cleaner_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "notification_cleaner";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
NotificationCleanerConfig notification_cleaner_configFromJson(const json& j) {
    NotificationCleanerConfig config;
