...
namespace {
    EventHashUtilsConfig g_config;
    std::mutex g_mutex;
    EventHashUtilsStatus g_status;
    EventHashUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* event_hash_utils_stateToString(HashUtilsState s) {
    switch (s) {
        case HashUtilsState::UNKNOWN: return "UNKNOWN";
        case HashUtilsState::INIT: return "INIT";
        case HashUtilsState::PROCESSING: return "PROCESSING";
        case HashUtilsState::DONE: return "DONE";
        case HashUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

HashUtilsState event_hash_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return HashUtilsState::UNKNOWN;
    if (s == "INIT") return HashUtilsState::INIT;
    if (s == "PROCESSING") return HashUtilsState::PROCESSING;
    if (s == "DONE") return HashUtilsState::DONE;
    if (s == "ERROR") return HashUtilsState::ERROR;
    return HashUtilsState::UNKNOWN;
}
...
bool event_hash_utils_init(const EventHashUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = HashUtilsState::INIT;
...
EventHashUtilsResponse event_hash_utils_handle(const EventHashUtilsRequest& req) {
    ...
    g_status.state = HashUtilsState::PROCESSING;
    // Validate request
...
void event_hash_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = EventHashUtilsConfig{};
    g_status = EventHashUtilsStatus{};
    g_status.state = HashUtilsState::UNKNOWN;
}
...
std::string event_hash_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "event_hash_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
