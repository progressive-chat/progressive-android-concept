...
namespace {
    RoomServerAclUtilsConfig g_config;
    std::mutex g_mutex;
    RoomServerAclUtilsStatus g_status;
    RoomServerAclUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* room_server_acl_utils_stateToString(ServerAclUtilsAction s) {
    switch (s) {
        case ServerAclUtilsAction::UNKNOWN: return "UNKNOWN";
        case ServerAclUtilsAction::INIT: return "INIT";
        case ServerAclUtilsAction::PROCESSING: return "PROCESSING";
        case ServerAclUtilsAction::DONE: return "DONE";
        case ServerAclUtilsAction::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

ServerAclUtilsAction room_server_acl_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return ServerAclUtilsAction::UNKNOWN;
    if (s == "INIT") return ServerAclUtilsAction::INIT;
    if (s == "PROCESSING") return ServerAclUtilsAction::PROCESSING;
    if (s == "DONE") return ServerAclUtilsAction::DONE;
    if (s == "ERROR") return ServerAclUtilsAction::ERROR;
    return ServerAclUtilsAction::UNKNOWN;
}
...
bool room_server_acl_utils_init(const RoomServerAclUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = ServerAclUtilsAction::INIT;
...
RoomServerAclUtilsResponse room_server_acl_utils_handle(const RoomServerAclUtilsRequest& req) {
    RoomServerAclUtilsResponse resp;
    ...
    g_status.state = ServerAclUtilsAction::PROCESSING;
    // Validate request
...
void room_server_acl_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = RoomServerAclUtilsConfig{};
    g_status = RoomServerAclUtilsStatus{};
    g_status.state = ServerAclUtilsAction::UNKNOWN;
}
...
std::string room_server_acl_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "room_server_acl_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
RoomServerAclUtilsConfig room_server_acl_utils_configFromJson(const json& j) {
    RoomServerAclUtilsConfig config;
