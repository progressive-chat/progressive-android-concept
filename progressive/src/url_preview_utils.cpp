...
namespace {
    UrlPreviewUtilsConfig g_config;
    std::mutex g_mutex;
    UrlPreviewUtilsStatus g_status;
    UrlPreviewUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* url_preview_utils_stateToString(UrlPreviewUtilsState s) {
    switch (s) {
        case UrlPreviewUtilsState::UNKNOWN: return "UNKNOWN";
        case UrlPreviewUtilsState::INIT: return "INIT";
        case UrlPreviewUtilsState::PROCESSING: return "PROCESSING";
        case UrlPreviewUtilsState::DONE: return "DONE";
        case UrlPreviewUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

UrlPreviewUtilsState url_preview_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return UrlPreviewUtilsState::UNKNOWN;
    if (s == "INIT") return UrlPreviewUtilsState::INIT;
    if (s == "PROCESSING") return UrlPreviewUtilsState::PROCESSING;
    if (s == "DONE") return UrlPreviewUtilsState::DONE;
    if (s == "ERROR") return UrlPreviewUtilsState::ERROR;
    return UrlPreviewUtilsState::UNKNOWN;
}
...
bool url_preview_utils_init(const UrlPreviewUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = UrlPreviewUtilsState::INIT;
...
UrlPreviewUtilsResponse url_preview_utils_handle(const UrlPreviewUtilsRequest& req) {
    ...
    g_status.state = UrlPreviewUtilsState::PROCESSING;
    // Validate request
...
void url_preview_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = UrlPreviewUtilsConfig{};
    g_status = UrlPreviewUtilsStatus{};
    g_status.state = UrlPreviewUtilsState::UNKNOWN;
}
...
std::string url_preview_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "url_preview_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
