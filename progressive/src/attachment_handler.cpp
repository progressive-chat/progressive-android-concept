...
namespace {
    AttachmentHandlerConfig g_config;
    std::mutex g_mutex;
    AttachmentHandlerStatus g_status;
    AttachmentHandlerStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* attachment_handler_stateToString(AttachmentHandlerState s) {
    switch (s) {
        case AttachmentHandlerState::UNKNOWN: return "UNKNOWN";
        case AttachmentHandlerState::INIT: return "INIT";
        case AttachmentHandlerState::PROCESSING: return "PROCESSING";
        case AttachmentHandlerState::DONE: return "DONE";
        case AttachmentHandlerState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

AttachmentHandlerState attachment_handler_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return AttachmentHandlerState::UNKNOWN;
    if (s == "INIT") return AttachmentHandlerState::INIT;
    if (s == "PROCESSING") return AttachmentHandlerState::PROCESSING;
    if (s == "DONE") return AttachmentHandlerState::DONE;
    if (s == "ERROR") return AttachmentHandlerState::ERROR;
    return AttachmentHandlerState::UNKNOWN;
}
...
bool attachment_handler_init(const AttachmentHandlerConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = AttachmentHandlerState::INIT;
...
AttachmentHandlerResponse attachment_handler_handle(const AttachmentHandlerRequest& req) {
    AttachmentHandlerResponse resp;
    ...
    g_status.state = AttachmentHandlerState::PROCESSING;
    // Validate request
...
void attachment_handler_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = AttachmentHandlerConfig{};
    g_status = AttachmentHandlerStatus{};
    g_status.state = AttachmentHandlerState::UNKNOWN;
}
...
std::string attachment_handler_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "attachment_handler";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
AttachmentHandlerConfig attachment_handler_configFromJson(const json& j) {
    AttachmentHandlerConfig config;
