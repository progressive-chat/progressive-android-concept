...
namespace {
    RoomKickerConfig g_config;
    std::mutex g_mutex;
    RoomKickerStatus g_status;
    RoomKickerStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* room_kicker_stateToString(KickerAction s) {
    switch (s) {
        case KickerAction::UNKNOWN: return "UNKNOWN";
        case KickerAction::INIT: return "INIT";
        case KickerAction::PROCESSING: return "PROCESSING";
        case KickerAction::DONE: return "DONE";
        case KickerAction::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

KickerAction room_kicker_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return KickerAction::UNKNOWN;
    if (s == "INIT") return KickerAction::INIT;
    if (s == "PROCESSING") return KickerAction::PROCESSING;
    if (s == "DONE") return KickerAction::DONE;
    if (s == "ERROR") return KickerAction::ERROR;
    return KickerAction::UNKNOWN;
}
...
bool room_kicker_init(const RoomKickerConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = KickerAction::INIT;
...
RoomKickerResponse room_kicker_handle(const RoomKickerRequest& req) {
    ...
    g_status.state = KickerAction::PROCESSING;
    // Validate request
...
void room_kicker_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = RoomKickerConfig{};
    g_status = RoomKickerStatus{};
    g_status.state = KickerAction::UNKNOWN;
}
