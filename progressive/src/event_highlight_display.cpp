...
namespace {
    EventHighlightDisplayConfig g_config;
    std::mutex g_mutex;
    EventHighlightDisplayStatus g_status;
    EventHighlightDisplayStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* event_highlight_display_stateToString(HighlightDisplayState s) {
    switch (s) {
        case HighlightDisplayState::UNKNOWN: return "UNKNOWN";
        case HighlightDisplayState::INIT: return "INIT";
        case HighlightDisplayState::PROCESSING: return "PROCESSING";
        case HighlightDisplayState::DONE: return "DONE";
        case HighlightDisplayState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

HighlightDisplayState event_highlight_display_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return HighlightDisplayState::UNKNOWN;
    if (s == "INIT") return HighlightDisplayState::INIT;
    if (s == "PROCESSING") return HighlightDisplayState::PROCESSING;
    if (s == "DONE") return HighlightDisplayState::DONE;
    if (s == "ERROR") return HighlightDisplayState::ERROR;
    return HighlightDisplayState::UNKNOWN;
}
...
bool event_highlight_display_init(const EventHighlightDisplayConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = HighlightDisplayState::INIT;
...
EventHighlightDisplayResponse event_highlight_display_handle(const EventHighlightDisplayRequest& req) {
    EventHighlightDisplayResponse resp;
    ...
    g_status.state = HighlightDisplayState::PROCESSING;
    // Validate request
...
void event_highlight_display_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = EventHighlightDisplayConfig{};
    g_status = EventHighlightDisplayStatus{};
    g_status.state = HighlightDisplayState::UNKNOWN;
}
...
std::string event_highlight_display_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "event_highlight_display";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
EventHighlightDisplayConfig event_highlight_display_configFromJson(const json& j) {
    EventHighlightDisplayConfig config;
