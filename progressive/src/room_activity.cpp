...
namespace {
    RoomActivityConfig g_config;
    std::mutex g_mutex;
    RoomActivityStatus g_status;
    RoomActivityStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* room_activity_stateToString(ActivityAction s) {
    switch (s) {
        case ActivityAction::UNKNOWN: return "UNKNOWN";
        case ActivityAction::INIT: return "INIT";
        case ActivityAction::PROCESSING: return "PROCESSING";
        case ActivityAction::DONE: return "DONE";
        case ActivityAction::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

ActivityAction room_activity_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return ActivityAction::UNKNOWN;
    if (s == "INIT") return ActivityAction::INIT;
    if (s == "PROCESSING") return ActivityAction::PROCESSING;
    if (s == "DONE") return ActivityAction::DONE;
    if (s == "ERROR") return ActivityAction::ERROR;
...
bool room_activity_init(const RoomActivityConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = ActivityAction::INIT;
...
RoomActivityResponse room_activity_handle(const RoomActivityRequest& req) {
    ...
    g_status.state = ActivityAction::PROCESSING;
    // Validate request
...
void room_activity_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = RoomActivityConfig{};
    g_status = RoomActivityStatus{};
    g_status.state = ActivityAction::UNKNOWN;
}
