...
namespace {
    ThreePidUtilsConfig g_config;
    std::mutex g_mutex;
    ThreePidUtilsStatus g_status;
    ThreePidUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* three_pid_utils_stateToString(ThreePidUtilsState s) {
    switch (s) {
        case ThreePidUtilsState::UNKNOWN: return "UNKNOWN";
        case ThreePidUtilsState::INIT: return "INIT";
        case ThreePidUtilsState::PROCESSING: return "PROCESSING";
        case ThreePidUtilsState::DONE: return "DONE";
        case ThreePidUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

ThreePidUtilsState three_pid_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return ThreePidUtilsState::UNKNOWN;
    if (s == "INIT") return ThreePidUtilsState::INIT;
    if (s == "PROCESSING") return ThreePidUtilsState::PROCESSING;
    if (s == "DONE") return ThreePidUtilsState::DONE;
    if (s == "ERROR") return ThreePidUtilsState::ERROR;
    return ThreePidUtilsState::UNKNOWN;
}
...
bool three_pid_utils_init(const ThreePidUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = ThreePidUtilsState::INIT;
...
ThreePidUtilsResponse three_pid_utils_handle(const ThreePidUtilsRequest& req) {
    ...
    g_status.state = ThreePidUtilsState::PROCESSING;
    // Validate request
...
void three_pid_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = ThreePidUtilsConfig{};
    g_status = ThreePidUtilsStatus{};
    g_status.state = ThreePidUtilsState::UNKNOWN;
}
