...
namespace {
    MegolmUtilsConfig g_config;
    std::mutex g_mutex;
    MegolmUtilsStatus g_status;
    MegolmUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* megolm_utils_stateToString(MegolmUtilsState s) {
    switch (s) {
        case MegolmUtilsState::UNKNOWN: return "UNKNOWN";
        case MegolmUtilsState::INIT: return "INIT";
        case MegolmUtilsState::PROCESSING: return "PROCESSING";
        case MegolmUtilsState::DONE: return "DONE";
        case MegolmUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

MegolmUtilsState megolm_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return MegolmUtilsState::UNKNOWN;
    if (s == "INIT") return MegolmUtilsState::INIT;
    if (s == "PROCESSING") return MegolmUtilsState::PROCESSING;
    if (s == "DONE") return MegolmUtilsState::DONE;
    if (s == "ERROR") return MegolmUtilsState::ERROR;
    return MegolmUtilsState::UNKNOWN;
}
...
bool megolm_utils_init(const MegolmUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = MegolmUtilsState::INIT;
...
MegolmUtilsResponse megolm_utils_handle(const MegolmUtilsRequest& req) {
    ...
    g_status.state = MegolmUtilsState::PROCESSING;
    // Validate request
...
void megolm_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = MegolmUtilsConfig{};
    g_status = MegolmUtilsStatus{};
    g_status.state = MegolmUtilsState::UNKNOWN;
}
