...
namespace {
    VoiceBroadcastUtilsConfig g_config;
    std::mutex g_mutex;
    VoiceBroadcastUtilsStatus g_status;
    VoiceBroadcastUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* voice_broadcast_utils_stateToString(VoiceBroadcastUtilsState s) {
    switch (s) {
        case VoiceBroadcastUtilsState::UNKNOWN: return "UNKNOWN";
        case VoiceBroadcastUtilsState::INIT: return "INIT";
        case VoiceBroadcastUtilsState::PROCESSING: return "PROCESSING";
        case VoiceBroadcastUtilsState::DONE: return "DONE";
        case VoiceBroadcastUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

VoiceBroadcastUtilsState voice_broadcast_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return VoiceBroadcastUtilsState::UNKNOWN;
    if (s == "INIT") return VoiceBroadcastUtilsState::INIT;
    if (s == "PROCESSING") return VoiceBroadcastUtilsState::PROCESSING;
    if (s == "DONE") return VoiceBroadcastUtilsState::DONE;
    if (s == "ERROR") return VoiceBroadcastUtilsState::ERROR;
    return VoiceBroadcastUtilsState::UNKNOWN;
}
...
bool voice_broadcast_utils_init(const VoiceBroadcastUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = VoiceBroadcastUtilsState::INIT;
...
VoiceBroadcastUtilsResponse voice_broadcast_utils_handle(const VoiceBroadcastUtilsRequest& req) {
    VoiceBroadcastUtilsResponse resp;
    ...
    g_status.state = VoiceBroadcastUtilsState::PROCESSING;
    // Validate request
...
void voice_broadcast_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = VoiceBroadcastUtilsConfig{};
    g_status = VoiceBroadcastUtilsStatus{};
    g_status.state = VoiceBroadcastUtilsState::UNKNOWN;
}
...
std::string voice_broadcast_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "voice_broadcast_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
VoiceBroadcastUtilsConfig voice_broadcast_utils_configFromJson(const json& j) {
    VoiceBroadcastUtilsConfig config;
