...
namespace {
    LoginTokenUtilConfig g_config;
    std::mutex g_mutex;
    LoginTokenUtilStatus g_status;
    LoginTokenUtilStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* login_token_util_stateToString(LoginTokenUtilState s) {
    switch (s) {
        case LoginTokenUtilState::UNKNOWN: return "UNKNOWN";
        case LoginTokenUtilState::INIT: return "INIT";
        case LoginTokenUtilState::PROCESSING: return "PROCESSING";
        case LoginTokenUtilState::DONE: return "DONE";
        case LoginTokenUtilState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

LoginTokenUtilState login_token_util_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return LoginTokenUtilState::UNKNOWN;
    if (s == "INIT") return LoginTokenUtilState::INIT;
    if (s == "PROCESSING") return LoginTokenUtilState::PROCESSING;
    if (s == "DONE") return LoginTokenUtilState::DONE;
    if (s == "ERROR") return LoginTokenUtilState::ERROR;
    return LoginTokenUtilState::UNKNOWN;
}
...
bool login_token_util_init(const LoginTokenUtilConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = LoginTokenUtilState::INIT;
...
LoginTokenUtilResponse login_token_util_handle(const LoginTokenUtilRequest& req) {
    ...
    g_status.state = LoginTokenUtilState::PROCESSING;
    // Validate request
...
void login_token_util_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = LoginTokenUtilConfig{};
    g_status = LoginTokenUtilStatus{};
    g_status.state = LoginTokenUtilState::UNKNOWN;
}
...
std::string login_token_util_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "login_token_util";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
