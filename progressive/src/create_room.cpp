...
namespace {
    CreateRoomConfig g_config;
    std::mutex g_mutex;
    CreateRoomStatus g_status;
    CreateRoomStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* create_room_stateToString(CreateRoomState s) {
    switch (s) {
        case CreateRoomState::UNKNOWN: return "UNKNOWN";
        case CreateRoomState::INIT: return "INIT";
        case CreateRoomState::PROCESSING: return "PROCESSING";
        case CreateRoomState::DONE: return "DONE";
        case CreateRoomState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

CreateRoomState create_room_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return CreateRoomState::UNKNOWN;
    if (s == "INIT") return CreateRoomState::INIT;
    if (s == "PROCESSING") return CreateRoomState::PROCESSING;
    if (s == "DONE") return CreateRoomState::DONE;
    if (s == "ERROR") return CreateRoomState::ERROR;
    return CreateRoomState::UNKNOWN;
}
...
bool create_room_init(const CreateRoomConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = CreateRoomState::INIT;
...
CreateRoomResponse create_room_handle(const CreateRoomRequest& req) {
    ...
    g_status.state = CreateRoomState::PROCESSING;
    // Validate request
...
void create_room_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = CreateRoomConfig{};
    g_status = CreateRoomStatus{};
    g_status.state = CreateRoomState::UNKNOWN;
}
