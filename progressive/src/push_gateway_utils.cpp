...
namespace {
    PushGatewayUtilsConfig g_config;
    std::mutex g_mutex;
    PushGatewayUtilsStatus g_status;
    PushGatewayUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* push_gateway_utils_stateToString(GatewayUtilsKind s) {
    switch (s) {
        case GatewayUtilsKind::UNKNOWN: return "UNKNOWN";
        case GatewayUtilsKind::INIT: return "INIT";
        case GatewayUtilsKind::PROCESSING: return "PROCESSING";
        case GatewayUtilsKind::DONE: return "DONE";
        case GatewayUtilsKind::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

GatewayUtilsKind push_gateway_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return GatewayUtilsKind::UNKNOWN;
    if (s == "INIT") return GatewayUtilsKind::INIT;
    if (s == "PROCESSING") return GatewayUtilsKind::PROCESSING;
    if (s == "DONE") return GatewayUtilsKind::DONE;
    if (s == "ERROR") return GatewayUtilsKind::ERROR;
    return GatewayUtilsKind::UNKNOWN;
}
...
bool push_gateway_utils_init(const PushGatewayUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = GatewayUtilsKind::INIT;
...
PushGatewayUtilsResponse push_gateway_utils_handle(const PushGatewayUtilsRequest& req) {
    ...
    g_status.state = GatewayUtilsKind::PROCESSING;
    // Validate request
...
void push_gateway_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = PushGatewayUtilsConfig{};
    g_status = PushGatewayUtilsStatus{};
    g_status.state = GatewayUtilsKind::UNKNOWN;
}
...
std::string push_gateway_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "push_gateway_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
