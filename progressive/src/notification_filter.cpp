...
namespace {
    NotificationFilterConfig g_config;
    std::mutex g_mutex;
    NotificationFilterStatus g_status;
    NotificationFilterStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* notification_filter_stateToString(FilterLevel s) {
    switch (s) {
        case FilterLevel::UNKNOWN: return "UNKNOWN";
        case FilterLevel::INIT: return "INIT";
        case FilterLevel::PROCESSING: return "PROCESSING";
        case FilterLevel::DONE: return "DONE";
        case FilterLevel::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

FilterLevel notification_filter_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return FilterLevel::UNKNOWN;
    if (s == "INIT") return FilterLevel::INIT;
    if (s == "PROCESSING") return FilterLevel::PROCESSING;
    if (s == "DONE") return FilterLevel::DONE;
    if (s == "ERROR") return FilterLevel::ERROR;
...
bool notification_filter_init(const NotificationFilterConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = FilterLevel::INIT;
...
NotificationFilterResponse notification_filter_handle(const NotificationFilterRequest& req) {
    NotificationFilterResponse resp;
    ...
    g_status.state = FilterLevel::PROCESSING;
    // Validate request
...
void notification_filter_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = NotificationFilterConfig{};
    g_status = NotificationFilterStatus{};
    g_status.state = FilterLevel::UNKNOWN;
}
...
std::string notification_filter_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "notification_filter";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
NotificationFilterConfig notification_filter_configFromJson(const json& j) {
    NotificationFilterConfig config;
