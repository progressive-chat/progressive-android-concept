...
namespace {
    MessageEditConfig g_config;
    std::mutex g_mutex;
    MessageEditStatus g_status;
    MessageEditStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* message_edit_stateToString(EditPhase s) {
    switch (s) {
        case EditPhase::UNKNOWN: return "UNKNOWN";
        case EditPhase::INIT: return "INIT";
        case EditPhase::PROCESSING: return "PROCESSING";
        case EditPhase::DONE: return "DONE";
        case EditPhase::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

EditPhase message_edit_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return EditPhase::UNKNOWN;
    if (s == "INIT") return EditPhase::INIT;
    if (s == "PROCESSING") return EditPhase::PROCESSING;
    if (s == "DONE") return EditPhase::DONE;
    if (s == "ERROR") return EditPhase::ERROR;
    return EditPhase::UNKNOWN;
}
...
MessageEditResponse message_edit_handle(const MessageEditRequest& req) {
    ...
    g_status.state = EditPhase::PROCESSING;
    // Validate request
...
void message_edit_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = MessageEditConfig{};
    g_status = MessageEditStatus{};
    g_status.state = EditPhase::UNKNOWN;
}
