...
namespace {
    SyncStateUtilsConfig g_config;
    std::mutex g_mutex;
    SyncStateUtilsStatus g_status;
    SyncStateUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* sync_state_utils_stateToString(StateUtilsPhase s) {
    switch (s) {
        case StateUtilsPhase::UNKNOWN: return "UNKNOWN";
        case StateUtilsPhase::INIT: return "INIT";
        case StateUtilsPhase::PROCESSING: return "PROCESSING";
        case StateUtilsPhase::DONE: return "DONE";
        case StateUtilsPhase::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

StateUtilsPhase sync_state_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return StateUtilsPhase::UNKNOWN;
    if (s == "INIT") return StateUtilsPhase::INIT;
    if (s == "PROCESSING") return StateUtilsPhase::PROCESSING;
    if (s == "DONE") return StateUtilsPhase::DONE;
    if (s == "ERROR") return StateUtilsPhase::ERROR;
    return StateUtilsPhase::UNKNOWN;
}
...
bool sync_state_utils_init(const SyncStateUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = StateUtilsPhase::INIT;
...
SyncStateUtilsResponse sync_state_utils_handle(const SyncStateUtilsRequest& req) {
    ...
    g_status.state = StateUtilsPhase::PROCESSING;
    // Validate request
...
void sync_state_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = SyncStateUtilsConfig{};
    g_status = SyncStateUtilsStatus{};
    g_status.state = StateUtilsPhase::UNKNOWN;
}
...
std::string sync_state_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "sync_state_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
