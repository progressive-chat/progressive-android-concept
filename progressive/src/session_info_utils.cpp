...
namespace {
    SessionInfoUtilsConfig g_config;
    std::mutex g_mutex;
    SessionInfoUtilsStatus g_status;
    SessionInfoUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* session_info_utils_stateToString(InfoUtilsPhase s) {
    switch (s) {
        case InfoUtilsPhase::UNKNOWN: return "UNKNOWN";
        case InfoUtilsPhase::INIT: return "INIT";
        case InfoUtilsPhase::PROCESSING: return "PROCESSING";
        case InfoUtilsPhase::DONE: return "DONE";
        case InfoUtilsPhase::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

InfoUtilsPhase session_info_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return InfoUtilsPhase::UNKNOWN;
    if (s == "INIT") return InfoUtilsPhase::INIT;
    if (s == "PROCESSING") return InfoUtilsPhase::PROCESSING;
    if (s == "DONE") return InfoUtilsPhase::DONE;
    if (s == "ERROR") return InfoUtilsPhase::ERROR;
    return InfoUtilsPhase::UNKNOWN;
}
...
bool session_info_utils_init(const SessionInfoUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = InfoUtilsPhase::INIT;
...
SessionInfoUtilsResponse session_info_utils_handle(const SessionInfoUtilsRequest& req) {
    ...
    g_status.state = InfoUtilsPhase::PROCESSING;
    // Validate request
...
void session_info_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = SessionInfoUtilsConfig{};
    g_status = SessionInfoUtilsStatus{};
    g_status.state = InfoUtilsPhase::UNKNOWN;
}
...
std::string session_info_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "session_info_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
