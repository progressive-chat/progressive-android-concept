...
namespace {
    ReadMarkerUtilsConfig g_config;
    std::mutex g_mutex;
    ReadMarkerUtilsStatus g_status;
    ReadMarkerUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* read_marker_utils_stateToString(ReadMarkerUtilsState s) {
    switch (s) {
        case ReadMarkerUtilsState::UNKNOWN: return "UNKNOWN";
        case ReadMarkerUtilsState::INIT: return "INIT";
        case ReadMarkerUtilsState::PROCESSING: return "PROCESSING";
        case ReadMarkerUtilsState::DONE: return "DONE";
        case ReadMarkerUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

ReadMarkerUtilsState read_marker_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return ReadMarkerUtilsState::UNKNOWN;
    if (s == "INIT") return ReadMarkerUtilsState::INIT;
    if (s == "PROCESSING") return ReadMarkerUtilsState::PROCESSING;
    if (s == "DONE") return ReadMarkerUtilsState::DONE;
    if (s == "ERROR") return ReadMarkerUtilsState::ERROR;
    return ReadMarkerUtilsState::UNKNOWN;
}
...
bool read_marker_utils_init(const ReadMarkerUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = ReadMarkerUtilsState::INIT;
...
ReadMarkerUtilsResponse read_marker_utils_handle(const ReadMarkerUtilsRequest& req) {
    ...
    g_status.state = ReadMarkerUtilsState::PROCESSING;
    // Validate request
...
void read_marker_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = ReadMarkerUtilsConfig{};
    g_status = ReadMarkerUtilsStatus{};
    g_status.state = ReadMarkerUtilsState::UNKNOWN;
}
...
std::string read_marker_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "read_marker_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
