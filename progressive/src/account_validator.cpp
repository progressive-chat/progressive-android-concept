...
namespace {
    AccountValidatorConfig g_config;
    std::mutex g_mutex;
    AccountValidatorStatus g_status;
    AccountValidatorStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* account_validator_stateToString(AccountValidatorState s) {
    switch (s) {
        case AccountValidatorState::UNKNOWN: return "UNKNOWN";
        case AccountValidatorState::INIT: return "INIT";
        case AccountValidatorState::PROCESSING: return "PROCESSING";
        case AccountValidatorState::DONE: return "DONE";
        case AccountValidatorState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

AccountValidatorState account_validator_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return AccountValidatorState::UNKNOWN;
    if (s == "INIT") return AccountValidatorState::INIT;
    if (s == "PROCESSING") return AccountValidatorState::PROCESSING;
    if (s == "DONE") return AccountValidatorState::DONE;
    if (s == "ERROR") return AccountValidatorState::ERROR;
    return AccountValidatorState::UNKNOWN;
}

bool account_validator_init(const AccountValidatorConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = AccountValidatorState::INIT;
...
AccountValidatorResponse account_validator_handle(const AccountValidatorRequest& req) {
    AccountValidatorResponse resp;
    ...
    g_status.state = AccountValidatorState::PROCESSING;
    // Validate request
...
AccountValidatorStatus account_validator_getStatus() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_status;
}

void account_validator_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = AccountValidatorConfig{};
    g_status = AccountValidatorStatus{};
    g_status.state = AccountValidatorState::UNKNOWN;
}
...
std::string account_validator_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "account_validator";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
AccountValidatorStats account_validator_getStats() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_stats;
}

AccountValidatorConfig account_validator_configFromJson(const json& j) {
    AccountValidatorConfig config;
...
json account_validator_configToJson(const AccountValidatorConfig& config) {
    json j;
