...
namespace {
    RoomTypeDetectorConfig g_config;
    std::mutex g_mutex;
    RoomTypeDetectorStatus g_status;
    RoomTypeDetectorStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* room_type_detector_stateToString(TypeDetectorAction s) {
    switch (s) {
        case TypeDetectorAction::UNKNOWN: return "UNKNOWN";
        case TypeDetectorAction::INIT: return "INIT";
        case TypeDetectorAction::PROCESSING: return "PROCESSING";
        case TypeDetectorAction::DONE: return "DONE";
        case TypeDetectorAction::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

TypeDetectorAction room_type_detector_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return TypeDetectorAction::UNKNOWN;
    if (s == "INIT") return TypeDetectorAction::INIT;
    if (s == "PROCESSING") return TypeDetectorAction::PROCESSING;
    if (s == "DONE") return TypeDetectorAction::DONE;
    if (s == "ERROR") return TypeDetectorAction::ERROR;
    return TypeDetectorAction::UNKNOWN;
}

bool room_type_detector_init(const RoomTypeDetectorConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = TypeDetectorAction::INIT;
...
RoomTypeDetectorResponse room_type_detector_handle(const RoomTypeDetectorRequest& req) {
    RoomTypeDetectorResponse resp;
    ...
    g_status.state = TypeDetectorAction::PROCESSING;
    // Validate request
...
RoomTypeDetectorStatus room_type_detector_getStatus() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_status;
}

void room_type_detector_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = RoomTypeDetectorConfig{};
    g_status = RoomTypeDetectorStatus{};
    g_status.state = TypeDetectorAction::UNKNOWN;
}
...
std::string room_type_detector_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "room_type_detector";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
RoomTypeDetectorStats room_type_detector_getStats() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_stats;
}

RoomTypeDetectorConfig room_type_detector_configFromJson(const json& j) {
    RoomTypeDetectorConfig config;
...
json room_type_detector_configToJson(const RoomTypeDetectorConfig& config) {
    json j;
