...
namespace {
    CrossSigningStatusConfig g_config;
    std::mutex g_mutex;
    CrossSigningStatusStatus g_status;
    CrossSigningStatusStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* cross_signing_status_stateToString(ingStatusPhase s) {
    switch (s) {
        case ingStatusPhase::UNKNOWN: return "UNKNOWN";
        case ingStatusPhase::INIT: return "INIT";
        case ingStatusPhase::PROCESSING: return "PROCESSING";
        case ingStatusPhase::DONE: return "DONE";
        case ingStatusPhase::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

ingStatusPhase cross_signing_status_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return ingStatusPhase::UNKNOWN;
    if (s == "INIT") return ingStatusPhase::INIT;
    if (s == "PROCESSING") return ingStatusPhase::PROCESSING;
    if (s == "DONE") return ingStatusPhase::DONE;
    if (s == "ERROR") return ingStatusPhase::ERROR;
    return ingStatusPhase::UNKNOWN;
}
...
bool cross_signing_status_init(const CrossSigningStatusConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = ingStatusPhase::INIT;
...
CrossSigningStatusResponse cross_signing_status_handle(const CrossSigningStatusRequest& req) {
    CrossSigningStatusResponse resp;
    ...
    g_status.state = ingStatusPhase::PROCESSING;
    // Validate request
...
void cross_signing_status_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = CrossSigningStatusConfig{};
    g_status = CrossSigningStatusStatus{};
    g_status.state = ingStatusPhase::UNKNOWN;
}
...
std::string cross_signing_status_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "cross_signing_status";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
CrossSigningStatusConfig cross_signing_status_configFromJson(const json& j) {
    CrossSigningStatusConfig config;
