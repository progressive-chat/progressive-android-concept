...
namespace {
    OpenidTokenUtilsConfig g_config;
    std::mutex g_mutex;
    OpenidTokenUtilsStatus g_status;
    OpenidTokenUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* openid_token_utils_stateToString(OpenidTokenUtilsState s) {
    switch (s) {
        case OpenidTokenUtilsState::UNKNOWN: return "UNKNOWN";
        case OpenidTokenUtilsState::INIT: return "INIT";
        case OpenidTokenUtilsState::PROCESSING: return "PROCESSING";
        case OpenidTokenUtilsState::DONE: return "DONE";
        case OpenidTokenUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

OpenidTokenUtilsState openid_token_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return OpenidTokenUtilsState::UNKNOWN;
    if (s == "INIT") return OpenidTokenUtilsState::INIT;
    if (s == "PROCESSING") return OpenidTokenUtilsState::PROCESSING;
    if (s == "DONE") return OpenidTokenUtilsState::DONE;
    if (s == "ERROR") return OpenidTokenUtilsState::ERROR;
    return OpenidTokenUtilsState::UNKNOWN;
}
...
bool openid_token_utils_init(const OpenidTokenUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = OpenidTokenUtilsState::INIT;
...
OpenidTokenUtilsResponse openid_token_utils_handle(const OpenidTokenUtilsRequest& req) {
    ...
    g_status.state = OpenidTokenUtilsState::PROCESSING;
    // Validate request
...
void openid_token_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = OpenidTokenUtilsConfig{};
    g_status = OpenidTokenUtilsStatus{};
    g_status.state = OpenidTokenUtilsState::UNKNOWN;
}
...
std::string openid_token_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "openid_token_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
