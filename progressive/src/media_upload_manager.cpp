...
namespace {
    MediaUploadManagerConfig g_config;
    std::mutex g_mutex;
    MediaUploadManagerStatus g_status;
    MediaUploadManagerStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* media_upload_manager_stateToString(UploadManagerType s) {
    switch (s) {
        case UploadManagerType::UNKNOWN: return "UNKNOWN";
        case UploadManagerType::INIT: return "INIT";
        case UploadManagerType::PROCESSING: return "PROCESSING";
        case UploadManagerType::DONE: return "DONE";
        case UploadManagerType::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

UploadManagerType media_upload_manager_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return UploadManagerType::UNKNOWN;
    if (s == "INIT") return UploadManagerType::INIT;
    if (s == "PROCESSING") return UploadManagerType::PROCESSING;
    if (s == "DONE") return UploadManagerType::DONE;
    if (s == "ERROR") return UploadManagerType::ERROR;
    return UploadManagerType::UNKNOWN;
}
...
bool media_upload_manager_init(const MediaUploadManagerConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = UploadManagerType::INIT;
...
MediaUploadManagerResponse media_upload_manager_handle(const MediaUploadManagerRequest& req) {
    MediaUploadManagerResponse resp;
    ...
    g_status.state = UploadManagerType::PROCESSING;
    // Validate request
...
void media_upload_manager_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = MediaUploadManagerConfig{};
    g_status = MediaUploadManagerStatus{};
    g_status.state = UploadManagerType::UNKNOWN;
}
...
std::string media_upload_manager_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "media_upload_manager";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
MediaUploadManagerConfig media_upload_manager_configFromJson(const json& j) {
    MediaUploadManagerConfig config;
