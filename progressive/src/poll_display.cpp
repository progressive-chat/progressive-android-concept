...
namespace {
    PollDisplayConfig g_config;
    std::mutex g_mutex;
    PollDisplayStatus g_status;
    PollDisplayStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* poll_display_stateToString(PollDisplayState s) {
    switch (s) {
        case PollDisplayState::UNKNOWN: return "UNKNOWN";
        case PollDisplayState::INIT: return "INIT";
        case PollDisplayState::PROCESSING: return "PROCESSING";
        case PollDisplayState::DONE: return "DONE";
        case PollDisplayState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

PollDisplayState poll_display_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return PollDisplayState::UNKNOWN;
    if (s == "INIT") return PollDisplayState::INIT;
    if (s == "PROCESSING") return PollDisplayState::PROCESSING;
    if (s == "DONE") return PollDisplayState::DONE;
    if (s == "ERROR") return PollDisplayState::ERROR;
    return PollDisplayState::UNKNOWN;
}
...
bool poll_display_init(const PollDisplayConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = PollDisplayState::INIT;
...
PollDisplayResponse poll_display_handle(const PollDisplayRequest& req) {
    ...
    g_status.state = PollDisplayState::PROCESSING;
    // Validate request
...
void poll_display_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = PollDisplayConfig{};
    g_status = PollDisplayStatus{};
    g_status.state = PollDisplayState::UNKNOWN;
}
