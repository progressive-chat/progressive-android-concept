...
namespace {
    EventDisplayFormatterConfig g_config;
    std::mutex g_mutex;
    EventDisplayFormatterStatus g_status;
    EventDisplayFormatterStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* event_display_formatter_stateToString(DisplayFormatterState s) {
    switch (s) {
        case DisplayFormatterState::UNKNOWN: return "UNKNOWN";
        case DisplayFormatterState::INIT: return "INIT";
        case DisplayFormatterState::PROCESSING: return "PROCESSING";
        case DisplayFormatterState::DONE: return "DONE";
        case DisplayFormatterState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

DisplayFormatterState event_display_formatter_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return DisplayFormatterState::UNKNOWN;
    if (s == "INIT") return DisplayFormatterState::INIT;
    if (s == "PROCESSING") return DisplayFormatterState::PROCESSING;
    if (s == "DONE") return DisplayFormatterState::DONE;
    if (s == "ERROR") return DisplayFormatterState::ERROR;
    return DisplayFormatterState::UNKNOWN;
}
...
bool event_display_formatter_init(const EventDisplayFormatterConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = DisplayFormatterState::INIT;
...
EventDisplayFormatterResponse event_display_formatter_handle(const EventDisplayFormatterRequest& req) {
    EventDisplayFormatterResponse resp;
    ...
    g_status.state = DisplayFormatterState::PROCESSING;
    // Validate request
...
void event_display_formatter_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = EventDisplayFormatterConfig{};
    g_status = EventDisplayFormatterStatus{};
    g_status.state = DisplayFormatterState::UNKNOWN;
}
...
std::string event_display_formatter_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "event_display_formatter";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
EventDisplayFormatterConfig event_display_formatter_configFromJson(const json& j) {
    EventDisplayFormatterConfig config;
