...
namespace {
    RoomAliasUtilsConfig g_config;
    std::mutex g_mutex;
    RoomAliasUtilsStatus g_status;
    RoomAliasUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* room_alias_utils_stateToString(AliasUtilsAction s) {
    switch (s) {
        case AliasUtilsAction::UNKNOWN: return "UNKNOWN";
        case AliasUtilsAction::INIT: return "INIT";
        case AliasUtilsAction::PROCESSING: return "PROCESSING";
        case AliasUtilsAction::DONE: return "DONE";
        case AliasUtilsAction::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

AliasUtilsAction room_alias_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return AliasUtilsAction::UNKNOWN;
    if (s == "INIT") return AliasUtilsAction::INIT;
    if (s == "PROCESSING") return AliasUtilsAction::PROCESSING;
    if (s == "DONE") return AliasUtilsAction::DONE;
    if (s == "ERROR") return AliasUtilsAction::ERROR;
    return AliasUtilsAction::UNKNOWN;
}
...
bool room_alias_utils_init(const RoomAliasUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = AliasUtilsAction::INIT;
...
RoomAliasUtilsResponse room_alias_utils_handle(const RoomAliasUtilsRequest& req) {
    ...
    g_status.state = AliasUtilsAction::PROCESSING;
    // Validate request
...
void room_alias_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = RoomAliasUtilsConfig{};
    g_status = RoomAliasUtilsStatus{};
    g_status.state = AliasUtilsAction::UNKNOWN;
}
...
std::string room_alias_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "room_alias_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
