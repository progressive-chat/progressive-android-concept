...
namespace {
    PushConditionUtilsConfig g_config;
    std::mutex g_mutex;
    PushConditionUtilsStatus g_status;
    PushConditionUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* push_condition_utils_stateToString(ConditionUtilsKind s) {
    switch (s) {
        case ConditionUtilsKind::UNKNOWN: return "UNKNOWN";
        case ConditionUtilsKind::INIT: return "INIT";
        case ConditionUtilsKind::PROCESSING: return "PROCESSING";
        case ConditionUtilsKind::DONE: return "DONE";
        case ConditionUtilsKind::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

ConditionUtilsKind push_condition_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return ConditionUtilsKind::UNKNOWN;
    if (s == "INIT") return ConditionUtilsKind::INIT;
    if (s == "PROCESSING") return ConditionUtilsKind::PROCESSING;
    if (s == "DONE") return ConditionUtilsKind::DONE;
    if (s == "ERROR") return ConditionUtilsKind::ERROR;
    return ConditionUtilsKind::UNKNOWN;
}

bool push_condition_utils_init(const PushConditionUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = ConditionUtilsKind::INIT;
...
PushConditionUtilsResponse push_condition_utils_handle(const PushConditionUtilsRequest& req) {
    PushConditionUtilsResponse resp;
    ...
    g_status.state = ConditionUtilsKind::PROCESSING;
    // Validate request
...
PushConditionUtilsStatus push_condition_utils_getStatus() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_status;
}

void push_condition_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = PushConditionUtilsConfig{};
    g_status = PushConditionUtilsStatus{};
    g_status.state = ConditionUtilsKind::UNKNOWN;
}
...
std::string push_condition_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "push_condition_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
PushConditionUtilsStats push_condition_utils_getStats() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_stats;
}

PushConditionUtilsConfig push_condition_utils_configFromJson(const json& j) {
    PushConditionUtilsConfig config;
...
json push_condition_utils_configToJson(const PushConditionUtilsConfig& config) {
    json j;
