...
namespace {
    MessageLinkUtilsConfig g_config;
    std::mutex g_mutex;
    MessageLinkUtilsStatus g_status;
    MessageLinkUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* message_link_utils_stateToString(LinkUtilsPhase s) {
    switch (s) {
        case LinkUtilsPhase::UNKNOWN: return "UNKNOWN";
        case LinkUtilsPhase::INIT: return "INIT";
        case LinkUtilsPhase::PROCESSING: return "PROCESSING";
        case LinkUtilsPhase::DONE: return "DONE";
        case LinkUtilsPhase::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

LinkUtilsPhase message_link_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return LinkUtilsPhase::UNKNOWN;
    if (s == "INIT") return LinkUtilsPhase::INIT;
    if (s == "PROCESSING") return LinkUtilsPhase::PROCESSING;
    if (s == "DONE") return LinkUtilsPhase::DONE;
    if (s == "ERROR") return LinkUtilsPhase::ERROR;
    return LinkUtilsPhase::UNKNOWN;
}
...
bool message_link_utils_init(const MessageLinkUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = LinkUtilsPhase::INIT;
...
MessageLinkUtilsResponse message_link_utils_handle(const MessageLinkUtilsRequest& req) {
    ...
    g_status.state = LinkUtilsPhase::PROCESSING;
    // Validate request
...
void message_link_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = MessageLinkUtilsConfig{};
    g_status = MessageLinkUtilsStatus{};
    g_status.state = LinkUtilsPhase::UNKNOWN;
}
...
std::string message_link_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "message_link_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
