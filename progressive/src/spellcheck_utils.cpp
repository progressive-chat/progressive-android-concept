...
namespace {
    SpellcheckUtilsConfig g_config;
    std::mutex g_mutex;
    SpellcheckUtilsStatus g_status;
    SpellcheckUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* spellcheck_utils_stateToString(SpellcheckUtilsState s) {
    switch (s) {
        case SpellcheckUtilsState::UNKNOWN: return "UNKNOWN";
        case SpellcheckUtilsState::INIT: return "INIT";
        case SpellcheckUtilsState::PROCESSING: return "PROCESSING";
        case SpellcheckUtilsState::DONE: return "DONE";
        case SpellcheckUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

SpellcheckUtilsState spellcheck_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return SpellcheckUtilsState::UNKNOWN;
    if (s == "INIT") return SpellcheckUtilsState::INIT;
    if (s == "PROCESSING") return SpellcheckUtilsState::PROCESSING;
    if (s == "DONE") return SpellcheckUtilsState::DONE;
    if (s == "ERROR") return SpellcheckUtilsState::ERROR;
    return SpellcheckUtilsState::UNKNOWN;
}
...
bool spellcheck_utils_init(const SpellcheckUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = SpellcheckUtilsState::INIT;
...
SpellcheckUtilsResponse spellcheck_utils_handle(const SpellcheckUtilsRequest& req) {
    ...
    g_status.state = SpellcheckUtilsState::PROCESSING;
    // Validate request
...
void spellcheck_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = SpellcheckUtilsConfig{};
    g_status = SpellcheckUtilsStatus{};
    g_status.state = SpellcheckUtilsState::UNKNOWN;
}
...
std::string spellcheck_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "spellcheck_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
