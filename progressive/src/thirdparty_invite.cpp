...
namespace {
    ThirdpartyInviteConfig g_config;
    std::mutex g_mutex;
    ThirdpartyInviteStatus g_status;
    ThirdpartyInviteStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* thirdparty_invite_stateToString(ThirdpartyInviteState s) {
    switch (s) {
        case ThirdpartyInviteState::UNKNOWN: return "UNKNOWN";
        case ThirdpartyInviteState::INIT: return "INIT";
        case ThirdpartyInviteState::PROCESSING: return "PROCESSING";
        case ThirdpartyInviteState::DONE: return "DONE";
        case ThirdpartyInviteState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

ThirdpartyInviteState thirdparty_invite_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return ThirdpartyInviteState::UNKNOWN;
    if (s == "INIT") return ThirdpartyInviteState::INIT;
    if (s == "PROCESSING") return ThirdpartyInviteState::PROCESSING;
    if (s == "DONE") return ThirdpartyInviteState::DONE;
    if (s == "ERROR") return ThirdpartyInviteState::ERROR;
    return ThirdpartyInviteState::UNKNOWN;
}
...
bool thirdparty_invite_init(const ThirdpartyInviteConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = ThirdpartyInviteState::INIT;
...
ThirdpartyInviteResponse thirdparty_invite_handle(const ThirdpartyInviteRequest& req) {
    ...
    g_status.state = ThirdpartyInviteState::PROCESSING;
    // Validate request
...
void thirdparty_invite_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = ThirdpartyInviteConfig{};
    g_status = ThirdpartyInviteStatus{};
    g_status.state = ThirdpartyInviteState::UNKNOWN;
}
...
std::string thirdparty_invite_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "thirdparty_invite";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
