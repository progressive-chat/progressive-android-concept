...
namespace {
    RoomJoinRulesUtilsConfig g_config;
    std::mutex g_mutex;
    RoomJoinRulesUtilsStatus g_status;
    RoomJoinRulesUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* room_join_rules_utils_stateToString(JoinRulesUtilsAction s) {
    switch (s) {
        case JoinRulesUtilsAction::UNKNOWN: return "UNKNOWN";
        case JoinRulesUtilsAction::INIT: return "INIT";
        case JoinRulesUtilsAction::PROCESSING: return "PROCESSING";
        case JoinRulesUtilsAction::DONE: return "DONE";
        case JoinRulesUtilsAction::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

JoinRulesUtilsAction room_join_rules_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return JoinRulesUtilsAction::UNKNOWN;
    if (s == "INIT") return JoinRulesUtilsAction::INIT;
    if (s == "PROCESSING") return JoinRulesUtilsAction::PROCESSING;
    if (s == "DONE") return JoinRulesUtilsAction::DONE;
    if (s == "ERROR") return JoinRulesUtilsAction::ERROR;
    return JoinRulesUtilsAction::UNKNOWN;
}
...
bool room_join_rules_utils_init(const RoomJoinRulesUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = JoinRulesUtilsAction::INIT;
...
RoomJoinRulesUtilsResponse room_join_rules_utils_handle(const RoomJoinRulesUtilsRequest& req) {
    RoomJoinRulesUtilsResponse resp;
    ...
    g_status.state = JoinRulesUtilsAction::PROCESSING;
    // Validate request
...
void room_join_rules_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = RoomJoinRulesUtilsConfig{};
    g_status = RoomJoinRulesUtilsStatus{};
    g_status.state = JoinRulesUtilsAction::UNKNOWN;
}
...
std::string room_join_rules_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "room_join_rules_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
RoomJoinRulesUtilsConfig room_join_rules_utils_configFromJson(const json& j) {
    RoomJoinRulesUtilsConfig config;
