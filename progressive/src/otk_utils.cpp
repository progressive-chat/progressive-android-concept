...
namespace {
    OtkUtilsConfig g_config;
    std::mutex g_mutex;
    OtkUtilsStatus g_status;
    OtkUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* otk_utils_stateToString(OtkUtilsState s) {
    switch (s) {
        case OtkUtilsState::UNKNOWN: return "UNKNOWN";
        case OtkUtilsState::INIT: return "INIT";
        case OtkUtilsState::PROCESSING: return "PROCESSING";
        case OtkUtilsState::DONE: return "DONE";
        case OtkUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

OtkUtilsState otk_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return OtkUtilsState::UNKNOWN;
    if (s == "INIT") return OtkUtilsState::INIT;
    if (s == "PROCESSING") return OtkUtilsState::PROCESSING;
    if (s == "DONE") return OtkUtilsState::DONE;
    if (s == "ERROR") return OtkUtilsState::ERROR;
    return OtkUtilsState::UNKNOWN;
}

bool otk_utils_init(const OtkUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = OtkUtilsState::INIT;
...
OtkUtilsResponse otk_utils_handle(const OtkUtilsRequest& req) {
    OtkUtilsResponse resp;
    ...
    g_status.state = OtkUtilsState::PROCESSING;
    // Validate request
...
OtkUtilsStatus otk_utils_getStatus() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_status;
}

void otk_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = OtkUtilsConfig{};
    g_status = OtkUtilsStatus{};
    g_status.state = OtkUtilsState::UNKNOWN;
}
...
OtkUtilsStats otk_utils_getStats() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_stats;
}

OtkUtilsConfig otk_utils_configFromJson(const json& j) {
    OtkUtilsConfig config;
...
json otk_utils_configToJson(const OtkUtilsConfig& config) {
    json j;
