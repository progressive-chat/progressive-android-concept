...
namespace {
    EventOriginUtilsConfig g_config;
    std::mutex g_mutex;
    EventOriginUtilsStatus g_status;
    EventOriginUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* event_origin_utils_stateToString(OriginUtilsState s) {
    switch (s) {
        case OriginUtilsState::UNKNOWN: return "UNKNOWN";
        case OriginUtilsState::INIT: return "INIT";
        case OriginUtilsState::PROCESSING: return "PROCESSING";
        case OriginUtilsState::DONE: return "DONE";
        case OriginUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

OriginUtilsState event_origin_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return OriginUtilsState::UNKNOWN;
    if (s == "INIT") return OriginUtilsState::INIT;
    if (s == "PROCESSING") return OriginUtilsState::PROCESSING;
    if (s == "DONE") return OriginUtilsState::DONE;
    if (s == "ERROR") return OriginUtilsState::ERROR;
    return OriginUtilsState::UNKNOWN;
}
...
bool event_origin_utils_init(const EventOriginUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = OriginUtilsState::INIT;
...
EventOriginUtilsResponse event_origin_utils_handle(const EventOriginUtilsRequest& req) {
    ...
    g_status.state = OriginUtilsState::PROCESSING;
    // Validate request
...
void event_origin_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = EventOriginUtilsConfig{};
    g_status = EventOriginUtilsStatus{};
    g_status.state = OriginUtilsState::UNKNOWN;
}
...
std::string event_origin_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "event_origin_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
