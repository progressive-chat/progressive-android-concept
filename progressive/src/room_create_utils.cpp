...
namespace {
    RoomCreateUtilsConfig g_config;
    std::mutex g_mutex;
    RoomCreateUtilsStatus g_status;
    RoomCreateUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* room_create_utils_stateToString(CreateUtilsAction s) {
    switch (s) {
        case CreateUtilsAction::UNKNOWN: return "UNKNOWN";
        case CreateUtilsAction::INIT: return "INIT";
        case CreateUtilsAction::PROCESSING: return "PROCESSING";
        case CreateUtilsAction::DONE: return "DONE";
        case CreateUtilsAction::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

CreateUtilsAction room_create_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return CreateUtilsAction::UNKNOWN;
    if (s == "INIT") return CreateUtilsAction::INIT;
    if (s == "PROCESSING") return CreateUtilsAction::PROCESSING;
    if (s == "DONE") return CreateUtilsAction::DONE;
    if (s == "ERROR") return CreateUtilsAction::ERROR;
    return CreateUtilsAction::UNKNOWN;
}
...
bool room_create_utils_init(const RoomCreateUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = CreateUtilsAction::INIT;
...
RoomCreateUtilsResponse room_create_utils_handle(const RoomCreateUtilsRequest& req) {
    ...
    g_status.state = CreateUtilsAction::PROCESSING;
    // Validate request
...
void room_create_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = RoomCreateUtilsConfig{};
    g_status = RoomCreateUtilsStatus{};
    g_status.state = CreateUtilsAction::UNKNOWN;
}
...
std::string room_create_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "room_create_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
