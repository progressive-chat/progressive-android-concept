...
namespace {
    PushTranslatorConfig g_config;
    std::mutex g_mutex;
    PushTranslatorStatus g_status;
    PushTranslatorStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* push_translator_stateToString(TranslatorKind s) {
    switch (s) {
        case TranslatorKind::UNKNOWN: return "UNKNOWN";
        case TranslatorKind::INIT: return "INIT";
        case TranslatorKind::PROCESSING: return "PROCESSING";
        case TranslatorKind::DONE: return "DONE";
        case TranslatorKind::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

TranslatorKind push_translator_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return TranslatorKind::UNKNOWN;
    if (s == "INIT") return TranslatorKind::INIT;
    if (s == "PROCESSING") return TranslatorKind::PROCESSING;
    if (s == "DONE") return TranslatorKind::DONE;
    if (s == "ERROR") return TranslatorKind::ERROR;
    return TranslatorKind::UNKNOWN;
}
...
bool push_translator_init(const PushTranslatorConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = TranslatorKind::INIT;
...
PushTranslatorResponse push_translator_handle(const PushTranslatorRequest& req) {
    ...
    g_status.state = TranslatorKind::PROCESSING;
    // Validate request
...
void push_translator_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = PushTranslatorConfig{};
    g_status = PushTranslatorStatus{};
    g_status.state = TranslatorKind::UNKNOWN;
}
