...
namespace {
    RoomUpgradeUtilsConfig g_config;
    std::mutex g_mutex;
    RoomUpgradeUtilsStatus g_status;
    RoomUpgradeUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* room_upgrade_utils_stateToString(UpgradeUtilsAction s) {
    switch (s) {
        case UpgradeUtilsAction::UNKNOWN: return "UNKNOWN";
        case UpgradeUtilsAction::INIT: return "INIT";
        case UpgradeUtilsAction::PROCESSING: return "PROCESSING";
        case UpgradeUtilsAction::DONE: return "DONE";
        case UpgradeUtilsAction::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

UpgradeUtilsAction room_upgrade_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return UpgradeUtilsAction::UNKNOWN;
    if (s == "INIT") return UpgradeUtilsAction::INIT;
    if (s == "PROCESSING") return UpgradeUtilsAction::PROCESSING;
    if (s == "DONE") return UpgradeUtilsAction::DONE;
    if (s == "ERROR") return UpgradeUtilsAction::ERROR;
    return UpgradeUtilsAction::UNKNOWN;
}
...
bool room_upgrade_utils_init(const RoomUpgradeUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = UpgradeUtilsAction::INIT;
...
RoomUpgradeUtilsResponse room_upgrade_utils_handle(const RoomUpgradeUtilsRequest& req) {
    ...
    g_status.state = UpgradeUtilsAction::PROCESSING;
    // Validate request
...
void room_upgrade_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = RoomUpgradeUtilsConfig{};
    g_status = RoomUpgradeUtilsStatus{};
    g_status.state = UpgradeUtilsAction::UNKNOWN;
}
...
std::string room_upgrade_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "room_upgrade_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
