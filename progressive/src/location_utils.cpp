...
namespace {
    LocationUtilsConfig g_config;
    std::mutex g_mutex;
    LocationUtilsStatus g_status;
    LocationUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* location_utils_stateToString(LocationUtilsState s) {
    switch (s) {
        case LocationUtilsState::UNKNOWN: return "UNKNOWN";
        case LocationUtilsState::INIT: return "INIT";
        case LocationUtilsState::PROCESSING: return "PROCESSING";
        case LocationUtilsState::DONE: return "DONE";
        case LocationUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

LocationUtilsState location_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return LocationUtilsState::UNKNOWN;
    if (s == "INIT") return LocationUtilsState::INIT;
    if (s == "PROCESSING") return LocationUtilsState::PROCESSING;
    if (s == "DONE") return LocationUtilsState::DONE;
    if (s == "ERROR") return LocationUtilsState::ERROR;
    return LocationUtilsState::UNKNOWN;
}

bool location_utils_init(const LocationUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = LocationUtilsState::INIT;
...
LocationUtilsResponse location_utils_handle(const LocationUtilsRequest& req) {
    LocationUtilsResponse resp;
    ...
    g_status.state = LocationUtilsState::PROCESSING;
    // Validate request
...
LocationUtilsStatus location_utils_getStatus() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_status;
}

void location_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = LocationUtilsConfig{};
    g_status = LocationUtilsStatus{};
    g_status.state = LocationUtilsState::UNKNOWN;
}
...
LocationUtilsStats location_utils_getStats() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_stats;
}

LocationUtilsConfig location_utils_configFromJson(const json& j) {
...
json location_utils_configToJson(const LocationUtilsConfig& config) {
    json j;
