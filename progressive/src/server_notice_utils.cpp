...
namespace {
    ServerNoticeUtilsConfig g_config;
    std::mutex g_mutex;
    ServerNoticeUtilsStatus g_status;
    ServerNoticeUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* server_notice_utils_stateToString(NoticeUtilsStatus s) {
    switch (s) {
        case NoticeUtilsStatus::UNKNOWN: return "UNKNOWN";
        case NoticeUtilsStatus::INIT: return "INIT";
        case NoticeUtilsStatus::PROCESSING: return "PROCESSING";
        case NoticeUtilsStatus::DONE: return "DONE";
        case NoticeUtilsStatus::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

NoticeUtilsStatus server_notice_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return NoticeUtilsStatus::UNKNOWN;
    if (s == "INIT") return NoticeUtilsStatus::INIT;
    if (s == "PROCESSING") return NoticeUtilsStatus::PROCESSING;
    if (s == "DONE") return NoticeUtilsStatus::DONE;
    if (s == "ERROR") return NoticeUtilsStatus::ERROR;
    return NoticeUtilsStatus::UNKNOWN;
}
...
bool server_notice_utils_init(const ServerNoticeUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = NoticeUtilsStatus::INIT;
...
ServerNoticeUtilsResponse server_notice_utils_handle(const ServerNoticeUtilsRequest& req) {
    ServerNoticeUtilsResponse resp;
    ...
    g_status.state = NoticeUtilsStatus::PROCESSING;
    // Validate request
...
void server_notice_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = ServerNoticeUtilsConfig{};
    g_status = ServerNoticeUtilsStatus{};
    g_status.state = NoticeUtilsStatus::UNKNOWN;
}
...
std::string server_notice_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "server_notice_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
ServerNoticeUtilsConfig server_notice_utils_configFromJson(const json& j) {
    ServerNoticeUtilsConfig config;
