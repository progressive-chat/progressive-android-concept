...
namespace {
    BackupVersionUtilsConfig g_config;
    std::mutex g_mutex;
    BackupVersionUtilsStatus g_status;
    BackupVersionUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* backup_version_utils_stateToString(BackupVersionUtilsState s) {
    switch (s) {
        case BackupVersionUtilsState::UNKNOWN: return "UNKNOWN";
        case BackupVersionUtilsState::INIT: return "INIT";
        case BackupVersionUtilsState::PROCESSING: return "PROCESSING";
        case BackupVersionUtilsState::DONE: return "DONE";
        case BackupVersionUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

BackupVersionUtilsState backup_version_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return BackupVersionUtilsState::UNKNOWN;
    if (s == "INIT") return BackupVersionUtilsState::INIT;
    if (s == "PROCESSING") return BackupVersionUtilsState::PROCESSING;
    if (s == "DONE") return BackupVersionUtilsState::DONE;
    if (s == "ERROR") return BackupVersionUtilsState::ERROR;
    return BackupVersionUtilsState::UNKNOWN;
}

bool backup_version_utils_init(const BackupVersionUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = BackupVersionUtilsState::INIT;
...
BackupVersionUtilsResponse backup_version_utils_handle(const BackupVersionUtilsRequest& req) {
    BackupVersionUtilsResponse resp;
    ...
    g_status.state = BackupVersionUtilsState::PROCESSING;
    // Validate request
...
BackupVersionUtilsStatus backup_version_utils_getStatus() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_status;
}

void backup_version_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = BackupVersionUtilsConfig{};
    g_status = BackupVersionUtilsStatus{};
    g_status.state = BackupVersionUtilsState::UNKNOWN;
}
...
std::string backup_version_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "backup_version_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
BackupVersionUtilsStats backup_version_utils_getStats() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_stats;
}

BackupVersionUtilsConfig backup_version_utils_configFromJson(const json& j) {
    BackupVersionUtilsConfig config;
...
json backup_version_utils_configToJson(const BackupVersionUtilsConfig& config) {
    json j;
