...
namespace {
    NotificationSummaryConfig g_config;
    std::mutex g_mutex;
    NotificationSummaryStatus g_status;
    NotificationSummaryStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* notification_summary_stateToString(SummaryLevel s) {
    switch (s) {
        case SummaryLevel::UNKNOWN: return "UNKNOWN";
        case SummaryLevel::INIT: return "INIT";
        case SummaryLevel::PROCESSING: return "PROCESSING";
        case SummaryLevel::DONE: return "DONE";
        case SummaryLevel::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

SummaryLevel notification_summary_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return SummaryLevel::UNKNOWN;
    if (s == "INIT") return SummaryLevel::INIT;
    if (s == "PROCESSING") return SummaryLevel::PROCESSING;
    if (s == "DONE") return SummaryLevel::DONE;
    if (s == "ERROR") return SummaryLevel::ERROR;
    return SummaryLevel::UNKNOWN;
}
...
bool notification_summary_init(const NotificationSummaryConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = SummaryLevel::INIT;
...
NotificationSummaryResponse notification_summary_handle(const NotificationSummaryRequest& req) {
    NotificationSummaryResponse resp;
    ...
    g_status.state = SummaryLevel::PROCESSING;
    // Validate request
...
void notification_summary_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = NotificationSummaryConfig{};
    g_status = NotificationSummaryStatus{};
    g_status.state = SummaryLevel::UNKNOWN;
}
...
std::string notification_summary_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "notification_summary";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
NotificationSummaryConfig notification_summary_configFromJson(const json& j) {
    NotificationSummaryConfig config;
