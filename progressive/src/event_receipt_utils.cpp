...
namespace {
    EventReceiptUtilsConfig g_config;
    std::mutex g_mutex;
    EventReceiptUtilsStatus g_status;
    EventReceiptUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* event_receipt_utils_stateToString(ReceiptUtilsState s) {
    switch (s) {
        case ReceiptUtilsState::UNKNOWN: return "UNKNOWN";
        case ReceiptUtilsState::INIT: return "INIT";
        case ReceiptUtilsState::PROCESSING: return "PROCESSING";
        case ReceiptUtilsState::DONE: return "DONE";
        case ReceiptUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

ReceiptUtilsState event_receipt_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return ReceiptUtilsState::UNKNOWN;
    if (s == "INIT") return ReceiptUtilsState::INIT;
    if (s == "PROCESSING") return ReceiptUtilsState::PROCESSING;
    if (s == "DONE") return ReceiptUtilsState::DONE;
    if (s == "ERROR") return ReceiptUtilsState::ERROR;
    return ReceiptUtilsState::UNKNOWN;
}

bool event_receipt_utils_init(const EventReceiptUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = ReceiptUtilsState::INIT;
...
EventReceiptUtilsResponse event_receipt_utils_handle(const EventReceiptUtilsRequest& req) {
    EventReceiptUtilsResponse resp;
    ...
    g_status.state = ReceiptUtilsState::PROCESSING;
    // Validate request
...
EventReceiptUtilsStatus event_receipt_utils_getStatus() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_status;
}

void event_receipt_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = EventReceiptUtilsConfig{};
    g_status = EventReceiptUtilsStatus{};
    g_status.state = ReceiptUtilsState::UNKNOWN;
}
...
std::string event_receipt_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "event_receipt_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
EventReceiptUtilsStats event_receipt_utils_getStats() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_stats;
}

EventReceiptUtilsConfig event_receipt_utils_configFromJson(const json& j) {
    EventReceiptUtilsConfig config;
...
json event_receipt_utils_configToJson(const EventReceiptUtilsConfig& config) {
    json j;
