...
namespace {
    RoomRetentionUtilsConfig g_config;
    std::mutex g_mutex;
    RoomRetentionUtilsStatus g_status;
    RoomRetentionUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* room_retention_utils_stateToString(RetentionUtilsAction s) {
    switch (s) {
        case RetentionUtilsAction::UNKNOWN: return "UNKNOWN";
        case RetentionUtilsAction::INIT: return "INIT";
        case RetentionUtilsAction::PROCESSING: return "PROCESSING";
        case RetentionUtilsAction::DONE: return "DONE";
        case RetentionUtilsAction::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

RetentionUtilsAction room_retention_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return RetentionUtilsAction::UNKNOWN;
    if (s == "INIT") return RetentionUtilsAction::INIT;
    if (s == "PROCESSING") return RetentionUtilsAction::PROCESSING;
    if (s == "DONE") return RetentionUtilsAction::DONE;
    if (s == "ERROR") return RetentionUtilsAction::ERROR;
    return RetentionUtilsAction::UNKNOWN;
}
...
bool room_retention_utils_init(const RoomRetentionUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = RetentionUtilsAction::INIT;
...
RoomRetentionUtilsResponse room_retention_utils_handle(const RoomRetentionUtilsRequest& req) {
    RoomRetentionUtilsResponse resp;
    ...
    g_status.state = RetentionUtilsAction::PROCESSING;
    // Validate request
...
void room_retention_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = RoomRetentionUtilsConfig{};
    g_status = RoomRetentionUtilsStatus{};
    g_status.state = RetentionUtilsAction::UNKNOWN;
}
...
std::string room_retention_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "room_retention_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
RoomRetentionUtilsConfig room_retention_utils_configFromJson(const json& j) {
    RoomRetentionUtilsConfig config;
