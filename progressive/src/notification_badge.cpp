...
namespace {
    NotificationBadgeConfig g_config;
    std::mutex g_mutex;
    NotificationBadgeStatus g_status;
    NotificationBadgeStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* notification_badge_stateToString(BadgeLevel s) {
    switch (s) {
        case BadgeLevel::UNKNOWN: return "UNKNOWN";
        case BadgeLevel::INIT: return "INIT";
        case BadgeLevel::PROCESSING: return "PROCESSING";
        case BadgeLevel::DONE: return "DONE";
        case BadgeLevel::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

BadgeLevel notification_badge_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return BadgeLevel::UNKNOWN;
    if (s == "INIT") return BadgeLevel::INIT;
    if (s == "PROCESSING") return BadgeLevel::PROCESSING;
    if (s == "DONE") return BadgeLevel::DONE;
    if (s == "ERROR") return BadgeLevel::ERROR;
    return BadgeLevel::UNKNOWN;
}
...
bool notification_badge_init(const NotificationBadgeConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = BadgeLevel::INIT;
...
NotificationBadgeResponse notification_badge_handle(const NotificationBadgeRequest& req) {
    NotificationBadgeResponse resp;
    ...
    g_status.state = BadgeLevel::PROCESSING;
    // Validate request
...
void notification_badge_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = NotificationBadgeConfig{};
    g_status = NotificationBadgeStatus{};
    g_status.state = BadgeLevel::UNKNOWN;
}
...
std::string notification_badge_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "notification_badge";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
NotificationBadgeConfig notification_badge_configFromJson(const json& j) {
    NotificationBadgeConfig config;
