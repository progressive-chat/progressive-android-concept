...
namespace {
    AliasManagerConfig g_config;
    std::mutex g_mutex;
    AliasManagerStatus g_status;
    AliasManagerStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* alias_manager_stateToString(AliasManagerState s) {
    switch (s) {
        case AliasManagerState::UNKNOWN: return "UNKNOWN";
        case AliasManagerState::INIT: return "INIT";
        case AliasManagerState::PROCESSING: return "PROCESSING";
        case AliasManagerState::DONE: return "DONE";
        case AliasManagerState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

AliasManagerState alias_manager_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return AliasManagerState::UNKNOWN;
    if (s == "INIT") return AliasManagerState::INIT;
    if (s == "PROCESSING") return AliasManagerState::PROCESSING;
    if (s == "DONE") return AliasManagerState::DONE;
    if (s == "ERROR") return AliasManagerState::ERROR;
    return AliasManagerState::UNKNOWN;
}
...
bool alias_manager_init(const AliasManagerConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = AliasManagerState::INIT;
...
AliasManagerResponse alias_manager_handle(const AliasManagerRequest& req) {
    ...
    g_status.state = AliasManagerState::PROCESSING;
    // Validate request
...
void alias_manager_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = AliasManagerConfig{};
    g_status = AliasManagerStatus{};
    g_status.state = AliasManagerState::UNKNOWN;
}
