...
namespace {
    RoomUpgradeConfig g_config;
    std::mutex g_mutex;
    RoomUpgradeStatus g_status;
    RoomUpgradeStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* room_upgrade_stateToString(UpgradeAction s) {
    switch (s) {
        case UpgradeAction::UNKNOWN: return "UNKNOWN";
        case UpgradeAction::INIT: return "INIT";
        case UpgradeAction::PROCESSING: return "PROCESSING";
        case UpgradeAction::DONE: return "DONE";
        case UpgradeAction::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

UpgradeAction room_upgrade_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return UpgradeAction::UNKNOWN;
    if (s == "INIT") return UpgradeAction::INIT;
    if (s == "PROCESSING") return UpgradeAction::PROCESSING;
    if (s == "DONE") return UpgradeAction::DONE;
    if (s == "ERROR") return UpgradeAction::ERROR;
    return UpgradeAction::UNKNOWN;
}
...
bool room_upgrade_init(const RoomUpgradeConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = UpgradeAction::INIT;
...
RoomUpgradeResponse room_upgrade_handle(const RoomUpgradeRequest& req) {
    ...
    g_status.state = UpgradeAction::PROCESSING;
    // Validate request
...
void room_upgrade_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = RoomUpgradeConfig{};
    g_status = RoomUpgradeStatus{};
    g_status.state = UpgradeAction::UNKNOWN;
}
