...
namespace {
    BiometricAuthConfig g_config;
    std::mutex g_mutex;
    BiometricAuthStatus g_status;
    BiometricAuthStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* biometric_auth_stateToString(BiometricAuthState s) {
    switch (s) {
        case BiometricAuthState::UNKNOWN: return "UNKNOWN";
        case BiometricAuthState::INIT: return "INIT";
        case BiometricAuthState::PROCESSING: return "PROCESSING";
        case BiometricAuthState::DONE: return "DONE";
        case BiometricAuthState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

BiometricAuthState biometric_auth_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return BiometricAuthState::UNKNOWN;
    if (s == "INIT") return BiometricAuthState::INIT;
    if (s == "PROCESSING") return BiometricAuthState::PROCESSING;
    if (s == "DONE") return BiometricAuthState::DONE;
    if (s == "ERROR") return BiometricAuthState::ERROR;
    return BiometricAuthState::UNKNOWN;
}
...
bool biometric_auth_init(const BiometricAuthConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = BiometricAuthState::INIT;
...
BiometricAuthResponse biometric_auth_handle(const BiometricAuthRequest& req) {
    ...
    g_status.state = BiometricAuthState::PROCESSING;
    // Validate request
...
void biometric_auth_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = BiometricAuthConfig{};
    g_status = BiometricAuthStatus{};
    g_status.state = BiometricAuthState::UNKNOWN;
}
