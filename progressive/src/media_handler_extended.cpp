...
namespace {
    MediaHandlerExtendedConfig g_config;
    std::mutex g_mutex;
    MediaHandlerExtendedStatus g_status;
    MediaHandlerExtendedStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* media_handler_extended_stateToString(HandlerExtendedType s) {
    switch (s) {
        case HandlerExtendedType::UNKNOWN: return "UNKNOWN";
        case HandlerExtendedType::INIT: return "INIT";
        case HandlerExtendedType::PROCESSING: return "PROCESSING";
        case HandlerExtendedType::DONE: return "DONE";
        case HandlerExtendedType::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

HandlerExtendedType media_handler_extended_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return HandlerExtendedType::UNKNOWN;
    if (s == "INIT") return HandlerExtendedType::INIT;
    if (s == "PROCESSING") return HandlerExtendedType::PROCESSING;
    if (s == "DONE") return HandlerExtendedType::DONE;
    if (s == "ERROR") return HandlerExtendedType::ERROR;
    return HandlerExtendedType::UNKNOWN;
}
...
bool media_handler_extended_init(const MediaHandlerExtendedConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = HandlerExtendedType::INIT;
...
MediaHandlerExtendedResponse media_handler_extended_handle(const MediaHandlerExtendedRequest& req) {
    MediaHandlerExtendedResponse resp;
    ...
    g_status.state = HandlerExtendedType::PROCESSING;
    // Validate request
...
void media_handler_extended_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = MediaHandlerExtendedConfig{};
    g_status = MediaHandlerExtendedStatus{};
    g_status.state = HandlerExtendedType::UNKNOWN;
}
...
std::string media_handler_extended_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "media_handler_extended";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
MediaHandlerExtendedConfig media_handler_extended_configFromJson(const json& j) {
    MediaHandlerExtendedConfig config;
