...
namespace {
    ReadTrackerConfig g_config;
    std::mutex g_mutex;
    ReadTrackerStatus g_status;
    ReadTrackerStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* read_tracker_stateToString(ReadTrackerState s) {
    switch (s) {
        case ReadTrackerState::UNKNOWN: return "UNKNOWN";
        case ReadTrackerState::INIT: return "INIT";
        case ReadTrackerState::PROCESSING: return "PROCESSING";
        case ReadTrackerState::DONE: return "DONE";
        case ReadTrackerState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

ReadTrackerState read_tracker_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return ReadTrackerState::UNKNOWN;
    if (s == "INIT") return ReadTrackerState::INIT;
    if (s == "PROCESSING") return ReadTrackerState::PROCESSING;
    if (s == "DONE") return ReadTrackerState::DONE;
    if (s == "ERROR") return ReadTrackerState::ERROR;
    return ReadTrackerState::UNKNOWN;
}
...
bool read_tracker_init(const ReadTrackerConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = ReadTrackerState::INIT;
...
ReadTrackerResponse read_tracker_handle(const ReadTrackerRequest& req) {
    ...
    g_status.state = ReadTrackerState::PROCESSING;
    // Validate request
...
void read_tracker_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = ReadTrackerConfig{};
    g_status = ReadTrackerStatus{};
    g_status.state = ReadTrackerState::UNKNOWN;
}
