...
namespace {
    OpenidTokenConfig g_config;
    std::mutex g_mutex;
    OpenidTokenStatus g_status;
    OpenidTokenStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* openid_token_stateToString(OpenidTokenState s) {
    switch (s) {
        case OpenidTokenState::UNKNOWN: return "UNKNOWN";
        case OpenidTokenState::INIT: return "INIT";
        case OpenidTokenState::PROCESSING: return "PROCESSING";
        case OpenidTokenState::DONE: return "DONE";
        case OpenidTokenState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

OpenidTokenState openid_token_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return OpenidTokenState::UNKNOWN;
    if (s == "INIT") return OpenidTokenState::INIT;
    if (s == "PROCESSING") return OpenidTokenState::PROCESSING;
    if (s == "DONE") return OpenidTokenState::DONE;
    if (s == "ERROR") return OpenidTokenState::ERROR;
    return OpenidTokenState::UNKNOWN;
}
...
bool openid_token_init(const OpenidTokenConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = OpenidTokenState::INIT;
...
OpenidTokenResponse openid_token_handle(const OpenidTokenRequest& req) {
    ...
    g_status.state = OpenidTokenState::PROCESSING;
    // Validate request
...
void openid_token_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = OpenidTokenConfig{};
    g_status = OpenidTokenStatus{};
    g_status.state = OpenidTokenState::UNKNOWN;
}
