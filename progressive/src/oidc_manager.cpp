...
namespace {
    OidcManagerConfig g_config;
    std::mutex g_mutex;
    OidcManagerStatus g_status;
    OidcManagerStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* oidc_manager_stateToString(OidcManagerState s) {
    switch (s) {
        case OidcManagerState::UNKNOWN: return "UNKNOWN";
        case OidcManagerState::INIT: return "INIT";
        case OidcManagerState::PROCESSING: return "PROCESSING";
        case OidcManagerState::DONE: return "DONE";
        case OidcManagerState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

OidcManagerState oidc_manager_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return OidcManagerState::UNKNOWN;
    if (s == "INIT") return OidcManagerState::INIT;
    if (s == "PROCESSING") return OidcManagerState::PROCESSING;
    if (s == "DONE") return OidcManagerState::DONE;
    if (s == "ERROR") return OidcManagerState::ERROR;
    return OidcManagerState::UNKNOWN;
}
...
bool oidc_manager_init(const OidcManagerConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = OidcManagerState::INIT;
...
OidcManagerResponse oidc_manager_handle(const OidcManagerRequest& req) {
    ...
    g_status.state = OidcManagerState::PROCESSING;
    // Validate request
...
void oidc_manager_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = OidcManagerConfig{};
    g_status = OidcManagerStatus{};
    g_status.state = OidcManagerState::UNKNOWN;
}
