...
namespace {
    PermissionCheckerConfig g_config;
    std::mutex g_mutex;
    PermissionCheckerStatus g_status;
    PermissionCheckerStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* permission_checker_stateToString(PermissionCheckerState s) {
    switch (s) {
        case PermissionCheckerState::UNKNOWN: return "UNKNOWN";
        case PermissionCheckerState::INIT: return "INIT";
        case PermissionCheckerState::PROCESSING: return "PROCESSING";
        case PermissionCheckerState::DONE: return "DONE";
        case PermissionCheckerState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

PermissionCheckerState permission_checker_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return PermissionCheckerState::UNKNOWN;
    if (s == "INIT") return PermissionCheckerState::INIT;
    if (s == "PROCESSING") return PermissionCheckerState::PROCESSING;
    if (s == "DONE") return PermissionCheckerState::DONE;
    if (s == "ERROR") return PermissionCheckerState::ERROR;
    return PermissionCheckerState::UNKNOWN;
}
...
bool permission_checker_init(const PermissionCheckerConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = PermissionCheckerState::INIT;
...
PermissionCheckerResponse permission_checker_handle(const PermissionCheckerRequest& req) {
    PermissionCheckerResponse resp;
    ...
    g_status.state = PermissionCheckerState::PROCESSING;
    // Validate request
...
void permission_checker_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = PermissionCheckerConfig{};
    g_status = PermissionCheckerStatus{};
    g_status.state = PermissionCheckerState::UNKNOWN;
}
...
std::string permission_checker_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "permission_checker";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
PermissionCheckerConfig permission_checker_configFromJson(const json& j) {
    PermissionCheckerConfig config;
