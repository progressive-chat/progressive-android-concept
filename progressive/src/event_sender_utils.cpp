...
namespace {
    EventSenderUtilsConfig g_config;
    std::mutex g_mutex;
    EventSenderUtilsStatus g_status;
    EventSenderUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* event_sender_utils_stateToString(SenderUtilsState s) {
    switch (s) {
        case SenderUtilsState::UNKNOWN: return "UNKNOWN";
        case SenderUtilsState::INIT: return "INIT";
        case SenderUtilsState::PROCESSING: return "PROCESSING";
        case SenderUtilsState::DONE: return "DONE";
        case SenderUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

SenderUtilsState event_sender_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return SenderUtilsState::UNKNOWN;
    if (s == "INIT") return SenderUtilsState::INIT;
    if (s == "PROCESSING") return SenderUtilsState::PROCESSING;
    if (s == "DONE") return SenderUtilsState::DONE;
    if (s == "ERROR") return SenderUtilsState::ERROR;
    return SenderUtilsState::UNKNOWN;
}
...
bool event_sender_utils_init(const EventSenderUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = SenderUtilsState::INIT;
...
EventSenderUtilsResponse event_sender_utils_handle(const EventSenderUtilsRequest& req) {
    ...
    g_status.state = SenderUtilsState::PROCESSING;
    // Validate request
...
void event_sender_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = EventSenderUtilsConfig{};
    g_status = EventSenderUtilsStatus{};
    g_status.state = SenderUtilsState::UNKNOWN;
}
...
std::string event_sender_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "event_sender_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
