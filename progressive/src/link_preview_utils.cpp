...
namespace {
    LinkPreviewUtilsConfig g_config;
    std::mutex g_mutex;
    LinkPreviewUtilsStatus g_status;
    LinkPreviewUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* link_preview_utils_stateToString(LinkPreviewUtilsState s) {
    switch (s) {
        case LinkPreviewUtilsState::UNKNOWN: return "UNKNOWN";
        case LinkPreviewUtilsState::INIT: return "INIT";
        case LinkPreviewUtilsState::PROCESSING: return "PROCESSING";
        case LinkPreviewUtilsState::DONE: return "DONE";
        case LinkPreviewUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

LinkPreviewUtilsState link_preview_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return LinkPreviewUtilsState::UNKNOWN;
    if (s == "INIT") return LinkPreviewUtilsState::INIT;
    if (s == "PROCESSING") return LinkPreviewUtilsState::PROCESSING;
    if (s == "DONE") return LinkPreviewUtilsState::DONE;
    if (s == "ERROR") return LinkPreviewUtilsState::ERROR;
    return LinkPreviewUtilsState::UNKNOWN;
}
...
bool link_preview_utils_init(const LinkPreviewUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = LinkPreviewUtilsState::INIT;
...
LinkPreviewUtilsResponse link_preview_utils_handle(const LinkPreviewUtilsRequest& req) {
    ...
    g_status.state = LinkPreviewUtilsState::PROCESSING;
    // Validate request
...
void link_preview_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = LinkPreviewUtilsConfig{};
    g_status = LinkPreviewUtilsStatus{};
    g_status.state = LinkPreviewUtilsState::UNKNOWN;
}
...
std::string link_preview_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "link_preview_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
