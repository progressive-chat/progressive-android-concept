...
namespace {
    IdentityLookupConfig g_config;
    std::mutex g_mutex;
    IdentityLookupStatus g_status;
    IdentityLookupStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* identity_lookup_stateToString(LookupState s) {
    switch (s) {
        case LookupState::UNKNOWN: return "UNKNOWN";
        case LookupState::INIT: return "INIT";
        case LookupState::PROCESSING: return "PROCESSING";
        case LookupState::DONE: return "DONE";
        case LookupState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

LookupState identity_lookup_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return LookupState::UNKNOWN;
    if (s == "INIT") return LookupState::INIT;
    if (s == "PROCESSING") return LookupState::PROCESSING;
    if (s == "DONE") return LookupState::DONE;
    if (s == "ERROR") return LookupState::ERROR;
...
bool identity_lookup_init(const IdentityLookupConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = LookupState::INIT;
...
IdentityLookupResponse identity_lookup_handle(const IdentityLookupRequest& req) {
    ...
    g_status.state = LookupState::PROCESSING;
    // Validate request
...
void identity_lookup_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = IdentityLookupConfig{};
    g_status = IdentityLookupStatus{};
    g_status.state = LookupState::UNKNOWN;
}
