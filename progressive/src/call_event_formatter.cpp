...
namespace {
    CallEventFormatterConfig g_config;
    std::mutex g_mutex;
    CallEventFormatterStatus g_status;
    CallEventFormatterStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* call_event_formatter_stateToString(EventFormatterPhase s) {
    switch (s) {
        case EventFormatterPhase::UNKNOWN: return "UNKNOWN";
        case EventFormatterPhase::INIT: return "INIT";
        case EventFormatterPhase::PROCESSING: return "PROCESSING";
        case EventFormatterPhase::DONE: return "DONE";
        case EventFormatterPhase::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

EventFormatterPhase call_event_formatter_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return EventFormatterPhase::UNKNOWN;
    if (s == "INIT") return EventFormatterPhase::INIT;
    if (s == "PROCESSING") return EventFormatterPhase::PROCESSING;
    if (s == "DONE") return EventFormatterPhase::DONE;
    if (s == "ERROR") return EventFormatterPhase::ERROR;
    return EventFormatterPhase::UNKNOWN;
}
...
bool call_event_formatter_init(const CallEventFormatterConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = EventFormatterPhase::INIT;
...
CallEventFormatterResponse call_event_formatter_handle(const CallEventFormatterRequest& req) {
    CallEventFormatterResponse resp;
    ...
    g_status.state = EventFormatterPhase::PROCESSING;
    // Validate request
...
void call_event_formatter_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = CallEventFormatterConfig{};
    g_status = CallEventFormatterStatus{};
    g_status.state = EventFormatterPhase::UNKNOWN;
}
...
std::string call_event_formatter_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "call_event_formatter";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
CallEventFormatterConfig call_event_formatter_configFromJson(const json& j) {
    CallEventFormatterConfig config;
