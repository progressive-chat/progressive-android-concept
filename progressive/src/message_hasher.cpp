...
namespace {
    MessageHasherConfig g_config;
    std::mutex g_mutex;
    MessageHasherStatus g_status;
    MessageHasherStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* message_hasher_stateToString(HasherPhase s) {
    switch (s) {
        case HasherPhase::UNKNOWN: return "UNKNOWN";
        case HasherPhase::INIT: return "INIT";
        case HasherPhase::PROCESSING: return "PROCESSING";
        case HasherPhase::DONE: return "DONE";
        case HasherPhase::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

HasherPhase message_hasher_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return HasherPhase::UNKNOWN;
    if (s == "INIT") return HasherPhase::INIT;
    if (s == "PROCESSING") return HasherPhase::PROCESSING;
    if (s == "DONE") return HasherPhase::DONE;
    if (s == "ERROR") return HasherPhase::ERROR;
    return HasherPhase::UNKNOWN;
}
...
bool message_hasher_init(const MessageHasherConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = HasherPhase::INIT;
...
MessageHasherResponse message_hasher_handle(const MessageHasherRequest& req) {
    ...
    g_status.state = HasherPhase::PROCESSING;
    // Validate request
...
void message_hasher_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = MessageHasherConfig{};
    g_status = MessageHasherStatus{};
    g_status.state = HasherPhase::UNKNOWN;
}
