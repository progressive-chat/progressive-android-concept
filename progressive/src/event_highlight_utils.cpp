...
namespace {
    EventHighlightUtilsConfig g_config;
    std::mutex g_mutex;
    EventHighlightUtilsStatus g_status;
    EventHighlightUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* event_highlight_utils_stateToString(HighlightUtilsState s) {
    switch (s) {
        case HighlightUtilsState::UNKNOWN: return "UNKNOWN";
        case HighlightUtilsState::INIT: return "INIT";
        case HighlightUtilsState::PROCESSING: return "PROCESSING";
        case HighlightUtilsState::DONE: return "DONE";
        case HighlightUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

HighlightUtilsState event_highlight_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return HighlightUtilsState::UNKNOWN;
    if (s == "INIT") return HighlightUtilsState::INIT;
    if (s == "PROCESSING") return HighlightUtilsState::PROCESSING;
    if (s == "DONE") return HighlightUtilsState::DONE;
    if (s == "ERROR") return HighlightUtilsState::ERROR;
    return HighlightUtilsState::UNKNOWN;
}
...
bool event_highlight_utils_init(const EventHighlightUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = HighlightUtilsState::INIT;
...
EventHighlightUtilsResponse event_highlight_utils_handle(const EventHighlightUtilsRequest& req) {
    EventHighlightUtilsResponse resp;
    ...
    g_status.state = HighlightUtilsState::PROCESSING;
    // Validate request
...
void event_highlight_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = EventHighlightUtilsConfig{};
    g_status = EventHighlightUtilsStatus{};
    g_status.state = HighlightUtilsState::UNKNOWN;
}
...
std::string event_highlight_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "event_highlight_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
EventHighlightUtilsConfig event_highlight_utils_configFromJson(const json& j) {
    EventHighlightUtilsConfig config;
