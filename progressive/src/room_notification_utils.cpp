...
namespace {
    RoomNotificationUtilsConfig g_config;
    std::mutex g_mutex;
    RoomNotificationUtilsStatus g_status;
    RoomNotificationUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* room_notification_utils_stateToString(NotificationUtilsAction s) {
    switch (s) {
        case NotificationUtilsAction::UNKNOWN: return "UNKNOWN";
        case NotificationUtilsAction::INIT: return "INIT";
        case NotificationUtilsAction::PROCESSING: return "PROCESSING";
        case NotificationUtilsAction::DONE: return "DONE";
        case NotificationUtilsAction::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

NotificationUtilsAction room_notification_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return NotificationUtilsAction::UNKNOWN;
    if (s == "INIT") return NotificationUtilsAction::INIT;
    if (s == "PROCESSING") return NotificationUtilsAction::PROCESSING;
    if (s == "DONE") return NotificationUtilsAction::DONE;
    if (s == "ERROR") return NotificationUtilsAction::ERROR;
    return NotificationUtilsAction::UNKNOWN;
}
...
bool room_notification_utils_init(const RoomNotificationUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = NotificationUtilsAction::INIT;
...
RoomNotificationUtilsResponse room_notification_utils_handle(const RoomNotificationUtilsRequest& req) {
    RoomNotificationUtilsResponse resp;
    ...
    g_status.state = NotificationUtilsAction::PROCESSING;
    // Validate request
...
void room_notification_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = RoomNotificationUtilsConfig{};
    g_status = RoomNotificationUtilsStatus{};
    g_status.state = NotificationUtilsAction::UNKNOWN;
}
...
std::string room_notification_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "room_notification_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
RoomNotificationUtilsConfig room_notification_utils_configFromJson(const json& j) {
    RoomNotificationUtilsConfig config;
