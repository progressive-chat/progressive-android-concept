...
namespace {
    RoomMemberSorterConfig g_config;
    std::mutex g_mutex;
    RoomMemberSorterStatus g_status;
    RoomMemberSorterStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* room_member_sorter_stateToString(MemberSorterAction s) {
    switch (s) {
        case MemberSorterAction::UNKNOWN: return "UNKNOWN";
        case MemberSorterAction::INIT: return "INIT";
        case MemberSorterAction::PROCESSING: return "PROCESSING";
        case MemberSorterAction::DONE: return "DONE";
        case MemberSorterAction::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

MemberSorterAction room_member_sorter_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return MemberSorterAction::UNKNOWN;
    if (s == "INIT") return MemberSorterAction::INIT;
    if (s == "PROCESSING") return MemberSorterAction::PROCESSING;
    if (s == "DONE") return MemberSorterAction::DONE;
    if (s == "ERROR") return MemberSorterAction::ERROR;
    return MemberSorterAction::UNKNOWN;
}
...
bool room_member_sorter_init(const RoomMemberSorterConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = MemberSorterAction::INIT;
...
RoomMemberSorterResponse room_member_sorter_handle(const RoomMemberSorterRequest& req) {
    ...
    g_status.state = MemberSorterAction::PROCESSING;
    // Validate request
...
void room_member_sorter_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = RoomMemberSorterConfig{};
    g_status = RoomMemberSorterStatus{};
    g_status.state = MemberSorterAction::UNKNOWN;
}
...
std::string room_member_sorter_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "room_member_sorter";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
