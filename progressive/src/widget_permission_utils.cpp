...
namespace {
    WidgetPermissionUtilsConfig g_config;
    std::mutex g_mutex;
    WidgetPermissionUtilsStatus g_status;
    WidgetPermissionUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* widget_permission_utils_stateToString(WidgetPermissionUtilsState s) {
    switch (s) {
        case WidgetPermissionUtilsState::UNKNOWN: return "UNKNOWN";
        case WidgetPermissionUtilsState::INIT: return "INIT";
        case WidgetPermissionUtilsState::PROCESSING: return "PROCESSING";
        case WidgetPermissionUtilsState::DONE: return "DONE";
        case WidgetPermissionUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

WidgetPermissionUtilsState widget_permission_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return WidgetPermissionUtilsState::UNKNOWN;
    if (s == "INIT") return WidgetPermissionUtilsState::INIT;
    if (s == "PROCESSING") return WidgetPermissionUtilsState::PROCESSING;
    if (s == "DONE") return WidgetPermissionUtilsState::DONE;
    if (s == "ERROR") return WidgetPermissionUtilsState::ERROR;
    return WidgetPermissionUtilsState::UNKNOWN;
}

bool widget_permission_utils_init(const WidgetPermissionUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = WidgetPermissionUtilsState::INIT;
...
WidgetPermissionUtilsResponse widget_permission_utils_handle(const WidgetPermissionUtilsRequest& req) {
    WidgetPermissionUtilsResponse resp;
    ...
    g_status.state = WidgetPermissionUtilsState::PROCESSING;
    // Validate request
...
WidgetPermissionUtilsStatus widget_permission_utils_getStatus() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_status;
}

void widget_permission_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = WidgetPermissionUtilsConfig{};
    g_status = WidgetPermissionUtilsStatus{};
    g_status.state = WidgetPermissionUtilsState::UNKNOWN;
}
...
std::string widget_permission_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "widget_permission_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
WidgetPermissionUtilsStats widget_permission_utils_getStats() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_stats;
}

WidgetPermissionUtilsConfig widget_permission_utils_configFromJson(const json& j) {
    WidgetPermissionUtilsConfig config;
...
json widget_permission_utils_configToJson(const WidgetPermissionUtilsConfig& config) {
    json j;
