...
namespace {
    EventRetentionConfig g_config;
    std::mutex g_mutex;
    EventRetentionStatus g_status;
    EventRetentionStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* event_retention_stateToString(RetentionState s) {
    switch (s) {
        case RetentionState::UNKNOWN: return "UNKNOWN";
        case RetentionState::INIT: return "INIT";
        case RetentionState::PROCESSING: return "PROCESSING";
        case RetentionState::DONE: return "DONE";
        case RetentionState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

RetentionState event_retention_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return RetentionState::UNKNOWN;
    if (s == "INIT") return RetentionState::INIT;
    if (s == "PROCESSING") return RetentionState::PROCESSING;
    if (s == "DONE") return RetentionState::DONE;
    if (s == "ERROR") return RetentionState::ERROR;
...
bool event_retention_init(const EventRetentionConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = RetentionState::INIT;
...
EventRetentionResponse event_retention_handle(const EventRetentionRequest& req) {
    ...
    g_status.state = RetentionState::PROCESSING;
    // Validate request
...
void event_retention_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = EventRetentionConfig{};
    g_status = EventRetentionStatus{};
    g_status.state = RetentionState::UNKNOWN;
}
