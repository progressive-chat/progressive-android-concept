...
namespace {
    MarkdownParserConfig g_config;
    std::mutex g_mutex;
    MarkdownParserStatus g_status;
    MarkdownParserStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* markdown_parser_stateToString(MarkdownParserState s) {
    switch (s) {
        case MarkdownParserState::UNKNOWN: return "UNKNOWN";
        case MarkdownParserState::INIT: return "INIT";
        case MarkdownParserState::PROCESSING: return "PROCESSING";
        case MarkdownParserState::DONE: return "DONE";
        case MarkdownParserState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

MarkdownParserState markdown_parser_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return MarkdownParserState::UNKNOWN;
    if (s == "INIT") return MarkdownParserState::INIT;
    if (s == "PROCESSING") return MarkdownParserState::PROCESSING;
    if (s == "DONE") return MarkdownParserState::DONE;
    if (s == "ERROR") return MarkdownParserState::ERROR;
    return MarkdownParserState::UNKNOWN;
}
...
bool markdown_parser_init(const MarkdownParserConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = MarkdownParserState::INIT;
...
MarkdownParserResponse markdown_parser_handle(const MarkdownParserRequest& req) {
    ...
    g_status.state = MarkdownParserState::PROCESSING;
    // Validate request
...
void markdown_parser_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = MarkdownParserConfig{};
    g_status = MarkdownParserStatus{};
    g_status.state = MarkdownParserState::UNKNOWN;
}
