...
namespace {
    InputToolsConfig g_config;
    std::mutex g_mutex;
    InputToolsStatus g_status;
    InputToolsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* input_tools_stateToString(InputToolsState s) {
    switch (s) {
        case InputToolsState::UNKNOWN: return "UNKNOWN";
        case InputToolsState::INIT: return "INIT";
        case InputToolsState::PROCESSING: return "PROCESSING";
        case InputToolsState::DONE: return "DONE";
        case InputToolsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

InputToolsState input_tools_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return InputToolsState::UNKNOWN;
    if (s == "INIT") return InputToolsState::INIT;
    if (s == "PROCESSING") return InputToolsState::PROCESSING;
    if (s == "DONE") return InputToolsState::DONE;
    if (s == "ERROR") return InputToolsState::ERROR;
    return InputToolsState::UNKNOWN;
}
...
bool input_tools_init(const InputToolsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = InputToolsState::INIT;
...
InputToolsResponse input_tools_handle(const InputToolsRequest& req) {
    ...
    g_status.state = InputToolsState::PROCESSING;
    // Validate request
...
void input_tools_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = InputToolsConfig{};
    g_status = InputToolsStatus{};
    g_status.state = InputToolsState::UNKNOWN;
}
