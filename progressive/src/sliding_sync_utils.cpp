...
namespace {
    SlidingSyncUtilsConfig g_config;
    std::mutex g_mutex;
    SlidingSyncUtilsStatus g_status;
    SlidingSyncUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* sliding_sync_utils_stateToString(SlidingSyncUtilsState s) {
    switch (s) {
        case SlidingSyncUtilsState::UNKNOWN: return "UNKNOWN";
        case SlidingSyncUtilsState::INIT: return "INIT";
        case SlidingSyncUtilsState::PROCESSING: return "PROCESSING";
        case SlidingSyncUtilsState::DONE: return "DONE";
        case SlidingSyncUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

SlidingSyncUtilsState sliding_sync_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return SlidingSyncUtilsState::UNKNOWN;
    if (s == "INIT") return SlidingSyncUtilsState::INIT;
    if (s == "PROCESSING") return SlidingSyncUtilsState::PROCESSING;
    if (s == "DONE") return SlidingSyncUtilsState::DONE;
    if (s == "ERROR") return SlidingSyncUtilsState::ERROR;
    return SlidingSyncUtilsState::UNKNOWN;
}

bool sliding_sync_utils_init(const SlidingSyncUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = SlidingSyncUtilsState::INIT;
...
SlidingSyncUtilsResponse sliding_sync_utils_handle(const SlidingSyncUtilsRequest& req) {
    SlidingSyncUtilsResponse resp;
    ...
    g_status.state = SlidingSyncUtilsState::PROCESSING;
    // Validate request
...
SlidingSyncUtilsStatus sliding_sync_utils_getStatus() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_status;
}

void sliding_sync_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = SlidingSyncUtilsConfig{};
    g_status = SlidingSyncUtilsStatus{};
    g_status.state = SlidingSyncUtilsState::UNKNOWN;
}
...
std::string sliding_sync_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "sliding_sync_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
SlidingSyncUtilsStats sliding_sync_utils_getStats() {
    std::lock_guard<std::mutex> lock(g_mutex);
    return g_stats;
}

SlidingSyncUtilsConfig sliding_sync_utils_configFromJson(const json& j) {
    SlidingSyncUtilsConfig config;
...
json sliding_sync_utils_configToJson(const SlidingSyncUtilsConfig& config) {
    json j;
