...
namespace {
    RainbowUtilsConfig g_config;
    std::mutex g_mutex;
    RainbowUtilsStatus g_status;
    RainbowUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* rainbow_utils_stateToString(RainbowUtilsState s) {
    switch (s) {
        case RainbowUtilsState::UNKNOWN: return "UNKNOWN";
        case RainbowUtilsState::INIT: return "INIT";
        case RainbowUtilsState::PROCESSING: return "PROCESSING";
        case RainbowUtilsState::DONE: return "DONE";
        case RainbowUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

RainbowUtilsState rainbow_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return RainbowUtilsState::UNKNOWN;
    if (s == "INIT") return RainbowUtilsState::INIT;
    if (s == "PROCESSING") return RainbowUtilsState::PROCESSING;
    if (s == "DONE") return RainbowUtilsState::DONE;
    if (s == "ERROR") return RainbowUtilsState::ERROR;
    return RainbowUtilsState::UNKNOWN;
}
...
bool rainbow_utils_init(const RainbowUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = RainbowUtilsState::INIT;
...
RainbowUtilsResponse rainbow_utils_handle(const RainbowUtilsRequest& req) {
    ...
    g_status.state = RainbowUtilsState::PROCESSING;
    // Validate request
...
void rainbow_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = RainbowUtilsConfig{};
    g_status = RainbowUtilsStatus{};
    g_status.state = RainbowUtilsState::UNKNOWN;
}
