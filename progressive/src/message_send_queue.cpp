...
namespace {
    MessageSendQueueConfig g_config;
    std::mutex g_mutex;
    MessageSendQueueStatus g_status;
    MessageSendQueueStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* message_send_queue_stateToString(SendQueuePhase s) {
    switch (s) {
        case SendQueuePhase::UNKNOWN: return "UNKNOWN";
        case SendQueuePhase::INIT: return "INIT";
        case SendQueuePhase::PROCESSING: return "PROCESSING";
        case SendQueuePhase::DONE: return "DONE";
        case SendQueuePhase::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

SendQueuePhase message_send_queue_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return SendQueuePhase::UNKNOWN;
    if (s == "INIT") return SendQueuePhase::INIT;
    if (s == "PROCESSING") return SendQueuePhase::PROCESSING;
    if (s == "DONE") return SendQueuePhase::DONE;
    if (s == "ERROR") return SendQueuePhase::ERROR;
    return SendQueuePhase::UNKNOWN;
}
...
bool message_send_queue_init(const MessageSendQueueConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = SendQueuePhase::INIT;
...
MessageSendQueueResponse message_send_queue_handle(const MessageSendQueueRequest& req) {
    ...
    g_status.state = SendQueuePhase::PROCESSING;
    // Validate request
...
void message_send_queue_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = MessageSendQueueConfig{};
    g_status = MessageSendQueueStatus{};
    g_status.state = SendQueuePhase::UNKNOWN;
}
...
std::string message_send_queue_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "message_send_queue";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
