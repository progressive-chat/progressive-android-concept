...
namespace {
    ReadReceiptFormatterConfig g_config;
    std::mutex g_mutex;
    ReadReceiptFormatterStatus g_status;
    ReadReceiptFormatterStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* read_receipt_formatter_stateToString(ReadReceiptFormatterState s) {
    switch (s) {
        case ReadReceiptFormatterState::UNKNOWN: return "UNKNOWN";
        case ReadReceiptFormatterState::INIT: return "INIT";
        case ReadReceiptFormatterState::PROCESSING: return "PROCESSING";
        case ReadReceiptFormatterState::DONE: return "DONE";
        case ReadReceiptFormatterState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

ReadReceiptFormatterState read_receipt_formatter_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return ReadReceiptFormatterState::UNKNOWN;
    if (s == "INIT") return ReadReceiptFormatterState::INIT;
    if (s == "PROCESSING") return ReadReceiptFormatterState::PROCESSING;
    if (s == "DONE") return ReadReceiptFormatterState::DONE;
    if (s == "ERROR") return ReadReceiptFormatterState::ERROR;
    return ReadReceiptFormatterState::UNKNOWN;
}
...
bool read_receipt_formatter_init(const ReadReceiptFormatterConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = ReadReceiptFormatterState::INIT;
...
ReadReceiptFormatterResponse read_receipt_formatter_handle(const ReadReceiptFormatterRequest& req) {
    ReadReceiptFormatterResponse resp;
    ...
    g_status.state = ReadReceiptFormatterState::PROCESSING;
    // Validate request
...
void read_receipt_formatter_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = ReadReceiptFormatterConfig{};
    g_status = ReadReceiptFormatterStatus{};
    g_status.state = ReadReceiptFormatterState::UNKNOWN;
}
...
std::string read_receipt_formatter_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "read_receipt_formatter";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
ReadReceiptFormatterConfig read_receipt_formatter_configFromJson(const json& j) {
    ReadReceiptFormatterConfig config;
