...
namespace {
    WidgetUrlCheckConfig g_config;
    std::mutex g_mutex;
    WidgetUrlCheckStatus g_status;
    WidgetUrlCheckStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* widget_url_check_stateToString(WidgetUrlCheckState s) {
    switch (s) {
        case WidgetUrlCheckState::UNKNOWN: return "UNKNOWN";
        case WidgetUrlCheckState::INIT: return "INIT";
        case WidgetUrlCheckState::PROCESSING: return "PROCESSING";
        case WidgetUrlCheckState::DONE: return "DONE";
        case WidgetUrlCheckState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

WidgetUrlCheckState widget_url_check_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return WidgetUrlCheckState::UNKNOWN;
    if (s == "INIT") return WidgetUrlCheckState::INIT;
    if (s == "PROCESSING") return WidgetUrlCheckState::PROCESSING;
    if (s == "DONE") return WidgetUrlCheckState::DONE;
    if (s == "ERROR") return WidgetUrlCheckState::ERROR;
    return WidgetUrlCheckState::UNKNOWN;
}
...
bool widget_url_check_init(const WidgetUrlCheckConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = WidgetUrlCheckState::INIT;
...
WidgetUrlCheckResponse widget_url_check_handle(const WidgetUrlCheckRequest& req) {
    ...
    g_status.state = WidgetUrlCheckState::PROCESSING;
    // Validate request
...
void widget_url_check_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = WidgetUrlCheckConfig{};
    g_status = WidgetUrlCheckStatus{};
    g_status.state = WidgetUrlCheckState::UNKNOWN;
}
...
std::string widget_url_check_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "widget_url_check";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
