...
namespace {
    TopicParserConfig g_config;
    std::mutex g_mutex;
    TopicParserStatus g_status;
    TopicParserStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* topic_parser_stateToString(TopicParserState s) {
    switch (s) {
        case TopicParserState::UNKNOWN: return "UNKNOWN";
        case TopicParserState::INIT: return "INIT";
        case TopicParserState::PROCESSING: return "PROCESSING";
        case TopicParserState::DONE: return "DONE";
        case TopicParserState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

TopicParserState topic_parser_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return TopicParserState::UNKNOWN;
    if (s == "INIT") return TopicParserState::INIT;
    if (s == "PROCESSING") return TopicParserState::PROCESSING;
    if (s == "DONE") return TopicParserState::DONE;
    if (s == "ERROR") return TopicParserState::ERROR;
    return TopicParserState::UNKNOWN;
}
...
bool topic_parser_init(const TopicParserConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = TopicParserState::INIT;
...
TopicParserResponse topic_parser_handle(const TopicParserRequest& req) {
    ...
    g_status.state = TopicParserState::PROCESSING;
    // Validate request
...
void topic_parser_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = TopicParserConfig{};
    g_status = TopicParserStatus{};
    g_status.state = TopicParserState::UNKNOWN;
}
