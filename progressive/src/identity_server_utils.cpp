...
namespace {
    IdentityServerUtilsConfig g_config;
    std::mutex g_mutex;
    IdentityServerUtilsStatus g_status;
    IdentityServerUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* identity_server_utils_stateToString(ServerUtilsState s) {
    switch (s) {
        case ServerUtilsState::UNKNOWN: return "UNKNOWN";
        case ServerUtilsState::INIT: return "INIT";
        case ServerUtilsState::PROCESSING: return "PROCESSING";
        case ServerUtilsState::DONE: return "DONE";
        case ServerUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

ServerUtilsState identity_server_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return ServerUtilsState::UNKNOWN;
    if (s == "INIT") return ServerUtilsState::INIT;
    if (s == "PROCESSING") return ServerUtilsState::PROCESSING;
    if (s == "DONE") return ServerUtilsState::DONE;
    if (s == "ERROR") return ServerUtilsState::ERROR;
    return ServerUtilsState::UNKNOWN;
}
...
bool identity_server_utils_init(const IdentityServerUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = ServerUtilsState::INIT;
...
IdentityServerUtilsResponse identity_server_utils_handle(const IdentityServerUtilsRequest& req) {
    IdentityServerUtilsResponse resp;
    ...
    g_status.state = ServerUtilsState::PROCESSING;
    // Validate request
...
void identity_server_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = IdentityServerUtilsConfig{};
    g_status = IdentityServerUtilsStatus{};
    g_status.state = ServerUtilsState::UNKNOWN;
}
...
std::string identity_server_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "identity_server_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
IdentityServerUtilsConfig identity_server_utils_configFromJson(const json& j) {
    IdentityServerUtilsConfig config;
