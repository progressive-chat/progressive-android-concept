...
namespace {
    ReactionAggregatorConfig g_config;
    std::mutex g_mutex;
    ReactionAggregatorStatus g_status;
    ReactionAggregatorStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* reaction_aggregator_stateToString(ReactionAggregatorState s) {
    switch (s) {
        case ReactionAggregatorState::UNKNOWN: return "UNKNOWN";
        case ReactionAggregatorState::INIT: return "INIT";
        case ReactionAggregatorState::PROCESSING: return "PROCESSING";
        case ReactionAggregatorState::DONE: return "DONE";
        case ReactionAggregatorState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

ReactionAggregatorState reaction_aggregator_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return ReactionAggregatorState::UNKNOWN;
    if (s == "INIT") return ReactionAggregatorState::INIT;
    if (s == "PROCESSING") return ReactionAggregatorState::PROCESSING;
    if (s == "DONE") return ReactionAggregatorState::DONE;
    if (s == "ERROR") return ReactionAggregatorState::ERROR;
    return ReactionAggregatorState::UNKNOWN;
}
...
bool reaction_aggregator_init(const ReactionAggregatorConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = ReactionAggregatorState::INIT;
...
ReactionAggregatorResponse reaction_aggregator_handle(const ReactionAggregatorRequest& req) {
    ReactionAggregatorResponse resp;
    ...
    g_status.state = ReactionAggregatorState::PROCESSING;
    // Validate request
...
void reaction_aggregator_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = ReactionAggregatorConfig{};
    g_status = ReactionAggregatorStatus{};
    g_status.state = ReactionAggregatorState::UNKNOWN;
}
...
std::string reaction_aggregator_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "reaction_aggregator";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
ReactionAggregatorConfig reaction_aggregator_configFromJson(const json& j) {
    ReactionAggregatorConfig config;
