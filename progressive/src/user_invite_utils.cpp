...
namespace {
    UserInviteUtilsConfig g_config;
    std::mutex g_mutex;
    UserInviteUtilsStatus g_status;
    UserInviteUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* user_invite_utils_stateToString(InviteUtilsState s) {
    switch (s) {
        case InviteUtilsState::UNKNOWN: return "UNKNOWN";
        case InviteUtilsState::INIT: return "INIT";
        case InviteUtilsState::PROCESSING: return "PROCESSING";
        case InviteUtilsState::DONE: return "DONE";
        case InviteUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

InviteUtilsState user_invite_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return InviteUtilsState::UNKNOWN;
    if (s == "INIT") return InviteUtilsState::INIT;
    if (s == "PROCESSING") return InviteUtilsState::PROCESSING;
    if (s == "DONE") return InviteUtilsState::DONE;
    if (s == "ERROR") return InviteUtilsState::ERROR;
    return InviteUtilsState::UNKNOWN;
}
...
bool user_invite_utils_init(const UserInviteUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = InviteUtilsState::INIT;
...
UserInviteUtilsResponse user_invite_utils_handle(const UserInviteUtilsRequest& req) {
    ...
    g_status.state = InviteUtilsState::PROCESSING;
    // Validate request
...
void user_invite_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = UserInviteUtilsConfig{};
    g_status = UserInviteUtilsStatus{};
    g_status.state = InviteUtilsState::UNKNOWN;
}
...
std::string user_invite_utils_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "user_invite_utils";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
