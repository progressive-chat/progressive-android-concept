...
namespace {
    CallModelsConfig g_config;
    std::mutex g_mutex;
    CallModelsStatus g_status;
    CallModelsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* call_models_stateToString(ModelsPhase s) {
    switch (s) {
        case ModelsPhase::UNKNOWN: return "UNKNOWN";
        case ModelsPhase::INIT: return "INIT";
        case ModelsPhase::PROCESSING: return "PROCESSING";
        case ModelsPhase::DONE: return "DONE";
        case ModelsPhase::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

ModelsPhase call_models_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return ModelsPhase::UNKNOWN;
    if (s == "INIT") return ModelsPhase::INIT;
    if (s == "PROCESSING") return ModelsPhase::PROCESSING;
    if (s == "DONE") return ModelsPhase::DONE;
    if (s == "ERROR") return ModelsPhase::ERROR;
    return ModelsPhase::UNKNOWN;
}
...
bool call_models_init(const CallModelsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = ModelsPhase::INIT;
...
CallModelsResponse call_models_handle(const CallModelsRequest& req) {
    ...
    g_status.state = ModelsPhase::PROCESSING;
    // Validate request
...
void call_models_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = CallModelsConfig{};
    g_status = CallModelsStatus{};
    g_status.state = ModelsPhase::UNKNOWN;
}
