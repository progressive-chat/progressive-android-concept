...
namespace {
    SecretsUtilsConfig g_config;
    std::mutex g_mutex;
    SecretsUtilsStatus g_status;
    SecretsUtilsStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* secrets_utils_stateToString(SecretsUtilsState s) {
    switch (s) {
        case SecretsUtilsState::UNKNOWN: return "UNKNOWN";
        case SecretsUtilsState::INIT: return "INIT";
        case SecretsUtilsState::PROCESSING: return "PROCESSING";
        case SecretsUtilsState::DONE: return "DONE";
        case SecretsUtilsState::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

SecretsUtilsState secrets_utils_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return SecretsUtilsState::UNKNOWN;
    if (s == "INIT") return SecretsUtilsState::INIT;
    if (s == "PROCESSING") return SecretsUtilsState::PROCESSING;
    if (s == "DONE") return SecretsUtilsState::DONE;
    if (s == "ERROR") return SecretsUtilsState::ERROR;
    return SecretsUtilsState::UNKNOWN;
}
...
bool secrets_utils_init(const SecretsUtilsConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = SecretsUtilsState::INIT;
...
SecretsUtilsResponse secrets_utils_handle(const SecretsUtilsRequest& req) {
    ...
    g_status.state = SecretsUtilsState::PROCESSING;
    // Validate request
...
void secrets_utils_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = SecretsUtilsConfig{};
    g_status = SecretsUtilsStatus{};
    g_status.state = SecretsUtilsState::UNKNOWN;
}
