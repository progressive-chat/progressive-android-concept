...
namespace {
    EncryptionKeyBackupConfig g_config;
    std::mutex g_mutex;
    EncryptionKeyBackupStatus g_status;
    EncryptionKeyBackupStats g_stats;
    bool g_initialized = false;
} // anonymous namespace

const char* encryption_key_backup_stateToString(KeyBackupMode s) {
    switch (s) {
        case KeyBackupMode::UNKNOWN: return "UNKNOWN";
        case KeyBackupMode::INIT: return "INIT";
        case KeyBackupMode::PROCESSING: return "PROCESSING";
        case KeyBackupMode::DONE: return "DONE";
        case KeyBackupMode::ERROR: return "ERROR";
        default: return "UNKNOWN";
    }
}

KeyBackupMode encryption_key_backup_stateFromString(const std::string& s) {
    if (s == "UNKNOWN") return KeyBackupMode::UNKNOWN;
    if (s == "INIT") return KeyBackupMode::INIT;
    if (s == "PROCESSING") return KeyBackupMode::PROCESSING;
    if (s == "DONE") return KeyBackupMode::DONE;
    if (s == "ERROR") return KeyBackupMode::ERROR;
    return KeyBackupMode::UNKNOWN;
}
...
bool encryption_key_backup_init(const EncryptionKeyBackupConfig& config) {
    ...
    g_config = config;
    g_initialized = true;
    g_status.state = KeyBackupMode::INIT;
...
EncryptionKeyBackupResponse encryption_key_backup_handle(const EncryptionKeyBackupRequest& req) {
    EncryptionKeyBackupResponse resp;
    ...
    g_status.state = KeyBackupMode::PROCESSING;
    // Validate request
...
void encryption_key_backup_reset() {
    std::lock_guard<std::mutex> lock(g_mutex);
    g_initialized = false;
    g_config = EncryptionKeyBackupConfig{};
    g_status = EncryptionKeyBackupStatus{};
    g_status.state = KeyBackupMode::UNKNOWN;
}
...
std::string encryption_key_backup_process(const std::string& input) {
    ...
    try {
        ...
        result["module"] = "encryption_key_backup";
        return result.dump();
    } catch (const std::exception& e) {
        result["error"] = e.what();
...
EncryptionKeyBackupConfig encryption_key_backup_configFromJson(const json& j) {
    EncryptionKeyBackupConfig config;
